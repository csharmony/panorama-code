...
var HudMissionPanel = (function () {
  ...
  var _UpdateMission = function () {
    ...
    if (
      ...
      _m_missionId === 0 ||
      _m_missionId === "0" ||
      GameStateAPI.GetMapBSPName() === "lobby_mapveto"
    ) {
      _DeleteMissionPanel()
      return
    }
    ...
    _m_elMission = OperationMission.CreateMission(_m_cp, oMissionDetails, false)
    ...
    if (isList) {
      var aIncomplete = oMissionDetails.aSubQuests.filter(
        (element) =>
          element.nUncommitted < 1 && element.nsubQuestPointsRemaining > 0
      )

      OperationMission.HudIncompleteSubMissions(_m_elMission, aIncomplete)
...
;(function () {
  $.RegisterForUnhandledEvent(
    "GameState_OnMatchStart",
    HudMissionPanel.OnMatchStart
  )
  $.RegisterForUnhandledEvent(
    "GameState_LevelInitPreEntity",
    HudMissionPanel.LevelTransitionStart
  )
  $.RegisterForUnhandledEvent(
    "OnQuestProgressMade",
    HudMissionPanel.UpdateProgress
  )
  $.RegisterForUnhandledEvent("OnRoundMVPShown", HudMissionPanel.OnReceiveMVP)
  $.RegisterForUnhandledEvent(
    "OnRoundFreezeTimeEnd",
    HudMissionPanel.OnRoundFreezeTimeEnd
  )
  $.RegisterForUnhandledEvent(
    "SurvivalSpawnSelectModeChange",
    HudMissionPanel.SurvivalMatchStart
  )
})()
