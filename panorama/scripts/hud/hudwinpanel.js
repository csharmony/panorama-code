...
var HudWinPanel = (function () {
  ...
  function _Init() {
    ...
    $.RegisterForUnhandledEvent(
      "HudWinPanel_ShowRoundImpactScoreReport",
      _ShowRoundImpactScoreReport
    )
  ...
  function _OnReceivePlayerHurt(attackerXuid, victimXuid, damage) {
    ...
    if (!_m_bCanvasIsReady) {
      $.Schedule(
        0.5,
        function (attackerXuid, victimXuid, damage) {
    ...
    _UpdateDamage(
      wasDamageGiven ? victimXuid : attackerXuid,
      healthRemoved,
      numHits,
      returnedHealthRemoved,
      returnHits
    )
  }
  ...
  function _ShowRoundImpactScoreReport(msg) {
    ...
    $.GetContextPanel().SetDialogVariable(
      "player_name",
      GameStateAPI.GetPlayerName(_m_localXuid)
    )

    _m_canvasHeightInPixels =
      _m_elCanvas.actuallayoutheight / _m_elCanvas.actualuiscale_y
    _m_canvasWidthInPixels =
      _m_elCanvas.actuallayoutwidth / _m_elCanvas.actualuiscale_x
    ...
    if (_m_arrTimelineEvents.length > 0) {
      var FinalTOdds =
        _m_arrTimelineEvents[_m_arrTimelineEvents.length - 1]["terrorist_odds"]
      _m_winningTeam = FinalTOdds == 100 ? 2 : FinalTOdds == 0 ? 3 : ""
    }

    _m_xRange =
      _m_arrTimelineEvents.length + _m_arrPersonalDamageEvents.length + 1.5
    ...
    _m_elCanvas.DrawSoftLinePointsJS(
      plotPoints.length,
      _flattenArrayOfTuples(plotPoints),
      4,
      1.0,
      drawColor
    )
    ...
    var roundRestartDelay = Number(
      GameInterfaceAPI.GetSettingString("mp_round_restart_delay")
    )
  ...
  function _ExtractTPersonalDamageEvents(arrEvents) {
    ...
    arrEvents.forEach(function (oEvent, index) {
      ...
      if (isLivingPlayer && oDamage) arrResults.push(oEvent)
    })
  ...
  function _ExtractLivingEnemies(arrEvents) {
    ...
    arrEvents.forEach(function (oEvent, index) {
      ...
      var isEnemy =
        oVictimData &&
        oVictimData["team_number"] != localTeam &&
  ...
  function _ProcessTimelineEvents(
    ...
    arrEvents.forEach(function (oEvent, index) {
      ...
      if (index == 0) delta = oEvent["terrorist_odds"] - nStartingOdds
        ...
        delta =
          oEvent["terrorist_odds"] - arrEvents[index - 1]["terrorist_odds"]

      var sfx =
        delta < 0
          ? "UIPanorama.round_report_line_down"
          : "UIPanorama.round_report_line_up"

      var delay = index * _m_timeslice

      Scheduler.Schedule(
        delay,
        function (oEvent, plotPoint, sfx) {
  ...
  function _ShowWinnerPlaque() {
    ...
    elWinningLogo.SetImage(
      bCT
        ? "file://{images}/icons/ui/ct_logo_1c.svg"
        : "file://{images}/icons/ui/t_logo_1c.svg"
    )
    ...
    var sfx =
      localTeamNumber == WinningTeamNumber
        ? "UIPanorama.round_report_round_won"
        : "UIPanorama.round_report_round_lost"
  ...
  function _Colorize() {
    var bCT = _m_winningTeam == 3
    $.GetContextPanel()
      .FindChildrenWithClassTraverse("team-colorize")
      .forEach((el) => {
  ...
  function _UpdateDamage(
    ...
    if (elDamage.returnHealthRemoved > 0 || elDamage.healthRemoved > 0) {
      ...
      if (BEAM_ONLY_ON_DAMAGE) {
        var elTeamColorBar = $.GetContextPanel().FindChildTraverse(
          "bar-" + xuid
        )
        if (elTeamColorBar) {
  ...
  function _AddDamageToDamagePanel(oEvent, plotPoint) {
    var elDamageContainer =
      $.GetContextPanel().FindChildTraverse("DamageContainer")
    ...
    var elDamage = $.CreatePanel(
      "Panel",
      elDamageContainer,
      "Damage-" + victimData["xuid"]
    )
    ...
    if (BEAM_ONLY_ON_DAMAGE) {
      var elTeamColorBar = $.CreatePanel(
        "Panel",
        _m_elPlotContainer,
        "bar-" + victimData["xuid"]
      )
  ...
  function _PlotStartingOdds(nStartingOdds, startPlotPoint) {
    ...
    $.GetContextPanel().SetDialogVariable(
      "starting_chance",
      _ConvertToLocalOdds(nStartingOdds) + "%"
    )
  ...
  function _DecoratePoint(oEvent, plotPoint) {
    ...
    if (victimData) {
      ...
      elEventMain.FindChildTraverse("JsAvatarTeamColor").style.washColor =
        "rgb(" + rgbColor + ")"
    ...
    if (chance == 100) {
      ...
      elEventChance.FindChildTraverse("EventChanceNumber").style.color =
        "#00ff00"
    } else if (chance == 0) {
      ...
      elEventChance.FindChildTraverse("EventChanceNumber").style.color =
        _RemapToRedGreenRGB(chance - _m_prevChance, -20, 20)
    }
    ...
    var sfx =
      delta > 0
        ? "UIPanorama.round_report_odds_up"
        : delta < 0
  ...
  function _RemapToRedGreenRGB(val, min, max) {
    ...
    if (val >= 20) rgb = "rgb(0,255,0)"
    else if (val > 0) rgb = "rgb(100,255,0)"
    else if (val < -20) rgb = "rgb(255,0,0)"
    else if (val < 0) rgb = "rgb(255,100,0)"
  ...
  function _Reset() {
    ...
    var bUseInEye =
      ...
      (localTeamNumber != 2 && localTeamNumber != 3)
    _m_localXuid = bUseInEye
      ? GameStateAPI.GetHudPlayerXuid()
      : GameStateAPI.GetLocalPlayerXuid()

    _m_teamPerspective =
      localTeamNumber == 2 || localTeamNumber == 3 ? localTeamNumber : 2
    var bCT = _m_teamPerspective == 3

    _m_elCanvas = $.GetContextPanel().FindChildTraverse("RisCanvas")
    _m_elPlotContainer =
      $.GetContextPanel().FindChildTraverse("RisPlotContainer")
    ...
    $.GetContextPanel().SetDialogVariable(
      "team",
      GameStateAPI.GetTeamClanName(bCT ? "CT" : "TERRORIST")
    )
