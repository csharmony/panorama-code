...
var EndOfMatch = (function () {
  ...
  function _Initialize() {
    $.Schedule(1, (_) => {
      $.DispatchEvent("EndOfMatch_Latch")
    })
    ...
    _m_cP.Data()._m_scoreboardVisible =
      mode == "cooperative" || mode == "coopmission"
    ...
    _m_cP
      .FindChildrenWithClassTraverse("eom-panel")
      .forEach(function (elPanel) {
        ...
        elRBtn.SetPanelEvent(
          "onactivate",
          _NavigateToTab.bind(undefined, elPanel.id)
        )
        elRBtn.FindChildTraverse("id-navbar-button__label").text = $.Localize(
          elPanel.id
        )
      })
  ...
  function _StartTestShow(mockData) {
    if (_m_cP.id !== "PanelToTest") return
  ...
  function _ShowNextPanel() {
    ...
    if (
      _m_cP.Data()._m_currentPanelIndex < _m_cP.Data()._m_arrPanelObjects.length
    ) {
      if (
        _m_cP.Data()._m_currentPanelIndex ===
          _m_cP.Data()._m_arrPanelObjects.length - 1 &&
  ...
  return {
    ...
    EnableToggleBetweenScoreboardAndCharacters:
      _EnableToggleBetweenScoreboardAndCharacters,
