...
var MainMenuStoreTile = (function () {
  ...
  var _Init = function () {
    ...
    elImage.itemid =
      !isMarketItem && !useItemId && LootListItemID ? LootListItemID : id
    ...
    if (elItem.Data().oData.usetinynames)
      strItemName = $.Localize(
        InventoryAPI.GetRawDefinitionKey(id, "item_name") + "_tinyname"
      )
  ...
  var _OnActivateStoreItem = function (elItem, id, type) {
    ...
    if (type === "market") {
      elItem.SetPanelEvent(
        "onactivate",
        _OpenOverlayToMarket.bind(undefined, id)
      )
      ...
      if (InventoryAPI.GetItemTypeFromEnum(id) === "coupon") {
        ...
        elItem.SetPanelEvent(
          "onactivate",
          _ShowDecodePopup.bind(undefined, id, displayItemId, type)
        )
      } else if (ItemInfo.GetLootListCount(id) > 0) {
        elItem.SetPanelEvent(
          "onactivate",
          _ShowDecodePopup.bind(undefined, id, id, type)
        )
      } else
        elItem.SetPanelEvent(
          "onactivate",
          _ShowInpsectPopup.bind(undefined, id, type)
        )
    } else
      elItem.SetPanelEvent(
        "onactivate",
  ...
  var _ShowInpsectPopup = function (id) {
    var strExtraSettings = elItem.Data().oData.extrapopupfullscreenstyle
