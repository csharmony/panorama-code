...
var MainmenuWatchNotice = (function () {
  ...
  function _EventsReceived(eventsAsString) {
    if (ADD_DEBUG_EVENT == 1) _m_prevSchedeventsString = ""
  ...
  function _FindNoticeEvents() {
    ...
    for (var jdx in _m_arrEvents) {
      ...
      if (
        (oEvent["is_featured"] || oEvent["is_official"]) &&
        "live_matches" in oEvent &&
        Object.keys(oEvent["live_matches"]).length > 0
      ) {
  ...
  function _PopulateEvent(oEvent) {
    var elEvent = $.CreatePanel(
      "Panel",
      _m_elWatchNoticeContainer,
      oEvent["event_id"]
    )
    elEvent.BLoadLayoutSnippet("snippet-wn-event", oEvent["event_id"])

    var elCarouselContainer = elEvent.FindChildTraverse(
      "id-watchnotice__matches-carousel-container"
    )
    ...
    $.CreatePanel(
      "CarouselNav",
      elCarouselContainer,
      "Match-Carousel-Nav--" + oEvent["event_id"],
      {
    ...
    if (!("logo_url" in oEvent));
    {
      ...
      if (elLogo) elLogo.SetImage(oEvent["logo_url"])
    }
    ...
    if ("is_official" in oEvent && oEvent["is_official"] == true) {
      ...
      _m_arrFavorites &&
      _m_arrFavorites.includes(Number(oEvent["event_id"]))
    ) {
    ...
    elEvent.SetDialogVariable(
      "watchnotice_fave_month",
      GetEventMonthString(oEvent)
    )
    elEvent.SetDialogVariable(
      "watchnotice_reason",
      $.Localize(locReason, elEvent)
    )
