...
var MapDraft = (function () {
  ...
  function _PlaySoundEffect(strSoundEffect, msThrottleRequired = 0) {
    ...
    if (msThrottleRequired && msThrottleRequired > 0) {
      if (msTimestampNow - _m_msLastSoundTimestamp < msThrottleRequired) return
    }
  ...
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_IngameDraft_DraftUpdate",
    _Update
  )
  ...
  function _Update() {
    ...
    if (
      sGameUiState === "CSGO_GAME_UI_STATE_LOADINGSCREEN" ||
      MatchDraftAPI.GetDraft() !== "ingame" ||
      MatchDraftAPI.GetIngamePhase() < 1
    ) {
    ...
    if (MatchDraftAPI.GetIngamePhase() != _m_nPhase) {
      ...
      if (
        ingameTeamToActNow &&
        ingameTeamToActNow ==
          GameStateAPI.GetPlayerTeamNumber(MyPersonaAPI.GetXuid())
      ) {
  ...
  var _UpdatePhaseProgressBar = function () {
    var aChildren = _m_cp
      .FindChildInLayoutFile("id-map-draft-phasebar-container")
      .Children()
    aChildren.forEach((phase) => {
      var nPhaseBarIndex = parseInt(phase.GetAttributeString("data-phase", ""))
      phase.SetHasClass(
        "map-draft-phasebar--ban",
        !_m_isThisPhasePick && nPhaseBarIndex === _m_nPhase
      )
      phase.SetHasClass(
        "map-draft-phasebar--pick",
        _m_isThisPhasePick && nPhaseBarIndex === _m_nPhase
      )
      ...
      phase.FindChildInLayoutFile("id-map-draft-phase-name").text = $.Localize(
        "matchdraft_phase_" + nPhaseBarIndex
      )
      ...
      if (nPhaseBarIndex === _m_nPhase) {
        ...
        phase.FindChildInLayoutFile("id-map-draft-phase-timer").timeleft =
          nTimeRemaining
      }
    })
  }
  ...
  var _UpdateButtonsRow = function () {
    var elContainer = _m_rowsContainer.FindChildInLayoutFile(
      _m_rowPhaseName + _m_nPhase
    )

    if (!elContainer) {
      elContainer = $.CreatePanel(
        "Panel",
        _m_rowsContainer,
        _m_rowPhaseName + _m_nPhase
      )
    ...
    if (_m_nPhase === 1) {
      ...
      for (var i = 0; i < aVoteIds.length; i++) {
        ...
        if (
          _m_nPhase !== 4 ||
          (_m_nPhase === 4 &&
            MatchDraftAPI.GetIngameTeamToActNow() !==
              GameStateAPI.GetPlayerTeamNumber(MyPersonaAPI.GetXuid())) ||
          (_m_nPhase === 4 &&
            MatchDraftAPI.GetIngameTeamToActNow() ===
              GameStateAPI.GetPlayerTeamNumber(MyPersonaAPI.GetXuid()) &&
  ...
  var _MakeButton = function (elContainer, oBtnData) {
    ...
    if (!elButton) {
      ...
      elButton
        .FindChildInLayoutFile("draft-phase-button-selectorimg")
        .SetImage(oBtnData.selectorimg)
      elButton.SetDialogVariable("mapname", $.Localize(oBtnData.name))

      var elStatusText = elButton.FindChildInLayoutFile(
        "draft-phase-button-statustext"
      )
      elStatusText.text = $.Localize(oBtnData.statustext)

      elButton.SetPanelEvent(
        "onactivate",
        _OnActivateVoteTile.bind(undefined, elContainer, oBtnData)
      )
    ...
    elButton.SetHasClass(
      "map-draft-phase-button__status--positive",
      oBtnData.ispick
    )
    ...
    if (
      ...
      elButton.SetHasClass(
        "map-draft-phase-button--vetoed",
        oBtnData.mapstatus === "veto"
      )
      elButton.enabled = false
      return
    }

    var aVotedXuids = MatchDraftAPI.GetIngameXuidsForVote(
      oBtnData.voteid
    ).split(",")
    elButton.SetHasClass(
      "map-draft-phase-button--selected",
      aVotedXuids.indexOf(MyPersonaAPI.GetXuid()) !== -1
    )

    if (MatchDraftAPI.GetIngameXuidsForVote(oBtnData.voteid)) {
      var aVoteIds = MatchDraftAPI.GetIngameWinningVotes().split(",")
      elButton.SetHasClass(
        "map-draft-phase-button--winning-vote",
        aVoteIds.indexOf(oBtnData.voteid) !== -1
      )
    } else {
      elButton.SetHasClass("map-draft-phase-button--winning-vote", false)
    }

    var elAvatarsContainer = elButton.FindChildInLayoutFile(
      "id-map-draft-phase-avatars-container"
    )
  ...
  var _UpdateActionText = function () {
    ...
    _m_cp
      .FindChildInLayoutFile("id-map-draft-phase-info")
      .SetHasClass("map-draft-phase-info--hidden", isWaiting)
    _m_cp
      .FindChildInLayoutFile("id-map-draft-phase-waiting")
      .SetHasClass("map-draft-phase-info--hidden", !isWaiting)

    if (isWaiting) {
      _m_cp.FindChildInLayoutFile("id-map-draft-phase-wait").text = $.Localize(
        "#matchdraft_phase_action_wait_" + _m_nPhase
      )
      return
    }
    ...
    var nPickedMaps = elContainer
      .Children()
      .filter((btn) => btn.BHasClass("map-draft-phase-button--selected"))
    _m_cp.SetDialogVariableInt("maps", nPickedMaps.length)
    _m_phaseTitleText.text = $.Localize(
  ...
  var _MakeLargeMap = function (elContainer, style) {
    ...
    var mapPickId = aMapIds.filter(
      (id) => MatchDraftAPI.GetIngameMapIdState(id) === "pick"
    )[0]
    var mapName = DeepStatsAPI.MapIDToString(parseInt(mapPickId))
    var elMapImage = elContainer.FindChildInLayoutFile(
      "id-map-draft-phase-pick-map-image"
    )

    if (!elMapImage) {
      elMapImage = $.CreatePanel(
        "Panel",
        elContainer,
        "id-map-draft-phase-pick-map-image"
      )
    ...
    elMapImage.style.backgroundImage =
    ...
    if (style) {
      ...
      var nStartingTeam =
        MatchDraftAPI.GetIngameTeamWithFirstChoice() ===
        MatchDraftAPI.GetIngameTeamStartingCT()
          ? nOtherTeam
          : nYourTeam
      ...
      elContainer.FindChildInLayoutFile("id-map-draft-starting-team").visible =
        true
      elContainer
        .FindChildInLayoutFile("id-map-draft-starting-team-icon")
        .SetImage("file://{images}/icons/" + teamLogo)
  ...
  function _PopulatePlayerList() {
    ...
    for (var iTeam = 0; iTeam < teamNames.length; ++iTeam) {
      ...
      var teamPanelId =
        iYourXuidTeamIdx === iTeam
          ? "id-map-draft-phase-your-team"
          : "id-map-draft-phase-other-team"
      var elTeammates = _m_cp
        .FindChildInLayoutFile(teamPanelId)
        .FindChild("id-map-draft-phase-avatars")
