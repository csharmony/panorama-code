...
var Loadout = (function () {
  ...
  var _AddSort = function () {
    var elDropdown = $.GetContextPanel().FindChildInLayoutFile(
      "LoadoutSortDropdown"
    )
  ...
  var _HideShowRadialLoadout = function (bShow) {
    $.GetContextPanel()
      .FindChildInLayoutFile("LoadoutRadialSelector")
  ...
  var _OnShuffleToggle = function () {
    var elBtn = $.GetContextPanel().FindChildInLayoutFile(
      "LoadoutShuffleToggle"
    )
    var slot = _GetUsesWedges()
      ? _GetSelectededRadialButton()
      : _GetSelectededLoadoutSlot()
    var ItemId = _GetUsesWedges()
      ? _GetItemIdFromSelectedWedge()
      : LoadoutAPI.GetItemID(_GetSelectedTeamAsString(slot), slot)

    LoadoutAPI.SetShuffleEnabled(
      ItemId,
      _GetSelectedTeamAsString(slot),
      elBtn.checked
    )
  ...
  var _UpdateShuffleToggle = function (teamAsNumber) {
    ...
    if (_GetUsesWedges()) {
      weaponType = InventoryAPI.GetItemDefinitionName(ItemId).replace(
        "weapon_",
        ""
      )
      ...
      elLabel.SetDialogVariable(
        "team",
        $.Localize("#SFUI_MainMenu_TeamIcon_" + teamAsString)
      )
    ...
    elBtn.visible =
      loadoutSlot !== "spray0" &&
      ItemInfo.IsShuffleAllowed(ItemId) &&
      ItemInfo.CountItemsInInventoryForShuffleSlot(ItemId, teamAsString) > 1

    var nameString =
      loadoutSlot === "melee" ||
      loadoutSlot === "clothing_hands" ||
      loadoutSlot === "musickit" ||
      loadoutSlot === "customplayer"
  ...
  var _HideShowSingleItem = function (bShow) {
    $.GetContextPanel()
      .FindChildInLayoutFile("LoadoutSingleItem")
      .SetHasClass("hidden", !bShow)
  }
  ...
  var _FillOutItemData = function (itemId, loadoutSlot) {
    ...
    var elTitle = $.GetContextPanel().FindChildInLayoutFile(
      "LoadoutSingleItemLabel"
    )
    var elRarity = $.GetContextPanel().FindChildInLayoutFile(
      "LoadoutSingleItemRarity"
    )
    ...
    var elSingleItemPanel =
      $.GetContextPanel().FindChildInLayoutFile("LoadoutSingleItem")
    elSingleItemPanel.SetHasClass("extra-tall", loadoutSlot === "customplayer")

    var elCharControls = $.GetContextPanel().FindChildInLayoutFile(
      "id-character-buttons-container"
    )
    ...
    var elBackground = $.GetContextPanel().FindChildInLayoutFile(
      "loadout-single-item__bg"
    )
  ...
  var _InitCharacterItemPanel = function (charItemId) {
    $("#id-character-buttons-container").SetHasClass(
      "hidden",
      !ItemInfo.IsCharacter(charItemId)
    )

    if (!ItemInfo.IsCharacter(charItemId)) return
    ...
    CharacterButtons.InitCharacterButtons(
      $("#id-character-buttons"),
      $("#LoadoutSingleItemModel"),
      characterToolbarButtonSettings
    )
  }

  var _ShowFlairReset = function (loadoutSlot, noIdEquipped) {
    var elClearBtn =
      $.GetContextPanel().FindChildInLayoutFile("LoadoutResetFlair")
  ...
  var _UpdateItemList = function () {
    var loadoutSlot = _GetUsesWedges()
      ? _GetSelectededRadialButton()
      : _GetSelectededLoadoutSlot()
    var elListerToUpdate =
      $.GetContextPanel().FindChildInLayoutFile("LoadoutItemList")
    var elDropdown = $.GetContextPanel().FindChildInLayoutFile(
      "LoadoutSortDropdown"
    )
    var elShuffle = $.GetContextPanel().FindChildInLayoutFile(
      "LoadoutShuffleToggle"
    )
    var sortType = elDropdown.GetSelected() ? elDropdown.GetSelected().id : ""

    var defName =
      _GetUsesWedges() && elShuffle.checked
        ? InventoryAPI.GetItemDefinitionName(_GetItemIdFromSelectedWedge())
        : ""
    ...
    var team =
      _GetSelectedTeamAsString(loadoutSlot) === "noteam"
        ? ""
        : _GetSelectedTeamAsString(loadoutSlot)

    var loadoutSlotParams = defName
      ? ",only_econ_items,item_definition:" + defName + "," + team
      : "," + team
  ...
  var _GetSelectedTab = function () {
    var elSlots = $.GetContextPanel().FindChildInLayoutFile(
      "LoadoutSlotSelectionRadios"
    )
    var elSelectedSlot = elSlots
      .Children()
  ...
  var _GetUsesWedges = function () {
    return _GetSelectedTab().GetAttributeString("data-wedge", "") === "true"
      ? true
      : false
  }
  ...
  var _GetSelectedWedge = function () {
    ...
    var wedgeList = elWedges
      .Children()
      .filter((wedge) => wedge.checked === true)
    return wedgeList[0]
  }
  ...
  var _GetSelectededTeam = function (loadoutSlot) {
    ...
    var elSelectedTeam = elTeams
      .Children()
      .filter((slot) => slot.checked === true)
    return _SlotHasNoTeamAsTeam(loadoutSlot)
      ? 0
      : elSelectedTeam[0].GetAttributeString("data-team", "")
  }
  ...
  var _ShowItemsForSelectedSlot = function () {
    if (_GetUsesWedges()) {
      ...
      $.DispatchEvent(
        "ShowLoadout",
        loadoutSlot,
        Number(_GetSelectededTeam(loadoutSlot))
      )

      var wedges = $.GetContextPanel()
        .FindChildInLayoutFile("ItemWheel")
        .Children()
  ...
  var _ToggleTeam = function () {
    var elTeams = $.GetContextPanel()
      .FindChildInLayoutFile("LoadoutSelectTeam")
      .Children()
  ...
  var _OnReadyForDisplay = function () {
    ...
    Loadout.PlayerEquipSlotChangedHandler = $.RegisterForUnhandledEvent(
      "PanoramaComponent_Inventory_PlayerEquipSlotChanged",
      Loadout.UpdateForSingleSlotItems
    )
    Loadout.InventoryUpdatedHandler = $.RegisterForUnhandledEvent(
      "PanoramaComponent_MyPersona_InventoryUpdated",
      Loadout.UpdateInteriorPreviewAndItemTiles
    )
  }
  ...
  var _OnUnreadyForDisplay = function () {
    if (Loadout.PlayerEquipSlotChangedHandler) {
      $.UnregisterForUnhandledEvent(
        "PanoramaComponent_Inventory_PlayerEquipSlotChanged",
        Loadout.PlayerEquipSlotChangedHandler
      )
    ...
    if (Loadout.InventoryUpdatedHandler) {
      $.UnregisterForUnhandledEvent(
        "PanoramaComponent_MyPersona_InventoryUpdated",
        Loadout.InventoryUpdatedHandler
      )
  ...
  var _SetLoadoutToSlot = function (itemSlot, subslot, numTeam) {
    ...
    var elSelectedSlot = elSlots
      .Children()
      .filter((slot) => slot.GetAttributeString("data-slot", "") === itemSlot)
    ...
    if (_GetUsesWedges()) {
      ...
      var wedgeList = elWedges
        .Children()
        .filter(
          (wedge) => wedge.GetAttributeString("equip_slot", "") === subslot
        )
  ...
  var _SelectMinimumItemsForShuffle = function (itemID, team) {
    ...
    if (elLoadoutItemList && elLoadoutItemList.IsValid()) {
      for (var i = 0; i < elLoadoutItemList.count; ++i) {
        if (LoadoutAPI.GetItemCountInShuffle(itemID, team) >= 2) break
  ...
  var _ShowShuffleContextMenu = function () {
    ...
    UiToolkitAPI.ShowSimpleContextMenu(
      "LoadoutShuffleOptions",
      "LoadoutShuffleCtxMenu",
      items
    )
  }
...
;(function () {
  ...
  $.RegisterEventHandler(
    "ReadyForDisplay",
    $.GetContextPanel(),
    Loadout.OnReadyForDisplay
  )
  $.RegisterEventHandler(
    "UnreadyForDisplay",
    $.GetContextPanel(),
    Loadout.OnUnreadyForDisplay
  )
  $.RegisterForUnhandledEvent(
    "LoadoutWedgePressed",
    Loadout.LoadoutWedgePressed
  )
