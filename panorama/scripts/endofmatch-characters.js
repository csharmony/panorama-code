...
var EOM_Characters = (function () {
  ...
  const CAMERA_POSITIONS = [10, 11, 12, 13, 14, 15, 22, 23, 24, 25]
  ...
  function _SetMainTeamLogo(teamName) {
    ...
    var myTeamLogoPath =
      "file://{images}/icons/ui/" +
      (teamName == "CT" ? "ct_logo_1c.svg" : "t_logo_1c.svg")
    var elMyTeamLogo = elRoot.FindChildTraverse(
      "id-eom-chars__layout__logo--myteam"
    )
  ...
  function _SetTeamLogo(team) {
    ...
    var teamLogoPath =
      "file://{images}/icons/ui/" +
      (team == "ct" ? "ct_logo_1c.svg" : "t_logo_1c.svg")
    var elTeamLogo = elRoot.FindChildTraverse(
      "id-eom-chars__layout__logo--" + team
    )
  ...
  function _CollectPlayersForMode(mode) {
    ...
    switch (mode) {
      ...
      case "competitive": {
        ...
        if (_m_localPlayer) {
          arrPlayerList = arrPlayerList.filter(
            (player) => player["xuid"] != _m_localPlayer["xuid"]
          )
      ...
      case "gungameprogressive": {
        ...
        arrPlayerList[0] = _m_arrAllPlayersMatchDataJSO.filter(
          (o) => o["xuid"] == arrPlayerXuids[0]
        )[0]
        arrPlayerList[1] = _m_arrAllPlayersMatchDataJSO.filter(
          (o) => o["xuid"] == arrPlayerXuids[1]
        )[0]
        arrPlayerList[2] = _m_arrAllPlayersMatchDataJSO.filter(
          (o) => o["xuid"] == arrPlayerXuids[2]
        )[0]
  ...
  function _CollectPlayersMatchingXuids(arrXuids) {
    return _m_arrAllPlayersMatchDataJSO.filter((o) =>
      arrXuids.includes(o["xuid"])
    )
  }
  ...
  function _CollectPlayersOfTeam(teamName) {
    ...
    return _m_arrAllPlayersMatchDataJSO.filter(
      (o) => o["teamnumber"] == teamNum
    )
  }
  ...
  var _DisplayMe = function () {
    ...
    var localPlayerSet = _m_arrAllPlayersMatchDataJSO.filter(
      (oPlayer) => oPlayer["xuid"] == MockAdapter.GetLocalPlayerXuid()
    )
    ...
    if (localPlayer && !_ShowWinningTeam(mode)) {
      ...
      if (oMatchEndData) teamNumToShow = oMatchEndData["winning_team_number"]
    ...
    if (_m_localPlayer) {
      var arrLocalPlayer = _m_localPlayer.hasOwnProperty("items")
        ? _m_localPlayer.items.filter((oItem) =>
            ItemInfo.IsCharacter(oItem.itemid)
          )
        : []
      var localPlayerModel = arrLocalPlayer.length > 0 ? arrLocalPlayer[0] : ""
      var localPlayerCheer = localPlayerModel
        ? ItemInfo.GetDefaultCheer(localPlayerModel["itemid"])
        : ""
    ...
    arrPlayerList.forEach(function (oPlayer, index) {
      if (oPlayer) {
        ...
        if ("items" in oPlayer) {
          playerModelItem = oPlayer["items"].filter((oItem) =>
            ItemInfo.IsCharacter(oItem["itemid"])
          )[0]
        }

        cheer = playerModelItem
          ? ItemInfo.GetDefaultCheer(playerModelItem["itemid"])
          : ""

        if (oPlayer != _m_localPlayer && mapCheers[cheer] == 1) {
          cheer = ""
        }
  ...
  function _ApplyAccolades(elCLU, arrPlayerList, bNoGimmes) {
    if (!arrPlayerList || arrPlayerList.length == 0) return 0
    ...
    arrPlayerList.forEach(function (oPlayer) {
      ...
      if (oTitle == undefined) {
        ...
        var accoladeName = GameStateAPI.GetAccoladeLocalizationString(
          Number(oTitle["eaccolade"])
        )
        ...
        if (!(bNoGimmes && accoladeName.includes("gimme_"))) {
          ...
          elAccolade.SetDialogVariable(
            "eom-accolade-the",
            $.Localize("accolade_the")
          )
          ...
          elAccolade.SetDialogVariableTime(
            "eom-accolade-value-time",
            Number(value)
          )
          elAccolade.SetDialogVariableInt(
            "eom-accolade-value-int",
            Number(value)
          )
          ...
          elAccolade.SetDialogVariable(
            "eom-accolade-title",
            $.Localize("accolade_" + accoladeName + secondPlaceSuffix)
          )
          elAccolade.SetDialogVariable(
            "eom-accolade-desc",
            $.Localize(
              "accolade_" + accoladeName + "_desc" + secondPlaceSuffix,
              elAccolade
            )
          )
      ...
      function _DisplayAccolade(panel, charPanel, index) {
        ...
        if (panel && panel.IsValid()) panel.AddClass("reveal")
        ...
        $.Schedule(0.1, (_) => {
          if (charPanel && charPanel.IsValid()) {
            charPanel
              .FindChildTraverse("id-charlineup__model-preview-panel")
              .TriggerSlowmo(
                _m_freeze_time - 1.5 * (index * DELAY_PER_PLAYER),
                UNFREEZE_TIME
              )
          }
        })
      ...
      $.Schedule(
        ACCOLADE_START_TIME + index * DELAY_PER_PLAYER,
        _DisplayAccolade.bind(undefined, elAccolade, elCharacter, index)
      )
    })
  }
    ...
    switch (mode) {
      ...
      case "no longer used but force player to have a spot":
        if (_m_localPlayer && _m_localPlayer in arrPlayerList) {
          localPlayerPosition = Math.min(
            arrPlayerList.indexOf(_m_localPlayer),
            7
          )
          arrPlayerList = arrPlayerList.filter(
            (player) => player["xuid"] != _m_localPlayer["xuid"]
