...
var OperationMissionCard = (function () {
  ...
  var _UpdateMissionCard = function (idx, elParent) {
    ...
    if (oCardDetails) {
      ...
      var elContainer = elMissionCard.FindChildInLayoutFile(
        "id-mission-card-missions-container"
      )
      ...
      if (oCardDetails.bShowLock) {
        ...
        for (
          var iMission = 0;
          iMission < oCardDetails.quests.length;
          ++iMission
        ) {
          ...
          nActiveMissionInCardCount =
            currentlyPlayingMissionId === missionId
              ? nActiveMissionInCardCount++
              : nActiveMissionInCardCount
        }
        ...
        aMissions.forEach((element) => {
          var elMission = OperationMission.CreateMission(
            elContainer,
            element,
            oCardDetails.isunlocked
          )

          OperationMission.UpdateMissionDisplay(
            elMission,
            element,
            oCardDetails.isunlocked,
            oCardDetails.id
        ...
        if (GameStateAPI.IsLocalPlayerPlayingMatch()) {
          elMissionCard.SetHasClass(
            "no-mission-warning",
            nActiveMissionInCardCount < 1
          )
        }
      }
    }
  ...
  var _GetMissionCardDetails = function (idx) {
    var oCardDetails = MissionsAPI.GetSeasonalOperationMissionCardDetails(
    ...
    if (oCardDetails) {
      ...
      oCardDetails.bShowLock =
        oCardDetails.quests.length <= 0 ||
  ...
  var _CreateMissionCard = function (oCardDetails, elParent) {
    ...
    if (
      !elParent.FindChildInLayoutFile(_m_missionCardPrefix + oCardDetails.id)
    ) {
      elParent.RemoveAndDeleteChildren()

      elMissionCard = $.CreatePanel(
        "Panel",
        elParent,
      ...
      elMissionCard.BLoadLayout(
        "file://{resources}/layout/operation/operation_mission_card.xml",
        false,
        false
      )
    } else {
      elMissionCard = elParent.FindChildInLayoutFile(
        _m_missionCardPrefix + oCardDetails.id
      )
    }
  ...
  var _UpdateMissionCardDisplay = function (elMissionCard, oCardDetails) {
    elMissionCard
      .FindChildInLayoutFile("id-mission-card-tag")
      .SetHasClass(
        "hidden",
        oCardDetails.idx !== OperationUtil.GetOperationInfo().nActiveCardIndex
      )

    elMissionCard.SetDialogVariable(
      "mission_name",
      $.Localize(oCardDetails.name)
    )
    elMissionCard.SetDialogVariableInt(
      "card_points_needed",
  ...
  var _UpdateUnlockTimer = function (elMissionCard) {
    ...
    if (_m_missionBacklogIndex !== InventoryAPI.GetMissionBacklog()) {
      if (
        InventoryAPI.GetMissionBacklog() &&
        InventoryAPI.GetMissionBacklog() > 0
      ) {
        _m_missionBacklogIndex = InventoryAPI.GetMissionBacklog()

        _GetMissionCardDetails(
          elMissionCard.Data().idx,
          elMissionCard.GetParent()
        )
      } else {
    ...
    if (
      elMissionCard.IsValid() &&
      elMissionCard.Data().idx === _m_missionBacklogIndex
    ) {
      ...
      if (seconds && seconds !== 0) {
        ...
        elMissionCard.SetDialogVariable(
          "unlock_time",
          FormatText.SecondsToSignificantTimeString(seconds)
        )
      }

      _m_missionUnlockTimerHandler = $.Schedule(
        5,
        _UpdateUnlockTimer.bind(undefined, elMissionCard)
      )
    }
  }
  ...
  var _UpdateEarnedPoints = function (elMissionCard, oCardDetails) {
    ...
    var elLabel = elMissionCard.FindChildInLayoutFile(
      "id-mission-card-stars-text"
    )
    elLabel.text = $.Localize("#op_mission_card_points", elMissionCard)
    elMissionCard.SetHasClass(
      "card-complete",
      nEarnedPoints >= oCardDetails.operational_points
    )
  }
