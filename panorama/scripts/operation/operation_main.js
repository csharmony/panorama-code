...
var OperationMain = (function () {
  ...
  var _Init = function () {
    ...
    if (OperationUtil.UpdateOldStars().ids.length > 0) {
      $.Schedule(0.3, OperationUtil.OpenUpSell)
    }
  ...
  var _CheckUsersOperationStatus = function () {
    ...
    var aChildren = elVisibleBar
      .FindChildInLayoutFile("op-main-progressbar-rewards")
      .Children()

    _m_activeRewardBtn = aChildren.filter(
      (element) =>
        element.Data().oReward.itempremium.ids[0] ===
          oRewardToHighlight.itempremium.ids[0] &&
  ...
  var _SetupHeader = function (oStatus) {
    if (oStatus.bPremiumUser) {
      $.GetContextPanel().SetDialogVariableInt(
        "total_value",
        oStatus.nTierUnlocked
      )
    }

    $.GetContextPanel().FindChildInLayoutFile("id-op-main-header").visible =
      oStatus.bPremiumUser
    $.GetContextPanel().FindChildInLayoutFile(
      "id-op-main-header-preview"
    ).visible = !oStatus.bPremiumUser
  }
  ...
  var _SetPurchaseBtn = function () {
    ...
    elUpSell.FindChildInLayoutFile("op-main-upsell-label").text = $.Localize(
      bPremiumUser
        ? "#op_get_more_stars"
        : sUserOwnedOperationPassItemID
    ...
    elUpSell.SetPanelEvent(
      "onactivate",
      OperationUtil.OpenUpSell.bind(undefined)
    )

    var elPassSaleDiscount = elUpSell.FindChildInLayoutFile(
      "id-op-reward-open-operation-hub-passsalediscount"
    )
    elPassSaleDiscount.visible =
      !bPremiumUser && sUserOwnedOperationPassItemID ? false : true
    ...
    if (bPremiumUser) {
      ...
      storeids.forEach(function (defIndex) {
        ...
        if (sPctReductionPerStar && sPctReductionPerStar !== "-0%") {
          ...
          if (numericStarDiscount < minDiscount)
            minDiscount = numericStarDiscount
          if (numericStarDiscount > maxDiscount)
            maxDiscount = numericStarDiscount
        }
      })

      if (sPctReduction) {
        elPassSaleDiscount.SetDialogVariable(
          "salediscount",
          "" + minDiscount + "%"
        )
        sPctReduction = $.Localize(
          "#Store_Price_UpToSale",
          elPassSaleDiscount
        ).toUpperCase()
      }
    }
    ...
    else elPassSaleDiscount.visible = false
  }
  ...
  var _UpdateDefaultProgressData = function (oStatus, openToRewardIndex) {
    ...
    var activeIndex =
      openToRewardIndex > -1
        ? Math.floor(openToRewardIndex / SECTIONS_IN_BAR)
        : Math.floor(oStatus.nTierUnlocked / SECTIONS_IN_BAR)
    ...
    _m_oProgressFlipModule.AddParamToCallbackData(
      "sectionStart",
      SECTIONS_IN_BAR * activeIndex
    )
    _m_oProgressFlipModule.AddParamToCallbackData(
      "sectionEnd",
      SECTIONS_IN_BAR * activeIndex + SECTIONS_IN_BAR
    )
  ...
  var _UpdateRewardTiles = function () {
    ...
    for (var i = 0; i < maxTiles; i++) {
      numTileRow = aTiles[i]
        ? parseInt(aTiles[i].rewardsData[0].uiOrder)
        : numTileRow
      if (numTileRow !== currentRow) {
        currentRow = numTileRow
        var elTitle = GetPanel(
          "id-op-rewards-section-",
          currentRow,
          elParent,
          "Label"
        )
      ...
      if (aTiles[i].rewardsData[0].imagePath) {
        elTile
          .FindChildInLayoutFile("id-op-reward-image")
          .SetImage(
            "file://{images}" + aTiles[i].rewardsData[0].imagePath + ".png"
          )
      } else {
        elTile.FindChildInLayoutFile("id-op-reward-image").itemid =
          aTiles[i].rewardId
      }

      elTile.SetHasClass("small", currentRow !== 1)
      elTile.FindChildInLayoutFile("id-op-reward-name").text =
        InventoryAPI.GetItemName(aTiles[i].rewardId)

      elTile.Data().oReward = aTiles[i].rewardsData[0]
      elTile.Data().showSingleReward =
        aTiles[i].rewardsData.length === 1 ? true : false
      elTile.Data().nCategoryButtonIdx = i
      elTile.SetPanelEvent(
        "onactivate",
        _OnSelectReward.bind(undefined, elTile)
      )
  ...
  var _GetRewardsForTiles = function () {
    ...
    for (
      var i = _m_oProgressFlipModule.oData.oCallbackData.sectionStart;
      ...
      if (aRewards[i].isGap !== "none") {
        ...
        if (
          !aTiles.find((tile) => {
            return tile["rewardId"] && tile["rewardId"] === rewardId
              ? true
              : false
          })
        ) {
          aTiles.push({
            rewardId: rewardId,
            rewardsData: aRewards.filter(
              (reward) =>
                reward.idx >=
                  _m_oProgressFlipModule.oData.oCallbackData.sectionStart &&
                reward.idx <=
                  _m_oProgressFlipModule.oData.oCallbackData.sectionEnd &&
    ...
    aTiles.sort(function (a, b) {
      return (
        parseInt(a.rewardsData[0].uiOrder) - parseInt(b.rewardsData[0].uiOrder)
      )
    })
    return aTiles
  }
  ...
  var _TransitionEvent = function (elBar) {
    elBar.OnPropertyTransitionEndEvent = function (panelName, propertyName) {
      if (elBar.id === panelName && propertyName === "transform") {
        if (
          !elBar.BIsTransparent() &&
          (elBar.BHasClass("flip-panel-anim-up-show") ||
            elBar.BHasClass("flip-panel-anim-down-show"))
        ) {
          _UpdateRewardTiles()
          _ShowPurchaseSpecificStarsBtn()

          var aChildren = _m_cp
            .FindChildInLayoutFile("id-op-rewards-list")
            .FindChildrenWithClassTraverse("op-main-reward")
          var aSelected = aChildren.filter((element) => element.checked)

          _m_activeRewardBtn =
            aSelected.length < 1 ? aChildren[0] : aSelected[0]
          _SetActiveReward(_m_activeRewardBtn)
          _SetCheckedMatchingTile()
    ...
    $.RegisterEventHandler(
      ...
      elBar,
      elBar.OnPropertyTransitionEndEvent
    )
  }
  ...
  var _UpdateProgressBarDisplay = function (oData, isPrev = false) {
    function UpdateData(oData) {
      ...
      oData.oCallbackData.sectionEnd =
        SECTIONS_IN_BAR * oData.activeIndex + SECTIONS_IN_BAR

      var NextPanel = _m_oProgressFlipModule.DetermineHiddenPanel(
        oData.animPanelA,
        oData.animPanelB
      )
    ...
    if (isPrev) {
      --oData.activeIndex
      UpdateData(oData)
      _m_oProgressFlipModule.BtnPressPrevAnim(
        oData.animPanelA,
        oData.animPanelB
      )
    } else {
      ++oData.activeIndex
      UpdateData(oData)
      _m_oProgressFlipModule.BtnPressNextAnim(
        oData.animPanelA,
        oData.animPanelB
      )
    }
  ...
  var _UpdateProgressBarEnableDisable = function (oData) {
    ...
    oData.controlBtnNext.enabled =
      oData.activeIndex < oData.oCallbackData.maxIndex
  ...
  var _UpdateProgressBarSections = function (oData, elBarParent) {
    var elBar = elBarParent.FindChildInLayoutFile(
      "op-main-progressbar-sections"
    )
    elBar.RemoveAndDeleteChildren()

    for (
      var i = oData.oCallbackData.sectionStart;
      i <= oData.oCallbackData.sectionEnd;
      i++
    ) {
      var elSection = $.CreatePanel("Panel", elBar, "bar-section-" + i)
      elSection.BLoadLayoutSnippet("progressbar-section")
      elSection.SetHasClass(
        "op-main-progress__bar__inner--fill",
        i < oData.oCallbackData.nTierUnlocked
      )
    }
  }

  var _UpdateRewardsOnProgressBar = function (
    elBar,
    oRewardTohighlight = null
  ) {
    ...
    _UpdateRewardPanelsOnProgressBar(
      _GetRewardsInRange(),
      elBar,
      oRewardTohighlight
    )
  ...
  var _GetRewardToHighlight = function (rewardTohighlightIndex) {
    ...
    for (
      var i = _m_oProgressFlipModule.oData.oCallbackData.nTierUnlocked;
      i < count;
      i++
    ) {
  ...
  var _GetRewardsInRange = function () {
    ...
    var aGetRewardsInRange = oAllRewards.filter(
      (reward) =>
        !reward.isGap &&
  ...
  var _UpdateRewardPanelsOnProgressBar = function (
    aGetRewardsInRange,
    elBar,
    oRewardToHighlight = null
  ) {
    var elRewardsPanel = elBar.FindChildInLayoutFile(
      "op-main-progressbar-rewards"
    )

    aGetRewardsInRange.forEach(function (reward, index) {
      var elReward = elRewardsPanel.FindChildInLayoutFile(
        "bar-reward-" + reward.idx
      )
      var bPlayAnim = false

      if (!elReward) {
        var elReward = $.CreatePanel(
          "RadioButton",
          elRewardsPanel,
          "bar-reward-" + reward.idx,
          { group: "bar-reward" }
      ...
      if (reward.imagePathThumbnail) {
        elReward
          .FindChildInLayoutFile("id-op-progressbar-reward-image")
          .SetImage("file://{images}" + reward.imagePathThumbnail + ".png")
      } else {
        elReward.FindChildInLayoutFile(
          "id-op-progressbar-reward-image"
        ).itemid = reward.itempremium.ids[0]
      }

      elReward.SetDialogVariableInt("reward_index", reward.idx + 1)
      elReward
        .FindChildInLayoutFile("id-op-progressbar-reward-hitbox")
        .SetPanelEvent(
          "onactivate",
          _OnSelectReward.bind(undefined, elReward, elBar)
        )
      ...
      var rewardTohighlight = _m_activeRewardBtn
        ? _m_activeRewardBtn.Data().oReward
        : oRewardToHighlight

      if (rewardTohighlight) {
        if (oRewardToHighlight || _m_activeRewardBtn.Data().showSingleReward) {
          elReward.SetHasClass(
            "highlight",
      ...
      elReward.SetHasClass(
        "claimed",
        reward.idx < _m_oProgressFlipModule.oData.oCallbackData.nTierUnlocked
      )

      if (bPlayAnim) {
        $.Schedule(
          0.25,
          _PositionRewardOnProgressBar.bind(undefined, oAnimInfo)
        )
      }
    })
  }
  ...
  var _PositionRewardOnProgressBar = function (oAnimInfo) {
    var elSectionsContainer = oAnimInfo.bar.FindChildInLayoutFile(
      "op-main-progressbar-sections"
    )
    var elSection = elSectionsContainer.FindChild(
      "bar-section-" + oAnimInfo.idx
    )
    let pos = elSection.GetPositionWithinWindow()

    var widthOffset =
      -(
        Math.floor(oAnimInfo.panel.actuallayoutwidth / 2) -
        Math.floor(elSection.actuallayoutwidth / 2)
      ) / oAnimInfo.panel.actualuiscale_x

    oAnimInfo.panel.style.x =
      pos.x / oAnimInfo.panel.actualuiscale_x + widthOffset + "px;"
  ...
  var _OnSelectReward = function (
    ...
    var visibleBar = elBar
      ? elBar
      : _m_oProgressFlipModule.DetermineVisiblePanel(
          _m_oProgressFlipModule.oData.animPanelA,
          _m_oProgressFlipModule.oData.animPanelB
        )
    ...
    if (bTrackStats) {
      if (elSelected.Data().nCategoryButtonIdx !== undefined) {
        StoreAPI.RecordUIEvent(
          "OperationJournal_RewardTrackSelection_Category",
          elSelected.Data().nCategoryButtonIdx
        )
      } else {
        StoreAPI.RecordUIEvent(
          "OperationJournal_RewardTrackSelection_Individual",
          elSelected.Data().oReward.idx
        )
      }
    }
  }
  ...
  var _ShowPurchaseSpecificStarsBtn = function () {
    var elPurchaseBtn = _m_cp.FindChildInLayoutFile(
      "id-op-rewards-upsell-stars-btn"
    )

    var bShow =
      _m_activeRewardBtn &&
      _m_activeRewardBtn.Data().showSingleReward &&
      OperationUtil.GetOperationInfo().bPremiumUser &&
      _m_activeRewardBtn.Data().oReward.idx >=
        OperationUtil.GetOperationInfo().nTierUnlocked
    ...
    if (bShow) {
      ...
      elPurchaseBtn.SetPanelEvent(
        "onactivate",
        OperationUtil.OpenUpSell.bind(undefined, starsNeeded)
      )
    }
  }
  ...
  var _UpdateInspectPanel = function () {
    ...
    _m_cp
      .FindChildInLayoutFile("id-op-reward-tag")
      .SetHasClass(
        "hidden",
        rewardId !== _GetRewardToHighlight().itempremium.ids[0]
      )

    var itemsList = []
    var toolsKey = InventoryAPI.GetRawDefinitionKey(
      rewardId,
    ...
    var bIsCoin =
      InventoryAPI.GetRawDefinitionKey(rewardId, "item_type") ===
      "operation_coin"
    ...
    var btnPreviewCase = _m_cp.FindChildInLayoutFile(
      "id-op-main-inspect-preview-item"
    )
    if (bIsCase) {
      if (bIsCase) {
      ...
      if (
        ItemInfo.IsCharacter(itemsList[0]) ||
        ItemInfo.IsWeapon(itemsList[0])
      ) {
        var _getSelectedIdInLayoutChildrenRow = function () {
          var selectedId = ""
          var aPips = _m_cp
            .FindChildInLayoutFile("id-op-main-inspect-rarity")
            .Children()
          if (aPips.length > 0) {
        ...
        btnPreviewCase.SetPanelEvent("onmouseover", function () {
          var selectedId = _getSelectedIdInLayoutChildrenRow()
          var settingsForSelectedCharacter =
            ItemInfo.GetOrUpdateVanityCharacterSettings(selectedId)
          ItemInfo.PrecacheVanityCharacterSettings(settingsForSelectedCharacter)
        })
      }
    }

    btnPreviewCase.visible =
      bIsCase ||
      ItemInfo.IsCharacter(itemsList[0]) ||
      ItemInfo.IsWeapon(itemsList[0])

    _m_cp.SetDialogVariable("reward_name", ItemInfo.GetName(rewardId))
  ...
  var _UpdateWarning = function (bIsCoin, bIsCase, rewardId) {
    ...
    var bHideWarning =
      bIsCase || (bIsCoin && !OperationUtil.GetOperationInfo().bPremiumUser)
    ...
    if (bIsCoin) {
      ...
      var missionsThreshold = InventoryAPI.GetItemAttributeValue(
        id,
        "upgrade threshold"
      )
      ...
      if (
        !missionsThreshold ||
        missionsThreshold < 0 ||
        missionsThreshold > 1000
      ) {
        elWarning.SetHasClass("hidden", true)
        return
      }

      var missionsRemaining =
        missionsThreshold - OperationUtil.GetOperationInfo().nMissionsCompleted
      _m_cp.SetDialogVariableInt("missions_remaining", missionsRemaining)

      var missionText =
        missionsRemaining === 1
          ? $.Localize("op_rewards_mission")
          : $.Localize("op_rewards_missions")
  ...
  var _MakePips = function (itemsList, rewardId) {
    ...
    itemsList.forEach(function (itemId, index) {
      ...
      $.CreatePanel("ItemImage", elPip, "", {
        itemid: itemId,
        scaling: "stretch-to-fit-y-preserve-aspect"
      })

      $.CreatePanel("Panel", elPip, "id-pip-rarity", {
      ...
      var rarityColor =
        itemId === "0" && bIsCase ? "#ffd700" : ItemInfo.GetRarityColor(itemId)
      ...
      if (rarityColor) {
        elPip.FindChild("id-pip-rarity").style.backgroundColor =
          rarityColor + ";"
      }
  ...
  var _UpdateInpsectNameFlipModule = function (aItems, rewardId) {
    ...
    _m_oNamesFlipModuleInspect.AddParamToCallbackData(
      "maxIndex",
      aItems.length - 1
    )
    _m_oNamesFlipModuleInspect.UseCallback()
  ...
  var _UpdateInspectName = function (oData, isPrev = false) {
    ...
    function UpdateData(oData) {
      var NextPanel = _m_oNamesFlipModuleInspect.DetermineHiddenPanel(
        oData.animPanelA,
        oData.animPanelB
      )
      var displayItemId = ""
      ...
      if (_m_activeRewardBtn.Data().bShowPips === false) {
        ...
        displayItemId = bIsCase
          ? activeTileId
          : oData.oCallbackData.items[oData.activeIndex]
        NextPanel.GetParent().visible = true
        NextPanel.SetDialogVariable(
          "reward_item_name",
          ItemInfo.GetName(displayItemId)
        )
      ...
      _UpdateInspectModelPanelOrImage(
        displayItemId,
        oData.oCallbackData.rewardid
      )
    }
    ...
    if (isPrev) {
      --oData.activeIndex
      UpdateData(oData)
      _m_oNamesFlipModuleInspect.BtnPressPrevAnim(
        oData.animPanelA,
        oData.animPanelB
      )
      ...
      ++oData.activeIndex
      UpdateData(oData)
      _m_oNamesFlipModuleInspect.BtnPressNextAnim(
        oData.animPanelA,
        oData.animPanelB
      )
      if (oData.oCallbackData.items[oData.activeIndex + 1])
    ...
    if (oData.oCallbackData.items[oData.activeIndex + 1]) {
      _CacheWeaponMouseOver(
        oData.controlBtnNext,
        oData.oCallbackData.items[oData.activeIndex + 1],
        true
      )
    }
  ...
  function _UpdateInspectModelPanelOrImage(itemId, rewardId) {
    ...
    var displayItemId = ""
    if (
      _m_activeRewardBtn.Data().bIsCoin &&
      OperationUtil.GetOperationInfo().bPremiumUser
    ) {
    ...
    if (model && !_m_activeRewardBtn.Data().bIsGraffiti) {
      ...
      if (isCharacter) {
        var settings = ItemInfo.GetOrUpdateVanityCharacterSettings(
          displayItemId,
          "unowned"
        )
        ...
        elModel.SetFlashlightColor(4.16, 4.06, 5.2)
        ...
        elModel.SetCameraAngles(5.66, 179.31, 0.0)
      } else {
      ...
      elImage.SetImage(
        "file://{images}" +
          _m_activeRewardBtn.Data().oReward.imagePathInspect +
          ".png"
      )
    }
  }
  ...
  var _CacheWeaponMouseOver = function (elTile, id, islootlistItem = false) {
    ...
    if (count > 0 || islootlistItem) {
      var lootListItem = islootlistItem
        ? id
        : ItemInfo.GetLootListItemByIndex(id, 0)
      if (ItemInfo.IsWeapon(lootListItem)) {
        elTile.SetPanelEvent(
          "onmouseover",
          _CacheWeapon.bind(undefined, lootListItem)
        )
      }
    }
  }
...
;(function () {
  $.RegisterForUnhandledEvent(
    ...
    OperationMain.CheckUsersOperationStatus
  )
})()
