...
var OperationStore = (function () {
  ...
  var _Init = function () {
    ...
    _m_cp
      .FindChildInLayoutFile("id-op-store-get-more-points-btn")
      .GetChild(0)
      .AddClass("op-store-progress__pass-upsell__text-btn-dark")
    _m_cp
      .FindChildInLayoutFile("id-store-pass-upsell-text-btn")
      .GetChild(0)
      .AddClass("op-store-progress__pass-upsell__text-btn-dark")
  ...
  var _CheckUsersOperationStatus = function () {
    ...
    var totalStarsAvailiable =
      OperationUtil.GettotalPointsFromAvailableFromMissions()
  ...
  var _UpdateProgressBar = function (totalStarsAvailiable) {
    ...
    aCoins.forEach(function (coinId, index) {
      ...
      if (!elCoin) {
        var elCoin = $.CreatePanel(
          "Panel",
          elParent,
          "op-progress-coin" + index
        )
      ...
      if (nThreshold > 0) {
        elCoin
          .FindChildInLayoutFile("id-store-progress-bar-progress")
          .SetDialogVariableInt("threshold", elCoin.Data().upgradeThreshold)
        var progress =
          numStarsEarned <= nThreshold ? numStarsEarned : nThreshold
        elCoin
          .FindChildInLayoutFile("id-store-progress-bar-progress")
          .SetDialogVariableInt("progress", progress)
      } else {
    ...
    $.Schedule(0.1, function () {
      ...
      var coinWidth =
        aCoinPanels[0].actuallayoutwidth / aCoinPanels[0].actualuiscale_x
      var barWidth = elParent.actuallayoutwidth / elParent.actualuiscale_x
      var segmentWidth =
        (barWidth - coinWidth * aCoins.length) / totalStarsAvailiable - 1
      var coinLevel = 0
      ...
      if (!_m_placeBarSegments) {
        $.Schedule(0.1, function () {
          elParent.style.width = "fit-children;"
        })
    ...
    aRowTypes.forEach((type, index) => {
      ...
      aRewards.forEach((reward, index) => {
        ...
        if (strSetName === type) {
          ...
          strSetName !== "set_train_2021" &&
          Array.isArray(type) &&
          type.indexOf(reward.containerType) !== -1
        ) {
          aRow.push(reward)
          bHasTitle = false
      ...
      if (bHasTitle) {
        aRewardPanels.forEach((reward) => {
          elRow
            .FindChildInLayoutFile("id-op-store-reward-row-bg")
            .AddBlurPanel(reward)
        })
      }
    })
  ...
  var _AddRewardRow = function (index, bHasTitle) {
    var elContainer = $.GetContextPanel().FindChildInLayoutFile(
      "id-op-store-reward-rows"
    )
  ...
  var _UpdateRowDescPanel = function (elRow, aRow, bHasTitle) {
    ...
    if (setName) {
      elRow
        .FindChildInLayoutFile("id-op-store-reward-row-bg-icon")
        .SetImage("file://{images_econ}/econ/set_icons/" + setName + ".png")
      elRow.FindChildInLayoutFile(
        "id-op-store-reward-row-bg"
      ).style.backgroundImage =
        'url("file://{images}/operations/op11/' + setName + '_row_bg.png");'
      elRow.FindChildInLayoutFile(
        "id-op-store-reward-row-bg"
  ...
  var _AddRewards = function (elRow, aRow) {
    ...
    aRow.forEach((oRewardData, index) => {
      ...
      if (!elContainer.FindChildInLayoutFile("id-op-store-reward-" + index)) {
        elReward = $.CreatePanel(
          "Panel",
          elContainer,
          "id-op-store-reward-" + index
        )
        elReward.BLoadLayoutSnippet("reward-tile")
      } else {
        elReward = elContainer.FindChildInLayoutFile(
          "id-op-store-reward-" + index
        )
      }
  ...
  var _UpdateRewardTile = function (elRow, elReward, oRewardData) {
    var elImagesContainer = elReward.FindChildInLayoutFile(
      "id-op-store-item-tile-images"
    )

    if (oRewardData.containerType === "isCharacterLootlist") {
      for (var i = oRewardData.lootlist.length - 1; i >= 0; i--) {
        var bCreateSeperator = i > 0
        _LoadCharactorImages(
          elImagesContainer,
          oRewardData.lootlist[i],
          i,
          bCreateSeperator
        )
      }

      elImagesContainer.SetHasClass(
        "three-tile",
        oRewardData.lootlist.length === 3
      )
      ...
      oRewardData.containerType === "isStickerLootlist" ||
      oRewardData.containerType === "isGraffitiBox"
    ) {
      ...
      LoadItemImage(
        elImagesContainer,
        oRewardData.itempremium.ids[0],
        "",
        "op-store-item-tile__image center"
      )
      _SetTileBackgroundImage(elImagesContainer, "store_bg_sand", true)
    }

    _SetRewardName(elReward, oRewardData.itempremium.ids[0])
    elReward.FindChildInLayoutFile("id-op-store-item-cost").text =
      oRewardData.points
    elReward.SetPanelEvent(
      "onactivate",
      _OpenInspect.bind(undefined, oRewardData)
    )
  }

  var _LoadCharactorImages = function (
    elImagesContainer,
    rewardId,
    index,
    bCreateSeperator
  ) {
    var elImage = elImagesContainer.FindChildInLayoutFile(
      "id-op-shop-tile-item-image-" + rewardId
    )

    if (!elImage && rewardId) {
      var styleName = "op-store-reward-tile__image-char-" + index
      elImage = LoadItemImage(
        elImagesContainer,
        rewardId,
      ...
      if (bCreateSeperator) {
        $.CreatePanel("Panel", elImagesContainer, "", {
          class: "op-store-reward-tile__image-seperator"
        })
      }
    }
  }
  ...
  var _LoadWeaponImages = function (elParent, oReward) {
    var elCollectionIcon = $.CreatePanel(
      "ItemImage",
      elParent,
      "id-op-store-item-tile__icon",
      {
    ...
    if (!elImage) {
      ...
      elParent.SetPanelEvent("onmouseover", function () {
        InventoryAPI.PrecacheCustomMaterials(oReward.lootlist[0])
      })
    }
  }
  ...
  var _SetMultiItemImages = function (elParent, oReward) {
    ...
    var numItems = oReward.lootlistGroups
      ? oReward.lootlistGroups.length - 1
      : oReward.lootlist.length - 1

    elMultiItemTile.Children().forEach(function (elImage, index) {
      var lootlistIndex = index % 2 === 0 ? numItems - index : 0 + index
      elImage.itemid =
        oReward.lootlist[
          oReward.lootlistGroups
            ? oReward.lootlistGroups[lootlistIndex].idxBegin
            : lootlistIndex
        ]
  ...
  var _SetRarityColor = function (itemTile, color) {
    ...
    itemTile.FindChildInLayoutFile(
      "id-op-store-item-tile-rarity"
    ).style.backgroundColor = color
  }

  var _SetTileBackgroundImage = function (elParent, imageName, bFade = false) {
    var elPanelBg = $.CreatePanel("Image", elParent, "id-op-shop-tile-bg", {
      class: "op-store-item-tile__bg"
    })
    elPanelBg.style.backgroundImage =
      "url( 'file://{images}/operations/op11/" + imageName + ".png' );"
  ...
  var _SetPurchaseBtns = function (totalStarsAvailiable) {
    var elUpSell = $.GetContextPanel().FindChildInLayoutFile(
      "id-store-pass-upsell-text-btn"
    )
    var elGetStars = $.GetContextPanel().FindChildInLayoutFile(
      "id-op-store-get-more-points-btn"
    )
    ...
    elGetStars.SetPanelEvent(
      "onactivate",
      OperationUtil.OpenUpSell.bind(undefined, 0, true)
    )

    var elMissionsProgress = _m_cp.FindChildInLayoutFile(
      "id-op-store-pass-missions-progress"
    )
    elMissionsProgress.visible = bPremiumUser
    elMissionsProgress.SetDialogVariableInt(
      "mission_stars",
      bPremiumUser ? oi.nTierUnlocked : 0
    )
    elMissionsProgress.SetDialogVariableInt("max_stars", totalStarsAvailiable)

    if (bPremiumUser) {
      _m_cp.FindChildInLayoutFile("id-op-store-progress-pass-upsell").visible =
        false
      elGetStars.SetPanelEvent(
        "onactivate",
      ...
      var descString = $.GetContextPanel().FindChildInLayoutFile(
        "id-store-pass-upsell-desc"
      )
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    ...
    OperationStore.OnInventoryUpdate
  )
