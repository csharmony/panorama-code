...
var OperationTiers = (function () {
  var m_tiersPanel =
    $.GetContextPanel().FindChildInLayoutFile("id-op-tiers-panel")
  var m_tiersIsScrolling = false
  var m_scheduleHandler = null
  var m_tiersList = m_tiersPanel.FindChildInLayoutFile("id-tiers-list")
  var m_tiersTitlesSticky = m_tiersPanel.FindChildInLayoutFile(
    "id-tiers-list-header-sticky"
  )
  ...
  var _SetScrollStickyTitleBarThreshold = function () {
    m_stickyTitleBarThreshold =
      m_tiersPanel.FindChildInLayoutFile("id-tiers-list-header").contentheight *
      -1
  }
  ...
  var _UpdateTiersScrolling = function () {
    ...
    if (m_tiersIsScrolling) {
      m_tiersTitlesSticky.SetHasClass(
        "hidden",
        m_tiersList.scrolloffset_y > m_stickyTitleBarThreshold
      )
      m_tiersIsScrolling = false
    }

    m_scheduleHandler = $.Schedule(0.15, _UpdateTiersScrolling)
  }
  ...
  var _OnReadyForDisplay = function () {
    m_scrollEventHandler = $.RegisterEventHandler(
      "Scroll",
      m_tiersList,
      _OnScrollTiers
    )
  ...
  function _UpdateTierTitle(oTiersData) {
    ...
    for (var i = oTiersData.starsEarned; i < oTiersData.totalStars; i++) {
      var rewardId = MissionsAPI.GetSeasonalOperationTrackRewardSchema(
        oTiersData.seasonAccess,
        i,
        "item_name"
      )
      numStarsNeeded++
  ...
  function _GetTiersData(m_nSeasonAccess) {
    ...
    if (oTiersData.seasonAccess < 0 && oTiersData.seasonAccess) return false

    oTiersData.totalStars = MissionsAPI.GetSeasonalOperationTrackRewardsCount(
      oTiersData.seasonAccess
    )
    var numLoopingExtra = MissionsAPI.GetSeasonalOperationLoopingRewardsCount(
      oTiersData.seasonAccess
    )

    var idxOperation = InventoryAPI.GetCacheTypeElementIndexByKey(
      "SeasonalOperations",
      oTiersData.seasonAccess
    )
    if (
      idxOperation != undefined &&
      idxOperation != null &&
      ...
      oTiersData.starsEarned = InventoryAPI.GetCacheTypeElementFieldByIndex(
        "SeasonalOperations",
        idxOperation,
        "tier_unlocked"
      )
  ...
  var _GetTierRewardItems = function (oTiersData) {
    ...
    for (var i = 0; i < oTiersData.totalStars; i++) {
      var sRewardFree = MissionsAPI.GetSeasonalOperationTrackRewardSchema(
        oTiersData.seasonAccess,
        i,
        "item_name_free"
      )
      var sRewardName = MissionsAPI.GetSeasonalOperationTrackRewardSchema(
        oTiersData.seasonAccess,
        i,
        "item_name"
      )
      var sHighlight = MissionsAPI.GetSeasonalOperationTrackRewardSchema(
        oTiersData.seasonAccess,
        i,
        "highlight"
      )
      var sCallout = MissionsAPI.GetSeasonalOperationTrackRewardSchema(
        oTiersData.seasonAccess,
        i,
        "callout"
      )
      ...
      var sIntentionalGap = MissionsAPI.GetSeasonalOperationTrackRewardSchema(
        oTiersData.seasonAccess,
        i,
        "none"
      )
    ...
    nameList.forEach((rewardname) => {
      if (rewardname) {
        ...
        if (rewardname.startsWith("lootlist:")) {
          ...
          var nDefinitionIndex =
            InventoryAPI.GetItemDefinitionIndexFromDefinitionName(rewardname)
          itemidForReward = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
            nDefinitionIndex,
            0
          )
        }
  ...
  var _CreateUpdateItemTilesForRow = function (elRow, list, rewardRowData) {
    var elContainer = elRow.FindChildInLayoutFile(
      "id-tier-rewards-" + list.type
    )
    ...
    if (list.type === "premium") {
      elContainer.SetHasClass(
        "unlocked",
        OperationMain.IsPremium() && rewardRowData.isUnlocked
      )
    } else {
    ...
    if (list.itemIds.length === 1) {
      ...
      elContainer.SetPanelEvent(
        "onactivate",
        OnActivateRadioButton.bind(
          undefined,
          list.itemIds[0],
          list.type,
          rewardRowData,
      ...
      var lootlistItemId =
        InventoryAPI.GetLootListItemsCount(list.itemIds[0]) > 0
          ? ItemInfo.GetLootListItemByIndex(list.itemIds[0], 0)
          : null
      if (lootlistItemId) {
      ...
      list.itemIds.forEach(function (itemid, index) {
        if (index % 2 === 0) {
          elCarouselPage = $.CreatePanel("Panel", elCarousel, "", {
            class: "op-tier__reward-carousel__page"
          })
        }

        var elItem = _CreateItemImage(
          itemid,
          index,
          "RadioButton",
          elCarouselPage
        ...
        elItem
          .FindChildInLayoutFile("id-tier-row-item-selected")
          .SetImage("file://{images}/operations/brackets_half.svg")
        ...
        var lootlistItemId =
          InventoryAPI.GetLootListItemsCount(itemid) > 0
            ? ItemInfo.GetLootListItemByIndex(itemid, 0)
            : null
        if (lootlistItemId) {
  ...
  var _SetRowNumberText = function (elRow, rewardRowData) {
    ...
    elTier.FindChildInLayoutFile("id-tier-row-tier-label").text =
      rewardRowData.idx + 1
  }
  ...
  var _GetIndexesForInbetweenLandmarks = function (specialLandmarksList) {
    ...
    for (var i = 0; i < specialLandmarksList.length; i++) {
      ...
      if (i + 1 < specialLandmarksList.length) {
        ...
        var maxLandmarks = 40
        var percentOfTotalTiers =
          tiersInBetween /
          specialLandmarksList[specialLandmarksList.length - 1].idx
        var numTiersInBetween = maxLandmarks * percentOfTotalTiers
        var inBetweenTiersModulo = Math.ceil(
          numTiersInBetween <= 1
            ? tiersInBetween / 2
            : tiersInBetween / numTiersInBetween
        )
  ...
  var _MakeNavLandmarkPanels = function () {
    var specialLandmarksList = m_allRowsData.filter(
      (row) => row.isSpecialLandmark
    )

    var allLandmarksUpdated = _GetIndexesForInbetweenLandmarks(
      specialLandmarksList.filter((row) => row.idx < 100)
    )
    ...
    allLandmarksUpdated.forEach((element) => {
      ...
      function OnMouseOver(elEntry) {
        var elHighlight = m_tiersPanel.FindChildInLayoutFile(
          "id-tiers-nav-highlight"
        )
        elHighlight.visible = true

        elHighlight.style.y =
          (elEntry.actualyoffset / elEntry.GetParent().actuallayoutheight) *
            100 +
          "%;"
        elHighlight.FindChild("id-tiers-nav-highlight-label").text =
          InventoryAPI.GetItemName(_GetPremiumItemIdFromItemList(element))
      }

      elEntry.SetPanelEvent(
        "onactivate",
        _ScrollToFitRow.bind(undefined, element.idx)
      )
  ...
  var _MoreRewardsBtn = function (specialLandmarksList) {
    ...
    if (!m_elTierNav.FindChildInLayoutFile("tier-nav-more")) {
      elEntry = $.CreatePanel("Button", m_elTierNav, "tier-nav-more", {
        group: "tier-nav"
      })
      elEntry.BLoadLayoutSnippet("snippet-nav-entry")
    } else {
    ...
    elEntry.SetPanelEvent("onactivate", function () {
      var elContextMenu = UiToolkitAPI.ShowCustomLayoutContextMenuParameters(
        "",
        "",
        "file://{resources}/layout/context_menus/context_menu_rewards.xml",
        "callback=" + jsContextMenuCallbackHandle
      )

      elContextMenu.FindChildInLayoutFile("Contents").Data().aRewardsList =
        specialLandmarksList.filter((row) => row.idx > 100)
      elContextMenu.AddClass("ContextMenu_NoArrow")
    })

    var jsContextMenuCallbackHandle =
      UiToolkitAPI.RegisterJSCallback(_ScrollToFitRow)
  ...
  var _ScrollToFitRow = function (idx) {
    ...
    $.Schedule(0.2, function () {
      ...
      if (m_allRowsData[idx].itemLists[0].itemIds[0].length > 1) {
        var elItem = elRow
          .FindChildInLayoutFile("id-tier-rewards-premium")
          .FindChildInLayoutFile("id-tier-reward-0")
        if (m_allRowsData[idx].itemLists[0].itemIds.length > 1) {
          elItem.checked = true
        }
        var elContainer = elRow.FindChildInLayoutFile(
          "id-tier-rewards-" + m_allRowsData[idx].itemLists[0].type
        )
  ...
  var _NavListEvents = function () {
    ...
    elList.SetPanelEvent("onmouseover", function () {
      elHighlight.visible = true
    })
    elList.SetPanelEvent("onmouseout", function () {
      $.Schedule(0.25, _HideHighlighMouseOutsideNav)
    })
  }
...
var OperationInspect = (function () {
  ...
  var _Update = function (rewardId, type, rowData) {
    ...
    _FillLabelAsTypeWriter(
      aLetters,
      m_inspectPanel.FindChildInLayoutFile("id-tiers-inspect-name"),
      0.02,
      true
    )
  ...
  var _MakePips = function (itemsList, rewardId) {
    ...
    itemsList.forEach(function (itemId, index) {
      ...
      elPip.SetPanelEvent(
        "onactivate",
        onActivate.bind(undefined, itemId, rewardId, index, itemsList.length)
      )
      ...
      if (ItemInfo.IsWeapon(itemId)) {
        elPip.SetPanelEvent(
          "onmouseover",
          OperationTiers.CacheWeapon.bind(undefined, itemId)
        )
      }

      if (rarityColor) {
        elPip.FindChildInLayoutFile(
          "id-tier-inspect-pip-dot"
  ...
  var _InitInspectControls = function () {
    m_inspectPanel
      .FindChildInLayoutFile("op-tier__inspect__pips-next")
      .SetPanelEvent("onactivate", _OnNextPreview.bind(undefined, 1))
    m_inspectPanel
      .FindChildInLayoutFile("op-tier__inspect__pips-prev")
      .SetPanelEvent("onactivate", _OnNextPreview.bind(undefined, -1))

    m_inspectPanel
      .FindChildInLayoutFile("op-tier__inspect__pips-next")
      .SetPanelEvent("onmouseover", _NextWeaponPreviewCache.bind(undefined, 1))
    m_inspectPanel
      .FindChildInLayoutFile("op-tier__inspect__pips-prev")
      .SetPanelEvent("onmouseover", _NextWeaponPreviewCache.bind(undefined, -1))
  ...
  function _UpdateNextPrevBtnState(newIndex, maxPips) {
    m_inspectPanel.FindChildInLayoutFile(
      "op-tier__inspect__pips-prev"
    ).enabled = newIndex > 0
    m_inspectPanel.FindChildInLayoutFile(
      "op-tier__inspect__pips-next"
    ).enabled = newIndex < maxPips - 1
  ...
  function onActivate(itemId, rewardId, index, maxPips) {
    ...
    if (bIscase && itemId === "0") {
      ...
      var unusualItemImagePath =
        InventoryAPI.GetLootListUnusualItemImage(rewardId) + ".png"
      ...
      if (model) {
        ...
        if (isCharacter) {
          ...
          elModel.SetCameraAngles(4.81, 178.85, 0.0)
        } else {
    ...
    if (setName || isCharacter) {
      var elSetImage = $.CreatePanel("Image", elParent, "id-tier-inspect-set", {
      ...
      elSetImage.SetImage(
        "file://{images_econ}/econ/set_icons/" + setName + ".png"
      )
  ...
  function _FillLabelAsTypeWriter(
    aLetters,
    elLabel,
    delayforNextLetter,
    bClearLine = false
    ...
    function _AddLetterToLabel(index) {
      ...
      index++
      m_handlerSchedule = $.Schedule(
        delayforNextLetter,
        _AddLetterToLabel.bind(undefined, index)
      )
    }
  }

  var _PlayTransitionEffect = function () {
    var moviePlayer = m_inspectPanel.FindChildInLayoutFile(
      "id-tier-inspect-transition"
    )
    moviePlayer.SetMovie("file://{resources}/videos/digital_glitch.webm")
    moviePlayer.Play()

    var elParent = m_inspectPanel.FindChildInLayoutFile(
      "id-tier-inspect-image-model"
    )
    ...
    if (random <= 2) {
      moviePlayer.style.transform =
        "rotateZ( 90deg ) scaleX( 1.25 ) scaleY( 1.25 );"
    } else {
...
;(function () {
  ...
  $.RegisterEventHandler(
    "ReadyForDisplay",
    elTiers,
    OperationTiers.OnReadyForDisplay
  )
  $.RegisterEventHandler(
    "UnreadyForDisplay",
    elTiers,
    OperationTiers.OnUnreadyForDisplay
  )
})()
