...
var OperationMainMenu = (function () {
  ...
  var _ShowUpSell = function () {
    ...
    var elUpsell = $.GetContextPanel().FindChildInLayoutFile(
      "id-op-mainmenu-upsell"
    )
    elUpsell.AddClass("show")

    var btnPremium = elUpsell.FindChildInLayoutFile(
      "id-op-mainmenu-upsell-store"
    )
    btnPremium.SetPanelEvent(
      "onactivate",
    ...
    btnPremium.text = sUserOwnedOperationPassItemID
      ? "#SFUI_ConfirmBtn_ActivatePassNow"
      : "#op_get_premium"

    elUpsell.FindChildInLayoutFile("id-op-mainmenu-upsell-store-image").itemid =
      sFauxPassItemID

    var elMissions = $.GetContextPanel().FindChildInLayoutFile(
      "id-op-mainmenu-mission_select"
    )
    elMissions.SetPanelEvent("onactivate", onMissionSelect)

    var elPassSaleDiscount = elUpsell.FindChildInLayoutFile(
      "id-op-mainmenu-upsell-store-passsalediscount"
    )
  ...
  var _UpdateSelectedMissionCard = function (cardIndex) {
    ...
    var elLabel = $.GetContextPanel().FindChildInLayoutFile(
      "id-missions-selected-card-name"
    )

    var nWeek = cardIndex + 1
    elLabel.text =
      $.Localize("#op_mainmenu_mission_week_prefix") +
      " " +
      nWeek +
      ": " +
      $.Localize(jsoCardDetails.name)
  ...
  var _OpenMissionCardSelectContextMenu = function () {
    var contextMenuPanel = UiToolkitAPI.ShowCustomLayoutContextMenuParameters(
      ...
      "test=123456&callback=" + jsContextMenuMissionCardCallbackHandle
    )

    contextMenuPanel.AddClass("ContextMenu_NoArrow")
  }

  jsContextMenuMissionCardCallbackHandle = UiToolkitAPI.RegisterJSCallback(
    _UpdateSelectedMissionCard
  )

  var _UpdateMissionCard = function (idx) {
    var elMissionCard = $.GetContextPanel().FindChildInLayoutFile(
      "id-missions-mission-card"
    )

    OperationMissionCard.UpdateMissionCard(parseInt(idx), elMissionCard)
  }
  ...
  var _UpdateXpDisplay = function (oStatus) {
    ...
    if (numNextMissionsCompletedNeededForReward) {
      _m_cp.SetDialogVariableInt(
        "xp_missions_completed",
        oStatus.nMissionsCompleted - numPreviousMissionsCompletedForReward
      )
      _m_cp.SetDialogVariableInt(
        "xp_missions_needed",
        numNextMissionsCompletedNeededForReward -
          numPreviousMissionsCompletedForReward
      )
  ...
  function _OnStatsReceived() {
    ...
    if (
      _m_oLastMatch !== null &&
      _m_oLastMatch.matches[0].player.match_id !=
        oMatch.matches[0].player.match_id
    ) {
  ...
  var _ShowStatsPanel = function (oStatus) {
    ...
    if (!oStatus.bPremiumUser) {
      _m_cp
        .FindChildInLayoutFile("id-op-mainmenu-stats-section")
        .SetHasClass("hide", true)
      _m_cp
        .FindChildInLayoutFile("id-op-mainmenu-stats-upsell")
        .SetHasClass("hide", false)
      _m_cp.FindChildInLayoutFile("id-op-mainmenu-stats-store-image").itemid =
        OperationUtil.GetPassFauxId()
      ...
      _m_cp
        .FindChildInLayoutFile("id-op-mainmenu-stats")
    ...
    var bHasMatchData =
      _m_oLastMatch != null && _m_oLastMatch.matches.length > 0
    elStatPanel.SetHasClass("no-match-data", !bHasMatchData)

    if (!bHasMatchData) return

    var oMatch = _m_oLastMatch.matches[0].player

    var elResult = _m_cp.FindChildTraverse("id-op-mainmenu-stats-match-result")

    elResult.SetHasClass(
      "op-mainmenu__stats__match__win-loss--green",
      (oMatch.match_outcome & 0x3) == 1
    )
    elResult.SetHasClass(
      "op-mainmenu__stats__match__win-loss--red",
      (oMatch.match_outcome & 0x3) == 2
    )
    elResult.SetHasClass(
      "op-mainmenu__stats__match__win-loss--yellow",
      (oMatch.match_outcome & 0x3) == 0
    )

    elResult.SetHasClass(
      "op-mainmenu__stats__match__win-loss--dnf",
      oMatch.match_outcome & 0x4
    )
  ...
  function _UpdateStats(oMatch, mode) {
    var elStatsContainer = _m_cp.FindChildTraverse(
      "id-op-mainmenu-stats-match-stats"
    )

    elStatsContainer.SetDialogVariable(
      "kills",
      MatchStats.GetTotalKills(oMatch)
    )
    elStatsContainer.SetDialogVariable(
      "kdr",
      MatchStats.GetKillsPerDeath(oMatch)
    )
    elStatsContainer.SetDialogVariable(
      "adr",
      MatchStats.GetDamagePerRound(oMatch)
    )
    elStatsContainer.SetDialogVariable("dmg", MatchStats.GetTotalDamage(oMatch))

    elStatsContainer.SetDialogVariable(
      "hsp",
      MatchStats.GetHeadShotKillRate(oMatch) + "%"
    )
    elStatsContainer.SetDialogVariable(
      "mk",
      MatchStats.GetTotalMultiKills(oMatch)
    )
  }

  var _MapInfo = function (mapName) {
    var elMapIcon = _m_cp.FindChildInLayoutFile(
      "id-op-mainmenu-stats-match-map-icon"
    )
    elMapIcon.SetImage("file://{images}/map_icons/map_icon_" + mapName + ".svg")
    _m_cp.FindChildInLayoutFile("id-op-mainmenu-stats-match-map-name").text =
      $.Localize("SFUI_Map_" + mapName)

    var elBackground = _m_cp.FindChildInLayoutFile(
      "id-op-mainmenu-stats-background"
    )
    elBackground.style.backgroundImage =
      'url("file://{images}/map_icons/screenshots/360p/' + mapName + '.png")'
  ...
  var _MatchTimeAndMode = function (time, mode) {
    var elMapTime = _m_cp.FindChildInLayoutFile(
      "id-op-mainmenu-stats-match-map-time"
    )
  ...
  var _UpdateTeammates = function (arrMates) {
    var elMates = _m_cp.FindChildInLayoutFile(
      "id-op-mainmenu-stats-match-teammates"
    )
...
;(function () {
  $.RegisterForUnhandledEvent(
    "CSGOShowMainMenu",
    OperationMainMenu.ShowMainMenu
  )
  $.RegisterForUnhandledEvent(
    "CSGOHideMainMenu",
    OperationMainMenu.HideMainMenu
  )
