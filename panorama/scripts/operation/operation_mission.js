...
var OperationMission = (function () {
  ...
  var _LoadMissonSnippet = function (elContainer, oMissionDetails) {
    var elMission = $.CreatePanel(
      "Button",
      elContainer,
      oMissionDetails.missionId
    )

    elMission.BLoadLayout(
      "file://{resources}/layout/operation/operation_mission_snippets.xml",
      false,
      false
    )
    return elMission
  ...
  var _UpdateMissionDisplay = function (
    elMission,
    oMissionDetails,
    isunlocked,
    missionCardId = null,
    ...
    var elParent = elMission.FindChildInLayoutFile(
      "id-mission-segments-container"
    )

    if (oMissionDetails.missonType === "or") {
      var oCompletedOrQuest = oMissionDetails.aSubQuests.filter(
        (element) => element.nsubQuestPointsRemaining === 0
      )[0]

      var uncommitedPointsMeetGoal = false
      if (oCompletedOrQuest) {
        var elContainer = LoadSnippetByType(
          elParent,
          missionPanelId,
          "snippet-mission-segment-container"
        )
        _UpdateMissionDesc(
          elMission,
          oCompletedOrQuest.missionId,
          (bHideDesc = false)
        )
        _UpdateSegmentsMissionTypes(
          oMissionDetails,
          elContainer,
        ...
        for (var i = 0; i < oMissionDetails.aSubQuests.length; i++) {
          if (
            !uncommitedPointsMeetGoal &&
            oMissionDetails.aSubQuests[i].nUncommitted >=
              oMissionDetails.aSubQuests[i].nGoal
          ) {
            uncommitedPointsMeetGoal = true
          }
          _UpdateOrTypeMission(
            oMissionDetails,
            oMissionDetails.aSegmentsData[0],
            elParent,
            i
        ...
        var aStars = elParent
          .FindChildInLayoutFile("id-mission-segments-stars")
          .Children()
      ...
      for (var i = 0; i < oMissionDetails.nMissionSegments; i++) {
        _UpdateSegmentsMissionTypes(
          oMissionDetails,
          elContainer,
          oMissionDetails.aSegmentsData[i],
          null,
          i
        )
      }
    ...
    elMission.SetHasClass(
      "complete",
      oMissionDetails.nMissionPointsRemaining === 0 && isunlocked
    )
    elMission.SetHasClass(
      "hidebar",
      MatchStatsAPI.GetGameMode() === "cooperative" &&
        GameStateAPI.IsLocalPlayerPlayingMatch()
    )

    MissionsAPI.ApplyQuestDialogVarsToPanelJS(
      Number(oMissionDetails.missionItemId),
      elMission
    )
  }

  var _UpdateOrTypeMission = function (
    oMissionDetails,
    oSegmentData,
    elParent,
    index
    ...
    var elContainer = LoadSnippetByType(
      elParent,
      _m_missionPrefix + oSubQuestData.missionId,
      "snippet-mission-segment-container-or-type"
    )
    ...
    var segmentPanelId =
      _m_missionPrefix + oSubQuestData.missionId + "_segment" + index
    _CreateBars(
      elContainer.FindChildInLayoutFile("id-mission-bar"),
      oSegmentData,
      oSubQuestData,
      oMissionDetails,
      segmentPanelId
    ...
    var nEarnedDisplay = _GetTotalPointsEarned(
      oMissionDetails,
      oSegmentData,
      oSubQuestData
    )
    var elCount = _CreateSectionCount(
      elContainer.FindChildInLayoutFile("id-mission-bar"),
      oSubQuestData.nGoal,
      nEarnedDisplay
    )
    var isMissionCompleteWithUncommittedPoints =
      nEarnedDisplay >= oSubQuestData.nGoal &&
      oMissionDetails.nMissionPointsRemaining > 0
    ...
    if (index === 0) {
      var starsPanelId =
        _m_missionPrefix + "sudquest-stars" + oSubQuestData.missionId
      var elStarsContainer = LoadSnippetByType(
        elParent,
        starsPanelId,
        "snippet-mission-stars-container-or-type"
      )

      for (var k = 0; k < oMissionDetails.nOpPointsPerSegment; k++) {
        var starPanelId =
          _m_missionPrefix + oSubQuestData.missionId + "_star" + k
  ...
  var _UpdateSegmentsMissionTypes = function (
    oMissionDetails,
    elContainer,
    oSegmentData,
    oSubQuestData,
    ...
    var segmentPanelId =
      _m_missionPrefix + oMissionDetails.missionId + "_segment" + index
    _CreateBars(
      elContainer,
      oSegmentData,
      oSubQuestData,
    ...
    if (oMissionDetails.nMissionSegments === 1) {
      var nGoal =
        oMissionDetails.missonType === "or"
          ? oSubQuestData.nGoal
          : oSegmentData.nGoal
      var elCount = _CreateSectionCount(elContainer, nGoal, nEarnedDisplay)
      elCount.SetHasClass(
        "uncommitted",
        oMissionDetails.nUncommitted + oSegmentData.nEarned >=
          oSegmentData.nGoal && !oSegmentData.isComplete
      )
    }
  ...
  var _GetTotalPointsEarned = function (
    oMissionDetails,
    oSegmentData,
    oSubQuestData
  ) {
    ...
    var oData =
      oMissionDetails.missonType === "or" ? oSubQuestData : oSegmentData
    var nUncommitted =
      oMissionDetails.missonType === "or"
        ? oSubQuestData.nUncommitted
        : oMissionDetails.nUncommitted
  ...
  var _HudIncompleteSubMissions = function (elMission, aIncompleteMissions) {
    var elContainer = elMission.FindChildInLayoutFile(
      "id-snippet-hud-next-subquests"
    )

    if (aIncompleteMissions.length > 0) {
      for (var i = 0; i < aIncompleteMissions.length; i++) {
        var elSegment = LoadSnippetByType(
          elContainer,
          aIncompleteMissions[i].missionId,
          "snippet-hud-next-subquest"
        )
        OperationUtil.SetLocalizationStringAndVarsForMission(
          elSegment.FindChildInLayoutFile("id-subquest-desc"),
          Number(aIncompleteMissions[i].missionId),
          "loc_description"
        )
      }
    }
    ...
    aChildren.forEach((element) => {
      var nUncommitted = MissionsAPI.GetQuestPoints(
        Number(element.id),
        "uncommitted"
      )
      ...
      element.FindChildInLayoutFile("id-subquest-progress").visible =
        nUncommitted > 0 && nGoal > 1
      ...
      if (nUncommitted >= nGoal) {
        ...
        $.Schedule(1.5, function () {
          element.AddClass("hide-submission")
        })
      }
    })
  }
  ...
  var _UpdateMissionName = function (elMission, missionItemId) {
    elMission.FindChildInLayoutFile("id-mission-name").text =
      InventoryAPI.GetItemName(missionItemId)
  }
  ...
  var _UpdateMissionIcon = function (elMission, isunlocked, oMissionDetails) {
    ...
    elIcon.SetImage(
      "file://{images}/icons/ui/" + oMissionDetails.missionGameMode + ".svg"
    )
    elIcon.SetHasClass(
      "nocolor",
    ...
    elMission.FindChildInLayoutFile("id-mission-card-icon-locked").visible =
      !isunlocked
    elMission.FindChildInLayoutFile("id-mission-card-icon-complete").visible =
      !oMissionDetails.isReplayable &&
      oMissionDetails.nMissionPointsRemaining === 0 &&
      isunlocked
    elMission.FindChildInLayoutFile("id-mission-card-spinner").visble =
      isunlocked && currentlyPlayingMissionId === oMissionDetails.missionItemId
  ...
  var _EnableDisableMission = function (
    elMission,
    isunlocked,
    oMissionDetails
  ) {
  ...
  var _AddMissionActions = function (
    elMission,
    oMissionDetails,
    missionCardId
  ) {
    elMission.SetPanelEvent(
      "onactivate",
    ...
    if (
      ...
      oMissionDetails.missonType === "sequential"
    ) {
      elMission.SetPanelEvent(
        "onmouseover",
        _ShowMissionTooltip.bind(undefined, elMission, oMissionDetails)
      )
  ...
  var _CreateBars = function (
    ...
    var isSingleBar =
      oMissionDetails.nMissionSegments === 1 ||
      oMissionDetails.missonType === "or"
        ? true
        : false
    ...
    for (var i = 0; i < nSegments; i++) {
      ...
      var elBar = LoadSnippetByType(
        elContainer,
        segmentSubMissionPanelId,
        "snippet-mission-segment"
      )
      elBar.SetHasClass(
        "op_mission-card__mission__bar-container-seq",
        oMissionDetails.missonType === "sequential"
      )
      elBar.SetHasClass(
      ...
      if (oMissionDetails.missonType === "or") {
        ...
        var oSubQuest =
          oMissionDetails.aSubQuests[oSegmentData.nPreviousGoal + i]
        nPercentComplete = oSubQuest.nPercentComplete

        nPercentCompleteUncommitted =
          oSubQuest.nUncommitted < oSubQuest.nGoal ? 0 : 100
      }

      elBar.FindChildInLayoutFile("id-mission-card-bar").style.width =
        nPercentComplete + "%;"
      elBar.FindChildInLayoutFile(
        "id-mission-card-bar-uncommitted"
      ).style.width = nPercentCompleteUncommitted + "%;"
    }
  }

  var _CreateMissionStar = function (
    elContainer,
    oSegmentData,
    oMissionDetails,
    starPanelId,
    ...
    var elStar = LoadSnippetByType(
      elContainer,
      starPanelId,
      "snippet-mission-star"
    )
  ...
  var _CreateSectionCount = function (elContainer, nGoal, nEarnedDisplay) {
    ...
    if (!elContainer.FindChildInLayoutFile("id-section-count")) {
      elSectionCount = $.CreatePanel("Label", elContainer, "id-section-count", {
  ...
  var LoadSnippetByType = function (elContainer, snippetPanelId, snippetName) {
    ...
    if (!elContainer.FindChildInLayoutFile(snippetPanelId)) {
      elSnippet = $.CreatePanel("Panel", elContainer, snippetPanelId)
  ...
  var _SetMissionOnActivate = function (
    missionCardId,
    MissionItemID,
    nSeasonAccess
  ) {
    ...
    if (
      OperationUtil.IsMissionLockedBehindPremiumOperationPass(
        missionCardId,
        MissionItemID,
        nSeasonAccess
      )
    ...
    UiToolkitAPI.ShowCustomLayoutPopupParameters(
      ...
      "message=" +
        $.Localize("#op_mission_activate") +
        "&" +
        "requestedMissonCardId=" +
        missionCardId +
        "&" +
        "seasonAccess=" +
        nSeasonAccess +
  ...
  var _ShowMissionTooltip = function (elMission, oMissionDetails) {
    ...
    UiToolkitAPI.ShowCustomLayoutParametersTooltip(
      ...
      "type=" +
        oMissionDetails.missonType +
        "&" +
        "gamemode=" +
        oMissionDetails.missionGameMode +
        "&" +
        "mission-id=" +
        oMissionDetails.missionId +
        "&" +
