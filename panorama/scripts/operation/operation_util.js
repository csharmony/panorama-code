...
var OperationUtil = (function () {
  ...
  var _ValidateOperationInfo = function (nSeasonAccess) {
    ...
    if (
      nSeasonAccess < 0 ||
      nSeasonAccess === null ||
      nSeasonAccess === undefined
    )
      return false

    m_nSeasonAccess = nSeasonAccess

    m_nCoinRank = MyPersonaAPI.GetMyMedalRankByType(
      m_nSeasonAccess + 1 + "Operation$OperationCoin"
    )

    m_bPrime = PartyListAPI.GetFriendPrimeEligible(MyPersonaAPI.GetXuid())
    m_nRewardsCount =
      MissionsAPI.GetSeasonalOperationTrackRewardsCount(m_nSeasonAccess)
    m_nRedeemableGoodsCount =
      MissionsAPI.GetSeasonalOperationRedeemableGoodsCount(m_nSeasonAccess)
    m_nLoopingRewardsCount =
      MissionsAPI.GetSeasonalOperationLoopingRewardsCount(m_nSeasonAccess)
    m_numMissionsRewardThresholds =
      MissionsAPI.GetSeasonalOperationXpRewardsThresholds(m_nSeasonAccess)
    ...
    if (
      ...
      m_numMissionsCompleted = InventoryAPI.GetCacheTypeElementFieldByIndex(
        "SeasonalOperations",
        idxOperation,
        "missions_completed"
      )
      m_numTierUnlocked = InventoryAPI.GetCacheTypeElementFieldByIndex(
        "SeasonalOperations",
        idxOperation,
        "tier_unlocked"
      )
      if (m_nRedeemableGoodsCount && m_nRedeemableGoodsCount > 0) {
        var spt = InventoryAPI.GetCacheTypeElementFieldByIndex(
          "SeasonalOperations",
          idxOperation,
          "season_pass_time"
        )
      ...
      m_nActiveCardIndex =
        MissionsAPI.GetSeasonalOperationMissionCardActiveIdx(m_nSeasonAccess)
      m_numRedeemableBalance = InventoryAPI.GetCacheTypeElementFieldByIndex(
        "SeasonalOperations",
        idxOperation,
        "redeemable_balance"
      )
  ...
  var _GetObjValue = function (bHasStoreItems, rewardIndex, item) {
    ...
    if (bHasStoreItems) {
      data = MissionsAPI.GetSeasonalOperationRedeemableGoodsSchema(
        m_nSeasonAccess,
        rewardIndex,
        item.value
      )
    } else {
      data = MissionsAPI.GetSeasonalOperationTrackRewardSchema(
        m_nSeasonAccess,
        rewardIndex,
        item.value
      )
  ...
  var _GetRewardsData = function () {
    ...
    var nRewardsCount = bHasStoreItems
      ? m_nRedeemableGoodsCount
      : m_nRewardsCount
    ...
    for (var i = 0; i < nRewardsCount; i++) {
          ...
          if (reward) {
            ...
            if (reward.startsWith("lootlist:")) {
              ...
              var nDefinitionIndex =
                InventoryAPI.GetItemDefinitionIndexFromDefinitionName(reward)
      ...
      if (_rewardData.RewardItemGroups) {
        ...
        var numGroups =
          parseInt(_rewardData.RewardItemGroups.substr(posUnderscore + 1)) + 1
        var strBaseLootlistName = _rewardData.RewardItemGroups.substr(
          0,
          posUnderscore + 1
        )
        ...
        for (var k = 0; k < numGroups; ++k) {
          ...
          if (moreItems.length > 0) {
            ...
            _rewardData.lootlistGroups.push({
              idxBegin: idxFistGroupElement,
              idxEnd: _rewardData.lootlist.length
            })
          }
        }
      } else {
        _rewardData.lootlist = _GetLootListForReward(
          _rewardData.itempremium.ids[0]
        )
      }
  ...
  var _GetLootListForReward = function (rewardId) {
    ...
    if (!count) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        if (
          InventoryAPI.DoesItemMatchDefinitionByName(itemId, "spraypaint") ||
          InventoryAPI.DoesItemMatchDefinitionByName(itemId, "spray")
        ) {
  ...
  var _GettotalPointsFromAvailableFromMissions = function () {
    ...
    var cardCount =
      MissionsAPI.GetSeasonalOperationMissionCardsCount(m_nSeasonAccess)
  ...
  var _GetMissionDetails = function (missionId) {
    ...
    if (numGraphCount > 0) {
      oMissionDetails.aSubQuests = _UpdateSubQuestData(
        Number(missionId),
        numGraphCount,
        oMissionDetails.missonType === "checklist"
      )
    }
  ...
  var _UpdateMissionDetailsObject = function (missionId) {
    ...
    return {
      ...
      missionDesc: MissionsAPI.GetQuestDefinitionField(
        missionId,
        "loc_description"
      ),
      nMissionSegments: MissionsAPI.GetQuestPoints(missionId, "count"),
      nMissionPointsRemaining: MissionsAPI.GetQuestPoints(
        missionId,
        "remaining"
      ),
      nOpPointsPerSegment: MissionsAPI.GetQuestDefinitionField(
        missionId,
        "operational_points"
      ),
      isReplayable: gameMode === "cooperative" || gameMode === "coopmission",
      isSingleMatch:
        MissionsAPI.GetQuestDefinitionField(missionId, "singlematch") === "1"
          ? true
          : false,
      ...
      missonType:
        numQuestGraphType === 1
          ? "sequential"
          : numQuestGraphType === 2 && missionGoal > 1
            ? "checklist"
            : numQuestGraphType === 2 && missionGoal === 1
              ? "or"
              : numQuestGraphType === 0
  ...
  var _SetLocalizationStringAndVarsForMission = function (
    elMissionPanel,
    nQuestID,
    strSchemaField
  ) {
  ...
  var _UpdateSegmentData = function (oMissionDetails) {
    ...
    for (var i = oMissionDetails.nMissionSegments; i-- > 0; ) {
      var nGoal = MissionsAPI.GetQuestPoints(
        oMissionDetails.missionId,
        "goal" + i
      )
      var nEarned =
        oMissionDetails.missionGoal - oMissionDetails.nMissionPointsRemaining
      ...
      var progressPercent =
        (nSegmentEarned / nSegmentIncrementalGoalDelta) * 100
      var oSegmentData = {}
  ...
  var _UpdateSubQuestData = function (missionId, numGraphCount, isChecklist) {
    ...
    for (var i = 0; i < numGraphCount; ++i) {
      ...
      var oData = {
        ...
        isComplete: nRemaining === 0 ? true : nEarned === nGoal,
        nPercentComplete: nRemaining === 0 ? 100 : (nEarned / nGoal) * 100,
        nPercentCompleteUncommitted:
          nUncommitted === 0 || !nUncommitted ? 0 : (nUncommitted / nGoal) * 100
      }
    ...
    if (isChecklist) {
      return aSubQuests.sort((a, b) => {
        return (
          b.nPercentComplete - a.nPercentComplete ||
          b.nPercentCompleteUncommitted - a.nPercentCompleteUncommitted
        )
      })
    }
    ...
    for (var iMission = 0; iMission < oCardDetails.quests.length; iMission++) {
      ...
      if (remaining > 0) {
        ...
        for (var i = 0; i < numLoops; ++i) {
          if (
            goal - remaining <
            MissionsAPI.GetQuestPoints(missionID, "goal" + i)
          )
            --numThresholds
          else break
        }
      ...
      totalPossilbePoints +=
        MissionsAPI.GetQuestDefinitionField(missionID, "operational_points") *
        MissionsAPI.GetQuestPoints(missionID, "count")
    }

    var oPoints = {
      totalCardPoints: totalCardPoints,
      totalCardPointsDisplay:
        totalCardPoints > oCardDetails.operational_points
          ? oCardDetails.operational_points
          : totalCardPoints,
  ...
  function _IfOperationEndedGetExtendedSeasonWithRedeemableBalance(
    bAlwaysShowOperationEndedMessageBox
  ) {
    ...
    if (bAlwaysShowOperationEndedMessageBox) {
      ...
      if (m_numRedeemableBalance <= 0) nActiveSeason = -1
    }

    if (nActiveSeason < 0)
      UiToolkitAPI.ShowGenericPopup(
        "#op_stars_shop_title",
        "#op_stars_shop_operation_over",
        ""
      )
  ...
  function _OpenUpSell(starsNeeded = 0, bForceOpenStarsPurchase = false) {
    function _OpenStarStore() {
      ...
      if (oOldStarsActivate.ids.length > 0) {
        elPopup.SetAttributeString(
          "oldstarstoactivate",
          oOldStarsActivate.ids.join(",")
        )
        elPopup.SetAttributeInt(
          "oldstarstoactivatevalue",
          oOldStarsActivate.value
        )
      }
    }
  ...
  var _UpdateOldStars = function () {
    var oDefNames = [{}, {}, {}]
    oDefNames[0] = {
      def: InventoryAPI.GetItemDefinitionName(
        InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(m_aStarDefIndexes[0], 0)
      ),
      value: 1
    }
    oDefNames[1] = {
      def: InventoryAPI.GetItemDefinitionName(
        InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(m_aStarDefIndexes[1], 0)
      ),
      value: 10
    }
    oDefNames[2] = {
      def: InventoryAPI.GetItemDefinitionName(
        InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(m_aStarDefIndexes[2], 0)
      ),
      value: 100
    }
  ...
  var _MissionsThatMatchYourMatchMakingSettings = function (
    SessionGameMode,
    sessionMaps,
    nSeasonAccess
  ) {
  ...
  var _GetMatchingMission = function (
    idx,
    jsoCardDetails,
    SessionGameMode,
    sessionMaps
    ...
    for (
      var iMission = 0;
      iMission < jsoCardDetails.quests.length;
      ++iMission
    ) {
      var MissionItemID = InventoryAPI.GetQuestItemIDFromQuestID(
        Number(jsoCardDetails.quests[iMission])
      )
      var gameMode = InventoryAPI.GetQuestGameMode(MissionItemID)
      var mapGroup = InventoryAPI.GetQuestMapGroup(MissionItemID)
      ...
      if (
        ...
        MissionsAPI.GetQuestPoints(
          jsoCardDetails.quests[iMission],
          "remaining"
        ) === 0
      ) {
  ...
  var _IsMissionLockedBehindPremiumOperationPass = function (
    missionCardId,
    MissionItemID,
    nSeasonAccess
  ) {
    ...
    if (mapGroup !== "mg_lobby_mapveto") return false

    if (_ValidateOperationInfo(nSeasonAccess) && m_bPremiumUser) return false

    return true
  }

  var _HasMatchtingMapGroup = function (sessionMaps, mapGroup) {
    return sessionMaps.filter((element) => mapGroup.includes(element)).length >
      0
      ? true
      : false
  }
  ...
  var _GetQuestGameElements = function (questID) {
    return MissionsAPI.GetQuestGameElements(questID).map(
      (elem) => gameElementDetails[elem]
    )
  }

  var _HasStoreItems = function () {
    return m_nRedeemableGoodsCount > 0 &&
      m_nRedeemableGoodsCount !== null &&
      m_nRedeemableGoodsCount != undefined
      ? true
      : false
  }
  ...
  return {
    ...
    IsMissionLockedBehindPremiumOperationPass:
      _IsMissionLockedBehindPremiumOperationPass,
    ...
    GettotalPointsFromAvailableFromMissions:
      _GettotalPointsFromAvailableFromMissions,
    MissionsThatMatchYourMatchMakingSettings:
      _MissionsThatMatchYourMatchMakingSettings,
    BlurMenu: _BlurMenu,
    UnblurMenu: _UnblurMenu,
    SetLocalizationStringAndVarsForMission:
      _SetLocalizationStringAndVarsForMission,
