...
var OperationStoreInspect = (function () {
  ...
  var _m_modelContainer = _m_cp.FindChildInLayoutFile(
    "id-op-inspect-models-container"
  )
  var _m_recievedRewardId = ""

  var _Init = function () {
    _m_cp
      .FindChildInLayoutFile("id-op-store-inspect-get-more-points-btn_label")
      .AddClass("op-store-progress__pass-upsell__text-btn")
    _m_cp
      .FindChildInLayoutFile("id-op-store-inspect-get-reward-btn_label")
      .AddClass("op-store-progress__pass-upsell__text-btn")
    _m_cp
      .FindChildInLayoutFile("id-op-store-inspect-get-reward-btn_label")
      .AddClass("op-store-progress__pass-upsell__text-btn--fit-children")

    _m_cp.FindChildInLayoutFile("id-store-inspect-loading").AddClass("hide")

    _m_nSeasonAccess = Number(
      $.GetContextPanel().GetAttributeString("seasonaccess", null)
    )
    ...
    if (
      !OperationUtil.ValidateCoinAndSeasonIndex(
        _m_nSeasonAccess,
        oStatus.nCoinRank
      )
    ) {
    ...
    var oReward = aRewards.find(
      (element) =>
        element.itempremium.ids[0] ===
        $.GetContextPanel().GetAttributeString("rewardid", null)
    )
    if (!oReward) {
      return
    }
    ...
    if (oReward.lootlistGroups) _m_aLootlistGroups = oReward.lootlistGroups
    ...
    if (_m_rewardtype === "isCharacterLootlist") {
      ...
      _m_cp
        .FindChildInLayoutFile("id-op-inspect-rewards-tray-container")
        .SetHasClass("has-model-panel", true)
      _m_cp.FindChildInLayoutFile("id-op-store-inspect-floor").visible = true
    } else if (
      _m_rewardtype === "isWeaponLootlist" ||
      _m_rewardtype === "isStickerLootlist" ||
      _m_rewardtype === "isGraffitiBox"
    ) {
      _SetupItemTray()
      _m_modelContainer.SetHasClass("narrow", _m_rewardtype !== "isGraffitiBox")
      _m_modelContainer.SetHasClass(
        "graffiti",
        _m_rewardtype == "isGraffitiBox"
      )
      _m_cp.SetHasClass("single-line", oReward.lootlist.length <= 8)
  ...
  var _SetUpCharModels = function () {
    ...
    var numParentWidth =
      _m_modelContainer.actuallayoutwidth / _m_modelContainer.actualuiscale_x

    var sizePerPanel = Math.floor(numParentWidth / numRewards)
    var aPosModels = []

    _m_aLootist.forEach(function (rewardId, index) {
      var elPlayerModel = _m_modelContainer.FindChildInLayoutFile(
        "id-op-shop-tile-char-model" + index
      )
      if (!elPlayerModel) {
        elPlayerModel = $.CreatePanel(
          "ItemPreviewPanel",
          _m_modelContainer,
          "id-op-store-inspect-char-model" + index
        )
        elPlayerModel.BLoadLayoutSnippet("snippet-model-char")

        var model = ItemInfo.GetModelPathFromJSONOrAPI(rewardId)
        elPlayerModel.SetScene(
        ...
        var elNameLabel = elPlayerModel.FindChildInLayoutFile(
          "id-store-inspect-item-name"
        )
        _UpdateItemName(elNameLabel, rewardId)

        var elPanel = elPlayerModel.FindChildInLayoutFile(
          "id-store-inspect-item-rarity"
        )
        _UpdateItemRarityColor(elPanel, rewardId)

        aPosModels.push(
          _PlaceCharsEvenlyInPanel(elPlayerModel, sizePerPanel, index)
        )
      }
    })
    ...
    $.Schedule(
      0.1,
      _CenterCharsInPanel.bind(undefined, numParentWidth, aPosModels)
    )
  }

  var _MakeCharButton = function (elParent, elPlayerModel, settings) {
    var elPlayerButton = elPlayerModel.FindChildInLayoutFile(
      "id-op-store-inspect-char-button"
    )

    elPlayerButton.SetPanelEvent(
      "onactivate",
      _OnActivateOpenInSeperateInspect.bind(undefined, elPlayerModel.rewardId)
    )
    elPlayerButton.SetPanelEvent("onmouseover", function () {
      elPlayerModel.SetDirectionalLightModify(0)
      elPlayerModel.SetDirectionalLightColor(7.0, 3.96, 2.07)

      $.DispatchEvent(
        "PlaySoundEffect",
        "submenu_dropdown_option_select",
        "MOUSE"
      )
    })
  ...
  var _UpdateCheerBtn = function (elPlayerModel, settings) {
    var elCheerBtn = elPlayerModel.FindChildInLayoutFile(
      "id-op-store-inspect__char-cheer"
    )
  ...
  var _CenterCharsInPanel = function (numParentWidth, aPosModels) {
    var numChildWidth =
      _m_modelContainer.Children()[0].actuallayoutwidth /
      _m_modelContainer.actualuiscale_x
  ...
  var _SetupItemTray = function () {
    var elParent = _m_cp.FindChildInLayoutFile(
      "id-op-inspect-rewards-tray-container"
    )
    ...
    if (_m_aLootlistGroups) {
      _m_aLootlistGroups.forEach(function (grp, index) {
        ...
        elGroup.SetPanelEvent(
          "onactivate",
          _PlaceGroupExpandedPanel.bind(undefined, elGroup, grp)
        )
  ...
  var _PlaceGroupExpandedPanel = function (elGroup, grp) {
    var elGroupExpanded = _m_cp.FindChildInLayoutFile(
      "id-lootlist-grp-expanded"
    )
    ...
    var elParent = _m_cp.FindChildInLayoutFile(
      "id-op-inspect-all-models-container-group-expanded"
    )
    ...
    $.Schedule(0.1, function () {
      var tileWidth =
        elGroupExpanded.Children()[0].actuallayoutwidth /
        elGroupExpanded.actualuiscale_x
  ...
  var _RemoveGroupExpandedPanel = function (elGroupExpanded) {
    ...
    _m_cp
      .FindChildInLayoutFile("id-lootlist-grp-expanded")
  ...
  var fnMakeItemTrayButton = function (rewardId, elParent) {
    ...
    if (rewardId !== "0") {
      ...
      elThumbnail.SetPanelEvent(
        "onactivate",
        _OnActivateOpenInSeperateInspect.bind(undefined, rewardId)
      )
      elThumbnail.SetPanelEvent(
        "onmouseover",
        _CacheWeapon.bind(undefined, rewardId)
      )
      _UpdateItemRarityColor(
        elThumbnail.FindChildInLayoutFile(
          "id-store-inspect-rewards-tray-rarity"
        ),
        rewardId
      )
  ...
  var _SetUpWeaponCaseModel = function () {
    ...
    elModelPanel.FindChildInLayoutFile("id-store-inspect-item-name").text =
      $.Localize("#SFUI_InvUse_Items_InContainer_Header")
    elModelPanel.FindChildInLayoutFile("id-store-inspect-item-rarity").visible =
      false
  }
  ...
  var MakeThumbnailImageForTray = function (elParent, rewardId) {
    ...
    var elImage = elThumbnail.FindChildInLayoutFile(
      "id-op-store-inspect-rewards-tray-image"
    )

    if (rewardId === "0") {
      var unusualItemImagePath =
        InventoryAPI.GetLootListUnusualItemImage(_m_rewardId) + ".png"
  ...
  var _UpdateTitle = function () {
    _m_cp.FindChildInLayoutFile("id-op-inspect-title").text =
      InventoryAPI.GetItemName(_m_rewardId)
  ...
  var _UpdateBtns = function () {
    ...
    _m_cp.SetDialogVariableInt(
      "your_stars",
      OperationUtil.GetOperationInfo().nRedeemableBalance
    )
    _m_cp.SetDialogVariable("cost_stars", _m_rewardCost)
    _m_cp.SetDialogVariable("item_name", ItemInfo.GetName(_m_rewardId))

    if (_m_rewardFlags) {
      var elFlagsNotice = _m_cp.FindChildInLayoutFile(
        "id-op-inspect-shop-get-confirm-flags-notice-" + _m_rewardFlags
      )
      if (elFlagsNotice) {
    ...
    var bSingularReward =
      _m_rewardtype === "isWeaponsCase" ||
      (_m_rewardtype === "isCharacterLootlist" && _m_aLootist.length === 1)

    var elRewardGetDesc = _m_cp.FindChildInLayoutFile(
      "id-op-store-inspect-get-reward-text"
    )
    elRewardGetDesc.text = bSingularReward
      ? $.Localize("#op_coin_get_reward_case", _m_cp)
      : $.Localize("#op_coin_get_reward_item", _m_cp)

    var elCostBtn = _m_cp.FindChildInLayoutFile(
      "id-op-store-inspect-get-reward-btn"
    )

    elCostBtn.enabled =
      _m_rewardCost <= OperationUtil.GetOperationInfo().nRedeemableBalance &&
      OperationUtil.GetOperationInfo().bPremiumUser

    elCostBtn.SetPanelEvent(
      "onactivate",
      OnActivateShowHideConfirm.bind(undefined, true)
    )

    var elGetStars = _m_cp.FindChildInLayoutFile(
      "id-op-store-inspect-get-more-points-btn_label"
    )
    if (!OperationUtil.GetOperationInfo().bPremiumUser) {
      elGetStars.text = $.Localize("#op_pass_upsell")

      _m_cp
        .FindChildInLayoutFile("id-op-store-inspect-get-more-points-btn")
        .SetPanelEvent("onactivate", OperationUtil.OpenUpSell.bind(undefined))
    } else {
      var starsNeeded =
        _m_rewardCost - OperationUtil.GetOperationInfo().nRedeemableBalance
      if (starsNeeded > 0) {
    ...
    _m_cp
      .FindChildInLayoutFile("id-op-inspect-shop-get-reward-btn_cancel")
      .SetPanelEvent("onactivate", function () {
        $.DispatchEvent("UIPopupButtonClicked", "")
      })

    _m_cp
      .FindChildInLayoutFile("id-op-inspect-shop-get-confirm-btn_label")
      .SetPanelEvent("onactivate", OnActivateConfirmReward)

    _m_cp
      .FindChildInLayoutFile("id-op-inspect-shop-get-confirm-btn_cancel")
      .SetPanelEvent(
        "onactivate",
        OnActivateShowHideConfirm.bind(undefined, false)
      )

    _m_cp.FindChildInLayoutFile(
      "id-op-inspect-shop-get-confirm-desc-label"
    ).text = bSingularReward
  ...
  var OnActivateShowHideConfirm = function (bshow) {
    ...
    var elStarsRow = _m_cp.FindChildInLayoutFile(
      "id-op-inspect-shop-balance-panel"
    )
    elCostRow.SetHasClass("hide", bshow)
    elStarsRow.SetHasClass("hide", bshow)

    var elConfrimRow = _m_cp.FindChildInLayoutFile(
      "id-op-inspect-shop-get-confirm"
    )
    elConfrimRow.SetHasClass("hide", !bshow)

    _m_cp
      .FindChildInLayoutFile("id-op-inspect-all-models-container")
      .SetHasClass("in-confirm-mode", bshow)
    _m_cp.FindChildInLayoutFile(
      "id-op-inspect-all-models-container"
    ).hittestchildren = !bshow
  }

  var OnActivateConfirmReward = function () {
    MissionsAPI.ActionRedeemOperationGoods(_m_nSeasonAccess, _m_rewardIndex)
    _m_cp.FindChildInLayoutFile(
      "id-op-inspect-shop-get-confirm-btn_label"
    ).enabled = false
    _m_cp.FindChildInLayoutFile(
      "id-op-inspect-shop-get-confirm-btn_cancel"
    ).enabled = false
    _StartRevealAnim()
  }
  ...
  var _StartRevealAnim = function () {
    ...
    $.Schedule(4, function () {
      ...
      if (_m_recievedRewardId) {
        var isSticker = ItemInfo.ItemMatchDefName(
          _m_recievedRewardId,
          "sticker"
        )
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    ...
    OperationStoreInspect.UpdateBtns
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Inventory_ItemCustomizationNotification",
    OperationStoreInspect.OnItemCustomizationNotification
  )
  $.RegisterForUnhandledEvent("UnblurOperationPanel", function () {
    OperationUtil.UnblurMenu(_m_cp)
  })
  $.RegisterForUnhandledEvent("BlurOperationPanel", function () {
    OperationUtil.BlurMenu(_m_cp)
  })
})()
