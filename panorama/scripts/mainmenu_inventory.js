...
var InventoryPanel = (function () {
  ...
  var _m_elInventoryMain =
    $.GetContextPanel().FindChildInLayoutFile("InventoryMain")
  var _m_elSelectItemForCapabilityPopup =
    $.GetContextPanel().FindChildInLayoutFile("SelectItemForCapabilityPopup")
  var _m_elInventorySearch =
    $.GetContextPanel().FindChildInLayoutFile("InvSearchPanel")
  ...
  var _CreateCategoriesNavBar = function () {
    var categories = StripEmptyStringsFromArray(
      InventoryAPI.GetCategories().split(",")
    )
  ...
  var _CreateInvCategoryDropdown = function (categories) {
    var elNavBar = $.CreatePanel(
      "Panel",
      _m_elInventoryMain,
      "InvCategories-NavBarParent",
      {
        class: "content-navbar content-navbar--dropdown content-navbar--short"
      }
    )

    var elDropdownContainer = $.CreatePanel(
      "Panel",
      elNavBar,
      "InvDropdownContainer",
      {
        class: "vertical-center left-right-flow"
      }
    )

    var elDropDown = $.CreatePanel(
      "DropDown",
      elDropdownContainer,
      "InvCategoriesDropdown",
      { class: "PopupButton" }
    )
    var tag = ""
    ...
    for (var i = 0; i < categories.length; i++) {
      tag = categories[i]
      var newEntry = $.CreatePanel(
        "Label",
        elDropDown,
        "InvCategoryDropDown" + tag,
        {
    ...
    elDropDown.SetPanelEvent(
      "oninputsubmit",
      _OnCategoryDropDownSubmit.bind(undefined, elDropDown)
    )
  ...
  var _CreateNavBar = function (idForNavBar, elParent, useSmallStyle) {
    var elNavBar = $.CreatePanel(
      "Panel",
      elParent,
      idForNavBar + "-NavBarParent",
      {
        class: "content-navbar"
      }
    )

    if (useSmallStyle) elNavBar.AddClass("content-navbar--short")

    var navBarClass = useSmallStyle
      ? "content-navbar__tabs--small" + " " + "content-navbar__tabs"
      : "content-navbar__tabs"
    var elNavBarButtonsContainer = $.CreatePanel(
      "Panel",
      elNavBar,
      idForNavBar + "-NavBar",
      {
  ...
  var _MakeNavBarButtons = function (elNavBar, listOfTags, onActivate) {
    ...
    listOfTags.forEach(function (tag, i) {
      ...
      if (catagory === "InvCategories") metaData = _GetMetadata(tag, "", "")
      else metaData = _GetMetadata(catagory, tag, "")

      var nameToken = _GetValueForKeyFromMetadata("nametoken", metaData)

      if (!nameToken) {
        nameToken = _GetValueForKeyFromMetadata("nameprefix", metaData)
        if (nameToken !== "") nameToken = nameToken + tag
      }
  ...
  var _CreateSubmenusAndListerPanelsForEachCategory = function (categories) {
    var elContent = _m_elInventoryMain.FindChildInLayoutFile(
      "InventoryMenuContent"
    )

    categories.forEach(function (tag, i) {
      if (tag) {
        var subCategories = StripEmptyStringsFromArray(
          InventoryAPI.GetSubCategories(tag).split(",")
        )
  ...
  var _UpdateActiveInventoryList = function () {
    var activePanel =
      _m_elInventoryMain.FindChildInLayoutFile(_m_activeCategory)
  ...
  var _NameFromTag = function (tag) {
    ...
    if (tag === "any") nameToken = "#Inv_Category_any"
    else nameToken = tag
  ...
  var _AddSortDropdownToNavBar = function (elNavBar, bIsCapabliltyPopup) {
    ...
    if (!elDropdown) {
      var elDropdownParent = $.CreatePanel(
        "Panel",
        elNavBar,
        "InvExtraNavOptions"
      )
      ...
      if (!bIsCapabliltyPopup) {
        elDropdown.SetPanelEvent(
          "oninputsubmit",
          _UpdateSort.bind(undefined, elDropdown)
        )
      }

      elDropdown.SetSelected(
        GameInterfaceAPI.GetSettingString("cl_inventory_saved_sort2")
      )
    }
  }
  ...
  var _UpdateSort = function (elDropdown) {
    ...
    if (activePanel) {
      ...
      if (
        typeof elDropdown.GetSelected().id === "string" &&
        elDropdown.GetSelected().id !==
          GameInterfaceAPI.GetSettingString("cl_inventory_saved_sort2")
      ) {
  ...
  var _AddTopRightExtraButtonsToInventory = function () {
    if (!$("#TopRightExtraButtons")) {
      ...
      var elTopRightExtraButtons = $.CreatePanel(
        "Panel",
        elParent,
        "TopRightExtraButtons"
      )
  ...
  var _ShowLoadout = function () {
    var elLoadoutContainer =
      $.GetContextPanel().FindChildInLayoutFile("InvLoadoutPanel")
  ...
  var _GetActiveCategoryLister = function (activePanel) {
    if (activePanel) {
      var elList = activePanel.FindChildInLayoutFile(
        _m_activeCategory + "-List"
      )
  ...
  var _GetSelectedSubCategoryPanel = function (activePanel) {
    var elSubCategoryNavBar = activePanel.FindChildInLayoutFile(
      _m_activeCategory + "-NavBar"
    )
  ...
  var _GetMetadata = function (category, subCategory, group) {
    var metaData = JSON.parse(
      InventoryAPI.GetInventoryStructureJSON(category, subCategory, group)
    )
  ...
  var _ShowInventoryMainListers = function () {
    if (!_m_elInventorySearch.BHasClass(_m_HiddenContentClassname))
      _UpdateSearchPanelVisibility(false)
    else _m_elInventoryMain.RemoveClass(_m_HiddenContentClassname)
  }
  ...
  var _UpdateActiveItemList = function (
    elListerToUpdate,
    category,
    subCategory,
    sortString,
  ...
  var _ShowHideNoItemsMessage = function (elLister, capabilityFilter) {
    ...
    if (capabilityFilter != "" && _SelectedCapabilityInfo.initialItemId != "") {
      elLabel.SetDialogVariable(
        "type",
        ItemInfo.GetName(_SelectedCapabilityInfo.initialItemId)
      )
      if (
        _SelectedCapabilityInfo.capability === "can_stattrack_swap" &&
        !InventoryAPI.IsTool(_SelectedCapabilityInfo.initialItemId)
      )
        elLabel.text = $.Localize(
          "#inv_empty_lister_for_stattrackswap",
          elLabel
        )
      else if (_SelectedCapabilityInfo.capability === "can_collect")
        elLabel.text = $.Localize("#inv_empty_lister_nocaskets", elLabel)
      else elLabel.text = $.Localize("#inv_empty_lister_for_use", elLabel)
    } else {
      elLabel.SetDialogVariable(
        "type",
        $.Localize(elSubCat[0].GetAttributeString("nice-name", ""))
      )
  ...
  var _OnReadyForDisplay = function () {
    ...
    if (!_m_elInventoryMain.updatePlayerEquipSlotChangedHandler) {
      _m_elInventoryMain.updatePlayerEquipSlotChangedHandler =
        $.RegisterForUnhandledEvent(
          "PanoramaComponent_Inventory_PlayerEquipSlotChanged",
          InventoryPanel.ShowNotification
        )
    }
  ...
  var _InventoryUpdated = function () {
    ...
    if (
      $.GetContextPanel().BHasClass(_m_HiddenContentClassname) ||
      _m_isCapabliltyPopupOpen
    )
      return
  ...
  var _UpdateLoadoutButtonState = function () {
    ...
    if (elInvLoadoutBtn) {
      ...
      elInvLoadoutBtn.SetPanelEvent(
        "onmouseover",
        elInvLoadoutBtn.enabled
          ? function () {}
          : function () {
              UiToolkitAPI.ShowTextTooltip(
                elInvLoadoutBtn.id,
                "#tooltip_loadout_disabled"
              )
            }
      )
  ...
  var _PromptShowSelectItemForCapabilityPopup = function (
    titletxt,
    messagetxt,
    capability,
    itemid,
  ...
  var _ShowSelectItemForCapabilityPopup = function (
    capability,
    itemid,
    itemid2
  ) {
    ...
    var elDropDownParent =
      _m_elSelectItemForCapabilityPopup.FindChildInLayoutFile(
        "CapabilityPopupSortContainer"
      )
    ...
    elDropdown.SetPanelEvent(
      "oninputsubmit",
      _UpdatePopup.bind(undefined, itemid, capability)
    )
  ...
  var _CloseSelectItemForCapabilityPopup = function () {
    ...
    if (
      _m_elSelectItemForCapabilityPopup.BHasClass(_m_HiddenContentClassname)
    ) {
      return
    }
  ...
  var _UpdatePopup = function (id, capability) {
    var elList = _m_elSelectItemForCapabilityPopup.FindChildInLayoutFile(
      "ItemListForCapability"
    )

    if (!elList)
      elList = $.CreatePanel(
        "InventoryItemList",
        _m_elSelectItemForCapabilityPopup,
        "ItemListForCapability"
      )
  ...
  var _SetUpCasketPopup = function (capability, elList) {
    var elActionBar = _m_elSelectItemForCapabilityPopup.FindChildInLayoutFile(
    ...
    if (capability === "casketstore" || capability === "casketretrieve") {
      ...
      _UpdateMultiSelectDisplay(
        elActionBar.FindChildInLayoutFile("CapabilityPopupMultiStatus")
      )
    } else {
  ...
  var _SetCapabilityPopupTitle = function (id, capability, elList) {
    ...
    var elImage =
      _m_elSelectItemForCapabilityPopup.FindChildInLayoutFile("CapItemImage")
    elImage.itemid = id
  ...
  var _UpdateSelectItemForCapabilityPopup = function (
    ...
    if (
      !_m_elSelectItemForCapabilityPopup ||
      !_m_elSelectItemForCapabilityPopup.IsValid()
    )
      return false

    var elMultiItemPortion =
      _m_elSelectItemForCapabilityPopup.FindChildInLayoutFile(
        "CapabilityPopupMultiStatus"
      )
    ...
    if (bSelected) {
      ...
      if (_SelectedCapabilityInfo.multiselectItemIds.hasOwnProperty(itemid)) {
        ...
        _SelectedCapabilityInfo.multiselectItemIdsArray.splice(
          _SelectedCapabilityInfo.multiselectItemIdsArray.indexOf(itemid),
          1
        )
      }
    }
  ...
  var _UpdateMultiSelectDisplay = function (elMultiItemPortion) {
    elMultiItemPortion.SetDialogVariableInt(
      "count",
      _SelectedCapabilityInfo.multiselectItemIdsArray.length
    )
    elMultiItemPortion.FindChildInLayoutFile(
      "CapabilityPopupMultiStatusBtn"
      ...
      case "casketretrieve":
        ...
        UiToolkitAPI.ShowCustomLayoutPopupParameters(
          ...
          "op=remove" +
            ...
            "&spinner=1" +
            "&casket_item_id=" +
            _SelectedCapabilityInfo.initialItemId +
            "&subject_item_id=" +
            strItemIDs
        )
        break
  ...
  var _DeleteItemAnim = function (id) {
    ...
    $.Schedule(0.3, _DeleteItem.bind(undefined, id))
  }
  ...
  var _LoadEquipNotification = function () {
    var elParent = $.GetContextPanel().FindChildInLayoutFile(
      "InventoryMainContainer"
    )

    var elNotification = $.CreatePanel(
      "Panel",
      elParent,
      "InvNotificationEquip"
    )
    elNotification.BLoadLayout(
      "file://{resources}/layout/notification/notification_equip.xml",
      false,
      false
    )
  }
  ...
  var _ShowNotification = function (
    ...
    var elNotification = $.GetContextPanel().FindChildInLayoutFile(
      "InvNotificationEquip"
    )
    EquipNotification.ShowEquipNotification(
      elNotification,
      slotString,
      newEquippedItemId
    )
  ...
  var _UpdateCraftingPanelVisibility = function (bShow) {
    ...
    if (bShow) {
      if (elCrafting.BHasClass(_m_HiddenContentClassname)) {
        ...
        $.GetContextPanel()
          .FindChildTraverse("Crafting-Items")
          .SetReadyForDisplay(true)
        $.GetContextPanel()
          .FindChildTraverse("Crafting-Ingredients")
          .SetReadyForDisplay(true)

        var RecipeId = InventoryAPI.GetTradeUpContractItemID()
        var strCraftingFilter = InventoryAPI.GetItemAttributeValue(
          RecipeId,
          "recipe filter"
        )
  ...
  var _UpdateItemListCallback = function () {
    if (
      _SelectedCapabilityInfo.popupVisible === true &&
      _SelectedCapabilityInfo.capability
    ) {
      _UpdatePopup(
        _SelectedCapabilityInfo.initialItemId,
        _SelectedCapabilityInfo.capability
  ...
  return {
    ...
    PromptShowSelectItemForCapabilityPopup:
      _PromptShowSelectItemForCapabilityPopup,
...
;(function () {
  ...
  $.RegisterEventHandler(
    "ReadyForDisplay",
    elJsInventory,
    InventoryPanel.OnReadyForDisplay
  )
  $.RegisterEventHandler(
    "UnreadyForDisplay",
    elJsInventory,
    InventoryPanel.ClosePopups
  )
  $.RegisterEventHandler("Cancelled", elJsInventory, InventoryPanel.ClosePopups)

  $.RegisterForUnhandledEvent(
    "PromptShowSelectItemForCapabilityPopup",
    InventoryPanel.PromptShowSelectItemForCapabilityPopup
  )
  $.RegisterForUnhandledEvent(
    "ShowSelectItemForCapabilityPopup",
    InventoryPanel.ShowSelectItemForCapabilityPopup
  )
  $.RegisterForUnhandledEvent(
    "UpdateSelectItemForCapabilityPopup",
    InventoryPanel.UpdateSelectItemForCapabilityPopup
  )
  $.RegisterForUnhandledEvent(
    "HideSelectItemForCapabilityPopup",
    InventoryPanel.CloseSelectItemForCapabilityPopup
  )
  $.RegisterForUnhandledEvent(
    "CapabilityPopupIsOpen",
    InventoryPanel.SetIsCapabilityPopUpOpen
  )
  $.RegisterForUnhandledEvent(
    "RefreshActiveInventoryList",
    InventoryPanel.InventoryUpdated
  )
  $.RegisterForUnhandledEvent(
    "ShowDeleteItemConfirmationPopup",
    InventoryPanel.ShowDeleteItemConfirmation
  )
  $.RegisterForUnhandledEvent(
    "ShowUseItemOnceConfirmationPopup",
    InventoryPanel.ShowUseItemOnceConfirmation
  )
  $.RegisterForUnhandledEvent(
    "ShowResetMusicVolumePopup",
    InventoryPanel.ShowResetMusicConfirmation
  )
  $.RegisterForUnhandledEvent(
    "ShowLoadoutForItem",
    InventoryPanel.ShowLoadoutForItem
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Inventory_CraftIngredientAdded",
    function () {
      InventoryPanel.UpdateCraftingPanelVisibility(true)
    }
  )
  $.RegisterForUnhandledEvent("ShowTradeUpPanel", function () {
    InventoryPanel.UpdateCraftingPanelVisibility(true)
  })
})()
