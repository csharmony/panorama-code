...
;(function () {
  ...
  var _Initialize = function () {
    ...
    if (_m_pauseMenu) {
      ...
      $.RegisterForUnhandledEvent(
        "EndOfMatch_Show",
        _IsEndOfMatchActive.bind(undefined, true)
      )
      $.RegisterForUnhandledEvent(
        "EndOfMatch_Shutdown",
        _IsEndOfMatchActive.bind(undefined, false)
      )
  ...
  var _FinishUpdating = function () {
    if (!_m_currentlyUpdating) return
      ...
      if (elTeam) {
        ...
        for (var j = 0; j < playersCount; j++) {
          ...
          if (!teamHasLocalPlayer) {
            teamHasLocalPlayer =
              oLocalPlayerXuid === oInfo.teams[i].players[j].xuid ? true : false
          }
  ...
  function _UpdateStats(oLocalPlayer, oInfo) {
    ...
    if (oLocalPlayer) {
      if (oLocalPlayer.position_loc.slice(0, 1) === "#")
        _m_cP
          .FindChildInLayoutFile("id-stat-position")
          .SetLocalizationString(oLocalPlayer.position_loc)
      else
        _m_cP.FindChildInLayoutFile("id-stat-position").text =
          oLocalPlayer.position_loc

      var elContentspanel = _m_cP.FindChildInLayoutFile(
        "id-eom-survival-contents"
      )
      _TintPositionHexToTeam(elContentspanel, oLocalPlayer.position)

      _m_cP
        .FindChildInLayoutFile("id-stat-title")
        .SetLocalizationString(oLocalPlayer.alive_loc)

      var elStats = _m_cP.FindChildInLayoutFile(
        "id-eom-survival-stats-container"
      )
      ...
      if (
        !_m_cP
          .FindChildInLayoutFile("id-eom-survival-contents")
          .BHasClass("show")
      ) {
      ...
      _m_cP
        .FindChildInLayoutFile("id-eom-survival-damage-container")
        .RemoveClass("show")
    }
  }
  ...
  function _RemoveOldStats() {
    ...
    var elDamageReports = _m_cP.FindChildInLayoutFile(
      "id-eom-survival-damage-container"
    )
  ...
  function _CreateTeamAvatarPanel(elList, oTeam) {
    for (var iPlayer = 0; iPlayer < oTeam.players.length; ++iPlayer) {
      if (elList.FindChildInLayoutFile(oTeam.players[iPlayer].xuid)) return null
    }
  ...
  function _UpdatePlayer(elList, oPlayer, position = null) {
    ...
    if (!elPlayerPanel) {
      ...
      if (oPlayer.skillgroup) {
        ...
        elImage.SetImage(
          "file://{images}/icons/skillgroups/dangerzone" +
            oPlayer.skillgroup +
            ".svg"
        )
      } else {
      ...
      _InitAvatar(
        elPlayerPanel.FindChildInLayoutFile("JsSurvivalAvatar"),
        oPlayer.xuid
      )
  ...
  function _AddDeadStateToPlayer(elPlayerPanel, oPlayer) {
    if (!oPlayer.alive) {
      ...
      elPlayerPanel
        .FindChildInLayoutFile("JsSurvivalDeadIcon")
        .SetImage(iconPath)
  ...
  function _CreatePositionPanelForPlayer(elPlayer, position) {
    ...
    if (!position) {
      ...
      elPlayer.FindChildInLayoutFile("JsSurvivalPlacement").text = $.Localize(
        "#EOM_Position_" + position
      )
    }
  ...
  function _AddStatsRow(elStats, oStat) {
    ...
    elPanel
      .FindChildInLayoutFile("id-stat-name")
      .SetLocalizationString(oStat.name)
    elPanel
      .FindChildInLayoutFile("id-stat-value")
      .SetDialogVariableInt("value", oStat.value)
    elPanel
      .FindChildInLayoutFile("id-stat-value")
      .SetDialogVariableTime("value", oStat.value)
    elPanel
      .FindChildInLayoutFile("id-stat-value")
      .SetLocalizationString(oStat.value_loc)
  }
  ...
  function _MakeDamageReports(ateams) {
    ...
    for (var i = 0; i < ateams.length; i++) {
      ...
      for (var j = 0; j < playersCount; j++) {
        if (
          ateams[i].players[j].damage_to ||
          ateams[i].players[j].damage_from
        ) {
  ...
  function _AddDamagReport(oPlayer) {
    ...
    if (oPlayer.damage_to) {
      ...
      elPanel.FindChildInLayoutFile("JsSurvivalDamageGivenValue").text =
        oPlayer.damage_to
    }

    if (oPlayer.damage_from) {
      elPanel.FindChildInLayoutFile("JsSurvivalDamageTaken").AddClass("show")
      elPanel.FindChildInLayoutFile("JsSurvivalDamageTakenValue").text =
        oPlayer.damage_from
    }
    ...
    elPanel.MoveChildBefore(
      elAvatar,
      elPanel.FindChildInLayoutFile("JsSurvivalDamageList")
    )
