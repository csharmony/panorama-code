...
var matchInfo = (function () {
  ...
  function _SetMatchMessage(value, show, tab) {
    if (tab) {
      ...
      var elMessageContainer = tab.FindChildInLayoutFile(
        "id-match-message-container"
      )
  ...
  function _IsMatchMetadataFullyLoaded(elParentPanel) {
    return (
      (elParentPanel.matchListDescriptor == "live" &&
        elParentPanel.matchId != "gotv") ||
      MatchInfoAPI.GetMatchMetadataFullState(elParentPanel.matchId)
    )
  }
  ...
  function _DownloadFailedNotify(elParentPanel) {
    var canDownload =
      !(elParentPanel.matchListDescriptor === "downloaded") &&
      (MatchInfoAPI.GetMatchState(elParentPanel.matchInfo) === "recent" ||
        elParentPanel.isTournament)
    if (canDownload) {
      UiToolkitAPI.ShowGenericPopupYesNo(
        $.Localize("#WatchMenu_Info_Download_Failed"),
        $.Localize("#WatchMenu_Info_Download_Failed_Retry"),
        "",
        function () {
          _DownloadMatch(elParentPanel)
        },
        function () {}
      )
    } else {
      UiToolkitAPI.ShowGenericPopupOk(
        $.Localize("#WatchMenu_Info_Download_Failed"),
        $.Localize("#WatchMenu_Info_Download_Failed_Info"),
        "",
        function () {}
      )
    }
  ...
  function _WatchHighlights(elParentPanel) {
    MatchInfoAPI.WatchHighlights(
      elParentPanel.matchId,
      elParentPanel.activePlayerRow.playerXuid
    )
  }

  function _WatchLowlights(elParentPanel) {
    MatchInfoAPI.WatchLowlights(
      elParentPanel.matchId,
      elParentPanel.activePlayerRow.playerXuid
    )
  }
  ...
  function _ShareMatch(elParentPanel) {
    SteamOverlayAPI.CopyTextToClipboard(
      MatchInfoAPI.GetMatchShareToken(elParentPanel.matchId, "copyurl")
    )
    var elShareLinkButton = elParentPanel.FindChildInLayoutFile("id-mi-copy")
    UiToolkitAPI.HideTextTooltip()
    UiToolkitAPI.ShowTextTooltipOnPanel(
      elShareLinkButton,
      $.Localize("#WatchMenu_Share_Link_Copied")
    )
  }
  ...
  var _CanRedeem = function (elParentPanel) {
    ...
    var id = InventoryAPI.GetActiveTournamentCoinItemId(
      elParentPanel.tournamentIndex
    )
    if (!id || id === "0") {
      return false
    } else {
      ...
      var coinRedeemsPurchased = InventoryAPI.GetItemAttributeValue(
        id,
        "operation drops awarded 1"
      )
      if (coinRedeemsPurchased) coinLevel += coinRedeemsPurchased

      var redeemed = InventoryAPI.GetItemAttributeValue(
        id,
      ...
      if (
        elParentPanel.tournamentIndex == g_ActiveTournamentInfo.eventid &&
        g_ActiveTournamentInfo.itemid_charge &&
        ItemInfo.GetStoreSalePrice(
      ...
      var tournamentName = MatchInfoAPI.GetMatchTournamentName(
        elParentPanel.matchId
      )
      return (
        redeemsAvailable > 0 &&
  ...
  function _RedeemSouvenir(tournamentIndex, matchId) {
    UiToolkitAPI.ShowCustomLayoutPopupParameters(
      ...
      "matchid=" + matchId + "&" + "tournamentindex=" + tournamentIndex
    )
  }
  ...
  function _RefreshRoundWatchEnabled(elParentPanel) {
    ...
    var elStatsContainer = elParentPanel.FindChildInLayoutFile(
      "id-mi-round-stats__container"
    )
    ...
    for (var i = 1; i <= totalBars; i++) {
      ...
      if (!canWatch) {
        ...
        elRoundStats.SetPanelEvent(
          "onmouseover",
          _OnRoundMouseOver.bind(undefined, elRoundStats)
        )
        elRoundStats.SetPanelEvent("onmouseout", function () {
          UiToolkitAPI.HideTextTooltip()
        })
        elRoundStats.SetPanelEvent(
          "onactivate",
          _OnRoundActivate.bind(undefined, elParentPanel.matchId, i)
        )
      }
    }
  }
  ...
  function _UpdateMatchMenu(elParentPanel) {
    ...
    var elWatchHighlightsButton = elParentPanel.FindChildInLayoutFile(
      "id-mi-watch-highlights"
    )
    var elWatchLowlightsButton = elParentPanel.FindChildInLayoutFile(
      "id-mi-watch-lowlights"
    )
    var elDeleteButton = elParentPanel.FindChildInLayoutFile("id-mi-delete")
    var elDownloadingButton =
      elParentPanel.FindChildInLayoutFile("id-mi-downloading")
    var elDownloadFailedButton =
      elParentPanel.FindChildInLayoutFile("id-mi-error-delete")
    ...
    if (elParentPanel.matchListDescriptor != "live") {
      ...
      _ShowButton(
        elSouvenirButton,
        matchState !== "live" && _CanRedeem(elParentPanel)
      )

      var szSouvenirButtonHint = "#popup_redeem_souvenir_title"
      elSouvenirButton.SetPanelEvent("onmouseover", function () {
        UiToolkitAPI.ShowTextTooltipOnPanel(
          elSouvenirButton,
          szSouvenirButtonHint
        )
      })
      elSouvenirButton.SetPanelEvent("onmouseout", function () {
        UiToolkitAPI.HideTextTooltip()
      })
      ...
      if (elParentPanel.matchListDescriptor != "downloaded") {
        ...
        _EnableButton(
          elShareLinkButton,
          elParentPanel.matchShareToken != "" &&
            elParentPanel.matchShareToken != undefined
        )

        elDownloadButton.SetPanelEvent("onmouseover", function () {
          UiToolkitAPI.ShowTextTooltipOnPanel(
            elDownloadButton,
            szDownloadButtonHint
          )
        })
        elDownloadButton.SetPanelEvent("onmouseout", function () {
          UiToolkitAPI.HideTextTooltip()
        })
      } else {
      ...
      var bEnabledReelLightsButton =
        elParentPanel.activePlayerRow &&
        MatchInfoAPI.CanWatchHighlights(
          elParentPanel.matchId,
          elParentPanel.activePlayerRow.playerXuid
        )
  ...
  function _Refresh(elParentPanel) {
    function _ShowLoadingError(elBoundParentPanel) {
      ...
      _SetMatchMessage(
        $.Localize("CSGO_Watch_NoMatchData"),
        true,
        elBoundParentPanel
      )
      if (elBoundParentPanel.updateMatchInfoHandler) {
        $.UnregisterForUnhandledEvent(
          "PanoramaComponent_MatchInfo_StateChange",
          elBoundParentPanel.updateMatchInfoHandler
        )
      }
    ...
    if (_IsMatchMetadataFullyLoaded(elParentPanel)) {
      ...
      elParentPanel.downloadFailedHandler = $.Schedule(
        3.0,
        _ShowLoadingError.bind(undefined, elParentPanel)
      )
      elParentPanel.updateMatchInfoHandler = $.RegisterForUnhandledEvent(
        "PanoramaComponent_MatchInfo_StateChange",
        _PopulateMatchInfo.bind(undefined, elParentPanel)
      )
    }
  }
  ...
  function _UpdateName(elParentPanel, elPlayerName) {
    if (elParentPanel.isTournament) {
      elPlayerName.text = MatchInfoAPI.GetMatchPlayerStat(
        elParentPanel.matchId,
        elPlayerName.playerXuid,
        "name"
      )
    } else {
  ...
  function _UpdateTitle(elParentPanel, playerXuid) {
    if (elParentPanel.isTournament) {
      elParentPanel.SetDialogVariable(
        "playerNameTitle",
        MatchInfoAPI.GetMatchPlayerStat(
          elParentPanel.matchId,
          playerXuid,
          "name"
        )
      ...
      elParentPanel.SetDialogVariable(
        "playerNameTitle",
        FriendsListAPI.GetFriendName(playerXuid)
      )
    }
    ...
    if (elWatchHighlightsButton) {
      elWatchHighlightsButton.SetPanelEvent("onmouseover", function () {
        UiToolkitAPI.ShowTextTooltipOnPanel(
          elWatchHighlightsButton,
          UiToolkitAPI.MakeStringSafe(
            $.Localize(
              "#WatchMenu_Watch_Highlights_Player_Selected",
              elParentPanel
            )
          )
        )
      })
      elWatchLowlightsButton.SetPanelEvent("onmouseover", function () {
        UiToolkitAPI.ShowTextTooltipOnPanel(
          elWatchLowlightsButton,
          UiToolkitAPI.MakeStringSafe(
            $.Localize(
              "#WatchMenu_Watch_Lowlights_Player_Selected",
              elParentPanel
            )
          )
        )
  ...
  function _Show(elParentPanel) {
    ...
    elParentPanel.updateMatchMenuHandler = $.RegisterForUnhandledEvent(
      "PanoramaComponent_MatchInfo_StateChange",
      _UpdateMatchMenu.bind(undefined, elParentPanel)
    )
  }
  ...
  function _Hide(elParentPanel) {
    for (var teamId in TEAMS) {
      var elTeam = elParentPanel.FindChildInLayoutFile(
        "players-table-" + TEAMS[teamId]
      )
      for (var i = 0; i < TEAMSIZE; i++) {
        ...
        if (elPlayerName.nameUpdateHandler) {
          $.UnregisterForUnhandledEvent(
            ...
            elPlayerName.nameUpdateHandler
          )
    ...
    if (elParentPanel.updateMatchInfoHandler) {
      $.UnregisterForUnhandledEvent(
        "PanoramaComponent_MatchInfo_StateChange",
        elParentPanel.updateMatchInfoHandler
      )
      elParentPanel.updateMatchInfoHandler = undefined
    }

    var elTitle = elParentPanel.FindChildInLayoutFile(
      "id-mi-player-stats-title"
    )
    if (elTitle.nameUpdateHandler) {
      $.UnregisterForUnhandledEvent(
        "PanoramaComponent_FriendsList_NameChanged",
        elTitle.nameUpdateHandler
      )
  ...
  function _FillRoundStats(elParentPanel, elPlayerRow) {
    ...
    if (elTitle.nameUpdateHandler == undefined) {
      elTitle.nameUpdateHandler = $.RegisterForUnhandledEvent(
        "PanoramaComponent_FriendsList_NameChanged",
        _UpdateTitle.bind(undefined, elParentPanel, elPlayerRow.playerXuid)
      )
    }
    ...
    if (isLive == false) {
      elParentPanel
        .FindChildInLayoutFile("id-mi-player-stats")
        .RemoveClass("mi-player-stats__collapse")
    }

    var elStatsContainer = elParentPanel.FindChildInLayoutFile(
      "id-mi-round-stats__container"
    )
    var elTickLabels = elParentPanel.FindChildInLayoutFile(
      "id-mi-round-stats__tick-labels"
    )

    var team0Score = MatchInfoAPI.GetMatchRoundScoreForTeam(
      elParentPanel.matchId,
      0
    )
    if (team0Score === undefined) team0Score = 0

    var team1Score = MatchInfoAPI.GetMatchRoundScoreForTeam(
      elParentPanel.matchId,
      1
    )
    if (team1Score === undefined) team1Score = 0
    ...
    var roundWins = MatchInfoAPI.GetMatchPlayerRoundStats(
      elParentPanel.matchId,
      elParentPanel.activePlayerRow.playerXuid,
      "round_wins"
    )
    roundWins = roundWins ? roundWins.split(",") : Array(totalRounds).fill(0)
    var mvps = MatchInfoAPI.GetMatchPlayerRoundStats(
      elParentPanel.matchId,
      elParentPanel.activePlayerRow.playerXuid,
      "mvps"
    )
    mvps = mvps ? mvps.split(",") : Array(totalRounds).fill(0)
    var kills = MatchInfoAPI.GetMatchPlayerRoundStats(
      elParentPanel.matchId,
      elParentPanel.activePlayerRow.playerXuid,
      "enemy_kills"
    )
    kills = kills ? kills.split(",") : Array(totalRounds).fill(0)
    var headshots = MatchInfoAPI.GetMatchPlayerRoundStats(
      elParentPanel.matchId,
      elParentPanel.activePlayerRow.playerXuid,
      "enemy_headshots"
    )
    headshots = headshots ? headshots.split(",") : Array(totalRounds).fill(0)
    var deaths = MatchInfoAPI.GetMatchPlayerRoundStats(
      elParentPanel.matchId,
      elParentPanel.activePlayerRow.playerXuid,
      "deaths"
    )
    ...
    function _IsMajorTick(n) {
      ...
      if (n == maxRounds) return true

      if (n == totalRounds) return true

      if (n > maxRounds && (n - maxRounds) % 6 == 0) return true
    }

    function _IsMinorTick(n) {
      if (n < maxRounds) {
        if (maxRounds % 5 == 0) return n % 5 == 0
        else if (maxRounds % 4 == 0) return n % 4 == 0
        else if (maxRounds <= 12 && maxRounds % 3 == 0) return n % 3 == 0
        else if (maxRounds <= 8 && maxRounds % 2 == 0) return n % 2 == 0
      } else {
      }
    ...
    function _IsRightOfHalftime(n) {
      if (n == maxRounds / 2 + 1) return true
    }

    function _IsLeftOfHalftime(n) {
      if (n == maxRounds / 2) return true
    }

    function _GetTickStyleForRound(n) {
      if (_IsRightOfHalftime(n)) return "mi-round-tick--right-of-team-switch"
      else if (_IsLeftOfHalftime(n)) return "mi-round-tick--left-of-team-switch"
      else if (_IsMajorTick(n)) return "mi-round-tick--major"
      else if (_IsMinorTick(n)) return "mi-round-tick--minor"
      else return "mi-round-tick--sub"
    }
    ...
    function _GetLabelForTick(n) {
      if (_IsRightOfHalftime(n) || _IsLeftOfHalftime(n)) return ""
      else if (_IsMajorTick(n) || _IsMinorTick(n)) return n
      else return ""
    }
    ...
    for (var i = 1; i <= totalRounds; i++) {
      ...
      var elRoundBar = elRoundStats.FindChildTraverse(
        "id-mi-round-summary-bar__container"
      )
      var elIconContainer = elRoundStats.FindChildTraverse(
        "id-mi-icons__container"
      )
      ...
      if (i > playedRounds) {
        ...
        var elEliminationWinIcons = elRoundStats.FindChildTraverse(
          "id-mi-eliminations-win"
        )
        for (var k = 0; k < 5; k++) {
          var kIcon = elEliminationWinIcons.FindChildTraverse(
            "id-mi-icon-elimination_" + k
          )
          var hIcon = elEliminationWinIcons.FindChildTraverse(
            "id-mi-icon-elimination--headshot_" + k
          )
          if (k >= nKills) {
            kIcon.AddClass("hide")
            hIcon.AddClass("hide")
        ...
        var elDeathIcon = elIconContainer.FindChildTraverse(
          "id-mi-elimination-death"
        )
        if (deaths[n] == 1) {
        ...
        elEliminationWinIcons.RemoveClass(
          "sb-tint--" + TEAMS[flipBit(currentTeamId)]
        )
  ...
  function _FillScoreboard(elParentPanel) {
    ...
    var currentTopPanelTeamId = MatchInfoAPI.GetMatchTournamentTeamID(
      elParentPanel.matchId,
      0
    )
    ...
    function Helper_FillTeamStats(teamId) {
      ...
      var elScoreboxBackdrop = elParentPanel.FindChildInLayoutFile(
        "id-sb-scorebox_backdrop--" + TEAMS[teamId]
      )
      if (elParentPanel.isTournament) {
        var tag = MatchInfoAPI.GetMatchTournamentTeamTag(
          elParentPanel.matchId,
          teamId
        )
        if (!tag) {
        ...
        elScoreboxBackdrop.SetImage(
          "file://{images}/tournaments/teams/" + tag.toLowerCase() + ".svg"
        )
        elScoreboxBackdrop.AddClass("scorebox_backdrop--tournament")
        elParentPanel.SetDialogVariable(
          "sb_team_name--" + TEAMS[teamId],
          MatchInfoAPI.GetMatchTournamentTeamName(elParentPanel.matchId, teamId)
        )
      } else {
        elParentPanel.SetDialogVariable(
          "sb_team_name--" + TEAMS[teamId],
          $.Localize("#teamname_" + TEAMS[teamId])
        )
      }
      elParentPanel.SetDialogVariable(
        "score_" + TEAMS[teamId],
        MatchInfoAPI.GetMatchRoundScoreForTeam(elParentPanel.matchId, teamId)
      )
      for (var i = 0; i < TEAMSIZE; i++) {
        var elPlayerRow = elTeam.GetChild(i)
        if (!elParentPanel.teamsFilled) {
          elPlayerRow.playerXuid =
            MatchInfoAPI.GetMatchPlayerXuidByIndexForTeam(
              elParentPanel.matchId,
              teamId,
              i
            )
        ...
        if (!elParentPanel.teamsFilled) {
          ...
          elAvatarImage.SetPanelEvent(
            "onactivate",
            _OpenPlayerCard.bind(undefined, playerXuid)
          )
          elAvatarTeamLogo.SetPanelEvent(
            "onactivate",
            _OpenPlayerCard.bind(undefined, playerXuid)
          )
        }
        if (elPlayerName.nameUpdateHandler == undefined) {
          elPlayerName.nameUpdateHandler = $.RegisterForUnhandledEvent(
            "PanoramaComponent_FriendsList_NameChanged",
            _UpdateName.bind(undefined, elParentPanel, elPlayerName)
          )
        }
        ...
        for (var p in PLAYERSTATS) {
          ...
          var elStatData = MatchInfoAPI.GetMatchPlayerStat(
            elParentPanel.matchId,
            playerXuid,
            PLAYERSTATS[p]
          )
          ...
          if (PLAYERSTATS[p] === "mvps") {
            if (elStatData == 0) {
              ...
              elPlayerRow
                .FindChildTraverse("mvps__panel")
                .RemoveClass("hide-mvps")
            }
          }
        }
      }
    ...
    if (elMatchMapIcon) {
      $.RegisterEventHandler(
        "ImageFailedLoad",
        elMatchMapIcon,
        setDefaultMapImage.bind(undefined, elMatchMapIcon)
      )
      elMatchMapIcon.SetImage(
        "file://{images}/map_icons/map_icon_" + rawMapName + ".svg"
      )
    }
    ...
    if (matchDuration === 1) minuteString = "CSGO_Watch_Minute"
    ...
    if (elParentPanel.matchListDescriptor === "live") {
      var round =
        1 +
        MatchInfoAPI.GetMatchRoundScoreForTeam(elParentPanel.matchId, 0) +
        MatchInfoAPI.GetMatchRoundScoreForTeam(elParentPanel.matchId, 1)
      ...
      elParentPanel.SetDialogVariable(
        "dateOrRound",
        $.Localize(progressionStateString)
      )
      elParentPanel.SetDialogVariable(
        "dateOrRoundLabel",
        $.Localize("#CSGO_Watch_Info_4")
      )
      elParentPanel.SetDialogVariable(
        "durationLabel",
        $.Localize("CSGO_Watch_Info_5")
      )
    } else {
      elParentPanel.SetDialogVariable(
        "dateOrRound",
        MatchInfoAPI.IsLive(elParentPanel.matchId)
          ? $.Localize("#CSGO_Watch_Cat_LiveMatches")
          : MatchInfoAPI.GetMatchTimestamp(elParentPanel.matchId)
      )
      elParentPanel.SetDialogVariable(
        "dateOrRoundLabel",
        $.Localize("#CSGO_Watch_Info_2")
      )
      elParentPanel.SetDialogVariable(
        "durationLabel",
        $.Localize("CSGO_Watch_Info_1")
      )
    }
  }
  ...
  function _FillServerLogTournamentInfo(elParentPanel) {
    ...
    function PopulateForTeam(nTeam) {
      ...
      elParentPanel.SetDialogVariable(
        "teamname" + nTeam,
        MatchInfoAPI.GetMatchTournamentTeamName(elParentPanel.matchId, nTeam)
      )
      elParentPanel.SetDialogVariable(
        "score" + nTeam,
        MatchInfoAPI.GetMatchRoundScoreForTeam(elParentPanel.matchId, nTeam)
      )
    }
    ...
    if (elEventID)
      elTournamentLogo.SetImage(
        "file://{images}/tournaments/events/tournament_logo_" +
          elEventID +
          ".svg"
      )
    ...
    elParentPanel.SetDialogVariable(
      "tournamentphase",
      $.Localize(
        MatchInfoAPI.GetMatchTournamentStageName(elParentPanel.matchId)
      )
    )
    elParentPanel.SetDialogVariable(
      "matchphase",
  ...
  function _Init(elParentPanel) {
    ...
    var bIsMinimalMatchInfo = MatchInfoAPI.IsServerLogTournamentMatch(
      elParentPanel.matchId
    )
    elParentPanel.SetHasClass("matchinfo--minimal", bIsMinimalMatchInfo)
    if (bIsMinimalMatchInfo) {
      var minimalInfoBody = $.CreatePanel(
        "Panel",
        elParentPanel,
        "minimal-match-info"
      )
      minimalInfoBody.BLoadLayoutSnippet(
        "matchinfo_serverlogtournament_minimal"
      )
      elParentPanel.MoveChildBefore(
        minimalInfoBody,
        elParentPanel.FindChild("footer")
      )
    }
    ...
    function Helper_CreateScoreboard(teamId) {
      ...
      for (var i = 0; i < TEAMSIZE; i++) {
        var playerXuid = MatchInfoAPI.GetMatchPlayerXuidByIndexForTeam(
          elParentPanel.matchId,
          teamId,
          i
        )
        var elPlayerRow = $.CreatePanel(
          "Panel",
          elTeam,
          "id-player-" + playerXuid
        )
        if (!playerXuid) {
        ...
        if (elParentPanel.matchListDescriptor != "live") {
          elPlayerRow.SetPanelEvent(
            "onactivate",
            _FillRoundStats.bind(undefined, elParentPanel, elPlayerRow)
          )
        ...
        elPlayerNameLabel.SetPanelEvent(
          "onactivate",
    ...
    elParentPanel.matchShareToken = MatchInfoAPI.GetMatchShareToken(
      elParentPanel.matchId,
      "text"
    )
    ...
    elParentPanel.updateMatchInfoHandler = undefined
    elParentPanel.teamsFilled = false
    var elColumnLabels = elParentPanel.FindChildInLayoutFile(
      "players-table__labels-row"
    )
    for (var p in PLAYERSTATS) {
    ...
    $.RegisterEventHandler(
      ...
      elParentPanel,
      _OnFadeOutEnd.bind(undefined, elParentPanel)
    )
    ...
    elDownloadButton.SetPanelEvent(
      "onactivate",
      _DownloadMatch.bind(undefined, elParentPanel)
    )
    elShareLinkButton.SetPanelEvent(
      "onactivate",
      _ShareMatch.bind(undefined, elParentPanel)
    )
    elShareLinkButton.SetDialogVariable(
      "matchcode",
      elParentPanel.matchShareToken
    )
    elShareLinkButton.SetPanelEvent("onmouseover", function () {
      UiToolkitAPI.ShowTextTooltipOnPanel(
        elShareLinkButton,
        $.Localize("#WatchMenu_Get_Share_Link")
      )
    })
    elShareLinkButton.SetPanelEvent("onmouseout", function () {
      UiToolkitAPI.HideTextTooltip()
    })
    elWatchButton.SetPanelEvent(
      "onactivate",
      _Watch.bind(undefined, elParentPanel)
    )
    elWatchHighlightsButton.SetPanelEvent(
      "onactivate",
      _WatchHighlights.bind(undefined, elParentPanel)
    )
    elWatchHighlightsButton.SetPanelEvent("onmouseover", function () {
      UiToolkitAPI.ShowTextTooltipOnPanel(
        elWatchHighlightsButton,
        $.Localize("#WatchMenu_Watch_Highlights")
      )
    })
    elWatchHighlightsButton.SetPanelEvent("onmouseout", function () {
      UiToolkitAPI.HideTextTooltip()
    })
    elWatchLowlightsButton.SetPanelEvent(
      "onactivate",
      _WatchLowlights.bind(undefined, elParentPanel)
    )
    elWatchLowlightsButton.SetPanelEvent("onmouseover", function () {
      UiToolkitAPI.ShowTextTooltipOnPanel(
        elWatchLowlightsButton,
        $.Localize("#WatchMenu_Watch_Lowlights")
      )
    })
    elWatchLowlightsButton.SetPanelEvent("onmouseout", function () {
      UiToolkitAPI.HideTextTooltip()
    })
    elDeleteButton.SetPanelEvent(
      "onactivate",
      _DeleteDemo.bind(undefined, elParentPanel)
    )
    elDeleteButton.SetPanelEvent("onmouseover", function () {
      UiToolkitAPI.ShowTextTooltipOnPanel(
        elDeleteButton,
        $.Localize("#WatchMenu_Delete")
      )
    })
    elDeleteButton.SetPanelEvent("onmouseout", function () {
      UiToolkitAPI.HideTextTooltip()
    })
    elDownloadFailedButton.SetPanelEvent(
      "onactivate",
      _DownloadFailedNotify.bind(undefined, elParentPanel)
    )
    elSouvenirButton.SetPanelEvent(
      "onactivate",
      _RedeemSouvenir.bind(
        undefined,
        elParentPanel.tournamentIndex,
        elParentPanel.matchId
      )
