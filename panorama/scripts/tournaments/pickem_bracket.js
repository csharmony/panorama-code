...
var PickEmBracket = (function () {
  ...
  var _UpdateAllSections = function (elPickemPanel, showIdPickIsSaved) {
    if (
      !elPickemPanel._oPickemData.oTournamentData ||
      !elPickemPanel._oPickemData.oInitData
    ) {
      return
    }

    var sectionCount =
      elPickemPanel._oPickemData.oTournamentData.sections.length
  ...
  var _UpdateSection = function (
    elPickemPanel,
    sectionIndex,
    showIdPickIsSaved
  ) {
    var groupsCount =
      elPickemPanel._oPickemData.oTournamentData.sections[sectionIndex].groups
        .length
  ...
  var _SetUpFirstDayTeams = function (elPickemPanel, sectionIndex, groupIndex) {
    var oGroupInfoForUpdate = _GetDataNeededToUpdateTeams(
      elPickemPanel,
      sectionIndex,
      groupIndex
    )
    ...
    for (var i = 0; i < oGroupInfoForUpdate.oGroupData.teamscount; i++) {
      ...
      var teamId = PredictionsAPI.GetGroupTeamIDByIndex(
        oGroupInfoForUpdate.tournamentId,
        oGroupInfoForUpdate.oGroupData.id,
        i
      )
  ...
  var _UpdatePicksInGroup = function (
    elPickemPanel,
    sectionIndex,
    groupIndex,
    showIdPickIsSaved
    ...
    PickemCommon.UpdateCorrectPickState(
      ...
      PredictionsAPI.GetGroupCorrectPicksByIndex(
        elPickemPanel._oPickemData.oTournamentData.tournamentid,
        oGroupInfoForUpdate.oGroupData.id,
        0
      ),
    ...
    oGroupInfoForUpdate.oGroupData.picks[0].storedefindex = notOwned
      ? PickemCommon.GetTeamItemDefIndex(
    ...
    if (showRemoveBtn) {
      elRemoveBtn.SetPanelEvent(
        "onactivate",
        _RemoveBracketsPicks.bind(undefined, elPickemPanel, elPick)
      )
    }
    ...
    PickemCommon.UpdateActionBarBtns(
      elPickemPanel,
      _GetListOfPicksWithNoOwnedItems,
      _MakePicksParams,
      _EnableApply
    )
  }

  var _UpdatePick = function (
    elPickemPanel,
    elPick,
    oGroupInfoForUpdate,
    oItemIdData,
    ...
    if (localTeamId) {
      ...
      if (
        sectionIndex !==
        elPickemPanel._oPickemData.oTournamentData.sections.length - 1
      ) {
    ...
    elPick.SetHasClass(
      "is-saved-pick",
      PickemCommon.IsPickSaved(oGroupInfoForUpdate.oGroupData.picks[0]) &&
        oGroupInfoForUpdate.oGroupData.canpick
    )
  ...
  var _SetIsLocked = function (elPickemPanel) {
    var startIndex = elPickemPanel._oPickemData.oInitData.sectionindex
    return elPickemPanel._oPickemData.oTournamentData.sections[startIndex]
      .isactive
  }
  ...
  var _GetListOfPicksWithNoOwnedItems = function (elPickemPanel) {
    var startIndex = elPickemPanel._oPickemData.oInitData.sectionindex
    var oSectionData =
      elPickemPanel._oPickemData.oTournamentData.sections[startIndex]
  ...
  var _MakePicksParams = function (elPickemPanel, useFakeItems = false) {
    ...
    for (var i = startIndex; i < sectionCount; i++) {
      var groupsList =
        elPickemPanel._oPickemData.oTournamentData.sections[i].groups
      ...
      for (var j = 0; j < groupsCount; j++) {
        ...
        if (groupsList[j].picks[0].localid) {
          ...
          strStickerItemId =
            oItemIdData.type === "fakeitem" && !useFakeItems
              ? ""
              : oItemIdData.itemid

          if (
            strStickerItemId &&
            idsForDisplayInConfimPopup.indexOf(strStickerItemId) === -1
          ) {
  ...
  var _EnableApply = function (elPickemPanel) {
    ...
    return bFoundDifferenceToApply
      ? strErrorString
        ? strErrorString
        : "ok"
      : false
  ...
  var _RemoveBracketsPicks = function (elPickemPanel, elPick) {
    ...
    for (var i = 0; i < targetCount; i++) {
      ...
      oGroupData =
        elPickemPanel._oPickemData.oTournamentData.sections[sectionIndex]
          .groups[groupIndex]
  ...
  var _GetDataNeededToUpdateTeams = function (
    elPickemPanel,
    sectionIndex,
    groupIndex
  ) {
  ...
  var _SetUpIsDraggable = function (elPickemPanel, elTeam, oGroupData) {
    ...
    elTeam._oteamData.dragStartHandle = $.RegisterEventHandler(
      "DragStart",
      elTeam,
      function (targetId, obj) {
        var elDraggable = $.CreatePanel(
          "Panel",
          elTeam,
          "draggable" + elTeam._oteamData.teamid,
          {
        ...
        elDraggable._oteamData.validtargets = _GetTargetsList(
          elTeam.GetParent()
        )
    ...
    elTeam._oteamData.dragEndHandle = $.RegisterEventHandler(
      "DragEnd",
      elTeam,
      function (targetId, obj) {
  ...
  var _HighlightValidTargets = function (elPickemPanel, elTeam, showTargets) {
    ...
    if (targetsList) {
      for (var i = 0; i < targetsList.length; i++) {
        elPickemPanel
          .FindChildInLayoutFile(targetsList[i])
          .SetHasClass("drag-valid-target", showTargets)
      }
    }
  }
  ...
  var _SetUpDragTargets = function (elPickemPanel, elPick) {
    ...
    $.RegisterEventHandler("DragDrop", elPick, function (dispayId, elDisplay) {
  ...
  var _PlaceTempPick = function (elPickemPanel, elDisplay) {
    ...
    for (var i = 0; i < validTargets.length; i++) {
      if (
        elPickemPanel._odraggableData.dragtarget &&
        elPickemPanel._odraggableData.dragtarget.IsValid()
      ) {
        if (validTargets[i] === elPickemPanel._odraggableData.dragtarget.id) {
          ...
          for (var j = 0; j < validTargets.length; j++) {
            var elValidTarget = elPickemPanel.FindChildInLayoutFile(
              validTargets[j]
            )
            ...
            if (
              ...
              teamsInGroup = elValidTarget.GetAttributeString(
                "data-teams-in-group",
                ""
              )
              elValidTarget.TriggerClass("dragdrop")
            } else if (
              _IsTeamIsFromSameGroup(
                elPickemPanel,
                teamsInGroup,
                oGroupData.picks[0].localid
              )
            ) {
              oGroupData.picks[0].localid = 0
            }
            ...
            if (
              validTargets[j] === elPickemPanel._odraggableData.dragtarget.id
            ) {
              foundPick = true
            }
          }
  ...
  var _UpdatePicksWorth = function (elPickemPanel) {
    var fistDayOfBracketIndex =
      elPickemPanel._oPickemData.oInitData.sectionindex
    var sectionCount =
      elPickemPanel._oPickemData.oTournamentData.sections.length -
      fistDayOfBracketIndex

    for (var i = 0; i < sectionCount; i++) {
      var elLabel = elPickemPanel.FindChildInLayoutFile(
        "id-pickem-group-worth" + i
      )
      var points =
        elPickemPanel._oPickemData.oTournamentData.sections[
          i + fistDayOfBracketIndex
        ].groups[0].pickworth

      PickemCommon.SetPointsWorth(
        elLabel,
        points,
        elPickemPanel._oPickemData.oInitData.tournamentid,
        i + fistDayOfBracketIndex
