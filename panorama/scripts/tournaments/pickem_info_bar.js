...
var PickEmInfoBar = (function () {
  ...
  var _SetUpHowToPlayLink = function (elPanel) {
    ...
    if (olinks.hasOwnProperty(tournamentNum)) {
      ...
      elLink.SetPanelEvent("onactivate", function () {
  ...
  var _UpdateTimer = function (elPanel, optbFromScheduledEvent) {
    if (
      optbFromScheduledEvent &&
      elPanel._oPickemData.oInitData.schPickEmInfoBarUpdateTimer
    ) {
    ...
    var oGroupData =
      elPanel._oPickemData.oTournamentData.sections[activeSectionIdx].groups[0]
    ...
    var secRemaining = PredictionsAPI.GetGroupRemainingPredictionSeconds(
      elPanel._oPickemData.oInitData.tournamentid,
      oGroupData.id
    )
    ...
    var sectionId = PredictionsAPI.GetEventSectionIDByIndex(
      elPanel._oPickemData.oInitData.tournamentid,
      elPanel._oPickemData.oInitData.sectionindex
    )
    var isActive = PredictionsAPI.GetSectionIsActive(
      elPanel._oPickemData.oInitData.tournamentid,
      sectionId
    )
    var canPick = PredictionsAPI.GetGroupCanPick(
      elPanel._oPickemData.oInitData.tournamentid,
      oGroupData.id
    )

    var elStatusBar = elPanel.FindChildTraverse("id-pickem-status")
    elStatusBar.SetHasClass(
      "pickem-header-status--active",
      isActive && canPick && secRemaining > 0
    )
    ...
    if (!isActive && canPick) {
      ...
      elStatus.SetDialogVariable(
        "time",
        FormatText.SecondsToSignificantTimeString(secRemaining)
      )
      elStatus.text = $.Localize("#pickem_timer", elStatus)

      if (!elPanel._oPickemData.oInitData.schPickEmInfoBarUpdateTimer) {
        elPanel._oPickemData.oInitData.schPickEmInfoBarUpdateTimer = $.Schedule(
          1,
          _UpdateTimer.bind(undefined, elPanel, true)
        )
      }
      return
    } else if (!canPick) {
      ...
      elStatus.SetDialogVariable(
        "time",
        FormatText.SecondsToSignificantTimeString(60)
      )
      elStatus.text = $.Localize("#pickem_timer", elStatus)
    }
  }
  ...
  var _UpdateScore = function (elPanel) {
    if (
      PickemCommon.GetTournamentIdNumFromString(
        elPanel._oPickemData.oInitData.tournamentid
      ) >= 15
    )
      return

    var pointsEarned = PredictionsAPI.GetMyPredictionsTotalPoints(
      elPanel._oPickemData.oInitData.tournamentid
    )
    var bronzePoints = PredictionsAPI.GetRequiredPredictionsPointsBronze(
      elPanel._oPickemData.oInitData.tournamentid
    )
    var silverPoints = PredictionsAPI.GetRequiredPredictionsPointsSilver(
      elPanel._oPickemData.oInitData.tournamentid
    )
    var goldPoints = PredictionsAPI.GetRequiredPredictionsPointsGold(
      elPanel._oPickemData.oInitData.tournamentid
    )
    var doesTournamentHaveAWinner = DoesTournamentHaveAWinner(
      elPanel._oPickemData.oInitData.tournamentid
    )
    ...
    var elPointsNeededLabel = elPointsNeeded.FindChildTraverse(
      "id-pickem-points-needed-Label"
    )
    var elPointsResultLabel = elPointsNeeded.FindChildTraverse(
      "id-pickem-points-result-Label"
    )
    ...
    if (doesTournamentHaveAWinner) {
      ...
      elPointsNeeded.SetDialogVariable(
        "level",
        $.Localize("#pickem_level_" + nextLevel)
      )
      ...
      elPointsNeededLabel.text = $.Localize(
        "#pickem_points_needed_next_level",
        elPointsNeeded
      )
    }
  }
  ...
  var _SetUpLiveButton = function (elPanel) {
    ...
    if (bIsMinimalMatchInfo) matchId = ""
    ...
    if (matchId) {
      for (var i = 0; i < 2; i++) {
        ...
        elBtn
          .FindChildInLayoutFile("JsTournamentLiveMatchTeam" + i)
  ...
  var SetTrophyImage = function (elPanel, resultLevel) {
    ...
    if (GetTrophyIconPath(tournamentId) !== "") {
      ...
      elTrophyImage.SetImage(
        "file://{images_econ}/" +
          GetTrophyIconPath(tournamentId) +
          resultLevel +
          ".png"
      )
    }
  ...
  var DoesTournamentHaveAWinner = function (tournamentId) {
    ...
    var numDayID = PredictionsAPI.GetEventSectionIDByIndex(
      tournamentId,
      numSections - 1
    )
    ...
    var WinnerTeamId = PredictionsAPI.GetGroupCorrectPicksByIndex(
      tournamentId,
      numGroupId,
      0
    )
