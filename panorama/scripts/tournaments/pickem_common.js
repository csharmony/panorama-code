...
var PickemCommon = (function () {
  ...
  var _IsDataLoaded = function (elPanel) {
    var listState = MatchListAPI.GetState(
      elPanel._oPickemData.oInitData.tournamentid
    )
    var elLoadingStatus = elPanel.FindChildInLayoutFile(
      "id-pickem-loading-status"
    )
    ...
    if (listState === "none") {
      ...
      elPanel._oPickemData.timerhandle = $.Schedule(
        5,
        _MatchStatsLoadedTimeout.bind(undefined, elPanel)
      )

      _UpdateLoadingStatusMessage(
        elPanel,
        $.Localize("#CSGO_Watch_Loading_PickEm"),
        true
      )
      return false
    } else if (listState === "ready") {
      var isLoaded = PredictionsAPI.GetMyPredictionsLoaded(
        elPanel._oPickemData.oInitData.tournamentid
      )
      var sectionsCount = PredictionsAPI.GetEventSectionsCount(
        elPanel._oPickemData.oInitData.tournamentid
      )
      ...
      if (
        !isLoaded ||
        !sectionsCount ||
        elPanel._oPickemData.oInitData.sectionindex === -1
      ) {
        _CancelMatchStatsLoadedTimeout(elPanel)
        elPanel._oPickemData.timerhandle = $.Schedule(
          5,
      ...
      elPanel._oPickemData.oTournamentData = _MakeTournamentDataObject(
        elPanel._oPickemData.oInitData.tournamentid
      )
  ...
  var _GetSectionData = function (tournamentId, dayIndex) {
    var sectionId = PredictionsAPI.GetEventSectionIDByIndex(
      tournamentId,
      dayIndex
    )

    var aGroups = []
    var groupsCount = PredictionsAPI.GetSectionGroupsCount(
      tournamentId,
      sectionId
    )
  ...
  var _GetGroupData = function (tournamentId, sectionId, idx) {
    var groupId = PredictionsAPI.GetSectionGroupIDByIndex(
      tournamentId,
      sectionId,
      idx
    )
  ...
  var _UpdateLoadingStatusMessage = function (
    elPanel,
    messageText,
    showSpinner
  ) {
  ...
  var _ReadyForDisplay = function (elPanel) {
    ...
    elPanel._oPickemData.eventhandleprediction = $.RegisterForUnhandledEvent(
      ...
      elPanel._oPickemData.oInitData.oPickemType.UpdatePrediction.bind(
        undefined,
        elPanel
      )
    )

    elPanel._oPickemData.eventhandlepurchaseUpdate =
      $.RegisterForUnhandledEvent(
        "PanoramaComponent_Store_PurchaseCompleted",
        elPanel._oPickemData.oInitData.oPickemType.PurchaseComplete.bind(
          undefined,
          elPanel
        )
      )

    elPanel._oPickemData.eventhandleinventoryUpdate =
      $.RegisterForUnhandledEvent(
    ...
    if (
      PredictionsAPI.GetMyPredictionsLoaded(
        elPanel._oPickemData.oInitData.tournamentid
      )
    ) {
  ...
  var _UnregisterEvents = function (elPanel) {
    $.UnregisterForUnhandledEvent(
      "PanoramaComponent_MatchList_StateChange",
      elPanel._oPickemData.eventhandle
    )
    $.UnregisterForUnhandledEvent(
      "PanoramaComponent_MatchList_PredictionUploaded",
      elPanel._oPickemData.eventhandleprediction
    )
    $.UnregisterForUnhandledEvent(
      "PanoramaComponent_Store_PurchaseCompleted",
      elPanel._oPickemData.eventhandlepurchaseUpdate
  ...
  var _UpdateImageForPick = function (
    oItemIdData,
    elItemImage,
    localTeamId,
    useSvg = false
    ...
    if (bValidTeamID) {
      if (useSvg) {
        ...
        elItemImage.SetImage(
          "file://{images}/tournaments/teams/" + teamTag + ".svg"
        )
      } else {
  ...
  var _UpdateCorrectPickState = function (
    tournamentId,
    oGroupData,
    correctPicks,
    localTeamId,
    ...
    if (correctPicks) {
      ...
      if (isCorrect) {
        var tournamentNum =
          PickemCommon.GetTournamentIdNumFromString(tournamentId)
        if (tournamentNum >= 15) {
        ...
        var pluralString =
          oGroupData.pickworth === 1
            ? $.Localize("#pickem_point")
            : $.Localize("#pickem_points")
  ...
  var _ShowPickItemNotOwnedWarning = function (
    isSectionActive,
    oGroupData,
    oItemIdData,
    elItemNotOwned,
  ...
  var _SetPointsWorth = function (elLabel, points, tournamentid, sectionindex) {
    ...
    var pluralString =
      points === 1 ? $.Localize("#pickem_point") : $.Localize("#pickem_points")
  ...
  var _SetTeamImage = function (
    ...
    var yourItemId = PredictionsAPI.GetMyPredictionItemIDForTeamID(
      tournamentId,
      elTeam._oteamData.teamid
    )
    var tournamentNum = PickemCommon.GetTournamentIdNumFromString(tournamentId)

    if (
      ((!yourItemId || yourItemId === "0" || yourItemId === 0) &&
        !useFakeItemId) ||
      tournamentNum >= 15
    ) {
      ...
      szImageToUse =
        "file://{images_econ}/" +
        InventoryAPI.GetItemInventoryImage(yourItemId) +
        ".png"
  ...
  var _ShowHideRemoveBtn = function (
    isSectionActive,
    canPick,
    localTeamId,
    elRemoveButton
  ...
  var _IsPickSaved = function (oPick) {
    if (
      oPick.localid === oPick.savedid &&
      oPick.localid !== 0 &&
      oPick.localid !== undefined
    ) {
      return true
    }
  ...
  var _GetYourPicksItemIdData = function (tournamentId, userPickTeamID) {
    ...
    if (
      ...
      yourItemId = PredictionsAPI.GetFakeItemIDToRepresentTeamID(
        tournamentId,
        userPickTeamID
      )
  ...
  var _UpdateActionBarBtns = function (
    elPanel,
    _funcListOfPicksWithNoOwnedItems,
    _funcMakePicksParams,
    _funcEnableApply
  ) {
    var tournamentNum = PickemCommon.GetTournamentIdNumFromString(
      elPanel._oPickemData.oInitData.tournamentid
    )
    var elPurchase = elPanel.FindChildInLayoutFile("id-pickem-getitems")
    ...
    if (tournamentNum >= 15) {
      ...
      _MouseOverEventsWithStickersToPurchase(
        elPurchase,
        "id-pickem-getitems",
        listStoreIndex
      )
  ...
  var _UpdatePurchaseBtnForPass = function (elPurchase, tournamentNum) {
    ...
    var _OpenContextMenu = function () {
      ...
      var itemIdLinked = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
    ...
    if (
      (!id || id === "0") &&
      tournamentNum === g_ActiveTournamentInfo.eventid
    ) {
      id = InventoryAPI.GetActiveTournamentCoinItemId(tournamentNum * -1)
      if (!id || id === "0") {
        id = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
          g_ActiveTournamentInfo.itemid_pass,
          0
        )
        ...
        elPurchase.FindChildInLayoutFile("id-pickem-getitems-label").text =
  ...
  var _UpdateApplyBtnForPass = function (
    elPanel,
    elApplyPicks,
    tournamentNum,
    _funcMakePicksParams
  ...
  var _MouseOverEventsWithStickersToPurchase = function (
    elButton,
    btnid,
    listStoreIndex
  ) {
    ...
    elButton.SetPanelEvent(
      "onmouseover",
      _OnMouseOver.bind(undefined, listStoreIndex)
    )
  ...
  var _EventsForPurchaseBtn = function (elPurchase, listStoreIndex) {
    ...
    elPurchase.SetPanelEvent(
      "onactivate",
      _OnActivatePurchase.bind(undefined, listStoreIndex)
    )
  }

  var _EventsForApplyBtn = function (
    elPanel,
    elApplyBtn,
    _funcListOfPicksWithNoOwnedItems,
    _funcMakePicksParams,
    ...
    elApplyBtn.SetPanelEvent(
      "onactivate",
      _OnApplyPicks.bind(undefined, elPanel, elApplyBtn, useFakeItems)
    )
  }
