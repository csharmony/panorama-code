...
var PickEmGroup = (function () {
  ...
  var _UpdateGroupPicks = function (elPanel) {
    if (
      !elPanel._oPickemData.oTournamentData ||
      !elPanel._oPickemData.oInitData
    ) {
      return
    }
    ...
    for (var i = 0; i < pickCount; i++) {
      ...
      var showRemoveBtn = PickemCommon.ShowHideRemoveBtn(
        elPanel._oPickemData.oTournamentData.sections[activeSectionIdx]
          .isactive,
        oGroupData.canpick,
        oGroupData.picks[i].localid,
        elRemoveBtn
      ...
      elPick.SetHasClass(
        "pickem-pick-placed",
        oGroupData.picks[i].localid ? true : false
      )
      elPick.SetHasClass(
        "is-saved-pick",
        PickemCommon.IsPickSaved(oGroupData.picks[i]) && oGroupData.canpick
      )
  ...
  var _MakePicksParams = function (elPanel, useFakeId = false) {
    ...
    for (var i = 0; i < count; ++i) {
      ...
      if (listPicks[i].localid) {
        ...
        strStickerItemId =
          oItemIdData.type === "fakeitem" && !useFakeId
            ? ""
            : oItemIdData.itemid
  ...
  var _UpdateTeams = function (elPanel) {
    ...
    var teamCount = PredictionsAPI.GetGroupTeamsPickableCount(
      tournamentId,
      groupId
    )
    ...
    for (var i = 0; i < teamCount; i++) {
      ...
      var uniqueId =
        tournamentId + elPanel._oPickemData.oInitData.xmltype + teamId
  ...
  var _SetIsAlreadyPicked = function (elPanel, elTeam) {
    ...
    var isAlreadyPick = PickemCommon.CheckIfTeamIsAlreadyPicked(
      oGroupData,
  ...
  var _TeamTooltips = function (elTeam) {
    var OnMouseOver = function (elTeam) {
      UiToolkitAPI.ShowTextTooltip(
        elTeam.id,
        PredictionsAPI.GetTeamName(elTeam._oteamData.teamid)
      )
  ...
  var _PlaceTempPick = function (elPanel, teamid) {
    ...
    if (
      elPanel._odraggableData.dragtarget &&
      elPanel._odraggableData.dragtarget.IsValid()
    ) {
      var pickIndex = elPanel._odraggableData.dragtarget.GetAttributeString(
        "data-pick-index",
        ""
      )
    } else {
  ...
  var _UpdatePrediction = function (elPanel) {
    ...
    if (!oGroupData || !oGroupData.pickscount) return
    ...
    for (var i = 0; i < oGroupData.pickscount; i++) {
      ...
      if (PickemCommon.IsPickSaved(oGroupData.picks[i]) && oGroupData.canpick) {
        elPick
          .FindChildInLayoutFile("id-pick-boundingbox")
          .TriggerClass("pickem-group-pick-update")
      }
    }
