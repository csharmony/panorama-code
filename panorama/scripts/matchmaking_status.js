...
function MatchmakingStatus(elMatchStatus) {
    ...
    if (_m_elStatusPanel.GetAttributeString("data-type", "") === "hud") {
      ...
      if (mode === "survival") {
        var teamCount = Number(
          GameInterfaceAPI.GetSettingString("sv_dz_team_count")
        )
  ...
  var _Init = function () {
    ...
    _handler_MatchmakingSessionUpdate = $.RegisterForUnhandledEvent(
      "PanoramaComponent_Lobby_MatchmakingSessionUpdate",
      _SessionUpdate
    )

    _handler_GC_Hello = $.RegisterForUnhandledEvent(
      "PanoramaComponent_GC_Hello",
      _SessionUpdate
    )

    _handler_HideMainMenu = $.RegisterForUnhandledEvent(
      "CSGOHideMainMenu",
      _OnHideMainMenu
    )
    _handler_HidePauseMenu = $.RegisterForUnhandledEvent(
      "CSGOHidePauseMenu",
      _OnHidePauseMenu
    )
    _handler_ShowPauseMenu = $.RegisterForUnhandledEvent(
      "CSGOShowPauseMenu",
      _OnShowMenu
    )
    _handler_ShowMainMenu = $.RegisterForUnhandledEvent(
      "CSGOShowMainMenu",
      _OnShowMenu
    )
  }
  ...
  var _Shutdown = function () {
    $.UnregisterForUnhandledEvent(
      "PanoramaComponent_Lobby_MatchmakingSessionUpdate",
      _handler_MatchmakingSessionUpdate
    )
    $.UnregisterForUnhandledEvent(
      "PanoramaComponent_GC_Hello",
      _handler_GC_Hello
  ...
  var _SessionUpdate = function () {
    if (!_m_elStatusPanel || !_m_elStatusPanel.IsValid()) return
  ...
  var _UpdateSearchWaitPanel = function (lobbySettings) {
    ...
    elStatusWait.FindChildInLayoutFile("MatchStatusWaitLabel").text =
      $.Localize("#party_waiting_lobby_leader")
  }

  var _SearchPanelSearching = function (lobbySettings) {
    var elStatusSearching = _m_elStatusPanel.FindChildInLayoutFile(
      "MatchStatusSearching"
    )
    ...
    var unavailableMatch =
      _GetSearchStatus().indexOf("unavailable") !== -1 ? true : false

    var elWarningIcon = elStatusSearching.FindChildInLayoutFile(
      "MatchStatusFailIcon"
    )
    elWarningIcon.SetHasClass("hidden", !unavailableMatch)

    var elSearchTime =
      elStatusSearching.FindChildInLayoutFile("MatchStatusTime")
    elSearchTime.SetHasClass("hidden", unavailableMatch)

    var elLabel = elStatusSearching.FindChildInLayoutFile(
      "MatchStatusSearchingLabel"
    )
  ...
  var _ShowMatchmakingWarnings = function (lobbySettings) {
    var elStatusWarnings =
      _m_elStatusPanel.FindChildInLayoutFile("MatchStatusWarning")
    ...
    if (isWarning)
      elStatusWarnings.FindChild("MatchStatusWarningLabel").text =
        $.Localize(serverWarning)
  }
  ...
  var _CheckForMatchingMissions = function (lobbySettings) {
    ...
    if (
      _IsSeaching() &&
      lobbySettings &&
      lobbySettings.mapgroupname &&
      _m_showMatchingMissions
    ) {
      OperationUtil.MissionsThatMatchYourMatchMakingSettings(
        lobbySettings.mode,
        lobbySettings.mapgroupname.split(","),
        nSeasonAccess
      )
