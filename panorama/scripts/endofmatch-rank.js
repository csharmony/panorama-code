...
var EOM_Rank = (function () {
  ...
  var _DisplayMe = function () {
    ...
    var elRankListerItems = _m_cP.FindChildInLayoutFile(
      "id-eom-rank__lister__items"
    )
    ...
    elCurrent.SetDialogVariable(
      "name",
      $.Localize("#XP_RankName_" + currentRank, elCurrent)
    )

    _m_cP
      .FindChildInLayoutFile("id-eom-rank__current__emblem")
      .SetImage("file://{images}/icons/xp/level" + currentRank + ".png")

    var newRank = currentRank < maxLevel ? currentRank + 1 : maxLevel

    elNew.SetDialogVariableInt("level", newRank)
    elNew.SetDialogVariable(
      "rank_new_name",
      $.Localize("#XP_RankName_" + newRank, elNew)
    )
    _m_cP.SetDialogVariable(
      "rank_new_rank",
      $.Localize("#XP_RankName_Display_Rank", elNew)
    )

    _m_cP
      .FindChildInLayoutFile("id-eom-rank__new-reveal__emblem")
      .SetImage("file://{images}/icons/xp/level" + newRank + ".png")
    ...
    var _AddXPBar = function (reason, xp) {
      ...
      $.Schedule(animTime, function () {
        if (!elBar.IsValid()) return 0

        var elRankSegment = $.CreatePanel(
          "Panel",
          elBar,
          "id-eom-rank__bar__segment"
        )
        ...
        $.DispatchEvent(
          "PlaySoundEffect",
          "UIPanorama.XP.Milestone_0" + _xpSoundNum.toString(),
          "eom-rank"
        )
        if (_xpSoundNum < 4) {
          _xpSoundNum++
        ...
        if (elRankListerItems && elRankListerItems.IsValid()) {
          elCurrentListerItem = $.CreatePanel(
            "Panel",
            elRankListerItems,
            "id-eom-rank__lister__items__" + reason
          )
          ...
          var elAmtLabel = elCurrentListerItem.FindChildTraverse(
            "id-eom-rank__lister__item__amt"
          )
          elAmtLabel.SetDialogVariable("xp", xp)
          elAmtLabel.text = $.Localize("#EOM_XP_Bar", elAmtLabel)
          elAmtLabel.AddClass(colorClass)

          var elDescLabel = elCurrentListerItem.FindChildTraverse(
            "id-eom-rank__lister__item__desc"
          )

          elDescLabel.SetDialogVariable(
            "gamemode",
            $.Localize("#SFUI_GameMode_" + MatchStatsAPI.GetGameMode())
          )

          elDescLabel.text = $.Localize(
            "#XP_Bonus_RankUp_" + reason,
            elDescLabel
          )
        }
      })
    ...
    Object.keys(oXpData["xp_earned"]).forEach(function (key, index) {
      ...
      if (totalXP + xp < xPPerLevel) {
        ...
        } else arrPostRankXP.push(xp_t(key, xp))
      }
    ...
    if (totalXP >= xPPerLevel) {
      _AnimSequenceNext(function () {
        ...
        if (_m_cP && _m_cP.IsValid()) {
          _m_cP
            .FindChildTraverse("id-eom-rank__bar--shine")
            .AddClass("eom-rank__bar--shine--on")
        }
      }, 1)
      ...
      _AnimSequenceNext(function () {
        ...
        elCurrentListerItem = $.CreatePanel(
          "Panel",
          elRankListerItems,
          "id-eom-rank__lister__items__" + "rank-up"
        )
        elCurrentListerItem.BLoadLayoutSnippet("snippet_rank__lister__item")

        elCurrentListerItem.RemoveClass("eom-rank__lister__item--appear")

        elCurrentListerItem.FindChildTraverse(
          "id-eom-rank__lister__item__amt"
        ).text = $.Localize("{s:rank_new_name}", elNew)
        elCurrentListerItem.FindChildTraverse(
          "id-eom-rank__lister__item__desc"
        ).text = $.Localize("{s:rank_new_rank}", elNew)
      }, 1)
      ...
      _AnimSequenceNext(function () {
        ...
        elBar
          .FindChildrenWithClassTraverse("eom-rank__bar__segment")
          .forEach((entry) => entry.DeleteAsync(0.0))
        ...
        elCurrent.SetDialogVariable(
          "name",
          $.Localize("#XP_RankName_" + newRank, elCurrent)
        )
        _m_cP.SetDialogVariable(
          "rank_new",
          $.Localize("#XP_RankName_Display", elCurrent)
        )

        _m_cP.FindChildTraverse("id-eom-rank__current__label").text =
          $.Localize("{s:rank_new}", elCurrent)

        _m_cP
          .FindChildTraverse("id-eom-rank__current__emblem")
          .SetImage("file://{images}/icons/xp/level" + newRank + ".png")
      }, 2)
  ...
  function _Start() {
    if (
      MockAdapter.GetMockData() &&
      !MockAdapter.GetMockData().includes("RANK")
    ) {
      _End()
      return
    }
