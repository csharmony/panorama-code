...
var InventorySearch = (function () {
  ...
  var _Init = function () {
    ...
    var elTextEntry =
      m_elSearchPanel.FindChildInLayoutFile("InvSearchTextEntry")
    elTextEntry.SetPanelEvent(
      "ontextentrychange",
      InventorySearch.OnEntryChanged
    )

    _PopulateSuggested(
      m_elSearchPanel.FindChildInLayoutFile("InvSearchSuggestionsList")
    )
    _TextEntrySettings.SetTextEntryPanel(elTextEntry)

    m_elSuggestedPanel = m_elSearchPanel.FindChildInLayoutFile(
  ...
  var _RegisterForInventoryUpdate = function () {
    m_InventoryUpdatedHandler = $.RegisterForUnhandledEvent(
  ...
  var _UpdateItemList = function () {
    ...
    var sortText = m_elSearchPanel
      .FindChildInLayoutFile("InvSortDropdown")
      .GetSelected().id
  ...
  var _PopulateSuggested = function (elSearchResults) {
    ...
    for (var entry of Object.values(settings)) {
      ...
      if (id !== "") _AddSuggestedEntry(id, elSearchResults)
    }
  }
  ...
  var _GetSuggested = function (sortType, searchText) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (
        itemId !== ItemInfo.IsEquippedForCT &&
      ...
      if (itemsValid > itemsNeedToCollect) break
    }

    var countValidItems = itemsList.length
    if (countValidItems > 0) {
      var index =
        countValidItems > itemsNeedToCollect
