...
var TeamSelectMenu = (function () {
  ...
  function _ShowPanel() {
    if (GameStateAPI.IsDemoOrHltv()) return

    var elBackgroundImage =
      $.GetContextPanel().FindChildInLayoutFile("BackgroundMapImage")
    var mapName = MockAdapter.GetMapBSPName()

    elBackgroundImage.SetImage(
      "file://{images}/map_icons/screenshots/1080p/" + mapName + ".png"
    )
    _GetAnimInfo()
  ...
  function _SelectTeam(team) {
    var currentTeamNumber = MockAdapter.GetPlayerTeamNumber(
  ...
  var _SetCharacterAnim = function (playerPanel, paramsettings) {
    ...
    var settings = ItemInfo.GetOrUpdateVanityCharacterSettings(
      LoadoutAPI.GetItemID(teamstring, "customplayer")
    )
    ...
    if (
      settings.charItemId ===
      LoadoutAPI.GetDefaultItem(teamstring, "customplayer")
    ) {
  ...
  var _PopulatePlayerList = function () {
    ...
    for (var iTeam = 0; iTeam < teamNames.length; ++iTeam) {
      ...
      listOfTeammatesPanels.forEach(function (element) {
        if (
          xuidsOnTeam.indexOf(element.id) === -1 ||
  ...
  var _UpdatePlayer = function (xuid, teamName) {
    ...
    if (!elTeammate) {
      ...
      var elAvatar = $.CreatePanel("Panel", elTeammate, xuid, {
        hittest: "true"
      })
      ...
      elAvatarImage.SetDefaultImage(
        "file://{images}/icons/scoreboard/avatar-" + teamName + ".png"
      )
      ...
      elTeammate.OnPropertyTransitionEndEvent = function (
        panelName,
        propertyName
      ) {
        if (propertyName === "opacity") {
          if (elTeammate.visible === true && elTeammate.BIsTransparent()) {
            elTeammate.DeleteAsync(0.0)
      ...
      $.RegisterEventHandler(
        ...
        elTeammate,
        elTeammate.OnPropertyTransitionEndEvent
      )
      ...
      var weaponItemId = bSameTeam
        ? MockAdapter.GetPlayerActiveWeaponItemId(xuid)
        : InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
            teamName == "CT" ? 42 : 59,
            0
          )

      elTeammate.SetPanelEvent(
        "onmouseover",
        OnMouseOver.bind(
          undefined,
          teamName,
      ...
      elTeammate.SetPanelEvent(
        "onmouseout",
        OnMouseOut.bind(undefined, teamName)
      )
    } else {
  ...
  function _SetPlayerModel(team, charItemId, weaponItemId) {
    var elChar =
      team == "CT"
        ? $.GetContextPanel().FindChildInLayoutFile("TeamCharCT")
        : $.GetContextPanel().FindChildInLayoutFile("TeamCharT")
  ...
  var _UpdateBotPlayerCount = function (countBots, countPlayers, team) {
    var labelCount = $("#BtnSelectTeam-" + team).FindChildInLayoutFile(
      "PlayerBotCount"
    )
    ...
    if (countPlayers === 1)
      labelCount.SetDialogVariable(
        "playerlabel",
        $.Localize("#team_select_player")
      )
    else
      labelCount.SetDialogVariable(
        "playerlabel",
        $.Localize("#team_select_players")
      )
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    "CSGOShowTeamSelectMenu",
    TeamSelectMenu.ShowPanel
  )
  $.RegisterForUnhandledEvent(
    "CSGOShowTeamSelectMenu_Test",
    TeamSelectMenu.ShowPanel_Test
  )

  $.RegisterForUnhandledEvent(
    "PlayerTeamChanged",
    TeamSelectMenu.PopulatePlayerList
  )

  $.RegisterForUnhandledEvent(
    "ServerForcingTeamJoin",
    TeamSelectMenu.ShowCancelButton
  )
