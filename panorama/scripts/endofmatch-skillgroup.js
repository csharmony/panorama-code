...
var EOM_Skillgroup = (function () {
  ...
  var _DisplayMe = function () {
    ...
    if (oData.mode === "survival" && currentRank < 1) {
      ...
      oData.image =
        "file://{images}/icons/skillgroups/" + modePrefix + "_expired.svg"
      ...
      var winNeededString =
        matchesNeeded === 1
          ? "#eom-skillgroup-needed-win"
          : "#eom-skillgroup-needed-wins"
      ...
      var modePrefix =
        oData.mode === "scrimcomp2v2"
          ? "skillgroup_wingman"
          : oData.mode === "survival"
            ? "skillgroup_dangerzone"
            : "skillgroup"
      var modelPath =
        "models/inventory_items/skillgroups/" +
        modePrefix +
        currentRank +
        ".mdl"
      oData.model = modelPath
      oData.rankInfo = $.Localize(
        oData.mode === "survival"
          ? "#skillgroup_" + currentRank + "dangerzone"
          : "RankName_" + currentRank
      )
  ...
  function _LoadAndShowRank() {
    _m_cP
      .FindChildInLayoutFile("id-eom-skillgroup")
      .BLoadLayoutSnippet("eom-skillgroup-display")
  }

  function _LoadAndShowNewRankReveal() {
    _m_cP
      .FindChildInLayoutFile("id-eom-skillgroup")
      .BLoadLayoutSnippet("eom-skillgroup-reveal")

    $.Schedule(0.75, _ShowIcon)
  ...
  function _FilloutRankData(oData) {
    var winString =
      oData.compWins === 1 ? "#eom-skillgroup-win" : "#eom-skillgroup-wins"
    var elDesc = _m_cP.FindChildInLayoutFile("id-eom-skillgroup__wins-desc")
    elDesc.text = $.Localize(winString, _m_cP)

    _m_cP.FindChildInLayoutFile("id-eom-skillgroup__rankname-label").text =
      oData.rankInfo
    _m_cP.FindChildInLayoutFile("id-eom-skillgroup__wins-label").text =
      oData.compWins

    var elRankDesc = _m_cP.FindChildInLayoutFile(
      "id-eom-skillgroup__rankname-desc"
    )
  ...
  function _Start() {
    if (
      ...
      !MockAdapter.GetMockData().includes("SKILLGROUP")
    ) {
      _End()
      return
    }
