...
var PartyMenu = (function () {
  ...
  var _RefreshPartyMembers = function () {
    ...
    var elPartyMembersList =
      $("#PartyList").FindChildInLayoutFile("PartyMembers")
    ...
    if (
      numPlayersActuallyInParty >= PartyListAPI.GetPartySessionUiThreshold() ||
      bIsSearching
    ) {
    ...
    $("#PartyList")
      .GetParent()
      .SetHasClass(
        "friendslist-party-searching",
        bIsSearching && numPlayersActuallyInParty <= 1
      )
  ...
  var _IsSessionActive = function () {
    if (!LobbyAPI.IsSessionActive()) {
      ...
      $("#PartyList")
        .GetParent()
        .SetHasClass("friendslist-party-searching", false)
      friendsList.HideLocalPlayer(false)
      return false
    }
  ...
  var _UpdateMembersList = function (lobbySettings, numPlayersActuallyInParty) {
    ...
    var numPlayersPossibleInMode = SessionUtil.GetMaxLobbySlotsForGameMode(
      lobbySettings.mode
    )
    ...
    for (var i = 0; i < maxAllowedInLobby; i++) {
      var xuid =
        i < numPlayersActuallyInParty ? PartyListAPI.GetXuidByIndex(i) : 0

      var isOverPossible =
        numPlayersActuallyInParty > numPlayersPossibleInMode ? true : false
  ...
  var _MakeNewPartyMemberTile = function (panelIdToLoad, xuid) {
    ...
    if (xuid !== 0 && xuid) _AddOpenPlayerCardAction(memberBtn, xuid)
    else _ClearExisitingOnActivateEvent(memberBtn)
  ...
  var _SetPartyMemberRank = function (elPartyMember, xuid) {
    ...
    if (
      wins < winsNeededForRank ||
      (wins >= winsNeededForRank && skillGroup < 1) ||
      !PartyListAPI.GetFriendPrimeEligible(xuid)
    ) {
    ...
    var imageName =
      skillgroupType !== "Competitive" ? skillgroupType : "skillgroup"
    elRank.SetImage(
  ...
  var _SetLobbyTitle = function (
    numPlayersPossibleInMode,
    numPlayersActuallyInParty
  ) {
    var elPanel = $("#PartyList").FindChildInLayoutFile("PartyListHeader")
    var isSoloSearch = numPlayersActuallyInParty === 1

    elPanel.FindChildInLayoutFile("PartyCancelBtn").visible =
      LobbyAPI.BIsHost() && _IsSearching()
  ...
  var _ClearExisitingOnActivateEvent = function (elPartyMember) {
    ...
    elPartyMember.SetPanelEvent(
      "onmouseover",
      OnMouseOver.bind(undefined, elPartyMember)
    )
  ...
  var _SessionUpdate = function (updateType) {
    if (LobbyAPI.IsSessionActive()) {
      if (m_eventRebuildPartyList == undefined) {
        m_eventRebuildPartyList = $.RegisterForUnhandledEvent(
          "PanoramaComponent_PartyList_RebuildPartyList",
          PartyMenu.RefreshPartyMembers
        )
      }
    } else {
  ...
  var _TintBgForSearch = function () {
    ...
    var isWarning =
      serverWarning !== "" && serverWarning !== undefined ? true : false

    $.GetContextPanel()
      .FindChildInLayoutFile("MatchStatusBackground")
      .SetHasClass("party-list__bg--warning", isWarning && _IsSeaching())
    $.GetContextPanel()
      .FindChildInLayoutFile("MatchStatusBackground")
      .SetHasClass("party-list__bg--searching", _IsSeaching())
  }
  ...
  var _UpdateLeaveBtn = function (numPlayersActuallyInParty) {
    ...
    elLeaveBtn.visible =
      !GameStateAPI.IsLocalPlayerPlayingMatch() && LobbyAPI.IsSessionActive()
  }
  ...
  var _ShowMatchmakingStatusTooltipEvent = function () {
    var btnSettings =
      $.GetContextPanel().FindChildInLayoutFile("MatchStatusInfo")
    ...
    btnSettings.SetPanelEvent("onmouseout", function () {
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_PartyList_PlayerActivityVoice",
    PartyMenu.PlayerActivityVoice
  )
