...
var mainmenu_watch_tournament = (function () {
  ...
  var _NavigateToTab = function (tab, tournament_id, oInitData = null) {
    ...
    if (!pressedTab && oInitData) {
      pressedTab = $.CreatePanel(
        "Panel",
        $.FindChildInContext("#JsTournamentContainer"),
        tab
      )

      pressedTab.BLoadLayout(
        "file://{resources}/layout/tournaments/pickem_" +
          oInitData.xmltype +
          ".xml",
        false,
        false
  ...
  var _AddEventHandlers = function (elPanel) {
    $.RegisterEventHandler(
      "ReadyForDisplay",
      elPanel,
      PickemCommon.ReadyForDisplay.bind(undefined, elPanel)
    )
    $.RegisterEventHandler(
      "UnreadyForDisplay",
      elPanel,
      PickemCommon.UnregisterEvents.bind(undefined, elPanel)
    )
    ...
    $.RegisterEventHandler(
      ...
      elPanel,
      elPanel.OnPropertyTransitionEndEvent
    )
  }
  ...
  var _PopulateTournamentNavBarButtons = function (
    tournament_id,
    elTournamentTab
  ) {
    var tournamentNumber =
      PickemCommon.GetTournamentIdNumFromString(tournament_id)
    var navBarPanel = elTournamentTab.FindChildTraverse("content-navbar__tabs")

    if (!elTournamentTab.hasSetUpNavBar) {
      var _CreateNavBarButton = function (
        buttonId,
        buttonTitle,
        targetTab,
        oInitData = null,
        ...
        elButton.SetPanelEvent(
          "onactivate",
          _NavigateToTab.bind(undefined, targetTab, tournament_id, oInitData)
        )

        return elButton
      }

      var isCurrentTourament =
        tournamentNumber === g_ActiveTournamentInfo.eventid

      var restrictions = LicenseUtil.GetCurrentLicenseRestrictions()
      var bDefaultToMatches =
        restrictions === false && isCurrentTourament ? false : true
      ...
      if (
        ...
        _CreateNavBarButton(
          "id-nav-pick-group",
          $.Localize("#CSGO_Fantasy_PickEm_Groups_Title"),
          "JsPickemGroup",
          {
            tournamentid: tournament_id,
            sectionindex: 1,
        ...
        _CreateNavBarButton(
          "id-nav-pick-playoffs",
          $.Localize("#CSGO_Fantasy_PickEm_Playoffs_Title"),
          "JsPickemPlayoffs",
          {
            tournamentid: tournament_id,
            sectionindex: 2,
      ...
      _m_matchesTab = _CreateNavBarButton(
        "id-nav-matches",
        $.Localize("#CSGO_Watch_Tournament_Matches_T2"),
        "JsTournamentMatches",
        null,
        bDefaultToMatches
      )
  ...
  var _InitializeTournamentsPage = function (tournament_id) {
    if (_m_bInitializedTournamentOnce) return

    var elParentPanel = _GetParentPanel(tournament_id)
    if (!elParentPanel) return

    _m_bInitializedTournamentOnce = true
    elParentPanel.SetDialogVariable(
      "tournament_name",
      $.Localize("#CSGO_Tournament_Event_Name_" + tournament_id.split(":")[1])
    )

    elParentPanel.FindChildInLayoutFile("id-tournament-title-bar").visible =
      elParentPanel.id !== "JsActiveTournament"

    _PopulateTournamentNavBarButtons(tournament_id, elParentPanel)
    elParentPanel.FindChildInLayoutFile("JsTournamentMatches").tournament_id =
      tournament_id
    ...
    if (isCurrentTourament) {
      ...
      $.DispatchEvent(
        "Activated",
        navBarPanel.FindChildInLayoutFile(tabIdToActivate),
        "mouse"
      )
    } else {
      $.DispatchEvent(
        "Activated",
        navBarPanel.FindChildInLayoutFile("id-nav-matches"),
        "mouse"
      )
    }
  ...
  var _SetStyleOverridesForTournament = function (
    elParentPanel,
    tournamentNumber
  ) {
