...
var EOM_Podium = (function () {
  ...
  function _AddPlayerToPodium(_xuid, _position) {
    if (_xuid === undefined) return

    var playerName = GameStateAPI.GetPlayerName(_xuid)

    var elPodiumPlayers = _m_cP.FindChildTraverse(
      "id-eom-podium__podium__players"
    )
    ...
    if (!elPlayer) {
      for (var i = 1; i <= _position; i++) {
        if (!elPodiumPlayers.FindChildTraverse("id-player--" + i)) {
          ...
          elPlayer
            .FindChildTraverse("id-eom-podium__podium__player__name-and-avatar")
            .AddClass("hidden")
        }
      }
    }

    var elPlayerNamePlaque = elPlayer.FindChildTraverse(
      "id-eom-podium__podium__player__plaque"
    )
    var elPlayerNameTag = elPlayer.FindChildTraverse(
      "id-eom-podium__podium__player__name-and-avatar"
    )

    elPlayerNameTag.RemoveClass("hidden")

    var PlaqueBg = elPlayerNamePlaque.FindChildTraverse(
      "id-eom-podium__podium__player__plaque__bg"
    )
    ...
    elPlayerNameTag.FindChildTraverse(
      "id-eom-podium__podium__player__name"
    ).text = playerName

    if (!GameStateAPI.IsFakePlayer(_xuid)) {
      elPlayerNameTag.FindChildTraverse(
        "id-eom-podium__podium__player__avatar"
      ).steamid = _xuid
    } else {
      var team = GameStateAPI.GetPlayerTeamName(_xuid)

      elPlayerNameTag
        .FindChildTraverse("id-eom-podium__podium__player__avatar")
        .AddClass("eom-podium__podium__player__avatar--" + team)
    }
    ...
    if (rank > 0) {
      var elRankImage = elPlayer.FindChildTraverse(
        "id-eom-podium__podium__player__rank"
      )
    ...
    if (skillGroup > 0) {
      var elRankImage = elPlayer.FindChildTraverse(
        "id-eom-podium__podium__player__skillgroup"
      )

      var imagepath =
        "file://{images}/icons/skillgroups/skillgroup" + skillGroup + ".png"
    ...
    Object.keys(_m_oBestStats).forEach((stat) => {
      if (_m_oBestStats[stat].m_leaderXuid == _xuid) {
        if (
          !(_xuid in oStatsPerPlayer) ||
          oStatsPerPlayer[_xuid] < MAX_STATS_PER_PLAYER
        ) {
          var elPanel = _m_oBestStats[stat].m_elPanel
          elPanel.SetParent(
            elPlayer.FindChildTraverse("id-eom-podium__podium__player__stats")
          )
    ...
    var elPlayerModel = $.CreatePanel(
      "ItemPreviewPanel",
      _m_cP.FindChildTraverse("id-eom-podium__podium__player-models"),
      "id-eom-podium__podium__player__model"
    )
    ...
    if (_xuid != GameStateAPI.GetLocalPlayerXuid()) {
      ...
      var elCommendButton = elPlayer.FindChildTraverse(
        "id-eom-podium__podium__player__commend--text"
      )

      elCommendButton.RemoveClass("hidden")
      elCommendButton.SetPanelEvent(
        "onactivate",
        onActivate.bind(undefined, _xuid)
      )
    }
  }
  ...
  function _CalculateTeamBestStats() {
    var _InitBestStatsLeaderPanel = function (stat) {
      if (!(stat in _m_oBestStats)) {
        var elHiddenStats = _m_cP.FindChildTraverse(
          "id-eom-podium-podium__hiddenstats"
        )
    ...
    var myTeamName = GameStateAPI.GetPlayerTeamName(
      GameStateAPI.GetLocalPlayerXuid()
    )

    for (var teamName in oPlayerList) {
      if (teamName !== myTeamName) continue
      ...
      for (var j in oPlayerList[teamName]) {
        ...
        Object.keys(oPlayerBestStats).forEach((stat) => {
          if (arrValidStats.indexOf(stat) === -1) return
  ...
  function _PopulateTeamPodium() {
    ...
    arrSortedTeam.sort(function (a, b) {
      return b[1] - a[1]
    })

    if (arrSortedTeam[4]) _AddPlayerToPodium(arrSortedTeam[4][0], 5)

    if (arrSortedTeam[3]) _AddPlayerToPodium(arrSortedTeam[3][0], 1)

    if (arrSortedTeam[2]) _AddPlayerToPodium(arrSortedTeam[2][0], 4)

    if (arrSortedTeam[1]) _AddPlayerToPodium(arrSortedTeam[1][0], 2)

    if (arrSortedTeam[0]) _AddPlayerToPodium(arrSortedTeam[0][0], 3)
  }
  ...
  function _Initialize() {
    ...
    _m_cP
      .FindChildrenWithClassTraverse("eom-event")
      .forEach(function (elPanel) {
        var panelXML =
          "file://{resources}/layout/" +
          elPanel.GetAttributeString("data-xml", "")
  ...
  return {
    ...
    Start: _Start,
    GetFreeForAllTopThreePlayers_Response:
      _GetFreeForAllTopThreePlayers_Response,
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    ...
    EOM_Podium.GetFreeForAllTopThreePlayers_Response
  )

  $.RegisterForUnhandledEvent(
    "EndOfMatch_GetFreeForAllPlayerPosition_Response",
    EOM_Podium.GetFreeForAllPlayerPosition_Response
  )

  $.RegisterForUnhandledEvent(
    "EOM_Podium_ShowNextEvent",
    EOM_Podium.ShowNextEvent
  )
})()
