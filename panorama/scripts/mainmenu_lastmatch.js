...
var LastMatch = (function () {
  ...
  var _Init = function () {
    ...
    _m_cp
      .FindChildInLayoutFile("id-mainmenu-lastmatch")
  ...
  var _RegisterForInventoryUpdate = function () {
    if (!_m_InventoryUpdatedHandler) {
      _m_InventoryUpdatedHandler = $.RegisterForUnhandledEvent(
        ...
        LastMatch.OnStatsReceived
      )
    }

    if (_m_DeepStatsEvtHandle === null) {
      _m_DeepStatsEvtHandle = $.RegisterForUnhandledEvent(
        "DeepStatsReceived",
        LastMatch.OnStatsReceived
      )
    }
  ...
  var _OnStatsReceived = function () {
    ...
    if (oMatch.matches.length > 0 && _HasSubscription()) {
  ...
  var _UpdateStatsPanel = function (oMatchFromDeepStats) {
    if (
      ...
      _m_oLastMatch.matches[0].player.match_id ===
        oMatchFromDeepStats.matches[0].player.match_id
    ) {
      return
    }
    ...
    elResult.SetHasClass(
      "mainmenu__lastmatch__match__win-loss--green",
      (oMatch.match_outcome & 0x3) == 1
    )
    elResult.SetHasClass(
      "mainmenu__lastmatch__match__win-loss--red",
      (oMatch.match_outcome & 0x3) == 2
    )
    elResult.SetHasClass(
      "mainmenu__lastmatch__match__win-loss--yellow",
      (oMatch.match_outcome & 0x3) == 0
    )
    elResult.SetHasClass(
      "mainmenu__lastmatch__match__win-loss--dnf",
  ...
  function _UpdateStats(oMatch, mode) {
    var elStatsContainer = _m_cp.FindChildTraverse(
      "id-mainmenu-lastmatch-match-stats"
    )
  ...
  var _MapInfo = function (mapName) {
    var elMapIcon = _m_cp.FindChildInLayoutFile(
      "id-mainmenu-lastmatch-match-map-icon"
    )
    elMapIcon.SetImage("file://{images}/map_icons/map_icon_" + mapName + ".svg")
    _m_cp.FindChildInLayoutFile("id-mainmenu-lastmatch-match-map-name").text =
      $.Localize("SFUI_Map_" + mapName)

    var elBackground = _m_cp.FindChildInLayoutFile(
      "id-mainmenu-lastmatch-background"
    )
  ...
  var _MatchTimeAndMode = function (time, mode) {
    var elMapTime = _m_cp.FindChildInLayoutFile(
      "id-mainmenu-lastmatch-match-map-time"
    )
  ...
  var _UpdateTeammates = function (arrMates) {
    var elMates = _m_cp.FindChildInLayoutFile(
      "id-mainmenu-lastmatch-match-teammates"
    )
