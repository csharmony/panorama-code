...
function UpdateStoreTiles() {
  ...
  Object.keys(banner_layout).forEach(function (key, index) {
    ...
    storeTile.BLoadLayout(
      "file://{resources}/layout/storeitem.xml",
      false,
      false
    )
    storeTile.SetAttributeInt("item_defidx", parseInt(key, 10))
    var ItemDef = JSON.parse(
      InventoryAPI.GetEconItemDefinition(parseInt(key, 10))
    )
    storeTile.FindChildInLayoutFile("ItemTitle").text = $.Localize(
      ItemDef.item_name
    )
    storeTile.FindChildInLayoutFile("ItemDescription").text = $.Localize(
      "#SFUI_Store_Hint_" + ItemDef.name.split(" ").join("_")
    )
...
;(function () {
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Store_UpdateStoreTiles",
    UpdateStoreTiles
  )
