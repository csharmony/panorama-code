...
var CharacterAnims = (function () {
  ...
  var _PlayAnimsOnPanel = function (
    importedSettings,
    bDontStompModel = false,
    makeDeepCopy = true
  ) {
    ...
    var settings = makeDeepCopy
      ? ItemInfo.DeepCopyVanityCharacterSettings(importedSettings)
      : importedSettings

    if (!settings.team || settings.team == "") settings.team = "ct"
    ...
    settings.arrModifiers.forEach((mod) =>
      playerPanel.ApplyActivityModifier(mod)
    )
    ...
    if ("flashlightColor" in settings && settings.flashlightColor !== "") {
      playerPanel.SetFlashlightColor(
        settings.flashlightColor[0],
        settings.flashlightColor[1],
        settings.flashlightColor[2]
      )
    }

    if ("ambientLightColor" in settings && settings.ambientLightColor !== "") {
      playerPanel.SetAmbientLightColor(
        settings.ambientLightColor[0],
        settings.ambientLightColor[1],
        settings.ambientLightColor[2]
      )
    }
  }
  ...
  var _GetValidCharacterModels = function (bUniquePerTeamModelsOnly) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (bUniquePerTeamModelsOnly) {
        if (uniqueTracker.hasOwnProperty(team + modelplayer)) continue
