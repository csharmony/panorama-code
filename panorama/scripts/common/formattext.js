...
var FormatText = (function () {
  ...
  var _SecondsToDDHHMMSSWithSymbolSeperator = function (rawSeconds) {
    ...
    for (var key in time) {
      ...
      if (returnRemaining) {
        var valueToShow =
          time[key] < 10 ? "0" + time[key].toString() : time[key].toString()
  ...
  var _SplitAbbreviateNumber = function (number, fixed = 0) {
    ...
    do {
      ...
      if ($.Localize(stringToken) != stringToken) break
    } while (--pow10 > 0)

    if ($.Localize(stringToken) == stringToken) return [number, ""]
  ...
  var _AbbreviateNumber = function (number) {
    ...
    do {
      ...
      if (_IsLocalizationValid($.Localize(stringToken))) break
    } while (--pow10 > 0)

    if (!_IsLocalizationValid($.Localize(stringToken))) return number
