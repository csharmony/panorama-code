...
var PromotedSettingsUtil = (function () {
  function _GetUnacknowledgedPromotedSettings() {
    if (g_PromotedSettings.length == 0) return []

    let settingsInfo = GameInterfaceAPI.GetSettingString(
    ...
    if (version === g_PromotedSettingsVersion) {
      ...
      const now = new Date()
      return g_PromotedSettings.filter(
  ...
  var hPromotedSettingsViewedEvt = $.RegisterForUnhandledEvent(
    ...
    function (id) {
      GameInterfaceAPI.SetSettingString(
        "cl_promoted_settings_acknowledged",
        "" + g_PromotedSettingsVersion + ":" + Date.now()
      )
