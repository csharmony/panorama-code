...
var ItemInfo = (function () {
  ...
  var _GetFormattedName = function (id) {
    ...
    if (InventoryAPI.HasCustomName(id)) {
      return new CFormattedText("#CSGO_ItemName_Custom", {
        item_name: strName
      })
    } else {
      ...
      if (splitLoc >= 0) {
        ...
        return new CFormattedText("#CSGO_ItemName_Painted", {
          item_name: strWeaponName,
          paintkit_name: strPaintName
        })
      }
  ...
  var _EnsureShuffleItemEquipped = function (itemID, team) {
    var equippedItemID = ItemInfo.GetItemIdForItemEquippedInLoadoutSlot(
      itemID,
      team
    )
  ...
  var _DeepCopyVanityCharacterSettings = function (inVanityCharacterSettings) {
    var modelRenderSettingsOneOffTempCopy = JSON.parse(
      JSON.stringify(inVanityCharacterSettings)
    )
  ...
  var _PrecacheVanityCharacterSettings = function (inVanityCharacterSettings) {
    if (inVanityCharacterSettings.weaponItemId)
      InventoryAPI.PrecacheCustomMaterials(
        inVanityCharacterSettings.weaponItemId
      )
    if (inVanityCharacterSettings.glovesItemId)
      InventoryAPI.PrecacheCustomMaterials(
        inVanityCharacterSettings.glovesItemId
      )
  }

  var _GetOrUpdateVanityCharacterSettings = function (
    optionalCharacterItemId,
    optionalState
  ) {
    var oSettings = {
      panel: undefined,
    ...
    if (
      optionalCharacterItemId &&
      InventoryAPI.IsValidItemID(optionalCharacterItemId)
    ) {
      var charTeam = ItemInfo.GetTeam(optionalCharacterItemId)
      if (charTeam.search("Team_CT") !== -1) oSettings.team = "ct"
      else if (charTeam.search("Team_T") !== -1) oSettings.team = "t"

      if (oSettings.team) oSettings.charItemId = optionalCharacterItemId
    }

    if (!oSettings.team) {
      oSettings.team = GameInterfaceAPI.GetSettingString(
    ...
    var _fnRollRandomLoadoutSlotAndWeapon = function (strTeam) {
      ...
      while (slots.length > 0) {
        ...
        myResult.weaponItemId = LoadoutAPI.GetItemID(
          strTeam,
          myResult.loadoutSlot
        )
        if (ItemInfo.IsWeapon(myResult.weaponItemId)) break
      }
      return myResult
    }

    oSettings.loadoutSlot = GameInterfaceAPI.GetSettingString(
      "ui_vanitysetting_loadoutslot_" + oSettings.team
    )
    ...
    oSettings.glovesItemId = LoadoutAPI.GetItemID(
      oSettings.team,
      "clothing_hands"
    )

    if (!oSettings.charItemId)
      oSettings.charItemId = LoadoutAPI.GetItemID(
        oSettings.team,
        "customplayer"
      )
    ...
    if (optionalState && optionalState === "unowned") {
      var randomResult = _fnRollRandomLoadoutSlotAndWeapon(oSettings.team)
      oSettings.loadoutSlot = randomResult.loadoutSlot
      oSettings.weaponItemId = LoadoutAPI.GetDefaultItem(
        oSettings.team,
        oSettings.loadoutSlot
      )
      oSettings.glovesItemId = LoadoutAPI.GetDefaultItem(
        oSettings.team,
        "clothing_hands"
      )
    }
  ...
  var _IsStatTrak = function (id) {
    var numIsStatTrak = InventoryAPI.GetRawDefinitionKey(
      id,
  ...
  var _IsEquippableThroughContextMenu = function (id) {
    var subSlot = _GetSlotSubPosition(id)
    return (
      subSlot === "flair0" || subSlot === "musickit" || subSlot === "spray0"
    )
  }
  ...
  var _GetFauxReplacementItemID = function (id, purpose) {
    if (purpose === "graffiti") {
      if (_ItemDefinitionNameSubstrMatch(id, "tournament_journal_")) {
        return _GetFauxItemIdForGraffiti(
          parseInt(InventoryAPI.GetItemAttributeValue(id, "sticker slot 0 id"))
        )
      }
    }
    return id
  ...
  var _GetModelPath = function (id, itemSchemaDef) {
    ...
    var isFanTokenOrShieldItem =
      itemSchemaDef.name &&
      itemSchemaDef.name.indexOf("tournament_journal_") != -1

    if (isSpray || isSprayPaint || isFanTokenOrShieldItem)
      return "vmt://spraypreview_" + id
    else if (_IsSticker(id) || _IsPatch(id)) return "vmt://stickerpreview_" + id
    else if (
      itemSchemaDef.hasOwnProperty("model_player") ||
      isMusicKit ||
      issMusicKitDefault
    )
  ...
  var _GetDefaultCheer = function (id) {
    ...
    if (itemSchemaDef["default_cheer"]) return itemSchemaDef["default_cheer"]
    else return ""
  }
  ...
  var _GetModelPathFromJSONOrAPI = function (id) {
    ...
    if (_GetSlotSubPosition(id) === "flair0") {
      pedistalModel = itemSchemaDef.hasOwnProperty("attributes")
        ? itemSchemaDef.attributes["pedestal display model"]
        : ""
    } else if (_ItemHasCapability(id, "decodable")) {
      pedistalModel = itemSchemaDef.hasOwnProperty("model_player")
        ? itemSchemaDef.model_player
        : ""
    }

    return pedistalModel === ""
      ? _GetModelPath(id, itemSchemaDef)
      : pedistalModel
  }
  ...
  return {
    ...
    BIsRewardPremium: function (id) {
      return InventoryAPI.BIsRewardPremium(id)
    },
    GetRewardTier: function (id) {
      return InventoryAPI.GetRewardTier(id)
    },
    ...
    GetItemIdForItemEquippedInLoadoutSlot:
      _GetItemIdForItemEquippedInLoadoutSlot,
