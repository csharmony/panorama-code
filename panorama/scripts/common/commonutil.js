...
var CommonUtil = (function () {
  ...
  function _SetRegionOnLabel(isoCode, elPanel, tooltip = true) {
    ...
    _SetDataOnLabelInternal(
      isoCode,
      isoCode,
      tooltip ? tooltipString : "",
      elPanel,
      tooltipString ? false : true
    )
  }
  ...
  function _SetLanguageOnLabel(isoCode, elPanel, tooltip = true) {
    ...
    if (isoCode) {
      ...
      if (sTranslated && sLocal && sTranslated === sLocal) {
        ...
        tooltipString =
          sTranslated && sLocal ? sTranslated + " (" + sLocal + ")" : ""
      }
  ...
  function _SetDataOnLabelInternal(
    isoCode,
    imgCode,
    tooltipString,
    elPanel,
    ...
    if (isoCode) {
      ...
      elLabel.style.backgroundImage =
        'url("file://{images}/regions/' + imgCode + '.png")'

      var elTTAnchor = elLabel.FindChildTraverse("region-tt-anchor")
      if (!elTTAnchor) {
        elTTAnchor = $.CreatePanel(
          "Panel",
          elLabel,
          elPanel.id + "-region-tt-anchor"
        )
      }

      if (tooltipString) {
        elLabel.SetPanelEvent("onmouseover", (_) =>
          UiToolkitAPI.ShowTextTooltip(elTTAnchor.id, tooltipString)
        )
        elLabel.SetPanelEvent("onmouseout", (_) =>
