...
var EOM_Drops = (function () {
  ...
  var _DisplayMe = function () {
    ...
    Object.keys(oDropList).forEach(function (key, index) {
      $.Schedule(animTime, function () {
        ...
        itemId = InventoryAPI.IsItemInfoValid(itemId)
          ? itemId
          : oDropList[key].faux_item_id
        ...
        if (color) {
          if (isAdDrop) {
            elDropContainer.FindChildInLayoutFile(
              "id-item-rarity"
        ...
        elDropContainer
          .FindChildInLayoutFile("id-item-rarity")
          .AddClass("reveal")

        if (isAdDrop) {
          elDropContainer
            .FindChildInLayoutFile("id-item-image")
            .SetImage("file://{images}/ui_textures/nonprime_item_icon_1.png")
        } else {
        ...
        if (isAdDrop) {
          elNameLabel.text = oDropList[key].is_local
        ...
        var elOwnerLabel =
          elDropContainer.FindChildInLayoutFile("id-item-owner-name")
        elOwnerLabel.AddClass("eom-drops__item__owner")
        elOwnerLabel.SetDialogVariable(
          "user_name",
          GameStateAPI.GetPlayerName(oDropList[key].owner_xuid)
        )
        ...
        TintSprayIcon.CheckIsSprayAndTint(
          itemId,
          elDropContainer.FindChildInLayoutFile("id-item-image")
        )
        ...
        if (
          (oDropList[key].is_local && rarityVal < 3) ||
          oDropList[key].reason === 3
        ) {
  ...
  function _Start() {
    if (
      ...
      !MockAdapter.GetMockData().includes("DROPS")
    ) {
      _End()
      return
    }
