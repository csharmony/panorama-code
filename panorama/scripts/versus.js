...
var Versus = (function () {
  ...
  function _Init(mockdata) {
    ...
    _m_totalTime =
      Number(
        GameInterfaceAPI.GetSettingString("sv_warmup_to_freezetime_delay")
      ) + 0.5
    ...
    _m_sTeamName = MockAdapter.GetPlayerTeamName(
      MockAdapter.GetLocalPlayerXuid()
    )
    ...
    _m_elPreviewPanel_MyTeam = _m_elMyTeam.FindChildTraverse(
      "id-charlineup__characters"
    )
    _m_elPreviewPanel_MyTeam.SetScene(
      "resource/ui/econ/VersusCharactersMine.res",
    ...
    _m_elMyTeam.SetDialogVariable(
      "teamname",
      MockAdapter.GetTeamClanName(_m_sTeamName)
    )
  ...
  function _SelectScene() {
    ...
    if (
      !_m_oPlayersData ||
      !_m_oPlayersData.scene ||
      !_m_oPlayersData.allplayerdata ||
      !_m_oPlayersData.allplayerdata.length
    )
      return false

    _m_oScene = $.LoadKeyValuesFile(
      "resource/characterscenes/" + _m_oPlayersData.scene + ".res",
      "MOD"
    )
    if (!_m_oScene) return false

    var numModelsCT = 0
    if ("CT" in _m_oScene) {
      while ("" + numModelsCT in _m_oScene["CT"]) ++numModelsCT
    }
    var numModelsT = 0
    if ("TERRORIST" in _m_oScene) {
      while ("" + numModelsT in _m_oScene["TERRORIST"]) ++numModelsT
    }

    var numModelsMax = numModelsCT > numModelsT ? numModelsCT : numModelsT
    if (numModelsMax <= 0) return false
    if (numModelsCT != numModelsT) return false
    ...
    for (var j = 0; j < _m_oPlayersData.allplayerdata.length; ++j) {
      ...
      if (teamnameForPlayer) {
        _m_oScene.players[teamnameForPlayer].push(
          _m_oPlayersData.allplayerdata[j]
        )
      }
    }
  ...
  function _AddCharacter(n, team) {
    ...
    if (n >= _m_oScene.players[team].length) return false
    ...
    var elIdentityContainer = elTeamPanel.FindChildTraverse(
      "id-vs-identity-container"
    )

    var elIdentity = $.CreatePanel(
      "Panel",
      elIdentityContainer,
      "id-identity-" + team + "-" + n
    )
    ...
    if (elAvatarImage) {
      ...
      var teamColor =
        thisPlayer.playercolor === undefined
          ? ""
          : GameStateAPI.GetPlayerColor("#" + thisPlayer.playercolor)
      if (teamColor) {
  ...
  function _AddScene(n, team, mdlName) {
    var elPanel =
      team == "enemy" ? _m_elPreviewPanel_EnemyTeam : _m_elPreviewPanel_MyTeam

    elPanel.SetActiveSceneContext(n)
    elPanel.SetSceneModel(mdlName)
  }

  function _AspectRatioTest(
    returnThisIf4x3,
    returnThisIf16x9,
    returnThisIf16x10
  ) {
  ...
  function _Show(teamOverride = undefined) {
    ...
    _AddScene(
      _m_oScene["numModels"],
      "mine",
      "models/weapons/pedestal_team01.mdl"
    )
    ...
    $.Schedule(wipeDuration / 2, function () {
      ...
      _m_elPreviewPanel_MyTeam.style.animationDuration =
        teamFadeInDuration + "s"
    ...
    if (1)
      $.Schedule(
        ...
        function (cTime) {
          ...
          var elIdentityContainer = _m_elMyTeam.FindChildTraverse(
            "id-vs-identity-container"
          )
            ...
            $.Schedule(
              ...
              function (panel) {
                ...
                if (index > 0) {
                  ...
                  var prevPosY = Number(
                    prevPosString.split(" ")[1].split("px")[0]
                  )
                }

                _m_elPreviewPanel_MyTeam.SetActiveSceneContext(index)
                var PanelPos =
                  _m_elPreviewPanel_MyTeam.GetBonePositionInPanelSpace("head_0")
                PanelPos.y -= 100
                ...
                if (0) {
                  ...
                  debugPanel.style.position =
                    PanelPos.x + "px " + PanelPos.y + "px 0px"
                }
          ...
          $.Schedule(0, function () {
            var elMyTeamName = _m_elMyTeam.FindChildTraverse(
              "id-charlineup__characters__teamname"
            )
