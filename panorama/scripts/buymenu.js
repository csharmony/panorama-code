...
var BuyMenu = (function () {
  ...
  var _UpdateCharacter = function (
    ...
    var elPreviewPanel =
      $.GetContextPanel().FindChildTraverse("id-buymenu-agent")
    if (!elPreviewPanel) return

    if (!weaponItemId) {
      weaponItemId = MockAdapter.GetPlayerActiveWeaponItemId(
    ...
    if (
      settings.charItemId == 0 ||
      settings.charItemId ===
        LoadoutAPI.GetDefaultItem(teamstring, "customplayer")
    ) {
      settings.modelOverride = MockAdapter.GetPlayerModel(
        MockAdapter.GetLocalPlayerXuid()
      )
    }
    ...
    elPreviewPanel.SetFlashlightAngle(21.01, 161.45, 0.0)
    elPreviewPanel.SetFlashlightPosition(51.49, -13.39, 75.93)
    elPreviewPanel.SetFlashlightAmount(1)

    if (bResetAgentAngles) elPreviewPanel.SetSceneAngles(0.0, 0.0, 0.0, true)
...
;(function () {
  $.RegisterForUnhandledEvent(
    "BuyMenu_UpdateCharacter",
    BuyMenu.UpdateCharacter
  )
})()
