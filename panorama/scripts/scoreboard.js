...
var arrSpectatorVisibleStats = ["ping", "avatar", "name"]

var arrStatsNotToTeamColorize = ["ping"]
...
var Scoreboard = (function () {
  ...
  function team_t(teamName) {
    ...
    function _UpdateCommendForPlayer(xuid, stat, value) {
      ...
      var playerCommend = _m_CommendLeaderboards[stat].find(
        (p) => p.m_xuid === xuid
      )
    ...
    function _DeletePlayerFromCommendsLeaderboards(xuid) {
      ;["leader", "teacher", "friendly"].forEach(function (stat) {
        var index = _m_CommendLeaderboards[stat].findIndex(
          (p) => p.m_xuid === xuid
        )
    ...
    function _ChangeCommendDisplay(xuid, stat, turnon) {
      ...
      if (!elPlayer || !elPlayer.IsValid()) return

      var elCommendationImage = elPlayer.FindChildTraverse(
        "id-sb-name__commendations__" + stat
      )
      if (!elCommendationImage || !elCommendationImage.IsValid()) return

      if (turnon) elCommendationImage.RemoveClass("hidden")
      else elCommendationImage.AddClass("hidden")
    }

    function _SortCommendLeaderboard(stat) {
      _m_CommendLeaderboards[stat].sort(function (a, b) {
    ...
    function _GetCommendBestXuid(stat) {
      switch (stat) {
        case "leader":
          return _GetCommendTopLeaderXuid()
        case "teacher":
          return _GetCommendTopTeacherXuid()
        case "friendly":
          return _GetCommendTopFriendlyXuid()
      }
    }
    ...
    function _GetCommendTopTeacherXuid() {
      ...
      if (teacher0 != _GetCommendTopLeaderXuid()) return teacher0
    ...
    function _GetCommendTopFriendlyXuid() {
      ...
      if (
        friendly0 != _GetCommendTopLeaderXuid() &&
        friendly0 != _GetCommendTopTeacherXuid()
      )
        return friendly0
      else if (
        friendly1 != _GetCommendTopLeaderXuid() &&
        friendly1 != _GetCommendTopTeacherXuid()
      )
        return friendly1
      else return friendly2
    }
    ...
    return {
      ...
      DeletePlayerFromCommendsLeaderboards:
        _DeletePlayerFromCommendsLeaderboards,
  ...
  function allplayers_t() {
    ...
    function _AddPlayer(xuid) {
      ...
      if (IsTeamASpecTeam(teamName)) teamName = "Spectator"
    ...
    function _DeletePlayerByXuid(xuid) {
      ...
      if (_m_arrPlayers[i].m_elPlayer && _m_arrPlayers[i].m_elPlayer.IsValid())
        _m_arrPlayers[i].m_elPlayer.DeleteAsync(0.0)
  ...
  function _PopulatePlayerList() {
    ...
    for (var teamName in oPlayerList) {
      ...
      for (var j in oPlayerList[teamName]) {
        ...
        if (!ShouldCreateTeamForMode(teamName)) continue
    ...
    function CreateTeam(team) {
      if (!ShouldCreateTeamForMode(team)) return
    ...
    function ShouldCreateTeamForMode(team) {
      ...
      if (mode == "cooperative" || mode == "coopmission") {
        return (
          team.toUpperCase() ===
          MockAdapter.GetCooperativePlayerTeamName().toUpperCase()
        )
      }
      return true
    }
  ...
  function _ChangeTeams(oPlayer, newTeam) {
    if (oPlayer.m_oStats["teamname"] == newTeam) return
    ...
    if (oldTeam) elPlayer.RemoveClass("sb-team--" + oldTeam)
  ...
  function _UpdateSpectatorButtons() {
    ...
    if (!elButtonPanel || !elButtonPanel.IsValid()) return

    var nCameraMan = parseInt(
      GameInterfaceAPI.GetSettingString("spec_autodirector_cameraman")
    )
  ...
  function _SortPlayer(idx) {
    if (_m_arrSortingPausedRefGetCounter != 0) return

    var oPlayer = _m_oPlayers.GetPlayerByIndex(idx)

    var elTeam = oPlayer.m_elTeam
    if (!elTeam || !elTeam.IsValid()) return
    ...
    for (var i = 0; i < children.length; i++) {
      if (oPlayer.m_xuid === children[i].m_xuid) continue

      var oCompareTargetPlayer = _m_oPlayers.GetPlayerByXuid(children[i].m_xuid)
      if (!oCompareTargetPlayer) continue
  ...
  function _GetMatchStatFn(stat) {
    var _fn = function (xuid) {
      ...
      if (allstats) return allstats[stat] == -1 ? "-" : allstats[stat]
    }
  ...
  function _CreateStatUpdateFn(stat) {
    ...
    switch (stat) {
      case "musickit":
        fn = function (oPlayer, bSilent = false) {
          if (MockAdapter.IsFakePlayer(oPlayer.m_xuid)) return
          ...
          var borrowedPlayerIndex = parseInt(
            GameInterfaceAPI.GetSettingString(
              "cl_borrow_music_from_player_index"
            )
          )

          if (borrowedPlayerIndex != 0 && isLocalPlayer) {
            borrowedXuid =
              GameStateAPI.GetPlayerXuidStringFromEntIndex(borrowedPlayerIndex)
          ...
          if (newStatValue !== oPlayer.m_oStats[stat]) {
            ...
            if (isLocalPlayer) {
              ...
              if (!elMusicKit || !elMusicKit.IsValid()) return

              var isValidMusicKit = newStatValue > 0
              elMusicKit.SetHasClass("hidden", !isValidMusicKit)
              if (isValidMusicKit) {
                _m_cP
                  .FindChildTraverse("id-sb-meta__musickit-unborrow")
                  .SetHasClass("hidden", !isBorrowed)

                var imagepath =
                  "file://{images_econ}/" +
                  InventoryAPI.GetItemInventoryImageFromMusicID(newStatValue) +
                  ".png"
                $("#id-sb-meta__musickit-image").SetImage(imagepath)
                $("#id-sb-meta__musickit-name").text = $.Localize(
                  InventoryAPI.GetMusicNameFromMusicID(newStatValue)
                )
              }
            }
          }
          ...
          var elPlayer = oPlayer.m_elPlayer
          if (elPlayer && elPlayer.IsValid()) {
            var elMusicKitIcon = elPlayer.FindChildTraverse(
              "id-sb-name__musickit"
            )
      ...
      case "kdr":
        fn = function (oPlayer, bSilent = false) {
          ...
          if (_m_overtime == 0) {
            ...
            var denom =
              oPlayer.m_oStats["deaths"] > 0 ? oPlayer.m_oStats["deaths"] : 1
      ...
      case "mvps":
        fn = function (oPlayer, bSilent = false) {
          ...
          if (newStatValue !== oPlayer.m_oStats[stat]) {
            ...
            if (!elMVPPanel || !elMVPPanel.IsValid()) return

            var elMVPStarImage = elMVPPanel.FindChildTraverse("star-image")
            if (!elMVPStarImage || !elMVPStarImage.IsValid()) return

            var elMVPStarNumberLabel =
              elMVPPanel.FindChildTraverse("star-count")
            if (!elMVPStarNumberLabel || !elMVPStarNumberLabel.IsValid()) return
      ...
      case "status":
        fn = function (oPlayer, bSilent) {
          ...
          if (newStatValue !== oPlayer.m_oStats[stat]) {
            ...
            if (!elStatusImage || !elStatusImage.IsValid()) return
      ...
      case "money":
        fn = function (oPlayer, bSilent = false) {
          ...
          var elLabel = elPanel.FindChildTraverse("label")
          if (!elLabel || !elLabel.IsValid()) return
      ...
      case "name":
        fn = function (oPlayer, bSilent = false) {
          if (!oPlayer.m_elPlayer || !oPlayer.m_elPlayer.IsValid()) return

          oPlayer.m_elPlayer.SetHasClass(
            "sb-row--localplayer",
            oPlayer.m_xuid === GetLocalPlayerId()
          )
          ...
          if (!elNameLabel || !elNameLabel.IsValid()) return

          elNameLabel.SetDialogVariable(
            "player_name",
            MockAdapter.GetPlayerName(oPlayer.m_xuid)
          )

          if (MockAdapter.GetPlayerClanTag(oPlayer.m_xuid) != "") {
            elNameLabel.SetDialogVariable(
              "player_clan",
              MockAdapter.GetPlayerClanTag(oPlayer.m_xuid)
            )
      ...
      case "friendly":
        fn = function (oPlayer, bSilent = false) {
          ...
          if (!MockAdapter.IsXuidValid(oPlayer.m_xuid)) {
            if (0) newStatValue = oPlayer.m_xuid
            else return
          } else {
            switch (stat) {
              case "leader":
                newStatValue = MockAdapter.GetPlayerCommendsLeader(
                  oPlayer.m_xuid
                )
                break
              case "teacher":
                newStatValue = MockAdapter.GetPlayerCommendsTeacher(
                  oPlayer.m_xuid
                )
                break
              case "friendly":
                newStatValue = MockAdapter.GetPlayerCommendsFriendly(
                  oPlayer.m_xuid
                )
                break
            }
      ...
      case "flair":
        fn = function (oPlayer, bSilent = false) {
          if (GameStateAPI.IsLatched()) return
          ...
          if (oPlayer.m_oStats[stat] !== newStatValue) {
            ...
            if (!elFlairImage || !elFlairImage.IsValid()) return

            var imagepath = InventoryAPI.GetFlairItemImage(oPlayer.m_xuid)

            elFlairImage.SetImage(
              "file://{images_econ}" + imagepath + "_small.png"
            )
          }
        }
        break
      ...
      case "avatar":
        fn = function (oPlayer, bSilent = false) {
          ...
          if (!elAvatarImage || !elAvatarImage.IsValid()) return
          ...
          if (MockAdapter.IsXuidValid(oPlayer.m_xuid)) {
            ...
            if (elAvatarImage.m_team != team) {
              elAvatarImage.RemoveClass(
                "sb-row__cell--avatar--" + elAvatarImage.m_team
              )
              elAvatarImage.AddClass("sb-row__cell--avatar--" + team)

              elAvatarImage.m_team = team
              elAvatarImage.SetDefaultImage(
                "file://{images}/icons/scoreboard/avatar-" + team + ".png"
              )
            }
          }
          ...
          var isEnemyTeamMuted =
            GameInterfaceAPI.GetSettingString("cl_mute_enemy_team") == "1"
          var isEnemy = MockAdapter.ArePlayersEnemies(
            oPlayer.m_xuid,
            GetLocalPlayerId()
          )

          var hasComAbusePenalty = MockAdapter.HasCommunicationAbuseMute(
            oPlayer.m_xuid
          )
          var isLocalPlayer = oPlayer.m_xuid == GetLocalPlayerId()
          ...
          oPlayer.m_elPlayer.SetHasClass(
            "muted",
            isMuted ||
              (isEnemy && isEnemyTeamMuted) ||
              (isLocalPlayer && hasComAbusePenalty)
          )
        }
        break
      ...
      case "skillgroup":
        fn = function (oPlayer, bSilent = false) {
          var newStatValue = MockAdapter.GetPlayerCompetitiveRanking(
            oPlayer.m_xuid
          )
          ...
          if (oPlayer.m_oStats[stat] !== newStatValue) {
            ...
            var elPlayer = oPlayer.m_elPlayer
            if (elPlayer && elPlayer.IsValid()) {
              var elSkillGroupImage = elPlayer.FindChildTraverse(
                "id-sb-skillgroup-image"
              )
              if (elSkillGroupImage && elSkillGroupImage.IsValid()) {
                ...
                if (newStatValue > 0) {
                  ...
                  if (mode == "scrimcomp2v2") imagePath = "wingman"
      ...
      case "rank":
        fn = function (oPlayer, bSilent = false) {
          ...
          if (oPlayer.m_oStats[stat] !== newStatValue) {
            ...
            if (!elRankImage || !elRankImage.IsValid()) return

            var imagepath = ""

            if (newStatValue > 0) {
              var imagepath =
                "file://{images}/icons/xp/level" + newStatValue + ".png"
            } else {
              var imagepath = ""
            }
      ...
      case "ggleader":
        fn = function (oPlayer, bSilent = false) {
          var isGGLeader =
            MockAdapter.GetTeamGungameLeaderXuid(
              oPlayer.m_oStats["teamname"]
            ) == oPlayer.m_xuid
              ? 1
              : 0

          _GenericUpdateStat(oPlayer, stat, () => {
            return isGGLeader
          })
        }
        break
    ...
    if (!bUnhandled) _m_oUpdateStatFns[stat] = fn
  }
    ...
    switch (mode) {
      ...
      case "casual":
        if (skirmish == "flyingscoutsman")
          return "snippet_scoreboard__row--demolition"
        else return "snippet_scoreboard-classic__row--casual"
  ...
  function _CreateLabelForStat(stat, set, isHidden) {
    ...
    if (!elLabelRow || !elLabelRow.IsValid()) return
    ...
    if (set !== "") {
      ...
      if (!elLabelSetContainer || !elLabelSetContainer.IsValid()) {
        elLabelSetContainer = $.CreatePanel(
          "Panel",
          elLabelRow,
          labelSetContainerId
        )
    ...
    if (!elStatPanel || !elStatPanel.IsValid()) {
      ...
      if (stat === "ping") {
        elStatLabel = $.CreatePanel(
          "Image",
          elStatPanel,
          "label-" + elStatPanel.id
        )
      ...
      if (toolTipString !== "") {
        ...
        elStatLabel.SetPanelEvent(
          "onmouseover",
          OnMouseOver.bind(undefined, elStatLabel.id, toolTipString)
        )
        elStatLabel.SetPanelEvent("onmouseout", function () {
      ...
      function _SetNewSortStat(stat) {
        ...
        var modeDefaultSortOrder = _GetSortOrderForMode(
          MockAdapter.GetGameModeInternalName(false)
        )
      ...
      elStatPanel.SetPanelEvent(
        "onactivate",
        _SetNewSortStat.bind(undefined, stat)
      )
    }
  }
  ...
  function _NewPlayerPanel(oPlayer) {
    if (!oPlayer.m_elTeam || !oPlayer.m_elTeam.IsValid()) return undefined

    oPlayer.m_elPlayer = $.CreatePanel(
      "Panel",
      oPlayer.m_elTeam,
      "player-" + oPlayer.m_xuid
    )
    ...
    function _InitStatCell(elStatCell, oPlayer) {
      if (!elStatCell || !elStatCell.IsValid()) return
      ...
      if (set !== "") {
        ...
        var elSetContainer =
          oPlayer.m_elPlayer.FindChildTraverse(SetContainerId)
        if (!elSetContainer || !elSetContainer.IsValid()) {
          elSetContainer = $.CreatePanel(
            "Panel",
            oPlayer.m_elPlayer,
            SetContainerId
          )
      ...
      if (idx++ % 2) elStatCell.AddClass("sb-row__cell--dark")
    ...
    if (MockAdapter.IsXuidValid(oPlayer.m_xuid)) {
      oPlayer.m_elPlayer.SetPanelEvent("onactivate", function () {
        ...
        if (!_m_hDenyInputToGame) {
          _m_hDenyInputToGame = UiToolkitAPI.AddDenyInputFlagsToGame(
            elPlayerCardContextMenu,
  ...
  function _UpdateMatchInfo() {
    ...
    _m_cP.SetDialogVariable(
      "server_name",
      _m_haveViewers ? "" : MockAdapter.GetServerName()
    )
    ...
    _m_cP.SetDialogVariable(
      "tournament_stage",
      MockAdapter.GetTournamentEventStage()
    )
    ...
    if (elMapLabel && elMapLabel.IsValid() && elMapLabel.text == "") {
      if (MatchStatsAPI.IsTournamentMatch()) {
        elMapLabel.text = $.Localize(
          "{s:tournament_stage} | {s:map_name}",
          _m_cP
        )
      } else {
        ...
        if (
          GameStateAPI.GetGameModeInternalName(true) === "competitive" &&
          GameTypesAPI.GetMapGroupAttribute(
            "mg_" + GameStateAPI.GetMapBSPName(),
            "competitivemod"
          ) === "unranked"
        ) {
          strLocalizeScoreboardTitle =
            $.Localize("#SFUI_RankType_Modifier_Unranked", _m_cP) +
            " | {s:map_name}"
          ...
          strLocalizeScoreboardTitle =
            $.Localize("#SFUI_GameModeCompetitiveTeams", _m_cP) +
            " | " +
            sMapName
        }
    ...
    if ($("#id-sb-meta__mode__image")) {
      if (GameStateAPI.IsQueuedMatchmakingMode_Team())
        $("#id-sb-meta__mode__image").SetImage(
          "file://{images}/icons/ui/competitive_teams.svg"
        )
      else
        $("#id-sb-meta__mode__image").SetImage(
          MockAdapter.GetGameModeImagePath()
        )
    }

    if ($("#sb-meta__labels__map"))
      $("#sb-meta__labels__map").SetImage(
        "file://{images}/map_icons/map_icon_" +
          MockAdapter.GetMapBSPName() +
          ".svg"
      )
    ...
    if (elCoopStats) {
      ...
      if (questID > 0) {
        ...
        var elLabel = elCoopStats.FindChildInLayoutFile(
          "MissionDescriptionLabel"
        )
        if (elLabel) {
          var strMissionDescriptionToken = MissionsAPI.GetQuestDefinitionField(
            questID,
            "loc_description"
          )
    ...
    var bind = GameInterfaceAPI.GetSettingString(
    ...
    if (elMouseBinding && elMouseBinding.IsValid()) {
      ...
      elMouseBinding.text = $.Localize(
        "#Scoreboard_Mouse_Enable_Instruction",
        elMouseBinding
      )
    }

    var elFooterWebsite = _m_cP.FindChildInLayoutFile(
  ...
  function _UpdateHLTVViewerNumber(nViewers) {
    ...
    if (nViewers > 0) {
      _m_cP.SetDialogVariable(
        "hltv_viewers",
        $.Localize("#Scoreboard_Viewers", _m_cP)
      )
      _m_haveViewers = true
    } else {
  ...
  function _UpdateRound(rnd, oScoreData, jsoTime) {
    if (!_SupportsTimeline(jsoTime)) return

    if (!oScoreData) return

    if (!jsoTime) return

    if (!("teamdata" in oScoreData)) return

    var elTimeline = _m_cP.FindChildInLayoutFile("id-sb-timeline__segments")
    if (!elTimeline || !elTimeline.IsValid()) return

    var elRnd = elTimeline.FindChildTraverse(rnd)
    if (!elRnd || !elRnd.IsValid()) return

    var elRndTop = elRnd.FindChildTraverse(
      "id-sb-timeline__segment__round--top"
    )
    var elRndBot = elRnd.FindChildTraverse(
      "id-sb-timeline__segment__round--bot"
    )
    ...
    if (rnd > jsoTime["rounds_played"]) {
      ...
      if (bCanClinch) {
        ...
        var bShowClinchTop =
          bThisRoundIsClinchTop && topClinchRound <= botClinchRound
        var bShowClinchBot =
          bThisRoundIsClinchBot && botClinchRound <= topClinchRound

        var thisRoundIsClinchAndShowIt = false

        if (bShowClinchTop) {
          elRndTop
            .FindChildTraverse("result")
            .SetImage(dictRoundResultImage["win"])
      ...
      elRnd
        .FindChildTraverse("id-sb-timeline__segment__round__tick")
        .RemoveClass("sb-team--CT")
      elRnd
        .FindChildTraverse("id-sb-timeline__segment__round__tick__label")
        .RemoveClass("sb-team--CT")
      elRnd
        .FindChildTraverse("id-sb-timeline__segment__round__tick")
        .RemoveClass("sb-team--TERRORIST")
      elRnd
        .FindChildTraverse("id-sb-timeline__segment__round__tick__label")
        .RemoveClass("sb-team--TERRORIST")
    ...
    if (
      MockAdapter.AreTeamsPlayingSwitchedSides() !==
      MockAdapter.AreTeamsPlayingSwitchedSidesInRound(rnd)
    ) {
      bFlippedSides = true
      var elTemp = elRndTop
      elRndTop = elRndBot
    ...
    if (typeof oScoreData["rounddata"][idx] !== "object") return
    ...
    if (oScoreData["rounddata"][idx]["result"].charAt(0) === "c") {
      ...
      elRndTop
        .FindChildTraverse("result")
        .SetImage(dictRoundResultImage[result])
      elRndTop
        .FindChildTraverse("result")
        .AddClass("sb-timeline__segment__round--active")

      elRndBot.FindChildTraverse("result").SetImage("")
      elRndBot
        .FindChildTraverse("result")
        .RemoveClass("sb-timeline__segment__round--active")
    ...
    var _UpdateCasualties = function (teamName, elRnd) {
      if (_m_oTeams[teamName]) {
        var livingCount =
          oScoreData["rounddata"][idx]["players_alive_" + teamName]
  ...
  function _ShowSurvivors(hide = false) {
    ...
    function CollectPanelsToToggleTransparency(el) {
      ...
      if (
        el.GetAttributeString(
          "data-casualty-mouse-over-toggle-transparency",
          "false"
        ) == "true"
      )
    ...
    arrPanelsToToggleTransparency.forEach((el) =>
      el.SetHasClass("transparent", hide)
    )
  }

  function _Casualties_OnMouseOver() {
    if (
      GameInterfaceAPI.GetSettingString("cl_scoreboard_survivors_always_on") ==
      "0"
    )
      _ShowSurvivors()

    UiToolkitAPI.ShowCustomLayoutTooltipStyled(
      "1",
      "id-tooltip-sb-casualties",
      "file://{resources}/layout/tooltips/tooltip_scoreboard_casualties.xml",
      "Tooltip_NoArrow"
    )
  }
  ...
  function _RoundLossBonusMoneyForTeam(teamname) {
    ...
    var nMaxLoss = parseInt(
      GameInterfaceAPI.GetSettingString("mp_consecutive_loss_max")
    )
    ...
    var nBaseAmount = parseInt(
      GameInterfaceAPI.GetSettingString("cash_team_loser_bonus")
    )
    var nConsecutiveBonus = parseInt(
      GameInterfaceAPI.GetSettingString(
  ...
  function _RoundLossBonusMoney_OnMouseOver_CT() {
    _m_cP.SetDialogVariable(
      "round_loss_income_team",
      $.Localize("#counter-terrorists")
    )
    _m_cP.SetDialogVariableInt(
      "round_loss_income_amount",
      _RoundLossBonusMoneyForTeam("CT")
    )
    var sTooltipText = $.Localize("#Scoreboard_lossmoneybonus_tooltip", _m_cP)
    UiToolkitAPI.ShowTextTooltip(
      "id-sb-timeline__round-loss-bonus-money",
      sTooltipText
    )
  }
  ...
  function _RoundLossBonusMoney_OnMouseOver_TERRORIST() {
    ...
    _m_cP.SetDialogVariableInt(
      "round_loss_income_amount",
      _RoundLossBonusMoneyForTeam("TERRORIST")
    )
    var sTooltipText = $.Localize("#Scoreboard_lossmoneybonus_tooltip", _m_cP)
    UiToolkitAPI.ShowTextTooltip(
      "id-sb-timeline__round-loss-bonus-money",
  ...
  function _UpdateTeamInfo(team) {
    ...
    _m_cP.SetDialogVariable(
      "sb_team_name--" + team,
      _m_oTeams[team].m_teamClanName
    )

    if (MockAdapter.GetTeamLogoImagePath(team) != "") {
      _m_oTeams[team].m_teamLogoImagePath =
        MockAdapter.GetTeamLogoImagePath(team)

      _m_cP
        .FindChildrenWithClassTraverse("sb-team-logo-background--" + team)
        .forEach(function (el) {
          el.style.backgroundImage =
            'url("file://{images}' + _m_oTeams[team].m_teamLogoImagePath + '")'
    ...
    _m_cP.SetDialogVariableInt(
      team + "_alive",
      MockAdapter.GetTeamLivingPlayerCount(team)
    )
    _m_cP.SetDialogVariableInt(
      team + "_total",
      MockAdapter.GetTeamTotalPlayerCount(team)
    )
  }
  function _UpdateTeams() {
    ...
    for (var team in _m_oTeams) {
      ...
      if (
        !oScoreData ||
        !("teamdata" in oScoreData) ||
        !(team in oScoreData["teamdata"])
      )
        continue
      ...
      if (team in oScoreData["teamdata"]) {
        _m_cP.SetDialogVariableInt(
          "sb_team_score--" + team,
          oScoreData["teamdata"][team]["score"]
        )
      }

      if ("score_1h" in oScoreData["teamdata"][team]) {
        _m_cP.SetDialogVariableInt(
          "sb_team_score_2--" + team,
          oScoreData["teamdata"][team]["score_1h"]
        )
      }

      if ("score_2h" in oScoreData["teamdata"][team]) {
        _m_cP.SetDialogVariableInt(
          "sb_team_score_3--" + team,
          oScoreData["teamdata"][team]["score_2h"]
        )
      }

      if ("score_ot" in oScoreData["teamdata"][team]) {
        _m_cP.SetDialogVariableInt(
          "sb_team_score_ot--" + team,
          oScoreData["teamdata"][team]["score_ot"]
        )
      }

      var elOTScore = _m_cP.FindChildTraverse("id-sb-timeline__score_ot")
      if (elOTScore && elOTScore.IsValid()) {
        elOTScore.SetHasClass(
          "hidden",
          !("score_ot" in oScoreData["teamdata"][team])
        )
        elOTScore.SetHasClass(
          "fade",
          !("score_ot" in oScoreData["teamdata"][team])
  ...
  function _UpdateAllRounds() {
    ...
    if (jsoTime["overtime"] > 0) {
      m_topScore =
        (jsoTime["maxrounds"] +
          (jsoTime["overtime"] - 1) * jsoTime["maxrounds_overtime"]) /
        2
      m_botScore =
        (jsoTime["maxrounds"] +
          (jsoTime["overtime"] - 1) * jsoTime["maxrounds_overtime"]) /
  ...
  function _UpdateScore_Classic() {
    ...
    _m_cP.SetDialogVariable(
      "match_phase",
      $.Localize("gamephase_" + jsoTime["gamephase"])
    )
  ...
  function _InsertTimelineDivider() {
    ...
    var elDivider = $.CreatePanel(
      "Panel",
      elTimeline,
      "id-sb-timeline__divider"
    )
  ...
  function _InitTimelineSegment(startRound, endRound, phase) {
    ...
    if (!elSegment || !elSegment.IsValid()) {
      ...
      elSegment.BLoadLayoutSnippet(
        "snippet_scoreboard-classic__timeline__segment"
      )
    }

    var elRoundContainer = elSegment.FindChildTraverse(
      "id-sb-timeline__round-container"
    )
    if (elRoundContainer && elRoundContainer.IsValid()) {
      for (var rnd = startRound; rnd <= endRound; rnd++) {
        ...
        if (!elRnd || !elRnd.IsValid()) {
          ...
          elRnd.BLoadLayoutSnippet(
            "snippet_scoreboard-classic__timeline__segment__round"
          )

          var elTop = elRnd.FindChildTraverse(
            "id-sb-timeline__segment__round--top"
          )
          elTop.BLoadLayoutSnippet(
            "snippet_scoreboard-classic__timeline__segment__round__data"
          )

          var elBot = elRnd.FindChildTraverse(
            "id-sb-timeline__segment__round--bot"
          )
          elBot.BLoadLayoutSnippet(
            "snippet_scoreboard-classic__timeline__segment__round__data"
          )

          if (rnd % 5 == 0) {
            elRnd.FindChildTraverse(
              "id-sb-timeline__segment__round__tick__label"
            ).text = rnd
          }
        }
      }
    ...
    if (
      MockAdapter.AreTeamsPlayingSwitchedSides() !==
      MockAdapter.AreTeamsPlayingSwitchedSidesInRound(endRound)
    ) {
      var elCTScore = elSegment.FindChildTraverse(
        "id-sb-timeline__segment__score__ct"
      )
      var elTScore = elSegment.FindChildTraverse(
        "id-sb-timeline__segment__score__t"
      )
  ...
  function _SupportsTimeline(jsoTime) {
    if (jsoTime == undefined) jsoTime = MockAdapter.GetTimeDataJSO()
  ...
  function _UpdateRoundLossBonus() {
    var elRoundLossBonusMoney = _m_cP.FindChildInLayoutFile(
      "id-sb-timeline__round-loss-bonus-money"
    )
    if (elRoundLossBonusMoney && elRoundLossBonusMoney.IsValid()) {
      ...
      if (
        parseInt(GameInterfaceAPI.GetSettingString("mp_consecutive_loss_max")) >
          0 &&
        parseInt(
          GameInterfaceAPI.GetSettingString(
            "cash_team_loser_bonus_consecutive_rounds"
        ...
        if (nLossT >= 0 && nLossCT >= 0) {
          ...
          for (var nClassIdx = 1; nClassIdx <= 4; ++nClassIdx) {
            elRoundLossBonusMoney.SetHasClass(
              "sb-timeline__round-loss-bonus-money__TERRORIST" + nClassIdx,
              nLossT >= nClassIdx
            )
          }
          ...
          for (var nClassIdx = 1; nClassIdx <= 4; ++nClassIdx) {
            elRoundLossBonusMoney.SetHasClass(
              "sb-timeline__round-loss-bonus-money__CT" + nClassIdx,
              nLossCT >= nClassIdx
            )
          }
        }
  ...
  function ToggleCasterButtonActive(buttonName, bActive) {
    ...
    if (
      bActive == false &&
      button.BHasClass("sb-spectator-control-button-notactive") == false
    ) {
      button.AddClass("sb-spectator-control-button-notactive")
    } else if (
      bActive == true &&
      button.BHasClass("sb-spectator-control-button-notactive") == true
    ) {
  ...
  function _CycleStats() {
    if (_m_dataSetGetCount === 0) return

    {
      _m_dataSetCurrent++

      if (_m_dataSetCurrent >= _m_dataSetGetCount) _m_dataSetCurrent = 0
    }
    ...
    for (var i = 0; i < _m_oPlayers.GetCount(); i++) {
      ...
      if (elPlayer && elPlayer.IsValid()) {
        var elSetContainer = elPlayer.FindChildTraverse(
          "id-sb-row__set-container"
        )
        if (elSetContainer && elSetContainer.IsValid()) {
  ...
  function _UpdateMuteVoiceState() {
    ...
    var elMuteImage = _m_cP.FindChildInLayoutFile(
      "id-sb-meta__mutevoice__image"
    )
    if (!elMuteImage) return
  ...
  function _BlockUgc() {
    ...
    var ugcBlockState =
      GameInterfaceAPI.GetSettingString("cl_hide_avatar_images") !== "0" ||
  ...
  function _UpdateUgcState() {
    ...
    var elBlockUgcImage = _m_cP.FindChildInLayoutFile(
      "id-sb-meta__blockugc__image"
    )
    if (!elBlockUgcImage) return
  ...
  function _CreateLabelsForRow(el) {
    ...
    if (stat != "") _CreateLabelForStat(stat, set, isHidden)
  }

  function _GetSortOrderForMode(mode) {
    if (GameStateAPI.IsQueuedMatchmakingMode_Team()) return sortOrder_tmm
  ...
  function _Initialize() {
    ...
    if (mode == "deathmatch") {
      if (
        GameInterfaceAPI.GetSettingString("mp_teammates_are_enemies") !== "0"
      ) {
        skirmish = "ffadm"
    ...
    switch (mode) {
      ...
      case "scrimcomp2v2":
        scoreboardTemplate =
          "snippet_scoreboard-classic--with-timeline--half-times"
        break
      ...
      case "casual":
        if (skirmish == "flyingscoutsman") {
          scoreboardTemplate =
            "snippet_scoreboard-classic--with-timeline--no-half-times"
        } else {
          scoreboardTemplate = "snippet_scoreboard-classic--no-timeline"
        }
        break
    ...
    if (MockAdapter.IsDemoOrHltv()) _m_cP.AddClass("IsDemoOrHltv")

    if (MatchStatsAPI.IsTournamentMatch()) _m_cP.AddClass("IsTournamentMatch")
  ...
  function _OnMouseActive() {
    ...
    var elServerViewers = _m_cP.FindChildTraverse(
  ...
  function _CloseScoreboard() {
    if (_m_updatePlayerHandler) {
      $.UnregisterForUnhandledEvent(
        "Scoreboard_UpdatePlayerByEntIndex",
        _m_updatePlayerHandler
      )
    ...
    _m_cP
      .FindChildrenWithClassTraverse("timer")
  ...
  function _OpenScoreboard() {
    ...
    if (!_m_updatePlayerHandler) {
      _m_updatePlayerHandler = $.RegisterForUnhandledEvent(
        "Scoreboard_UpdatePlayerByEntIndex",
        Scoreboard.UpdatePlayerByEntIndex
      )
    }
  }
  ...
  function _GetFreeForAllPlayerPosition(xuid) {
    ...
    for (var i = 0; i < elTeam.Children().length; i++) {
      if (elTeam.Children()[i].m_xuid == xuid) returnVal = i + 1
    }
  ...
  function _UpdateLeaderboard(strLBname) {
    ...
    if (
      activeQuestID > 0 &&
      LeaderboardsAPI.GetDetailsQuestID(hPrevDetails) === activeQuestID
    ) {
    ...
    if (lbState === "none") {
      ...
      if (cacheTimeMinutes > 3) {
        ...
        var hDetails = LeaderboardsAPI.GetEntryDetailsHandleByXuid(
          strLBname,
          MyPersonaAPI.GetXuid()
        )
        if (hDetails) UpdateCoopScores(hDetails)
      }
    }
  }
  ...
  function UpdateCoopScores(hDetailHandle) {
    ...
    var nScoreCount = LeaderboardsAPI.GetDetailsMatchScoreboardStatsCount(
      hDetailHandle,
      "score"
    )
    for (let i = 0; i < nScoreCount; ++i) {
      var strNameToken =
        LeaderboardsAPI.GetDetailsMatchScoreboardStatNameByIndex(
          hDetailHandle,
          "score",
          i
        )
      elLeaderboardStats.SetDialogVariable(
        "statname" + i,
        $.Localize(strNameToken)
      )
      FormatStatValueDialogVar()
      FormatStatScoreDialogVar()

      function FormatStatValueDialogVar() {
        var stat = LeaderboardsAPI.GetDetailsMatchScoreboardStatValueByIndex(
          hDetailHandle,
          "score",
          i
        )
        ...
        if (strNameToken === "#SFUI_Scoreboard_Coop_DmgToEnemyRatio") {
          ...
          strNameToken === "#SFUI_Scoreboard_Coop_TeamAccuracy" ||
      ...
      function FormatStatScoreDialogVar() {
        var score = LeaderboardsAPI.GetDetailsMatchScoreboardStatScoreByIndex(
          hDetailHandle,
          "score",
          i
        )
    ...
    var nBonusCount = LeaderboardsAPI.GetDetailsMatchScoreboardStatsCount(
      hDetailHandle,
      "bonus"
    )
    for (let i = 0; i < nBonusCount; ++i) {
      ...
      elLeaderboardStats.SetDialogVariableInt(
        "bonusval" + i,
        LeaderboardsAPI.GetDetailsMatchScoreboardStatValueByIndex(
          hDetailHandle,
          "bonus",
          i
        )
      ...
      elLeaderboardStats.SetDialogVariableInt(
        "bonusscore" + i,
        LeaderboardsAPI.GetDetailsMatchScoreboardStatScoreByIndex(
          hDetailHandle,
          "bonus",
          i
        )
    ...
    var nBonus = LeaderboardsAPI.GetDetailsMatchScoreboardStatScoreTotal(
      hDetailHandle,
      "bonus"
    )
    elLeaderboardStats.SetDialogVariableInt("bonus_score", nBonus)
    var nScore = LeaderboardsAPI.GetDetailsMatchScoreboardStatScoreTotal(
      hDetailHandle,
      "score"
    )
    elLeaderboardStats.SetDialogVariableInt("score", nScore)
    var nTotalScore = LeaderboardsAPI.GetDetailsMatchScoreboardStatScoreTotal(
      hDetailHandle,
      "all"
    )
  ...
  function GetCasterIsCameraman() {
    ...
    var bQ =
      MockAdapter.IsDemoOrHltv() &&
      nCameraMan != 0 &&
      MockAdapter.IsHLTVAutodirectorOn()
  ...
  function GetCasterIsHeard() {
    ...
    if (MockAdapter.IsDemoOrHltv()) {
      var bVoiceCaster = parseInt(
        GameInterfaceAPI.GetSettingString("voice_caster_enable")
      )
      bQ = bVoiceCaster
    }
  ...
  function GetCasterControlIsDisabled() {
    var bDisableWithControl = parseInt(
      GameInterfaceAPI.GetSettingString(
    ...
    var bQ =
      MockAdapter.IsDemoOrHltv() &&
      bDisableWithControl &&
      MockAdapter.IsHLTVAutodirectorOn() == false
  ...
  function GetCasterControlsXray() {
    var bXRay =
      MockAdapter.IsDemoOrHltv() &&
      parseInt(GameInterfaceAPI.GetSettingString("spec_cameraman_xray"))

    return bXRay
  }

  function GetCasterControlsUI() {
    var bSpecCameraMan = parseInt(
      GameInterfaceAPI.GetSettingString("spec_cameraman_ui")
    )
  ...
  function _ApplyPlayerCrosshairCode(panel, xuid) {
    UiToolkitAPI.ShowGenericPopupYesNo(
      ...
      "",
      function () {
        let code = GameStateAPI.GetCrosshairCode(xuid)
        MyPersonaAPI.BApplyCrosshairCode(code)
      },
      function () {}
    )
  }
  ...
  return {
    ...
    RoundLossBonusMoney_OnMouseOver_TERRORIST:
      _RoundLossBonusMoney_OnMouseOver_TERRORIST,
    RoundLossBonusMoney_OnMouseOut_TERRORIST:
      _RoundLossBonusMoney_OnMouseOut_TERRORIST,
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    "Scoreboard_ResetAndInit",
    Scoreboard.ResetAndInit
  )

  $.RegisterForUnhandledEvent(
    "Scoreboard_UpdateEverything",
    Scoreboard.UpdateEverything
  )
  ...
  $.RegisterForUnhandledEvent(
    "Scoreboard_OnMouseActive",
    Scoreboard.OnMouseActive
  )

  $.RegisterForUnhandledEvent(
    "Scoreboard_OnEndOfMatch",
    Scoreboard.OnEndOfMatch
  )

  $.RegisterForUnhandledEvent(
    "Scoreboard_UnborrowMusicKit",
    Scoreboard.UnborrowMusicKit
  )

  $.RegisterForUnhandledEvent(
    "Scoreboard_ToggleSetCasterIsCameraman",
    Scoreboard.ToggleSetCasterIsCameraman
  )
  $.RegisterForUnhandledEvent(
    "Scoreboard_ToggleSetCasterIsHeard",
    Scoreboard.ToggleSetCasterIsHeard
  )
  $.RegisterForUnhandledEvent(
    "Scoreboard_ToggleSetCasterControlsXray",
    Scoreboard.ToggleSetCasterControlsXray
  )
  $.RegisterForUnhandledEvent(
    "Scoreboard_ToggleSetCasterControlsUI",
    Scoreboard.ToggleSetCasterControlsUI
  )

  $.RegisterForUnhandledEvent(
    "GameState_RankRevealAll",
    Scoreboard.RankRevealAll
  )

  $.RegisterForUnhandledEvent(
    "Scoreboard_UpdateHLTVViewers",
    Scoreboard.UpdateHLTVViewerNumber
  )

  $.RegisterForUnhandledEvent(
    "Scoreboard_Casualties_OnMouseOver",
    Scoreboard.Casualties_OnMouseOver
  )
  $.RegisterForUnhandledEvent(
    "Scoreboard_Casualties_OnMouseOut",
    Scoreboard.Casualties_OnMouseOut
  )

  $.RegisterForUnhandledEvent(
    "Scoreboard_RoundLossBonusMoney_OnMouseOver_CT",
    Scoreboard.RoundLossBonusMoney_OnMouseOver_CT
  )
  $.RegisterForUnhandledEvent(
    "Scoreboard_RoundLossBonusMoney_OnMouseOut_CT",
    Scoreboard.RoundLossBonusMoney_OnMouseOut_CT
  )
  $.RegisterForUnhandledEvent(
    "Scoreboard_RoundLossBonusMoney_OnMouseOver_TERRORIST",
    Scoreboard.RoundLossBonusMoney_OnMouseOver_TERRORIST
  )
  $.RegisterForUnhandledEvent(
    "Scoreboard_RoundLossBonusMoney_OnMouseOut_TERRORIST",
    Scoreboard.RoundLossBonusMoney_OnMouseOut_TERRORIST
  )

  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Leaderboards_StateChange",
  ...
  $.RegisterForUnhandledEvent(
    "Scoreboard_ApplyPlayerCrosshairCode",
    Scoreboard.ApplyPlayerCrosshairCode
  )
})()
