...
var playerCard = (function () {
  ...
  var _Init = function () {
    ...
    _m_bShownInFriendsList = $.GetContextPanel().GetAttributeString(
      "data-slot",
      ""
    )
    ...
    if (!_m_isSelf) FriendsListAPI.RequestFriendProfileUpdateFromScript(_m_xuid)
  ...
  var _FillOutFriendCard = function () {
    if (_m_xuid) {
      ...
      _m_ShowLockedRankSkillGroupState =
        !_IsPlayerPrime() && _HasXpProgressToFreeze()
      ...
      if (_m_isSelf) {
        if (MyPersonaAPI.GetPipRankWins("Competitive") >= 0) {
          if (_m_bShownInFriendsList) _SetSkillGroup("competitive")
          else _SetAllSkillGroups()
        } else {
          var elToggleBtn =
            $.GetContextPanel().FindChildInLayoutFile("SkillGroupExpand")
      ...
      if (_m_bShownInFriendsList) {
        $.GetContextPanel()
          .FindChildInLayoutFile("JsPlayerCommendations")
          .AddClass("hidden")
        $.GetContextPanel()
          .FindChildInLayoutFile("JsPlayerPrime")
          .AddClass("hidden")
        _SetTeam()
      } else {
        _SetCommendations()
  ...
  var _ProfileUpdated = function (xuid) {
    if (_m_xuid === xuid) _FillOutFriendCard()
  }
  ...
  var _SetAvatar = function () {
    var elAvatarExisting =
      $.GetContextPanel().FindChildInLayoutFile("JsPlayerCardAvatar")

    if (!elAvatarExisting) {
      var elParent =
        $.GetContextPanel().FindChildInLayoutFile("JsPlayerCardTop")
      ...
      elParent.MoveChildBefore(
        elAvatar,
        $.GetContextPanel().FindChildInLayoutFile("JsPlayerCardName")
      )
    } else {
  ...
  var _SetPlayerBackground = function () {
    ...
    var flairItemId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
      flairDefIdx,
      0
    )
    ...
    elBgImage.style.backgroundImage = imagePath
      ? 'url("file://{images_econ}' + imagePath + '_large.png")'
      : "none"
  ...
  var _SetRank = function () {
    ...
    if (
      !MyPersonaAPI.IsInventoryValid() ||
      !_m_currentLvl ||
      (!_HasXpProgressToFreeze() && !_IsPlayerPrime())
    ) {
    ...
    var bHasRankToFreezeButNoPrestige = _m_ShowLockedRankSkillGroupState
      ? true
      : false
    ...
    var elXpBar =
      $.GetContextPanel().FindChildInLayoutFile("JsPlayerXpBarInner")
    var elXpBarInner =
      $.GetContextPanel().FindChildInLayoutFile("JsPlayerXpBarInner")
    ...
    var elRankText =
      $.GetContextPanel().FindChildInLayoutFile("JsPlayerRankName")

    elRankText.SetHasClass(
      "player-card-prime-text",
      bHasRankToFreezeButNoPrestige
    )

    elRank.SetHasClass(
      "player-card-nonprime-locked-xp-row",
      bHasRankToFreezeButNoPrestige
    )
    if (bHasRankToFreezeButNoPrestige) {
      elRankText.text = $.Localize("#Xp_RankName_Locked")
    } else {
      elRankText.SetDialogVariable(
        "name",
        $.Localize("#SFUI_XP_RankName_" + _m_currentLvl)
      )
    ...
    elRankIcon.SetImage(
      "file://{images}/icons/xp/level" + _m_currentLvl + ".png"
    )

    elRank.RemoveClass("hidden")

    var bPrestigeAvailable =
      _m_isSelf && _m_currentLvl >= InventoryAPI.GetMaxLevel()
    $.GetContextPanel()
      .FindChildInLayoutFile("GetPrestigeButton")
      .SetHasClass("hidden", !bPrestigeAvailable)
    if (bPrestigeAvailable) {
      $.GetContextPanel()
        .FindChildInLayoutFile("GetPrestigeButtonClickable")
        .SetPanelEvent("onactivate", _OnActivateGetPrestigeButtonClickable)
    }
  }
  ...
  var _SetAllSkillGroups = function () {
    var elSkillGroupContainer = $.GetContextPanel().FindChildInLayoutFile(
      "JsPlayerCardSkillGroupContainer"
    )
  ...
  var _UpdateSkillGroup = function (elSkillGroup, skillGroup, wins, type) {
    ...
    var typeModifier =
      _m_arrAdditionalSkillGroups.indexOf(type) >= 0 ? type : ""

    var imageName = typeModifier !== "" ? typeModifier : "skillgroup"
    var bNonPrimeButHasXpProgress = _m_ShowLockedRankSkillGroupState
      ? true
      : false

    if (bNonPrimeButHasXpProgress) {
      elSkillGroup
        .FindChildInLayoutFile("JsPlayerSkillLabel")
        .SetHasClass("player-card-prime-text", bNonPrimeButHasXpProgress)
      if (!_m_bShownInFriendsList) {
        elSkillGroup
          .FindChildInLayoutFile("JsPlayerSkillLabel")
          .SetHasClass(
            "player-card-prime-text--popup",
            bNonPrimeButHasXpProgress
          )
      }
    }
    ...
    if (wins < winsNeededForRank || isloading) {
      ...
      if (isloading) {
        elSkillGroup
          .FindChildInLayoutFile("JsPlayerSkillIcon")
          .SetImage(
            "file://{images}/icons/skillgroups/" + imageName + "_none.svg"
          )
        elSkillGroup.FindChildInLayoutFile("JsPlayerSkillLabel").text =
          $.Localize("#SFUI_LOADING")
      } else if (bNonPrimeButHasXpProgress) {
        elSkillGroup.FindChildInLayoutFile("JsPlayerSkillLabel").text =
          $.Localize("#skillgroup_locked")
        ...
        elSkillGroup.FindChildInLayoutFile("JsPlayerSkillLabel").text =
          $.Localize("#skillgroup_0" + typeModifier)
        elSkillGroup
          .FindChildInLayoutFile("JsPlayerSkillLabel")
          .SetDialogVariableInt("winsneeded", winsneeded)
        tooltipText = $.Localize(
          "#tooltip_skill_group_none" + typeModifier,
          elSkillGroup.FindChildInLayoutFile("JsPlayerSkillLabel")
        )
      }
      ...
      elSkillGroup
        .FindChildInLayoutFile("JsPlayerSkillIcon")
        .SetImage(
          "file://{images}/icons/skillgroups/" + imageName + "_expired.svg"
        )
      ...
      elSkillGroup.FindChildInLayoutFile("JsPlayerSkillLabel").text =
        $.Localize("#skillgroup_" + strSkillGroupString)
      tooltipText = $.Localize("#tooltip_skill_group_" + strSkillGroupString)
    } else {
      elSkillGroup
        .FindChildInLayoutFile("JsPlayerSkillIcon")
        .SetImage(
          "file://{images}/icons/skillgroups/" + imageName + skillGroup + ".svg"
        )
      ...
      if (bNonPrimeButHasXpProgress) {
        ...
        var skillGroupNamingSuffix =
          typeModifier && typeModifier !== "wingman" ? typeModifier : ""
        elSkillGroup.FindChildInLayoutFile("JsPlayerSkillLabel").text =
          $.Localize("#skillgroup_" + skillGroup + skillGroupNamingSuffix)
      }

      if (_m_isSelf)
        tooltipText = bNonPrimeButHasXpProgress
          ? $.Localize("#tooltip_skill_group_locked")
          : $.Localize("#tooltip_skill_group_generic" + typeModifier)
    }
    ...
    if (bNonPrimeButHasXpProgress) {
      ...
      tooltipText =
        tooltipText !== ""
          ? tooltipText + "<br><br>" + GetMatchWinsText(elSkillGroup, wins)
          : GetMatchWinsText(elSkillGroup, wins)
    }

    elSkillGroup.RemoveClass("hidden")
    if (!isloading) {
      elSkillGroup.SetPanelEvent(
        "onmouseover",
        _ShowSkillGroupTooltip.bind(undefined, tooltipLoc, tooltipText)
      )
      elSkillGroup.SetPanelEvent("onmouseout", _HideSkillGroupTooltip)
    }

    elSkillGroup.SetHasClass(
      "player-card-nonprime-locked-xp-row",
      _m_ShowLockedRankSkillGroupState
    )
  }
  ...
  var _SetPrimeUpsell = function () {
    var elUpsellPanel = $.GetContextPanel().FindChildInLayoutFile(
      "JsPlayerCardPrimeUpsell"
    )
    ...
    elUpsellPanel.FindChildInLayoutFile(
      "id-player-card-prime-upsell-xp"
    ).visible = !_HasXpProgressToFreeze() && !_IsPlayerPrime()
    elUpsellPanel.FindChildInLayoutFile(
      "id-player-card-prime-upsell-skillgroup"
    ).visible = !_HasXpProgressToFreeze() && !_IsPlayerPrime()
  }
  ...
  var _SetCommendations = function () {
    ...
    var elCommendsBlock = $.GetContextPanel().FindChildInLayoutFile(
      "JsPlayerCommendations"
    )

    for (var i = 0; i < catagoriesCount; i++) {
      catagories[i].value = FriendsListAPI.GetFriendCommendations(
        _m_xuid,
        catagories[i].key
      )

      var elCommend = $.GetContextPanel().FindChildInLayoutFile(
        "JsPlayer" + catagories[i].key
      )
    ...
    elCommendsBlock.SetHasClass(
      "hidden",
      countHiddenCommends === catagoriesCount && !_IsPlayerPrime()
    )
  }
  ...
  var _SetPrime = function () {
    ...
    if (!MyPersonaAPI.IsInventoryValid()) elPrime.AddClass("hidden")

    if (_IsPlayerPrime()) {
      elPrime.RemoveClass("hidden")
      return
    } else elPrime.AddClass("hidden")
  }
  ...
  var _SetTeam = function () {
    ...
    if (!teamName || !tournamentName) {
      $.GetContextPanel()
        .FindChildInLayoutFile("JsPlayerTeam")
        .AddClass("hidden")
      return
    }
    ...
    $.GetContextPanel()
      .FindChildInLayoutFile("JsPlayerCardSkillGroupContainer")
      .AddClass("hidden")
    $.GetContextPanel()
      .FindChildInLayoutFile("JsPlayerTeam")
      .RemoveClass("hidden")
    ...
    $.GetContextPanel()
      .FindChildInLayoutFile("JsTeamIcon")
      .SetImage("file://{images}/tournaments/teams/" + teamTag + ".svg")
    $.GetContextPanel().FindChildInLayoutFile("JsTeamLabel").text = teamName
    $.GetContextPanel().FindChildInLayoutFile("JsTournamentLabel").text =
      tournamentName
  }
  ...
  var _SetFlairItems = function () {
    ...
    var flairItemIdList = []
    var elFlairPanal = $.GetContextPanel().FindChildInLayoutFile(
      "FlairCarouselAndControls"
    )
    ...
    for (var i = 0; i < flairItems; i++) {
      var flairDefIdx = FriendsListAPI.GetFriendDisplayItemDefByIndex(
        _m_xuid,
        i
      )
      var flairItemId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
    ...
    $.GetContextPanel()
      .FindChildInLayoutFile("FlairCarousel")
  ...
  var _MakeFlairCarouselPages = function (elFlairPanal, flairItemIdList) {
    ...
    var elFlairCarousel =
      $.GetContextPanel().FindChildInLayoutFile("FlairCarousel")
    var elCarouselPage = null

    for (var i = 0; i < countFlairItems; i++) {
      if (i % 5 === 0) {
        elCarouselPage = $.CreatePanel("Panel", elFlairCarousel, "", {
          class: "playercard-flair-carousel__page"
        })
      }
      ...
      elFlair.SetPanelEvent(
        "onmouseover",
        onMouseOver.bind(undefined, flairItemIdList[i], panelName)
      )
  ...
  var _ShowHideAdditionalRanks = function () {
    ...
    _m_arrAdditionalSkillGroups.forEach((type) => {
      $.GetContextPanel()
        .FindChildInLayoutFile("JsPlayerCardSkillGroup-" + type)
        .SetHasClass("collapsed", !elToggleBtn.checked)
    })
  }
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    ...
    playerCard.FillOutFriendCard
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_NameChanged",
    playerCard.UpdateName
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_MedalsChanged",
    playerCard.UpdateAvatar
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_FriendsList_ProfileUpdated",
    playerCard.ProfileUpdated
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_PipRankUpdate",
    playerCard.SetAllSkillGroups
  )
  $.RegisterForUnhandledEvent(
  ...
  $.RegisterForUnhandledEvent(
    ...
    playerCard.FriendsListUpdateName
  )
})()
