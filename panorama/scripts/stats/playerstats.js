...
var playerstats = (function () {
  ...
  var _m_LineGraph = $.GetContextPanel().FindChildTraverse(
    "id-playerstats__linegraph"
  )
  var _m_MapGraph = $.GetContextPanel().FindChildTraverse(
    "id-playerstats__web-maps"
  )
  var _m_WeaponGraph = $.GetContextPanel().FindChildTraverse(
    "id-playerstats__web-weapons"
  )
  var _m_Heatmap = $.GetContextPanel().FindChildTraverse(
    "id-playerstats__heatmap"
  )

  var _m_elMatchHistory = $.GetContextPanel().FindChildTraverse(
    "PlayerStatsMatchHistory"
  )
  ...
  function _InitializeTimeRangeFilter() {
    var ival = parseInt(
      $.GetContextPanel()
        .FindChildTraverse("id-playerstats__range")
        .GetSelected()
        .GetAttributeString("value", "")
    )
  ...
  function _InitializeModeFilterFromSettings() {
    var elDropdown = $.GetContextPanel().FindChildTraverse(
      "id-playerstats__mode"
    )
    ...
    if (lastMatch && lastMatch.matches.length > 0) {
      ...
      timestamp +=
        1606723200 - DeepStatsAPI.MatchIDToLocalTime("3450411799665595200")
  ...
  function _GetModeFilter() {
    return $.GetContextPanel()
      .FindChildTraverse("id-playerstats__mode")
      .GetSelected()
      .GetAttributeString("value", "")
  }
  ...
  function _InitMatchDetailsPanel() {
    if (!_m_elSingleMatch) {
      _m_elSingleMatch = $.GetContextPanel().FindChildTraverse(
  ...
  function _Init() {
    ...
    if (!_m_SubscriptionStatusChangeUpdatedHandler) {
      _m_SubscriptionStatusChangeUpdatedHandler = $.RegisterForUnhandledEvent(
        "PanoramaComponent_MyPersona_RecurringSubscriptionStatusChange",
        _UpdateSubscriptionStatus
      )
    }
    ...
    $.RegisterForUnhandledEvent(
      "StatsProgressGraph_OnGraphUpdated",
      _UpdateLineGraph
    )
    ...
    $.RegisterEventHandler(
      "UnreadyForDisplay",
      $.GetContextPanel(),
      _OnUnreadyForDisplay
    )
    $.RegisterForUnhandledEvent("DeepStatsTimeoutGiveUp", _OnDeepStatsTimeOut)

    $.RegisterEventHandler(
      "DeepStatsPanel_OnStatDownloadProgress",
      _m_LineGraph,
      function () {
        $.Schedule(0.01, _OnStatDownloadProgress_LineGraph)
      }
    )
    $.RegisterEventHandler(
      "DeepStatsPanel_OnStatDownloadProgress",
      _m_MapGraph,
      function () {
        $.Schedule(0.01, _OnStatDownloadProgress_MapGraph)
      }
    )
    $.RegisterEventHandler(
      "DeepStatsPanel_OnStatDownloadProgress",
      _m_WeaponGraph,
      function () {
        $.Schedule(0.01, _OnStatDownloadProgress_WeaponGraph)
      }
    )
    ...
    var arrPanelsToAnimateIn =
      $.GetContextPanel().FindChildrenWithClassTraverse("rotatein")
  ...
  function _OnUnreadyForDisplay() {
    ...
    if (_m_SubscriptionStatusChangeUpdatedHandler) {
      $.UnregisterForUnhandledEvent(
        "PanoramaComponent_MyPersona_RecurringSubscriptionStatusChange",
        _m_SubscriptionStatusChangeUpdatedHandler
      )
  ...
  function _GetBestMateForPeriod(arrMatches) {
    ...
    var arrFilteredMates =
      Object.values(oMates.w3).length > 0
        ? oMates.w3
        : Object.keys(oMates.w2).length > 0
          ? oMates.w2
          : Object.keys(oMates.w1).length > 0
            ? oMates.w1
            : oMates.all
    ...
    if (arrFilteredMates.length == 0) return [0, 0]

    arrFilteredMates.forEach(function (mate, index) {
      var ratio =
        oMatesData[mate].rounds_lost == 0
          ? oMatesData[mate].rounds_won
          : oMatesData[mate].rounds_won / oMatesData[mate].rounds_lost
      if (ratio >= bestRatio) {
        bestRatio = ratio
  ...
  function _UpdateSubscriptionStatus() {
    ...
    var labelDesc = $.GetContextPanel().FindChildInLayoutFile(
      "id-subscription-status-desc"
    )
    ...
    if (rtRecurringSubscriptionNextBillingCycle) {
      var numSecondsTillNextBillingCycle = NewsAPI.GetNumSecondsTillGcTimestamp(
        rtRecurringSubscriptionNextBillingCycle
      )
      if (numSecondsTillNextBillingCycle < 60) {
        labelDesc.text = $.Localize(
          "#playerstats_subscription_expired",
          labelDesc
        )
        ...
        var decString = status
          ? "#playerstats_subscription_" + status
          : "#playerstats_subscription_renew_unknown"

        labelDesc.SetHasClass(
          "subscription-status__label--yellow",
          status === "renew_manually"
        )
        labelDesc.SetHasClass(
          "subscription-status__label--green",
          status === "renew_automatically"
        )

        if (
          numSecondsTillNextBillingCycle <= 24 * 3600 &&
          status === "renew_manually"
        ) {
          labelDesc.text = $.Localize(
            "#playerstats_subscription_expire_warning"
          )
        } else {
          var daysLeft = Math.floor(
            numSecondsTillNextBillingCycle / (24 * 3600 + 1)
          )

          labelDesc.SetDialogVariableInt("days", daysLeft)
          var daysString =
            numSecondsTillNextBillingCycle <= 24 * 3600
              ? $.Localize("#SFUI_Store_Timer_Day")
              : $.Localize("#SFUI_Store_Timer_Days")
    ...
    var btnGetSubscription = $.GetContextPanel().FindChildInLayoutFile(
      "id-get-subscription"
    )
    var btnManageSubscription = $.GetContextPanel().FindChildInLayoutFile(
  ...
  function _CreateTooltip(
    ...
    var xmlsrc =
      "file://{resources}/layout/tooltips/stats/tooltip_playerstats_generic.xml"
    var ttid = "tt_" + elPanel.id
    var parms = "title=" + title + "&text=" + text + "&class=" + additionalClass
    var onStatHoverOn_f = _OnMouseOverCustomLayoutTooltip.bind(
      undefined,
      elPanel.id,
      ttid,
      xmlsrc,
    ...
    elPanel.SetPanelEvent("onmouseover", function () {
      onStatHoverOn_f()
      if (typeof fnon === "function") fnon()
    })
    elPanel.SetPanelEvent("onmouseout", function () {
      onStatHoverOff_f()
      if (typeof fnoff === "function") fnoff()
    })
  }

  function _ResetYourRecord() {
    var elRecordFrame = $.GetContextPanel().FindChildTraverse(
      "PlayerStatsRecordFrame"
    )
  ...
  function _UpdateYourRecordAsycDeepstats() {
    ...
    if (elBlocker) elBlocker.visible = arrMatches.length == 0

    var elNoDataNotice = $.GetContextPanel().FindChildTraverse("NoDataNotice")
    if (elNoDataNotice)
      elNoDataNotice.style.opacity =
        arrMatches.length == 0 && oDeepStats.status == "complete" ? 1 : 0

    if (
      (!arrMatches || arrMatches.length == 0) &&
      oDeepStats.status != "complete"
    ) {
  ...
  function _OnStatDownloadProgress_LineGraph() {
    ...
    _m_arrLinegraphStats.forEach(function (stat, idx) {
      ...
      $.GetContextPanel().SetDialogVariable(
        strStatTotalPrefix + stat,
        prettyTotal
      )

      var denom =
        oStatAvgSuffix[stat] == "playerstats_suffix_per_cent" ? 10 : 1000
      $.GetContextPanel().SetDialogVariable(
        strStatAveragePrefix + stat,
        (avg / denom).toPrecision(3) + $.Localize(oStatAvgSuffix[stat])
      )
    })

    $.GetContextPanel().SetDialogVariable(
      "stat_total_wins",
      _m_LineGraph.GetAttributeInt("wins", 0)
    )
    $.GetContextPanel().SetDialogVariable(
      "stat_total_losses",
      _m_LineGraph.GetAttributeInt("losses", 0)
    )
    $.GetContextPanel().SetDialogVariable(
      "stat_total_ties",
      _m_LineGraph.GetAttributeInt("ties", 0)
    )
    ...
    if (elKills) {
      ...
      DigitPanelFactory.SetDigitPanelString(
        elKills,
        string,
        bAbbreviate ? suffix : ""
      )
  ...
  function _OnStatDownloadProgress_MapGraph() {
    ...
    if (elBestMap) {
      ...
      if (bValid) {
        elBestMap.SetDialogVariable(
          "value",
          $.Localize("SFUI_Map_" + mapString)
        )

        var elMapIcon = elBestMap.FindChildTraverse("MapIcon")
        var strMap = DeepStatsAPI.MapIDToString(_m_MapGraph.best_map)
        elMapIcon.SetImage(
          "file://{images}/map_icons/map_icon_" + strMap + ".svg"
        )
        IconUtil.SetupFallbackMapIcon(
          elMapIcon,
          "file://{images}/map_icons/map_icon_NONE.png"
        )
      }
    }
  ...
  function _OnStatDownloadProgress_WeaponGraph() {
    ...
    if (elBestWeaponContainer) {
      ...
      var weaponIdx = 0
      elBestWeaponContainer
        .FindChildrenWithClassTraverse("weapons__row")
        .forEach(function (elWeaponPanel) {
          var elImage = elWeaponPanel.FindChild("WeaponImage")
          var weaponScore = arrWeapons[weaponIdx++]
          if (elImage) {
            elImage.SetImage(
              "file://{images}/icons/equipment/" +
                InventoryAPI.GetHudIconFileName(weaponScore.weapon_id) +
                ".svg"
            )
          }
          elWeaponPanel.SetDialogVariable(
            "score",
            weaponScore.score.toPrecision(3)
          )
          elWeaponPanel.SetDialogVariable("total", weaponScore.total)
          var itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
            weaponScore.weapon_id,
            0
          )
  ...
  function _UpdateLineGraph() {
    ...
    var arrGraphPoints =
      $.GetContextPanel().FindChildrenWithClassTraverse("graph-point-avg")
    ...
    arrGraphPoints.forEach(function (elPoint, index) {
      ...
      elPoint.SetPanelEvent(
        "onactivate",
        onActivate_f.bind(this, _GetDateKeyFromTimestamp(timestamp))
      )
  ...
  function _isPercentage(index) {
    return index == 4 || index == 8 || index == 9 || index == 10 || index == 11
  }
  ...
  function _GetUnitSuffix(index) {
    if (_isPercentage(index)) return $.Localize("playerstats_suffix_per_cent")
    else return ""
  }

  function _UpdateGraphStatButtons() {
    _m_LineGraph
      .FindChildrenWithClassTraverse("stats-panel--graphbtn")
      .forEach(function (elBtn, index) {
        var valueNormal = parseInt(elBtn.GetAttributeInt("stat_average", 0))
        var valueNormal =
          valueNormal == 0
            ? 0
            : (
                valueNormal / (_isPercentage(index) ? 10.0 : 1000.0)
              ).toPrecision(3)
        ...
        if (elStatStatNormalLabel) {
          ...
          if (_isPercentage(index)) {
            elStatStatNormalLabel.text = $.Localize(
              "#playerstats_per_cent",
              elStatStatNormalLabel
            )
          } else {
            elStatStatNormalLabel.text = $.Localize(
              "#playerstats_no_units",
              elStatStatNormalLabel
            )
          }
        }
        ...
        if (elStatRankLabel) {
          ...
          if (_HasHistogram(index)) {
            elStatRankLabel.SetDialogVariable(
              "tt_value",
              valueNormal + _GetUnitSuffix(index)
            )
          ...
          elStatRankLabel.SetDialogVariable(
            "value",
            rank != -1 ? rank.toPrecision(2) : ""
          )

          if (_HasHistogram(index))
            elStatRankLabel.text = $.Localize(
              "#playerstats_per_cent",
              elStatRankLabel
            )
        }
  ...
  function _OnModeChanged() {
    ...
    if (
      GameInterfaceAPI.GetSettingString("ui_deepstats_toplevel_mode") !==
      _m_mode
    ) {
  ...
  function _OnMouseOverCustomLayoutTooltip(
    _panel,
    _tooltipId,
    _xmlsrc,
    _parms
