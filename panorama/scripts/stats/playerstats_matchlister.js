...
var MatchLister = (function () {
  function _Init(elPanel) {
    $.RegisterEventHandler(
      "ScrolledIntoView",
      elPanel.FindChildTraverse("MatchContainer"),
      _OnScrollIntoView
    )
    $.RegisterEventHandler(
      "ScrolledOutOfView",
  ...
  function _Populate(elPanel, nDays, nMode, sortStat) {
    ...
    if (
      !elPanel.m_nDays ||
      (nDays < elPanel.m_nDays && nDays != -1) ||
      elPanel.m_nDays == -1 ||
      nMode != elPanel.m_nMode
    ) {
      if (!elPanel.m_hasDummyMatches) elLister.RemoveAndDeleteChildren()
    ...
    if (
      ...
      Scheduler.Schedule(
        1.0,
        _Populate.bind(this, elPanel, nDays, nMode, sortStat),
        "MATCHES"
      )
      return
    }
    ...
    if (
      !arrMatches ||
      (arrMatches.length == 0 && oDeepStats.status == "complete")
    ) {
    ...
    arrMatches.forEach(function (oMatch, index) {
      ...
      if (elLister.FindChildTraverse(oMatch.match_id)) return
      ...
      if (!elDayContainer) {
        ...
        if (elDayContainer.id > arrChildren[numChildren - 1].id) {
          ...
          while (
            idx < numChildren &&
            arrChildren[idx] &&
            elDayContainer.id < arrChildren[idx].id
          )
            idx++
      ...
      elMatchDetails.SetPanelEvent(
        "onactivate",
        onActivate_f.bind(this, oMatch.match_id)
      )

      var parms =
        "class=" + "mode" + nMode + "&matchdata=" + JSON.stringify(oMatch)
      var xmlsrc =
        "file://{resources}/layout/tooltips/stats/tooltip_playerstats_matchlister_matchstats.xml"

      var ttid = "tt_" + elMatch.id
      var onDayHoverOn_f = _OnMouseOverCustomLayoutTooltip.bind(
        undefined,
        elMatch.id,
        ttid,
        xmlsrc,
