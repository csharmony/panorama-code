...
var SettingsMenu = (function () {
  ...
  var _AccountPrivacySettingsChanged = function () {
    var gameSettingPanel =
      $.GetContextPanel().FindChildInLayoutFile("GameSettings")
    if (gameSettingPanel != null) {
      var twitchTvSetting = gameSettingPanel.FindChildInLayoutFile(
        "accountprivacydropdown"
      )
  ...
  var _InitTab = function (tabID) {
    ...
    newPanel.BLoadLayout(
      "file://{resources}/layout/settings/" + XmlName + ".xml",
      false,
      false
    )
  ...
  var _Init = function () {
    for (let tab in tabInfo) {
      if (tab !== "Promoted" && tab !== "Search") _InitTab(tab)
    }
  }
...
;(function () {
  ...
  if (PromotedSettingsUtil.GetUnacknowledgedPromotedSettings().length > 0) {
    ...
    const now = new Date()
    if (
      g_PromotedSettings.filter(
        (setting) => setting.start_date <= now && setting.end_date > now
      ).length == 0
    )
  ...
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_AccountPrivacySettingsChanged",
  ...
  $.RegisterEventHandler(
    "ReadyForDisplay",
    $("#JsSettings"),
    SettingsMenu.OnSettingsMenuShown
  )
  $.RegisterEventHandler(
    "UnreadyForDisplay",
    $("#JsSettings"),
    SettingsMenu.OnSettingsMenuHidden
  )
  $.RegisterForUnhandledEvent(
    "SettingsMenu_NavigateToSetting",
    SettingsMenu.NavigateToSetting
  )
  $.RegisterForUnhandledEvent(
    "SettingsMenu_NavigateToSettingPanel",
    SettingsMenu.NavigateToSettingPanel
  )
})()
