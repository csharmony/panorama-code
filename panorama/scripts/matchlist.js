...
var matchList = (function () {
      ...
      if (activeTiles[i].markForDelete) {
        ...
        if (watchTile.downloadStateHandler) {
          $.UnregisterForUnhandledEvent(
            ...
            watchTile.downloadStateHandler
          )
  ...
  function _SelectFirstTile(parentPanel, elMatchList, matchListDescriptor) {
    if (
      elMatchList &&
      !elMatchList.activeButton &&
      elMatchList.GetChildCount() > 0
    ) {
      ...
      do {
        ...
        tileIsVisible =
          elFirstTile && !elFirstTile.BHasClass("MatchTile--Collapse")
        n = n + 1
  ...
  function _ReselectActiveTile(elListRoot) {
    ...
    if (elMatchList && elMatchList.activeButton) {
      ...
      _PopulateMatchInfo(
        elListRoot,
        elListRoot.matchListDescriptor,
        elMatchList.activeButton.matchId
      )
    } else {
  ...
  var _OnTournamentTeamSelected = function (
    elParentPanel,
    elMatchList,
    matchListDescriptor
  ) {
  ...
  var _OnTournamentSectionSelected = function (
    elParentPanel,
    elMatchList,
    matchListDescriptor
  ) {
  ...
  function MakeDropDownEntry(
    index,
    sectionDesc,
    sectionName,
    elMatchlistDropdown
  ) {
    var elSection = $.CreatePanel(
      "Label",
      elMatchlistDropdown,
      "group_" + sectionDesc,
      { text: sectionName }
  ...
  var _PopulateMatchlistDropdown = function (elParentPanel, tournamentId) {
    var elMatchlistDropdown = elParentPanel.FindChildTraverse(
      "id-match-list-selector"
    )
    ...
    for (var i = 0; i < nSections; i++) {
      ...
      sectionName = $.Localize(
        "#CSGO_MatchInfo_Stage_" + sectionName.replace(/\s+/g, "")
      )
    ...
    elMatchlistDropdown.SetPanelEvent(
      "oninputsubmit",
      _OnTournamentSectionSelected.bind(
        undefined,
        elParentPanel,
        elMatchList,
        tournamentId
  ...
  var _PopulateMatchTeamsDropdown = function (elParentPanel, tournamentId) {
    var elMatchistTeamDropdown = elParentPanel.FindChildTraverse(
      "id-match-list-selector-teams"
    )
    elMatchistTeamDropdown.ClearPanelEvent("oninputsubmit")
    elMatchistTeamDropdown.RemoveAllOptions()

    var elStageDropdown = elParentPanel.FindChildTraverse(
      "id-match-list-selector"
    )
    var sectionId = elStageDropdown
      .GetSelected()
      .GetAttributeString("section_id", 0)

    var teamsList = []
    var numGroups = PredictionsAPI.GetSectionGroupsCount(
      tournamentId,
      sectionId
    )
    ...
    MakeDropDownEntry(
      0,
      "allteams",
      "#Matchlist_Team_Selection",
      elMatchistTeamDropdown
    )
    teamsList.push("allteams")

    for (var j = 0; j < numGroups; j++) {
      var numGroupId = PredictionsAPI.GetSectionGroupIDByIndex(
        tournamentId,
        sectionId,
        j
      )
      var count = PredictionsAPI.GetGroupTeamsPickableCount(
        tournamentId,
        numGroupId
      )

      for (var h = 0; h < count; h++) {
        var teamId = PredictionsAPI.GetGroupTeamIDByIndex(
          tournamentId,
          numGroupId,
          h
        )
    ...
    var elMatchList = elParentPanel.FindChildTraverse("JsMatchList")
    elMatchistTeamDropdown.SetPanelEvent(
      "oninputsubmit",
      _OnTournamentTeamSelected.bind(
        undefined,
        elParentPanel,
        elMatchList,
        tournamentId
  ...
  function _UpdateMatchList(
    elTab,
    matchListDescriptor,
    optbFromMatchListChangeEvent
  ) {
  ...
  function _PopulateMatchInfo(parentPanel, matchListDescriptor, matchId) {
    ...
    var elButton = parentPanel.FindChildTraverse(
      matchListDescriptor + "_" + matchId
    )
    ...
    if (
      parentPanel.activeMatchInfoPanel &&
    ...
    parentPanel.activeMatchInfoPanel = parentInfoPanel.FindChild(
      "info_" + matchId
    )
    if (parentPanel.activeMatchInfoPanel == undefined) {
      parentPanel.activeMatchInfoPanel = $.CreatePanel(
        "Panel",
        parentInfoPanel,
        "info_" + matchId
      )
      ...
      parentPanel.activeMatchInfoPanel.BLoadLayout(
        "file://{resources}/layout/matchinfo.xml",
        false,
        false
      )
      parentPanel.activeMatchInfoPanel.tournament_id = parentPanel.tournament_id
      parentPanel.activeMatchInfoPanel.tournamentIndex =
        parentPanel.tournamentIndex
  ...
  function _RequestMatchListUpdate(elTab, matchListDescriptor) {
    function _ShowLoadingError(elBoundTab) {
      ...
      if (elBoundTab.tournament_id) {
        msg =
          "#CSGO_Watch_NoMatch_Tournament_" +
          elBoundTab.tournament_id.split(":")[1]
      } else {
        switch (elTab.id) {
          case "JsLive":
    ...
    if (elTab) {
      ...
      if (newState === "loading") {
        ...
        elTab.downloadFailedHandler = $.Schedule(
          3.0,
          _ShowLoadingError.bind(undefined, elTab)
        )
      }
      return newState
    }
  ...
  function _PopulateMatchList(parentPanel, matchListDescriptor) {
    ...
    function _ShowGOTVConfirmPopup(elListRoot) {
      ...
      UiToolkitAPI.ShowGenericPopupOkCancel(
        $.Localize("CSGO_Watch_Gotv_Theater"),
        $.Localize("CSGO_Watch_Gotv_Theater_tip"),
        "",
        function () {
          MatchListAPI.StartGOTVTheater("live")
        },
        _ReselectActiveTile.bind(undefined, elListRoot)
      )
    }
    ...
    function GetListOfMatchIds(
      ...
      for (var i = 0; i < unfilteredCount; i++) {
        ...
        if (tournamentIndex > 3) {
          matchId = PredictionsAPI.GetSectionMatchByIndex(
            matchListDescriptor,
            sectionDesc,
            i
          )
    ...
    if (unfilteredCount > 0 && parentPanel.tournament_id) {
      ...
      if (tournamentIndex > 3) {
        ...
        sectionDesc = elDropdown
          .GetSelected()
          .GetAttributeString("section_id", 0)
        unfilteredCount = PredictionsAPI.GetSectionMatchesCount(
          parentPanel.tournament_id,
          sectionDesc
        )

        var elStageDropdown = parentPanel.FindChildTraverse(
          "id-match-list-selector-teams"
        )
        var strTeamId = elStageDropdown
          .GetSelected()
          .GetAttributeString("section_id", 0)
        var nteamId = strTeamId === "allteams" ? 0 : Number(strTeamId)

        MatchIdsFiltered = GetListOfMatchIds(
          parentPanel.tournament_id,
          tournamentIndex,
          unfilteredCount,
          sectionDesc,
    ...
    if (nCount <= 0) {
      ...
      if (parentPanel.tournament_id) {
        msg =
          "#CSGO_Watch_NoMatch_Tournament_" +
          parentPanel.tournament_id.split(":")[1]
      } else {
    ...
    function _CreateOrValidateMatchTile(matchId) {
      var elMatchButton = elMatchList.FindChildInLayoutFile(
        matchListDescriptor + "_" + matchId
      )
      if (!elMatchButton || matchListDescriptor === "live") {
        ...
        if (matchId != "gotv") {
          elMatchButton.SetPanelEvent(
            "onactivate",
            _PopulateMatchInfo.bind(
              undefined,
              parentPanel,
              matchListDescriptor,
              matchId
          ...
          elMatchButton.SetPanelEvent(
            "onactivate",
            _ShowGOTVConfirmPopup.bind(undefined, parentPanel)
          )
        }
        elMatchButton.SetPanelEvent(
          "onmouseover",
          OnMouseOverButton.bind(
            undefined,
            parentPanel,
            matchListDescriptor + "_" + matchId
          )
      ...
      function _UpdateDownloadState(elBoundMatchButton) {
        if (elBoundMatchButton && !elBoundMatchButton.markForDelete) {
          var elDownloadIndicator =
            elBoundMatchButton.FindChildInLayoutFile("id-download-state")
          if (elDownloadIndicator) {
            var isDownloading = Boolean(
              MatchInfoAPI.GetMatchState(elBoundMatchButton.matchId) ===
              "downloading"
            )
            var canWatch = Boolean(
              MatchInfoAPI.CanWatch(elBoundMatchButton.matchId)
            )
            var isLive = Boolean(
              MatchInfoAPI.IsLive(elBoundMatchButton.matchId)
            )
      ...
      if (
        elMatchButton.downloadStateHandler == undefined &&
        elMatchButton.FindChildInLayoutFile("id-download-state")
      ) {
        elMatchButton.downloadStateHandler = $.RegisterForUnhandledEvent(
          "PanoramaComponent_MatchInfo_StateChange",
          _UpdateDownloadState.bind(undefined, elMatchButton)
        )
    ...
    if (
      matchListDescriptor === "live" &&
      elMatchList.FindChildInLayoutFile("live_gotv")
    ) {
