...
var mainmenu_watch_eventsched = (function () {
    ...
    _m_arrEvents.forEach((o) => {
      o["live_matches"].forEach((m) => {
        if (m["match_id"] == matchId) {
          $.DispatchEvent(
            "Tournaments_RequestMatch_Response",
            JSON.stringify(m)
          )
          return
        }
  ...
  function _ClearEmptyMonth(elMonth) {
    {
      ...
      if (!bAnyVisible) elMonth.RemoveClass("eventsched-month-show")
    }
  }
  ...
  function _UpdateAllFavorites() {
    ...
    if (!_m_arrFavorites || _m_arrFavorites.length === 0) return
    ...
    for (var idx in _m_arrEvents) {
      ...
      if (elEvent && elEvent.IsValid()) {
        ...
        var elFavoriteBtn = elEvent.FindChildTraverse(
          "id-capsule__main__favorite"
        )
  ...
  function _CreateOnGoingSection() {
    ...
    let elHeader = elContainer.FindChildTraverse(
      "id-eventsched__month-container__header"
    )
    if (elHeader) {
  ...
  function _CreateYearSection(idYear, startDate) {
    ...
    let elYearHeader = elYearContainer.FindChildTraverse(
      "id-eventsched__year-container__header"
    )
    if (elYearHeader) {
  ...
  function _CreateMonthSection(idMonth, startDate, elYearContainer) {
    ...
    let elMonthHeader = elMonthContainer.FindChildTraverse(
      "id-eventsched__month-container__header"
    )
    if (elMonthHeader) {
      var monthPaddedNumber = ("0" + (startDate.getMonth() + 1)).slice(-2)
  ...
  function _CreateEventPanel(elMonthContainer, oEvent, isOddEvent, isOngoing) {
    ...
    elEvent.m_isLive =
      "live_matches" in oEvent && oEvent["live_matches"].length > 0

    elEvent.m_event_id = oEvent["event_id"]

    elEvent.m_favoriteCount =
      "favorites" in oEvent ? Number(oEvent["favorites"]) : 0
  ...
  function _SetEventIsFeatured(oEvent, elEvent) {
    if (oEvent["is_official"]) {
      let elEventBtn = elEvent.FindChildTraverse(
        "id-eventsched__capsule__main__btn"
      )

      elEventBtn.SetPanelEvent(
        "onmouseover",
        _OnMouseOverTextTooltip.bind(
          undefined,
          elEvent.id,
          $.Localize("eventsched_official")
        )
      )
      ...
      elEvent.SetDialogVariable(
        "eventsched_fave_month",
        GetEventMonthString(oEvent)
      )
      elEvent.SetDialogVariable(
        "eventsched_featured",
        $.Localize("#eventsched_featured", elEvent)
      )
      ...
      elEventBtn.SetPanelEvent(
        ...
        _OnMouseOverTextTooltip.bind(
          undefined,
          elEvent.id,
          $.Localize("{s:eventsched_featured}", elEvent)
        )
      )
      elEventBtn.SetPanelEvent("onmouseout", _OnMouseOutTextTooltip)
    }
  ...
  function _SetEventDateStrings(elEvent, startDate, endDate) {
    let elStartDateLabel = elEvent.FindChildTraverse(
      "id-eventsched__dates__start"
    )
    ...
    if (startDate != undefined) {
      startMonthString = $.Localize(
        "SFUI_Date_Format_Month" + Number(startDate.getMonth() + 1)
      )
      elStartDateLabel.SetDialogVariable(
        "eventsched_date_month",
        startMonthString
      )
      startDatePaddedDayNumber = ("0" + startDate.getDate()).slice(-2)
      startDayString = $.Localize(
        "SFUI_Date_Format_DayOfMonth" + startDatePaddedDayNumber
      )
      elStartDateLabel.SetDialogVariable("eventsched_date_day", startDayString)
    }

    if (endDate != undefined) {
      endMonthString = $.Localize(
        "SFUI_Date_Format_Month" + Number(endDate.getMonth() + 1)
      )
      elEndDateLabel.SetDialogVariable("eventsched_date_month", endMonthString)
      endDatePaddedDayNumber = ("0" + endDate.getDate()).slice(-2)
      endDayString = $.Localize(
        "SFUI_Date_Format_DayOfMonth" + endDatePaddedDayNumber
      )
  ...
  function _SetEventTeams(oEvent, elEvent) {
    ...
    let elTeamMasterContainer = elEvent.FindChildTraverse(
      "id-eventsched__teams"
    )
    ...
    if (arrTeams) {
      ...
      for (let idx in arrTeams) {
        ...
        let elTeamSubContainer = elEvent.FindChildTraverse(
          "id-eventsched__teams__" + nRow
        )
        if (!elTeamSubContainer || !elTeamSubContainer.IsValid()) {
          elTeamSubContainer = $.CreatePanel(
            "Panel",
            elTeamMasterContainer,
            "id-eventsched__teams__" + nRow
          )
        ...
        let elTeamButton = $.CreatePanel(
          "Button",
          elTeamSubContainer,
          "button-" + oEvent["event_id"] + "_" + oTeam["name"]
        )
        ...
        if ("lineup" in oTeam && Object.keys(oTeam["lineup"]).length != 0) {
          ...
          for (let pIdx in oTeam["lineup"]) {
            ...
            if (
              "profile_photo_url" in oPlayer &&
              oPlayer["profile_photo_url"] !== ""
            ) {
              parms +=
                "&player_photo" + pIdx + "=" + oPlayer["profile_photo_url"]
              bHaveAnyPhoto = true
            } else {
              parms +=
                "&player_photo" +
                pIdx +
                "=" +
                "file://{images}/icons/ui/pro_player.svg"
            }
...

          {
            function OnSimpleContextMenu(url) {
              let items = []
              items.push({
                label: $.Localize("#eventsched_team_link"),
                jsCallback: function () {
            ...
            if (!onTeamActivate_f)
              onTeamActivate_f = OnSimpleContextMenu.bind(
                undefined,
                oTeam["link"]
              )

            xmlsrc =
              "file://{resources}/layout/tooltips/tooltip_eventsched_team_simple.xml"

            onTeamHoverOn_f = _OnMouseOverCustomLayoutTooltip.bind(
              undefined,
              elTeamButton.id,
              "tt_" + elTeamButton.id,
              xmlsrc,
            ...
            onTeamHoverOff_f = _OnMouseOutCustomLayoutTooltip.bind(
              undefined,
              "tt_" + elTeamButton.id
            )
          }
          ...
          onTeamHoverOn_f = textTooltipOn.bind(
            undefined,
            elTeamButton.id,
            oTeam["name"]
          )
          onTeamHoverOff_f = function () {
            UiToolkitAPI.HideTextTooltip()
          }
  ...
  function _SetEventURL(oEvent, elEvent) {
    if ("event_page_url" in oEvent) {
      ...
      let elLinkBtn = elEvent.FindChildTraverse(
        "id-eventsched__capsule__main__btn"
      )
      ...
      function OnSimpleContextMenu(url) {
        let items = []
        items.push({
          label: $.Localize("#eventsched_event_link"),
          jsCallback: function () {
      ...
      elLinkBtn.SetPanelEvent(
        "onactivate",
        OnSimpleContextMenu.bind(undefined, url)
      )
    }
  }
  ...
  function _SetEventFavoriteButton(oEvent, elEvent) {
    ...
    if (elFavoriteBtn && elFavoriteBtn.IsValid()) {
      ...
      var tooltipText = _m_IsPrime
        ? "#eventsched_favorite_tooltip_prime"
        : "#eventsched_favorite_tooltip_not_prime"

      var onMouseOver = function (id, tooltipText) {
        UiToolkitAPI.ShowTextTooltip(id, tooltipText)
      }
      elFavoriteBtn.SetPanelEvent(
        "onmouseover",
        onMouseOver.bind(undefined, oEvent["event_id"], tooltipText)
      )
      elFavoriteBtn.SetPanelEvent("onmouseout", function () {
        UiToolkitAPI.HideTextTooltip()
      })
      ...
      if (_m_arrFavorites) {
        var onActivate = function (element, btn) {
          ...
          TournamentsAPI.SetTournamentFavorite(
            Number(element.m_event_id),
            btn.IsSelected()
          )
          ...
          if (btn.IsSelected()) {
            ...
            if (idx !== -1) _m_arrFavorites.splice(idx, 1)
          }
        ...
        elFavoriteBtn.SetPanelEvent(
          "onactivate",
          onActivate.bind(undefined, elEvent, elFavoriteBtn)
        )
  ...
  function _SetEventLiveMatches(oEvent, elEvent, isOngoing) {
    if ("live_matches" in oEvent && oEvent["live_matches"].length > 0) {
      var elMatchContainer = elEvent.FindChildTraverse(
        "id-eventsched__capsule-container__matches"
      )
      ...
      for (var idx in oEvent["live_matches"]) {
        ...
        var elMatch = $.CreatePanel(
          "Panel",
          elMatchContainer,
          oMatch["match_id"]
        )
        elMatch.BLoadLayout(
          "file://{resources}/layout/watchmatchtile.xml",
          false,
          false
        )
        ...
        function _GetTeam(num) {
...

          {
            for (var idx in arrTeams) {
              ...
              if (oTeam["name"] == oMatch[matchTeamName]) return oTeam
            }
          }
      ...
      var elNoLiveMatchNotice = elMatchContainer.FindChildTraverse(
  ...
  function _PopulateLister() {
    ...
    for (var idx in _m_arrEvents) {
      ...
      if (
        "start_date_time" in oEvent &&
        "seconds" in oEvent["start_date_time"]
      ) {
        startTimeUTCSeconds = Number(oEvent["start_date_time"]["seconds"])
        startDate.setUTCSeconds(startTimeUTCSeconds)
      ...
      if (isOngoing) {
        ...
        if (!elMonthContainer || !elMonthContainer.IsValid()) {
          elMonthContainer = _CreateMonthSection(
            idMonth,
            startDate,
            elYearContainer
          )
      ...
      if (isOngoing) {
        ...
        for (let e = 0; e < nChildren; e++) {
          ...
          if (!el.hasOwnProperty("m_event_id")) continue
