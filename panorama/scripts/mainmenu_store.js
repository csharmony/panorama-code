...
var MainMenuStore = (function () {
  ...
  var _Init = function () {
    ...
    if (!MyPersonaAPI.IsConnectedToGC()) return
    ...
    if (
      ...
      "" !==
        StoreAPI.GetStoreItemSalePrice(
          InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
            g_ActiveTournamentStoreLayout[0][0],
            0
          ),
          1,
          ""
      ...
      var bCanSellCapsules =
        sRestriction !== "restricted" && sRestriction !== "xray"

      if (bCanSellCapsules || (!bCanSellCapsules && !oWinningTeam)) {
        m_elStore.SetDialogVariable(
          "tournament_name",
          $.Localize(
            "#CSGO_Tournament_Event_Location_" +
              NewsAPI.GetActiveTournamentEventID()
          )
        )
    ...
    if (nSeasonIndex && nSeasonIndex > 0) {
      ...
      m_elStore.SetDialogVariable(
        "operation_name",
        $.Localize("#" + opname + "_name")
      )
    }

    if (
      OperationUtil.ValidateCoinAndSeasonIndex(nSeasonIndex, oStatus.nCoinRank)
    ) {
  ...
  function _CheckInventoryFull() {
    if (
      InventoryAPI.GetInventoryUsagePercentage() >= INVENTORY_FULL_THRESHOLD
    ) {
      let elStorageContainerItem = $("#JsStorageContainerItem")

      $("#StorePanelInventoryFull").SetDialogVariableInt(
        "inv-usage",
        InventoryAPI.GetInventoryUsagePercentage()
      )
      ...
      if (!elItem) {
        ...
        elItem.BLoadLayout(
          "file://{resources}/layout/mainmenu_store_tile.xml",
          false,
          false
        )
      }
    ...
    var tournament = [
      {
        ...
        load_func: function (elpanel) {
          elpanel.BLoadLayout(
            "file://{resources}/layout/mainmenu_tournament_pass_status.xml",
            false,
            false
          )
        }
    ...
    var operation = [
      {
        ...
        load_func: function (elpanel) {
          ...
          if (aRewards && aRewards.length > 0) {
            ...
            for (
              var idxstoretile = 0;
              idxstoretile < aIds.length;
              ++idxstoretile
            ) {
              elpanel.FindChildInLayoutFile(
                "id-store-operation-char" + idxstoretile
              ).itemid = aIds[idxstoretile]
            }
            ...
            for (
              ...
              elpanel.FindChildInLayoutFile(
                "id-store-operation-item" + idxstoretile
              ).itemid = aIds[idxstoretile]
            }

            var elBalance = m_elStore.FindChildInLayoutFile(
              "id-store-operation-balance-container"
            )
  ...
  var _ProTeamsItems = function () {
    ...
    if (
      firstIdPrice === "" ||
      firstIdPrice === undefined ||
      firstIdPrice === null ||
      !_BAllowDisplayingItemInStore(aItemIds[0])
    ) {
      return null
    }
  ...
  var _CheckLicenseScreen = function () {
    ...
    if (restrictions) {
      elBanner.FindChildInLayoutFile("StorePanelLicenseBannerText").text =
        restrictions.license_msg
      elBanner
        .FindChildInLayoutFile("StorePanelLicenseBannerButton")
        .Children()[0].text = restrictions.license_act
    }

    var elMainMenuInput = m_elStore
    while (elMainMenuInput) {
      elMainMenuInput = elMainMenuInput.GetParent()
      if (elMainMenuInput.id === "MainMenuInput") break
    }
    if (elMainMenuInput) {
      elMainMenuInput.SetHasClass(
        "steam-license-restricted",
        restrictions !== false
      )
    }
  }
  ...
  var _BAllowDisplayingItemInStore = function (FauxItemId) {
    ...
    if (
      bIsCouponCrate &&
      ItemInfo.GetLootListCount(idToCheckForRestrictions) > 0
    ) {
      idToCheckForRestrictions = InventoryAPI.GetLootListItemIdByIndex(
        idToCheckForRestrictions,
        0
      )
    }

    var sDefinitionName = InventoryAPI.GetItemDefinitionName(
      idToCheckForRestrictions
    )
    if (sDefinitionName === "crate_stattrak_swap_tool") return true

    var bIsDecodable = ItemInfo.ItemHasCapability(
      idToCheckForRestrictions,
      "decodable"
    )
    var sRestriction = bIsDecodable
      ? InventoryAPI.GetDecodeableRestriction(idToCheckForRestrictions)
      : null
  ...
  var _CouponsSearchFilterCallback = function (strAction) {
    var elTextControl = $.GetContextPanel().FindChildInLayoutFile(
      "CouponsSearchBarFilterText"
    )
    ...
    if (strAction === "clear" || strAction === "browse") strFilterString = ""
    var arrSearchTokens = strFilterString
      .toLowerCase()
      .split(/[\s\|]/)
      .filter((subtoken) => (subtoken ? true : false))
    if (!arrSearchTokens.length) strFilterString = ""
    ...
    if (
      ...
      itemsByCategory.coupons.length > 8 &&
      arrSearchTokens.length > 0
    ) {
      for (var j = 0; j < itemsByCategory.coupons.length; ++j) {
        ...
        arrSearchTokens.forEach(
          (subtoken) =>
            (hasKeySearchToken =
              hasKeySearchToken && strItemName.indexOf(subtoken) >= 0)
        )
        if (hasKeySearchToken) results.push(obj)
      }

      if (results.length <= 0) {
        itemsByCategory.coupons = itemsByCategory.coupons.slice(0, 8)
        var elTextControlErrorLabel = $.GetContextPanel().FindChildInLayoutFile(
          "CouponsSearchBarFilterError"
        )
    ...
    if (results.length > 0) {
      strPageName = "Page-2"
      itemsByCategory.coupons = itemsByCategory.coupons
        .slice(0, 8)
        .concat(results)
    }
    ...
    if (!strFilterString) {
      strPageName = "Page-0"
      if (strAction === "browse") strPageName = "Page-2"
    }
    ...
    for (var i = 0; i < 9; ++i) {
      if (
        m_pendingItemsToPopulateScheduled.hasOwnProperty(prop) &&
    ...
    if (strFilterString) {
      var elTextControlNew = $.GetContextPanel().FindChildInLayoutFile(
        "CouponsSearchBarFilterText"
      )
    ...
    if (elPageChild) elCarousel.SetSelectedChild(elPageChild)
  ...
  var _GetStoreItems = function (itemsByCategory, bOptionalFullSearchResults) {
    ...
    if (!count || count < 1) return itemsByCategory
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (
        ...
        (strBannerEntryCustomFormatString =
          StoreAPI.GetBannerEntryCustomFormatString(i)).startsWith("coupon")
      ) {
        if (!_BAllowDisplayingItemInStore(FauxItemId)) continue
        ...
        if (itemsByCategory.coupons.length == 4) {
          itemsByCategory.coupons.push({
            snippet_name: "CouponsSearchBarItemSnippet",
            load_func: function (elItem) {
              elItem.SetHasClass(
                "store-panel__carousel__coupons_searchitem",
                true
              )
            }
          })
          itemsByCategory.coupons.push({
            snippet_name: "CouponsSearchBarItemDummy",
            load_func: function (elItem) {}
          })
          itemsByCategory.coupons.push({
            snippet_name: "CouponsSearchBarItemDummy",
        ...
        if (sLinkedCoupon) {
          var LinkedItemId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
            parseInt(sLinkedCoupon),
            0
          )
          ...
          itemsByCategory.coupons.push({
            ...
            isNewRelease: strBannerEntryCustomFormatString === "coupon_new"
          })
        ...
        if (
          ...
          var nCurrentLvl = FriendsListAPI.GetFriendLevel(
  ...
  var _GetCoupons = function (itemsByCategory) {
    ...
    if (count > 0) {
      for (var i = 0; i < count; i++) {
        var CouponDefIdx = InventoryAPI.GetCacheTypeElementFieldByIndex(
          "Coupons",
          i,
          "defidx"
        )
        if (!bCheckedExpirationTimestamp) {
          ...
          var ExpirationUTC = InventoryAPI.GetCacheTypeElementFieldByIndex(
            "Coupons",
            i,
            "expiration_date"
          )
        ...
        var CouponId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
          CouponDefIdx,
          0
        )
  ...
  var _MakeIndividualCarousel = function (itemsList, prop) {
    ...
    var elCarousel = $.CreatePanel("Carousel", elParent, "Carousel-" + prop)

    elCarousel.BLoadLayoutSnippet("StoreCarousel")

    $.CreatePanel("Panel", elParent, "", {
    ...
    $.CreatePanel("CarouselNav", elParent, "CarouselNav-" + prop, {
      class: "full-width vertical-center",
      carouselid: "Carousel-" + prop,
      hittest: "false"
    })

    m_pendingItemsToPopulateScheduled[prop].m_hScheduled = $.Schedule(
      0.1,
      _ScheduledPopulateCarousel.bind(undefined, prop)
    )
  ...
  var _ScheduledPopulateCarousel = function (prop) {
    ...
    else delete m_pendingItemsToPopulateScheduled[prop]
  }

  var _PrePopulateCarousel = function (prop) {
    if (!m_pendingItemsToPopulateScheduled[prop].m_itemsList) return false

    if (
      m_pendingItemsToPopulateScheduled[prop].m_idx >=
      m_pendingItemsToPopulateScheduled[prop].m_itemsList.length
    )
      return false
    ...
    if (!elCarousel) return false

    _PopulateCarousel(
      elCarousel,
      m_pendingItemsToPopulateScheduled[prop].m_itemsList,
      m_pendingItemsToPopulateScheduled[prop].m_idx,
      prop
    )
    ++m_pendingItemsToPopulateScheduled[prop].m_idx
  ...
  var _PopulateCarousel = function (elCarousel, itemList, i, type) {
    var itemsPerPage =
      type === "tournament" || type === "operation" || type === "proteams"
        ? 1
        : 4
    var elPage = null
    ...
    if (i % itemsPerPage === 0) {
      ...
      elPage.SetHasClass(
        "store-panel__carousel-page--single",
        type === "operation" || type === "proteams" || type === "tournament"
      )
    } else {
      elPage = elCarousel.FindChildInLayoutFile(
        "Page-" + Math.floor(i / itemsPerPage)
      )
    }

    var panelName =
      typeof itemList[i] === "object" &&
      type === "coupons" &&
      itemList[i].linkedid
        ? itemList[i].linkedid
        : itemList[i]
    ...
    if (itemList[i] === "prime") {
      ...
      typeof itemList[i] == "string" &&
      InventoryAPI.IsValidItemID(itemList[i])
    ) {
      ...
      elItem.BLoadLayout(
        "file://{resources}/layout/mainmenu_store_tile_linked.xml",
        false,
        false
      )
    } else if (
      typeof itemList[i] == "object" &&
      itemList[i].snippet_name &&
    ...
    if (i % itemsPerPage === 0) {
      if (i > 0) {
        ...
        if (type === "coupons" && i == 4) {
          ...
          elPage.SetAttributeString(
            "on-carousel-select-action",
            "couponssearch"
          )
        }
      }
  ...
  var _PrimeStoreItem = function (elItem, id, type) {
    elItem.SetHasClass(
      "store-panel__carousel__item__prime__full",
      type === "prime" ? true : false
    )
  ...
  var _SetCarouselSelectedChild = function (objSelectedChild) {
    var strAction = objSelectedChild.GetAttributeString(
      "on-carousel-select-action",
      ""
    )
    if (!strAction) return
    ...
    if (strAction === "couponssearch") {
      var elTextEntry = objSelectedChild.FindChildInLayoutFile(
        "CouponsSearchBarFilterText"
      )
      if (elTextEntry) {
  ...
  var _MakeTabBtn = function (prefix, type) {
    var elBtn = $.CreatePanel(
      "RadioButton",
      m_elStore.FindChildInLayoutFile("StoreNaveBar"),
      type
    )
    elBtn.BLoadLayoutSnippet("StoreNavBtn")
    elBtn
      .FindChildInLayoutFile("StoreTabLabel")
      .SetLocalizationString("#store_tab_" + type)

    elBtn.SetPanelEvent(
      "onactivate",
      MainMenuStore.OnNavigateTab.bind(undefined, prefix + type, type)
    )
  }
  ...
  var _SortTabs = function () {
    ...
    tabList.forEach((obj) =>
      dict.hasOwnProperty(obj.id) ? (tabelements[dict[obj.id]] = obj) : null
    )

    var fnMoveToFront = (obj) =>
      obj ? elParent.MoveChildBefore(obj, elParent.Children()[0]) : null
    tabelements.reverse()
    ...
    fnMoveToFront(
      tabelements[nCategoryIdx2 + (nCategoryIdx2 >= nCategoryIdx ? 1 : 0)]
    )
  ...
  var _AccountWalletUpdated = function () {
    ...
    if (
      MyPersonaAPI.GetLauncherType() === "perfectworld" &&
      MyPersonaAPI.GetSteamType() !== "china"
    ) {
    ...
    var balance =
      MyPersonaAPI.GetLauncherType() === "perfectworld"
        ? StoreAPI.GetAccountWalletBalance()
        : ""
  ...
  var _OpenTournamentMarketLink = function () {
    ...
    SteamOverlayAPI.OpenURL(
      SteamOverlayAPI.GetSteamCommunityURL() +
        "/market/search?q=&category_" +
        appid +
        "_Tournament%5B%5D=tag_Tournament" +
        g_ActiveTournamentInfo.eventid +
        "&appid=" +
        appid
    )
  }
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    ...
    MainMenuStore.CheckLicenseScreen
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_UpdateConnectionToGC",
    MainMenuStore.CheckLicenseScreen
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Store_AccountWalletUpdated",
    MainMenuStore.AccountWalletUpdated
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Store_PriceSheetChanged",
    MainMenuStore.Init
  )
  $.RegisterForUnhandledEvent(
    "FilterStoreCouponsDisplay",
    MainMenuStore.CouponsSearchFilterCallback
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_InventoryUpdated",
    MainMenuStore.OnInventoryUpdate
  )

  $.RegisterEventHandler(
    "SetCarouselSelectedChild",
    $.GetContextPanel(),
    MainMenuStore.SetCarouselSelectedChild
  )
})()
