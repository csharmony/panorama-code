...
var SettingsMenuGameSettings = (function () {
  var _InitSteamClanTagsPanel = function () {
    ...
    for (var i = 0; i < nNumClans; i++) {
      ...
      optionLabel = $.CreatePanel("Label", clanTagDropdown, id, {
        text: "{s:clanTag}"
      })
  ...
  var _OnCrosshairStyleChange = function () {
    let nStyle = parseInt(
      GameInterfaceAPI.GetSettingString("cl_crosshairstyle")
    )
    ...
    $("#CrosshairEditorPreview").SetHasClass(
      "dynamic-crosshair",
      nStyle === 0 || nStyle === 2 || nStyle === 3
    )

    let obsCrosshairs = parseInt(
      GameInterfaceAPI.GetSettingString("cl_show_observer_crosshair")
    )
