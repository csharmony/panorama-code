...
var AdvertisingToggle = (function () {
  ...
  var _OnActivate = function () {
    ...
    gameModes.forEach((entry) => {
      if (
        !PartyListAPI.IsPlayerForHireAdvertisingEnabledForGameMode(entry.mode)
      )
        return

      var sReplaces = [
        "<img src='file://{images}/icons/ui/cooperative.svg'",
      ...
      for (var k = 0; k < sReplaces.length; ++k) {
        if (sLocalizedMode.startsWith(sReplaces[k])) {
          sLocalizedMode =
            sReplaces[0] + sLocalizedMode.substr(sReplaces[k].length)
          break
        }
      }

      var labelLoc =
        entry.mode === advertisingMode
      ...
      items.push({
        label: labelLoc,
        style: "Icon",
        jsCallback: callbackFunction.bind(undefined, entry.mode)
      })
    })
    ...
    if (strAdvertising) {
      items.push({
        label: $.Localize("#advertising_for_hire_stop_looking"),
        style: "TopSeparator",
        jsCallback: callbackFunction.bind(undefined, "")
      })
    }
  ...
  var UpdateTooltip = function (isDisabled) {
    var OnMouseOver = function () {
      ...
      UiToolkitAPI.ShowTitleTextTooltip(
        "HireAdvertisingToggleContainer",
        "#advertising_for_hire_tooltip_title",
        tooltipText
      )
    }
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_PartyBrowser_LocalPlayerForHireAdvertisingChanged",
    AdvertisingToggle.UpdateToggle
  )
  $.RegisterForUnhandledEvent(
