...
var PlayMenu = (function () {
  ...
  var m_serverPrimeSetting =
    GameInterfaceAPI.GetSettingString("ui_playsettings_prime") === "1" ? 1 : 0
  ...
  function StartSearch() {
    ...
    if (m_isWorkshop) {
      ...
      if (
        !_CheckContainerHasAnyChildChecked(
          _GetMapListForServerTypeAndGameMode(m_activeMapGroupSelectionPanelID)
        )
      ) {
        _NoMapSelectedPopup()
      ...
      if (
        GameModeFlags.DoesModeUseFlags(m_gameModeSetting) &&
        !m_gameModeFlags[m_serverSetting + m_gameModeSetting]
      ) {
        btnStartSearch.RemoveClass("pressed")
      ...
      let settings =
        LobbyAPI.IsSessionActive() && !_GetTournamentOpponent()
  ...
  function _Init() {
    ...
    m_arrGameModeRadios.forEach(function (entry) {
      entry.SetPanelEvent("onactivate", function () {
        ...
        if (_IsSingleSkirmishString(entry.id)) {
          ...
          m_singleSkirmishMapGroup =
            _GetSingleSkirmishMapGroupFromSingleSkirmishString(entry.id)
        } else {
        ...
        if (
          (entry.id === "competitive" || entry.id === "scrimcomp2v2") &&
          !entry.FindChild("GameModeAlert").BHasClass("hidden")
        ) {
          if (
    ...
    m_arrGameModeRadios.forEach(function (entry) {
      if (_IsSingleSkirmishString(entry.id)) {
        m_arrSingleSkirmishMapGroups.push(
          _GetSingleSkirmishMapGroupFromSingleSkirmishString(entry.id)
        )
      }
    })
    ...
    elPermissionsButton.SetPanelEvent("onactivate", function () {
      var bCurrentlyPrivate =
        LobbyAPI.GetSessionSettings().system.access === "private"
      ...
      GameInterfaceAPI.SetSettingString(
        "lobby_default_privacy_bits",
        sNewAccessSetting === "public" ? "1" : "0"
      )
    ...
    $("#WorkshopSearchTextEntry").SetPanelEvent(
      "ontextentrychange",
    ...
    var strFavoriteMaps = GameInterfaceAPI.GetSettingString(
  ...
  function _OnDirectChallengeBtn() {
    if (inDirectChallenge()) {
      ...
      var savedKey = GameInterfaceAPI.GetSettingString(
      ...
      else _SetDirectChallengeKey(savedKey)
  ...
  function _SetDirectChallengeKey(key) {
    ...
    if (key && m_challengeKey != key) {
      $.Schedule(0.01, function () {
        var elHeader = $.GetContextPanel().FindChildTraverse(
          "JsDirectChallengeKey"
        )
    ...
    var DirectChallengeCheckBox = $.GetContextPanel().FindChildTraverse(
      "JsDirectChallengeBtn"
    )
  ...
  var _ClansInfoUpdated = function () {
    if (
      m_challengeKey &&
      $.GetContextPanel().GetAttributeString("code-type", "") === "g"
    ) {
  ...
  function _SetDirectChallengeIcons(type, id) {
    var elAvatar = $.GetContextPanel().FindChildInLayoutFile(
      "JsDirectChallengeAvatar"
    )
  ...
  function _AddGoToClanPageAction(elAvatar, id) {
    elAvatar.SetPanelEvent("onactivate", function () {
      SteamOverlayAPI.OpenUrlInOverlayOrExternalBrowser(
        "https://" + SteamOverlayAPI.GetSteamCommunityURL() + "/gid/" + id
      )
    })
  }
  ...
  function _OnDirectChallengeEdit() {
    ...
    m_popupChallengeKeyEntryValidate =
  ...
  function _DirectChallengeStartSearch() {
    ...
    var bValid = _IsChallengeKeyValid(
      m_challengeKey.toUpperCase(),
      oReturn,
      "set"
    )
    if (!bValid) {
  ...
  function _ShowNewMatchmakingModePopup() {
    return
    var setVersionTo = "3"
    var currentVersion = GameInterfaceAPI.GetSettingString(
  ...
  function _SetGameModeRadioButtonAvailableTooltip(
    gameMode,
    isAvailable,
    txtTooltip
  ) {
    var elGameModeSelectionRadios = $("#GameModeSelectionRadios")
    var elTab = elGameModeSelectionRadios
      ? elGameModeSelectionRadios.FindChildInLayoutFile(gameMode)
      : null
    if (elTab) {
      if (!isAvailable) {
  ...
  function _IsGameModeAvailable(serverType, gameMode) {
    ...
    if (gameMode === "cooperative" || gameMode === "coopmission") {
      ...
      var bGameModeMatchesLobby =
        questID && LobbyAPI.GetSessionSettings().game.mode === gameMode
      var bAvailable =
        bGameModeMatchesLobby &&
        MissionsAPI.GetQuestDefinitionField(questID, "gamemode") === gameMode
      _SetGameModeRadioButtonVisible(gameMode, bAvailable)
      return bAvailable
    }

    if (
      _IsValveOfficialServer(serverType) &&
      LobbyAPI.BIsHost() &&
  ...
  function _GetTournamentOpponent() {
    var elTeamDropdown = $.GetContextPanel().FindChildInLayoutFile(
      "TournamentTeamDropdown"
    )
    if (elTeamDropdown.GetSelected() === null) return ""
    return elTeamDropdown.GetSelected().GetAttributeString("data", "")
  }

  function _GetTournamentStage() {
    var elStageDropdown = $.GetContextPanel().FindChildInLayoutFile(
      "TournamentStageDropdown"
    )
    if (elStageDropdown.GetSelected() === null) return ""
    return elStageDropdown.GetSelected().GetAttributeString("data", "")
  }

  function _UpdateStartSearchBtn(isSearchingForTournament) {
    var btnStartSearch =
      $.GetContextPanel().FindChildInLayoutFile("StartMatchBtn")
    btnStartSearch.enabled = isSearchingForTournament
      ? _GetTournamentOpponent() != "" && _GetTournamentStage() != ""
      : true
  }

  function _UpdateTournamentButton(
    isHost,
    isSearching,
    settingsgamemapgroupname
  ) {
    var bIsOfficialCompetitive =
      m_gameModeSetting === "competitive" && _IsPlayingOnValveOfficial()
    ...
    if (isInTournament) {
      function AddDropdownOption(
        ...
        var newEntry = $.CreatePanel("Label", elDropdown, entryID, {
          data: strData
        })
      ...
      AddDropdownOption(
        elTeamDropdown,
        "PickOpponent",
        $.Localize("#SFUI_Tournament_Pick_Opponent"),
        "",
        strCurrentOpponent
      )
      var teamCount = CompetitiveMatchAPI.GetTournamentTeamCount(strTournament)
      for (var i = 0; i < teamCount; i++) {
        var strTeam = CompetitiveMatchAPI.GetTournamentTeamNameByIndex(
          strTournament,
          i
        )

        if (strTeamName === strTeam) continue
      ...
      elTeamDropdown.SetPanelEvent(
        "oninputsubmit",
        _UpdateStartSearchBtn.bind(undefined, isSearchingForTournament)
      )
      ...
      AddDropdownOption(
        elStageDropdown,
        "PickStage",
        $.Localize("#SFUI_Tournament_Stage"),
        "",
        strCurrentStage
      )
      var stageCount =
        CompetitiveMatchAPI.GetTournamentStageCount(strTournament)
      for (var i = 0; i < stageCount; i++) {
        var strStage = CompetitiveMatchAPI.GetTournamentStageNameByIndex(
          strTournament,
          i
        )
        AddDropdownOption(
  ...
  function _SyncDialogsFromSessionSettings(settings) {
    ...
    _SetDirectChallengeKey(
      settings.options.hasOwnProperty("challengekey")
        ? settings.options.challengekey
        : ""
    )
    if (!m_challengeKey) {
    ...
    if (
      m_gameModeSetting === "skirmish" &&
      settings.game.mapgroupname &&
      m_arrSingleSkirmishMapGroups.includes(settings.game.mapgroupname)
    ) {
      ...
      for (var i = 0; i < m_arrGameModeRadios.length; ++i) {
        ...
        if (m_singleSkirmishMapGroup) {
          if (_IsSingleSkirmishString(strGameModeForButton)) {
            if (
              m_singleSkirmishMapGroup ===
              _GetSingleSkirmishMapGroupFromSingleSkirmishString(
                strGameModeForButton
              )
            ) {
        ...
        var isAvailable = _IsGameModeAvailable(
          m_serverSetting,
          strGameModeForButton
        )
        m_arrGameModeRadios[i].enabled = isAvailable && isEnabled
        ;(strGameModeForButton === "competitive" ||
          strGameModeForButton === "scrimcomp2v2") &&
        ...
        if (
          ...
          if (m_arrGameModeRadios[i].FindChildInLayoutFile("GameModeAlert")) {
            m_arrGameModeRadios[i]
              .FindChildInLayoutFile("GameModeAlert")
              .SetHasClass("hidden", bHide)
          }
        }
      }
  ...
  function _UpdateDirectChallengePage(isSearching = false, isHost = true) {
    ...
    if ($("#RandomChallengeKey")) $("#RandomChallengeKey").enabled = bEnable

    if ($("#EditChallengeKey")) $("#EditChallengeKey").enabled = bEnable

    if ($("#ClanChallengeKey")) $("#ClanChallengeKey").enabled = bEnable

    if ($("#JsDirectChallengeBtn"))
      $("#JsDirectChallengeBtn").enabled =
        bEnable &&
        (MyPersonaAPI.HasPrestige() || MyPersonaAPI.GetCurrentLevel() >= 2)
  }
  ...
  function _OnChooseClanKeyBtn() {
    ...
    var clanKey =
      _GetChallengeKeyType(m_challengeKey) == "g" ? m_challengeKey : ""
  ...
  function _CreatePlayerTile(elTile, xuid, delay = 0) {
    elTile.BLoadLayout(
      "file://{resources}/layout/simple_player_tile.xml",
      false,
      false
    )
    ...
    $.Schedule(
      0.1,
      function (elTile, xuid) {
        if (!elTile || !elTile.IsValid()) return
        ...
        Scheduler.Schedule(
          delay,
          function () {
            if (elTile) elTile.RemoveClass("hidden")
          },
          "directchallenge"
        )
  ...
  function _OnPrivateQueuesUpdate() {
    var elMembersContainer = $.GetContextPanel().FindChildTraverse(
      "DirectChallengeQueueMembers"
    )
    if (!elMembersContainer) return
    ...
    if ($("#id-directchallenge-status__queue-members"))
      $("#id-directchallenge-status__queue-members").SetHasClass(
        "hidden",
        !_IsSearching()
      )
    ...
    if (!_IsSearching()) {
      ...
      if (elMembersContainer) elMembersContainer.RemoveAndDeleteChildren()
    ...
    var NumberOfMorePartiesNotShown =
      PartyBrowserAPI.GetPrivateQueuesMoreParties()

    if ($("#id-directchallenge-status")) {
      $.GetContextPanel().SetDialogVariableInt(
        "directchallenge_players",
        NumberOfPlayers
      )
      $.GetContextPanel().SetDialogVariableInt(
        "directchallenge_moreparties",
        NumberOfMorePartiesNotShown
      )
    ...
    for (var i = NumberOfParties; i-- > 0; ) {
      ...
      if (!elParty) {
        elParty = $.CreatePanel("Panel", elMembersContainer, partyId, {
          class: "directchallenge__party hidden"
        })
        elParty.SetHasClass("multi", arrMembers.length > 1)

        elMembersContainer.MoveChildBefore(
          elParty,
          elMembersContainer.Children()[0]
        )
        ...
        Scheduler.Schedule(
          delay,
          function (elParty) {
            if (elParty && elParty.IsValid()) elParty.RemoveClass("hidden")
          }.bind(this, elParty),
          "directchallenge"
        )

        arrMembers.forEach(function (xuid) {
          var elTile = $.CreatePanel("Panel", elParty, xuid, {
            class: "directchallenge__party__member"
          })
  ...
  function _GetAvailableMapGroups(gameMode, isPlayingOnValveOfficial) {
    ...
    if (gameModeCfg === undefined) return []

    var mapgroup = isPlayingOnValveOfficial
      ? gameModeCfg.mapgroupsMP
      : gameModeCfg.mapgroupsSP
    if (mapgroup !== undefined && mapgroup !== null) {
      return Object.keys(mapgroup)
    }

    if (
      (gameMode === "cooperative" || gameMode === "coopmission") &&
  ...
  function _GetMapGroupPanelID() {
    var gameModeId =
      m_gameModeSetting +
      (m_singleSkirmishMapGroup ? "@" + m_singleSkirmishMapGroup : "")
    var panelID =
      "gameModeButtonContainer_" +
      gameModeId +
      "_" +
      m_serverSetting +
      (inDirectChallenge() ? "_directchallenge" : "")
    return panelID
  }
  ...
  function _OnActivateMapOrMapGroupButton(mapgroupButton) {
    ...
    if (
      $.GetContextPanel().BHasClass("play-menu__lobbymapveto_activated") &&
      mapGroupNameClicked !== "mg_lobby_mapveto"
    ) {
      return
    }
    ...
    if (mapGroupName) {
      ...
      if (mapGroupName.toLowerCase().endsWith(siblingSuffix))
        mapGroupName = mapGroupName.substring(
          0,
          mapGroupName.length - siblingSuffix.length
        )
      else mapGroupName = mapGroupName + siblingSuffix

      var elParent = mapgroupButton.GetParent()
      if (elParent) elParent = elParent.GetParent()
      if (elParent && elParent.GetAttributeString("hassections", "")) {
        elParent.Children().forEach(function (section) {
          section.Children().forEach(function (tile) {
            ...
            if (
              mapGroupNameSibling.toLowerCase() === mapGroupName.toLowerCase()
            ) {
              tile.checked = false
            }
          })
  ...
  function _OnMapQuickSelect(mgName) {
    ...
    var prevSelection = _GetSelectedMapsForServerTypeAndGameMode(
      m_serverSetting,
      m_gameModeSetting,
      true
    )
    ...
    var newSelection = _GetSelectedMapsForServerTypeAndGameMode(
      m_serverSetting,
      m_gameModeSetting,
      true
    )
  ...
  function _ValidateMaps(arrMapList) {
    ...
    var arrMapButtons = _GetMapListForServerTypeAndGameMode(
      m_activeMapGroupSelectionPanelID
    )
    arrMapButtons.forEach((elMapTile) =>
      arrMapTileNames.push(elMapTile.GetAttributeString("mapname", ""))
    )

    var filteredMapList = arrMapList.filter((strMap) =>
      arrMapTileNames.includes(strMap)
    )
  ...
  function _GetMapGroupsWithAttribute(strAttribute, strValue) {
    ...
    elMapGroupContainer.Children().forEach(function (elMapBtn) {
      ...
      if (
        GameTypesAPI.GetMapGroupAttribute(mgName, strAttribute) === strValue
      ) {
  ...
  function _GetMapsFromQuickSelectMapGroup(mgName) {
    if (mgName === "favorites") {
      var mapsAsString = GameInterfaceAPI.GetSettingString(
        "ui_playsettings_custom_preset"
      )
      if (mapsAsString === "") return []
      else {
        ...
        if (arrMapList.length != filteredMapList.length)
          GameInterfaceAPI.SetSettingString(
            "ui_playsettings_custom_preset",
            filteredMapList.length > 0 ? filteredMapList.join(",") : ""
          )
      ...
      return _GetMapGroupsWithAttribute("grouptype", "active").filter(
        (x) => x !== "mg_lobby_mapveto"
      )
  ...
  function _MatchMapSelectionWithQuickSelect() {
    var elQuickSelectContainer = $.GetContextPanel().FindChildInLayoutFile(
      "JsQuickSelectParent"
    )
    if (!elQuickSelectContainer || m_isWorkshop) return

    elQuickSelectContainer
      .FindChildrenWithClassTraverse("preset-button")
      .forEach(function (elQuickBtn, index, aMapGroups) {
  ...
  function _LazyCreateMapListPanel() {
    ...
    if (panelID in m_mapSelectionButtonContainers) {
      ...
      if (elExistingContainer && strRequireTagNameToReuse) {
        var strExistingTagValue = elExistingContainer.GetAttributeString(
          strRequireTagNameToReuse,
          ""
        )
        bAllowReuseExistingContainer =
          strExistingTagValue === strRequireTagValueToReuse
      }

      var elFriendLeaderboards = elExistingContainer
        ? elExistingContainer.FindChildTraverse("FriendLeaderboards")
        : null
      if (elFriendLeaderboards) {
        var strEmbeddedLeaderboardName =
          elFriendLeaderboards.GetAttributeString("type", null)
        if (strEmbeddedLeaderboardName) {
          LeaderboardsAPI.Refresh(strEmbeddedLeaderboardName)
        }
      }

      if (bAllowReuseExistingContainer) return panelID
      else elExistingContainer.DeleteAsync(0.0)
    }
    ...
    if (inDirectChallenge()) {
      ...
      strSnippetNameOverride =
        "MapSelectionContainer_" + serverType + "_" + gameMode
    }
    ...
    if (container.BHasLayoutSnippet(strSnippetNameOverride)) {
      ...
      if (elMapTile) elMapTile.BLoadLayoutSnippet("MapGroupSelection")
    ...
    if (strRequireTagNameToReuse) {
      container.SetAttributeString(
        strRequireTagNameToReuse,
        strRequireTagValueToReuse
      )
    }

    var isPlayingOnValveOfficial = _IsValveOfficialServer(serverType)
    var arrMapGroups = _GetAvailableMapGroups(
      gameMode,
      isPlayingOnValveOfficial
    )

    if (gameMode === "skirmish" && m_singleSkirmishMapGroup) {
      _UpdateOrCreateMapGroupTile(
        m_singleSkirmishMapGroup,
        container,
        null,
        panelID + m_singleSkirmishMapGroup
      )
    } else {
      arrMapGroups.forEach(function (item, index, aMapGroups) {
        if (
          gameMode === "skirmish" &&
          m_arrSingleSkirmishMapGroups.includes(aMapGroups[index])
        ) {
          return
        }
        ...
        if (elSectionContainer)
          _UpdateOrCreateMapGroupTile(
            ...
            elSectionContainer,
            null,
            panelID + aMapGroups[index]
          )
      })
    }

    container.OnPropertyTransitionEndEvent = function (
      panelName,
      propertyName
    ) {
      if (
    ...
    $.RegisterEventHandler(
      ...
      container,
      container.OnPropertyTransitionEndEvent
    )
  ...
  function _PopulateQuickSelectBar(isSearching, isHost) {
    var elQuickSelectContainer = $.GetContextPanel().FindChildInLayoutFile(
      "jsQuickSelectionSetsContainer"
    )
    if (!elQuickSelectContainer) return

    if (m_isWorkshop) return
  ...
  function _EnableDisableQuickSelectBtns(isSearching, isHost) {
    ...
    elQuickSelectContainer
      .FindChildrenWithClassTraverse("preset-button")
      .forEach((element) => (element.enabled = bEnable))
  }

  function _SaveMapSelectionToCustomPreset(bSilent = false) {
    if (inDirectChallenge()) return

    var selectedMaps = _GetSelectedMapsForServerTypeAndGameMode(
      m_serverSetting,
      m_gameModeSetting,
      true
    )
    ...
    if (!bSilent) {
      ...
      $.GetContextPanel()
        .FindChildInLayoutFile("jsQuickSelectionSave")
        .TriggerClass("save")
    }
  ...
  function _GetPanelTypeForMapGroupTile(gameMode, singleSkirmishMapGroup) {
    ...
    return (bIsCompetitive || bIsSkirmish || bIsWingman) &&
      _IsValveOfficialServer(m_serverSetting)
      ? "ToggleButton"
      : "RadioButton"
  }

  function _UpdateOrCreateMapGroupTile(
    mapGroupName,
    container,
    elTilePanel,
    newTileID
    ...
    if (!p) {
      var panelType = _GetPanelTypeForMapGroupTile(
        m_gameModeSetting,
        m_singleSkirmishMapGroup
      )
      ...
      if (panelType === "RadioButton") {
        ...
        if (panelID.endsWith(mapGroupName))
          radioGroupID = panelID.substring(
            0,
            panelID.length - mapGroupName.length
          )
        else radioGroupID = container.id
    ...
    p.FindChildInLayoutFile("ActiveGroupIcon").visible =
      mg.grouptype === "active"
    ...
    if (keysList.length > 1) {
      ...
      p.FindChildInLayoutFile("MapSelectionButton").AddClass(
        "map-selection-btn-container--large"
      )
      p.FindChildInLayoutFile("MapGroupName").AddClass("fontSize-m")
    } else {
      if (
        m_gameModeSetting === "survival" &&
        _IsValveOfficialServer(m_serverSetting)
      ) {
      ...
      var iconPath =
        mapGroupName === "random"
          ? "file://{images}/icons/ui/random.svg"
          : "file://{images}/" + mg.icon_image_path + ".svg"
      var iconSize = GetIconSize()
      var mapGroupIcon = p
        .FindChildInLayoutFile("MapSelectionButton")
        .FindChildInLayoutFile("MapGroupCollectionIcon")

      if (mapGroupIcon) {
        mapGroupIcon.SetImage(iconPath)
      } else {
        mapGroupIcon = $.CreatePanel(
          "Image",
          p.FindChildInLayoutFile("MapSelectionButton"),
          "MapGroupCollectionIcon",
          {
        ...
        p.FindChildInLayoutFile("MapSelectionButton").MoveChildBefore(
          mapGroupIcon,
          p.FindChildInLayoutFile("MapGroupCollectionMultiIcons")
        )
      }

      function GetIconSize() {
        if (m_gameModeSetting === "survival") return "256"
        else if (m_gameModeSetting === "cooperative") return "256"
        else if (m_gameModeSetting === "coopmission") return "256"
    ...
    if (mapGroupName === "random") {
      mapImage = p
        .FindChildInLayoutFile("MapGroupImagesCarousel")
        .FindChildInLayoutFile("MapSelectionScreenshot")

      if (!mapImage) {
        mapImage = $.CreatePanel(
          "Panel",
          p.FindChildInLayoutFile("MapGroupImagesCarousel"),
          "MapSelectionScreenshot"
        )
        mapImage.AddClass("map-selection-btn__screenshot")
      }

      mapImage.style.backgroundImage =
        'url("file://{images}/map_icons/screenshots/360p/random.png")'
    ...
    for (var i = 0; i < keysList.length; i++) {
      mapImage = p
        .FindChildInLayoutFile("MapGroupImagesCarousel")
        .FindChildInLayoutFile("MapSelectionScreenshot" + i)
      if (!mapImage) {
        mapImage = $.CreatePanel(
          "Panel",
          p.FindChildInLayoutFile("MapGroupImagesCarousel"),
          "MapSelectionScreenshot" + i
        )
        mapImage.AddClass("map-selection-btn__screenshot")
      }
      ...
      if (m_gameModeSetting === "survival") {
        mapImage.style.backgroundImage =
          'url("file://{resources}/videos/' + keysList[i] + '_preview.webm")'
      } else {
        mapImage.style.backgroundImage =
          'url("file://{images}/map_icons/screenshots/360p/' +
          keysList[i] +
          '.png")'
      }
      mapImage.style.backgroundPosition = "50% 0%"
      ...
      if (keysList.length > 1) {
        var mapIconsContainer = p.FindChildInLayoutFile(
          "MapGroupCollectionMultiIcons"
        )
        if (numImagesPerMultiIconRow > 1) {
          var subContainerID =
            "MultiIconsSubPanel" + Math.floor(i / numImagesPerMultiIconRow)
        ...
        mapIcon = mapIconsContainer.FindChildInLayoutFile(
          subMapIconImagePanelID
        )
        if (!mapIcon) {
          mapIcon = $.CreatePanel(
            "Image",
        ...
        IconUtil.SetupFallbackMapIcon(
          mapIcon,
          "file://{images}/map_icons/map_icon_" + keysList[i]
        )
      }
    }

    if (numImagesPerMultiIconRow > 1) {
      p.FindChildInLayoutFile("MapGroupCollectionMultiIcons").AddClass(
        "map-selection-btn__groupmap-icons-2x"
      )
    }
    ...
    if (mg.tooltipID) {
      p.SetPanelEvent(
        "onmouseover",
        OnMouseOverMapTile.bind(undefined, p.id, mg.tooltipID, keysList)
      )
  ...
  function _GetRotatingMapGroupStatus(
    gameMode,
    singleSkirmishMapGroup,
    mapgroupname
  ) {
    m_timerMapGroupHandler = null
    var strSchedule =
      CompetitiveMatchAPI.GetRotatingOfficialMapGroupCurrentState(
        gameMode,
        mapgroupname
      )
    var elTimer = m_mapSelectionButtonContainers[
      m_activeMapGroupSelectionPanelID
    ].FindChildInLayoutFile("PlayMenuMapRotationTimer")
    ...
    if (elTimer) {
      if (strSchedule) {
        ...
        var numWait =
          FormatText.SecondsToDDHHMMSSWithSymbolSeperator(numSecondsRemaining)
        ...
        var mapGroupContainer =
          m_mapSelectionButtonContainers[
            m_activeMapGroupSelectionPanelID
          ].FindChildTraverse("MapTile")

        var mapGroupPanel =
          mapGroupContainer.FindChildInLayoutFile(mapGroupPanelID)
        if (!mapGroupPanel) {
          mapGroupContainer.RemoveAndDeleteChildren()
          var btnMapGroup = _UpdateOrCreateMapGroupTile(
            strCurrentMapGroup,
            mapGroupContainer,
            null,
            mapGroupPanelID
        ...
        m_timerMapGroupHandler = $.Schedule(
          1,
          _GetRotatingMapGroupStatus.bind(
            undefined,
            gameMode,
            singleSkirmishMapGroup,
            mapgroupname
  ...
  function _StartRotatingMapGroupTimer() {
    ...
    if (
      ...
      m_gameModeSetting === "survival" &&
      m_mapSelectionButtonContainers &&
      m_mapSelectionButtonContainers[m_activeMapGroupSelectionPanelID] &&
      m_mapSelectionButtonContainers[
        m_activeMapGroupSelectionPanelID
      ].Children()
    ) {
      var btnSelectedMapGroup = m_mapSelectionButtonContainers[
        m_activeMapGroupSelectionPanelID
      ]
        .Children()
        .filter((entry) => entry.GetAttributeString("mapname", "") !== "")

      if (btnSelectedMapGroup[0]) {
        var mapSelectedGroupName = btnSelectedMapGroup[0].GetAttributeString(
          "mapname",
          ""
        )
  ...
  function _SetMapGroupModifierLabelElements(mapName, elMapPanel) {
    var isUnrankedCompetitive =
      m_gameModeSetting === "competitive" &&
      _IsValveOfficialServer(m_serverSetting) &&
      GameTypesAPI.GetMapGroupAttribute(mapName, "competitivemod") ===
        "unranked"
    var isNew =
      !isUnrankedCompetitive &&
      GameTypesAPI.GetMapGroupAttribute(mapName, "showtagui") === "new"

    elMapPanel
      .FindChildInLayoutFile("MapGroupNewTag")
      .SetHasClass("hidden", !isNew || mapName === "mg_lobby_mapveto")

    elMapPanel
      .FindChildInLayoutFile("MapGroupNewTagYellowLarge")
      .SetHasClass("hidden", true)
    elMapPanel
      .FindChildInLayoutFile("MapSelectionTopRowIcons")
      .SetHasClass("tall", mapName === "mg_lobby_mapveto")

    elMapPanel
      .FindChildInLayoutFile("MapGroupUnrankedTag")
      .SetHasClass("hidden", !isUnrankedCompetitive)
  }

  function _ReloadLeaderboardLayoutGivenSettings(
    container,
    lbName,
    strTitleOverride,
    strPointsTitle
    ...
    elFriendLeaderboards.BLoadLayout(
  ...
  function _LoadLeaderboardsLayoutForContainer(container) {
    if (
      ...
      if (questID > 0) {
        ...
        var elFriendLeaderboards =
          container.FindChildTraverse("FriendLeaderboards")
        ...
        elDescriptionLabel.text = MissionsAPI.GetQuestDefinitionField(
          questID,
          "loc_description"
        )
  ...
  function _UpdateMapGroupButtons(isEnabled, isSearching, isHost) {
    ...
    if (
      (m_gameModeSetting === "competitive" ||
        m_gameModeSetting === "scrimcomp2v2") &&
      _IsPlayingOnValveOfficial()
    ) {
    ...
    if (!inDirectChallenge())
      _SetEnabledStateForMapBtns(
        m_mapSelectionButtonContainers[panelID],
        isSearching,
        isHost
      )
  ...
  function _SelectMapButtonsFromSettings(settings) {
    ...
    var aListMaps = _GetMapListForServerTypeAndGameMode(
      m_activeMapGroupSelectionPanelID
    )
  ...
  function _ShowHideStartSearchBtn(isSearching, isHost) {
    ...
    if (
      ...
      m_gameModeSetting === "competitive" &&
      _IsPlayingOnValveOfficial() &&
      PartyListAPI.GetCount() >= PartyListAPI.GetPartySessionUiThreshold()
    ) {
      numStyleToShow = PartyListAPI.GetCount()
      if (
        numStyleToShow > 5 ||
        0 == PartyListAPI.GetPartySessionUiThreshold()
      ) {
        numStyleToShow = 5
      }
    }
  ...
  function _UpdatePrimeBtn(isSearching, isHost) {
    ...
    var btnText = ""
    var tooltipText = ""
    var LocalPlayerHasPrime = PartyListAPI.GetFriendPrimeEligible(
    ...
    if (!LocalPlayerHasPrime) {
      ...
      var sPrice = StoreAPI.GetStoreItemSalePrice(
        InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(1353, 0),
        1,
        ""
      )
      ...
      elGetPrimeBtn.SetPanelEvent("onactivate", function () {
        ...
        UiToolkitAPI.ShowCustomLayoutPopup(
          "prime_status",
          "file://{resources}/layout/popups/popup_prime_status.xml"
        )
      })
    } else {
      if (!SessionUtil.DoesGameModeHavePrimeQueue(m_gameModeSetting)) {
        ...
        elPrimeText.SetPanelEvent("onmouseover", function () {
          UiToolkitAPI.ShowTextTooltip(elTextNA.id, tooltipText)
        })
        elPrimeText.SetPanelEvent("onmouseout", function () {
          UiToolkitAPI.HideTextTooltip()
        })
        ...
        if (SessionUtil.AreLobbyPlayersPrime()) {
          ...
          elTooglePrimeBtn.enabled =
            isHost && !isSearching && !inDirectChallenge()

          btnText = "#elevated_status_toggle_prime_only"
        } else {
          var oPrimeMembers = _GetPrimePartyMembers()
          elTooglePrimeBtn.SetDialogVariableInt(
            "prime_members",
            oPrimeMembers.prime
          )
        ...
        elTooglePrimeBtn.SetPanelEvent("onmouseover", function () {
          UiToolkitAPI.ShowTextTooltip(elTooglePrimeBtn.id, tooltipText)
        })
        elTooglePrimeBtn.SetPanelEvent("onmouseout", function () {
          UiToolkitAPI.HideTextTooltip()
        })
  ...
  function _UpdateSurvivalAutoFillSquadBtn(gameMode) {
    ...
    if (
      gameMode === "survival" &&
      _IsPlayingOnValveOfficial() &&
      PartyListAPI.GetCount() <= 1
    ) {
      elBtn.visible = true
      var bAutoFill = !(
        GameInterfaceAPI.GetSettingString("ui_playsettings_survival_solo") ===
        "1"
      )
    ...
    if (gameMode === "survival") {
      ...
      if (sPreviousType !== lbName) {
        _ReloadLeaderboardLayoutGivenSettings(
          container,
          lbName,
          "#CSGO_official_leaderboard_survival_" + lbType,
  ...
  function _UpdateWaitTime(elMapList) {
    ...
    for (var i = 0; i < childrenList.length; i++) {
      ...
      if (numWait) {
        ...
        elWaitTime.FindChild("MapGroupWaitTimeLabel").text = $.Localize(
          "#matchmaking_expected_wait_time",
          elWaitTime
        )
  ...
  function _GetSelectedMapsForServerTypeAndGameMode(
    serverType,
    gameMode,
    bDontToggleMaps = false
  ) {
    var isPlayingOnValveOfficial = _IsValveOfficialServer(serverType)
    ...
    if (!_CheckContainerHasAnyChildChecked(aListMapPanels)) {
      var preferencesMapsForThisMode = GameInterfaceAPI.GetSettingString(
        "ui_playsettings_maps_" + serverType + "_" + gameMode
      )

      if (!preferencesMapsForThisMode) preferencesMapsForThisMode = ""
      ...
      savedMapIds.forEach(function (strMapNameIndividual) {
        ...
        if (mapsWithThisName.length > 0) {
          if (!bDontToggleMaps) mapsWithThisName[0].checked = true
        }
      })

      if (
        aListMapPanels.length > 0 &&
        !_CheckContainerHasAnyChildChecked(aListMapPanels)
      ) {
        if (!bDontToggleMaps) aListMapPanels[0].checked = true
      }
    }

    if (serverType === "official" && gameMode === "survival") {
      return GameInterfaceAPI.GetSettingString(
        "ui_playsettings_maps_" + serverType + "_" + gameMode
      )
    }

    var selectedMaps = aListMapPanels
      .filter(function (e) {
        return e.checked
      })
  ...
  function _GetMapListForServerTypeAndGameMode(mapGroupOverride) {
    var mapGroupPanelID = !mapGroupOverride
      ? _LazyCreateMapListPanel()
      : mapGroupOverride
    var elParent = m_mapSelectionButtonContainers[mapGroupPanelID]

    if (
      m_gameModeSetting === "competitive" &&
      elParent.GetAttributeString("hassections", "")
    ) {
      var aListMapPanels = []
      elParent.Children().forEach(function (section) {
        section.Children().forEach(function (tile) {
      ...
      if (elMapTile) return elMapTile.Children()
      else return elParent.Children()
    } else {
  ...
  function _GetSingleSkirmishMapGroupFromSingleSkirmishString(entry) {
    return _GetSingleSkirmishMapGroupFromId(
      _GetSingleSkirmishIdFromSingleSkirmishString(entry)
    )
  }
  ...
  function _CheckContainerHasAnyChildChecked(aMapList) {
    if (aMapList.length < 1) return false

    return (
      aMapList.filter(function (map) {
        return map.checked
      }).length > 0
    )
  }
  ...
  function _ValidateSessionSettings() {
    ...
    if (!_IsGameModeAvailable(m_serverSetting, m_gameModeSetting)) {
      m_gameModeSetting = GameInterfaceAPI.GetSettingString(
        "ui_playsettings_mode_" + m_serverSetting
      )
      m_singleSkirmishMapGroup = null

      if (_IsSingleSkirmishString(m_gameModeSetting)) {
        m_singleSkirmishMapGroup =
          _GetSingleSkirmishMapGroupFromSingleSkirmishString(m_gameModeSetting)
        m_gameModeSetting = "skirmish"
      }
  ...
  function _LoadGameModeFlagsFromSettings() {
    m_gameModeFlags[m_serverSetting + m_gameModeSetting] = parseInt(
      GameInterfaceAPI.GetSettingString(
        "ui_playsettings_flags_" + m_serverSetting + "_" + m_gameModeSetting
      )
    )
  }
  ...
  function _ApplySessionSettings() {
    ...
    var gameModeFlags = m_gameModeFlags[m_serverSetting + m_gameModeSetting]
      ? m_gameModeFlags[m_serverSetting + m_gameModeSetting]
      : 0
    ...
    if (m_isWorkshop) selectedMaps = _GetSelectedWorkshopMap()
      ...
      selectedMaps = _GetSelectedMapsForServerTypeAndGameMode(
        serverType,
        gameMode
      )
    }
    ...
    if (m_isWorkshop) {
      ...
      if (m_singleSkirmishMapGroup) {
        singleSkirmishSuffix =
          "_" + _GetSingleSkirmishIdFromMapGroup(m_singleSkirmishMapGroup)
      }
      GameInterfaceAPI.SetSettingString(
        "ui_playsettings_mode_" + serverType,
        m_gameModeSetting + singleSkirmishSuffix
      )

      if (!inDirectChallenge()) {
        GameInterfaceAPI.SetSettingString(
          "ui_playsettings_maps_" +
            serverType +
            "_" +
            m_gameModeSetting +
            singleSkirmishSuffix,
  ...
  function _OnHideMainMenu() {
    $("#MapSelectionList")
      .FindChildrenWithClassTraverse("map-selection-btn__carousel")
  ...
  function _GetPlayType() {
    ...
    var playType = elDropDownEntry.GetAttributeString(
      "data-type",
      "(not_found)"
    )
    return playType
  ...
  function _ShowWorkshopMapInfoTooltip(panel) {
    ...
    if (text) UiToolkitAPI.ShowTextTooltip(panel.id, text)
  }
  ...
  function _LazyCreateWorkshopTab() {
    ...
    if (panelId in m_mapSelectionButtonContainers) return panelId
    ...
    for (var idxMap = 0; idxMap < numMaps; ++idxMap) {
      ...
      if (!mapInfo || !mapInfo.mapgroup) continue
  ...
  function _UpdateGameModeFlagsBtn() {
    ...
    $.GetContextPanel().SetDialogVariable(
      "gamemodeflag_setting",
      $.Localize(
        "#play_setting_gamemodeflags_" +
          m_gameModeSetting +
          "_" +
          m_gameModeFlags[m_serverSetting + m_gameModeSetting]
      )
  ...
  function _setAndSaveGameModeFlags(value) {
    ...
    if (!inDirectChallenge())
      GameInterfaceAPI.SetSettingString(
        "ui_playsettings_flags_" + m_serverSetting + "_" + m_gameModeSetting,
        m_gameModeFlags[m_serverSetting + m_gameModeSetting].toString()
      )
  }
  ...
  function _PlayTopNavDropdownChanged() {
    ...
    if (playType === "official" || playType === "listen") {
      ...
      if (
        ...
        UiToolkitAPI.ShowCustomLayoutPopup(
          "server_browser_popup",
          "file://{resources}/layout/popups/popup_serverbrowser.xml"
        )
      } else {
        if (m_bPerfectWorld) {
  ...
  function _UpdateBotDifficultyButton() {
    ...
    var bShowBotDifficultyButton =
      playType === "listen" || playType === "workshop"
  ...
  function _DisplayWorkshopModePopup() {
    ...
    for (var iMap = 0; iMap < elSelectedMaps.length; ++iMap) {
      var mapModes = elSelectedMaps[iMap]
        .GetAttributeString("data-workshop-modes", "")
        .split(",")

      if (iMap == 0) modes = mapModes
      else
        modes = modes.filter(function (mode) {
          return mapModes.includes(mode)
        })
    }
    ...
    UiToolkitAPI.ShowCustomLayoutPopupParameters(
      "workshop_map_mode",
      "file://{resources}/layout/popups/popup_workshop_mode_select.xml",
      "workshop-modes=" + $.UrlEncode(strModes)
    )
  }

  function _UpdateWorkshopMapFilter() {
    var filter = $.HTMLEscape(
      $("#WorkshopSearchTextEntry").text,
      true
    ).toLowerCase()
    ...
    for (var i = 0; i < children.length; ++i) {
      ...
      if (
        mapname &&
        mapname.text &&
        mapname.text.toLowerCase().includes(filter)
      ) {
        panel.visible = true
        continue
      }
...
;(function () {
  ...
  $.RegisterEventHandler(
    "ReadyForDisplay",
    $.GetContextPanel(),
    PlayMenu.ReadyForDisplay
  )
  $.RegisterEventHandler(
    "UnreadyForDisplay",
    $.GetContextPanel(),
    PlayMenu.UnreadyForDisplay
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Lobby_MatchmakingSessionUpdate",
    PlayMenu.SessionSettingsUpdate
  )
  ...
  $.RegisterForUnhandledEvent(
    ...
    PlayMenu.WorkshopSubscriptionsChanged
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_ClansInfoUpdated",
    PlayMenu.ClansInfoUpdated
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_InventoryUpdated",
    PlayMenu.InventoryUpdated
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_FriendsList_NameChanged",
    PlayMenu.OnPlayerNameChangedUpdate
  )

  $.RegisterForUnhandledEvent(
    "DirectChallenge_GenRandomKey",
    PlayMenu.OnDirectChallengeRandom
  )
  $.RegisterForUnhandledEvent(
    "DirectChallenge_EditKey",
    PlayMenu.OnDirectChallengeEdit
  )
  $.RegisterForUnhandledEvent(
    "DirectChallenge_CopyKey",
    PlayMenu.OnDirectChallengeCopy
  )
  $.RegisterForUnhandledEvent(
    "DirectChallenge_ChooseClanKey",
    PlayMenu.OnChooseClanKeyBtn
  )
  $.RegisterForUnhandledEvent(
    "DirectChallenge_ClanChallengeKeySelected",
    PlayMenu.OnClanChallengeKeySelected
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_PartyBrowser_PrivateQueuesUpdate",
    PlayMenu.OnPrivateQueuesUpdate
  )
})()
