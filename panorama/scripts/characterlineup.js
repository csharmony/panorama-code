...
var CharacterLineUp = (function () {
  ...
  function _Init(elCLU, oSettings) {
    ...
    elCLU.Data().m_overrideCharacterSpacing =
      oSettings["overrideCharacterSpacing"]

    elCLU.Data().m_elModelContainer = elCLU.FindChildTraverse(
  ...
  function _CreatePlayerModelPanel(elCLU, xuid) {
    var elPlayer = $.CreatePanel(
      "Panel",
      elCLU.Data().m_elModelContainer,
      "id-charlineup__character-" + xuid
    )
  ...
  function _SetCharacter(
    ...
    var elAvatarImage = elPlayer.FindChildTraverse(
      "id-charlineup__player__avatar"
    )
    if (elAvatarImage) {
      if (!isBot) {
    ...
    var myTeamName = MockAdapter.GetPlayerTeamName(
      MockAdapter.GetLocalPlayerXuid()
    )

    if (
      xuid != MockAdapter.GetLocalPlayerXuid() &&
      !GameStateAPI.IsFakePlayer(xuid) &&
      myTeamName == teamName &&
      elCLU.Data().m_displayCommendButton
    ) {
    ...
    var elPlayerModelPreviewPanel = elPlayer.FindChildTraverse(
      "id-charlineup__model-preview-panel"
    )
  ...
  function _AddPlayer(
    ...
    if (!elCLU.Data().m_bInitialized) return

    if (arrPlayerMasterList.includes(label)) return

    if (arrPlayerMasterList.length >= MAX_CHARACTERS) return
  ...
  function _Filter(elCLU, filterFn) {
    ...
    arrPlayerMasterList.forEach((label) => {
      if (!filterFn(label)) {
        var elPlayer = elCLU.FindChildTraverse(
          "id-charlineup__character-" + label
        )
