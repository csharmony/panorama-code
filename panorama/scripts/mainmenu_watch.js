...
var mainmenu_watch = (function () {
  ...
  function _PopulateStreamList(parentPanel) {
    ...
    if (count === 0) {
      ...
      matchList.SetListMessage(
        $.Localize("#CSGO_Watch_NoSteams"),
        true,
        parentPanel
      )
    ...
    for (var i = 0; i < count; i++) {
      ...
      var elStreamPanel = elStreamList.FindChildInLayoutFile(
        "TwitchStream_" + streamName
      )
      if (elStreamPanel == undefined) {
        var elStreamPanel = $.CreatePanel(
          "Button",
          elStreamList,
          "TwitchStream_" + streamName
        )
        var streamCountry = StreamsAPI.GetStreamCountryByName(streamName)
        elStreamPanel.BLoadLayout(
          "file://{resources}/layout/matchtiles/streams.xml",
          false,
          false
        )
        ...
        elStreamPanel
          .FindChildInLayoutFile("stream-button__blur-target")
          .AddBlurPanel(elStreamText)

        elStreamPanel.SetDialogVariable(
          "streamText",
          StreamsAPI.GetStreamTextDescriptionByName(streamName)
        )
        elStreamPanel.SetDialogVariable(
          "numberOfViewers",
          StreamsAPI.GetStreamViewersByName(streamName)
        )
        elStreamPanel.SetDialogVariable(
          "channel",
          StreamsAPI.GetStreamDisplayNameByName(streamName)
        )

        elStreamPanel
          .FindChildTraverse("TwitchThumb")
          .SetImage(StreamsAPI.GetStreamPreviewImageByName(streamName))
        CommonUtil.SetLanguageOnLabel(streamCountry, elStreamPanel)

        elStreamPanel.SetPanelEvent(
          "onactivate",
          _SendToTwitch.bind(undefined, streamName)
        )
      }
  ...
  function _PopulateTournamentPage(parentPanel) {
    ...
    if (!elTournamentList.FindChildTraverse("other-tournaments")) {
      elTournamentList.BLoadLayout(
        "file://{resources}/layout/matchtiles/tournament_page.xml",
        false,
        false
      )
      var pastTournamentPanel =
        elTournamentList.FindChildTraverse("other-tournaments")
      ...
      for (let i = maxTournaments; i >= 1; i--) {
        ...
        let elTournamentPanel = $.CreatePanel(
          "Panel",
          pastTournamentPanel,
          "Tournament_" + i
        )
        ...
        elTournamentPanel.SetDialogVariable(
          "tournament-title",
          $.Localize("#CSGO_Tournament_Event_Location_" + i)
        )

        let elTOLogo = elTournamentPanel.FindChildTraverse(
          "id-tournament-to-logo"
        )
        elTOLogo.SetImage(
          "file://{images}/tournaments/events/tournament_logo_" + i + ".svg"
        )
        ...
        let elLegendsContainer =
          elTournamentPanel.FindChildTraverse("JsLegendsContainer")

        let elPlayerRoot =
          elTournamentPanel.FindChildTraverse("JsPlayersContainer")

        let elHoverPanel = elTournamentPanel.FindChildTraverse(
          "JsChampionsHoverTarget"
        )
        _PopulateTeamPlayers(
          elPlayerRoot,
          elHoverPanel,
          elLegendsContainer,
          oWinningTeam,
        ...
        if (elModel) {
          elModel.SetCameraPosition(-10, 0, 0.0)
          elModel.SetCameraAngles(0.0, 0.0, 0.0)
    ...
    function _SetTeam(elTeam, oTeamData, uniqueIdentifier, bTooltip = true) {
      ...
      if (oTeamData) {
        ...
        let teamLogo =
          "file://{images}/tournaments/teams/" + teamTag.toLowerCase() + ".svg"
        ...
        if (bTooltip) {
          ...
          let elTooltipAnchor = $.CreatePanel(
            "Panel",
            elTeam,
            uniqueIdentifier + "_" + elTeam.id,
    ...
    function _PopulateTeamPlayers(
      ...
      elHoverPanel.SetPanelEvent(
        "onmouseover",
        function (elPlayerContainer, elLegendsContainer) {
          _RevealPlayers(elPlayerContainer, elLegendsContainer)
        }.bind(this, elPlayerContainer, elLegendsContainer)
      )
      elHoverPanel.SetPanelEvent(
        "onmouseout",
        function (elPlayerContainer, elLegendsContainer) {
          _HidePlayers(elPlayerContainer, elLegendsContainer)
        }.bind(this, elPlayerContainer, elLegendsContainer)
      )
      ...
      function _RevealPlayers(elPlayerContainer, elLegendsContainer) {
        ...
        arrElPlayers.forEach(function (elPlayer, i) {
          ...
          Scheduler.Schedule(
            delay,
            function (elPlayer) {
              if (elPlayer) elPlayer.RemoveClass("hidden")
  ...
  function _UpdateTab(elTab, optbFromMatchListChangeEvent) {
    ...
    switch (elTab.id) {
      ...
      case "JsDownloaded":
      case "JsLive":
        matchList.UpdateMatchList(
          elTab,
          MATCHLISTDESCRIPTOR[elTab.id],
  ...
  function _UpdateActiveTab() {
    if (_m_activeTab) {
      if (_m_activeTab.id === "JsActiveTournament") {
        $.DispatchEvent(
          "RefreshPickemPage",
          "tournament:" + g_ActiveTournamentInfo.eventid
        )
        return
      }
  ...
  function _NavigateToTab(
    ...
    if (isSubTab && !$.GetContextPanel().FindChildInLayoutFile(tab)) {
      ...
      newPanel = $.CreatePanel(
        "Panel",
        parent,
        "tournament_content_" + tournament_id
      )
  ...
  function _CloseSubMenuContent() {
    if (
      !_m_tabStack ||
      _m_tabStack.length == 0 ||
      !_m_tabStack[_m_tabStack.length - 1].visible
    ) {
      return false
    }
    _m_tabStack.pop()
    ...
    if (_m_tabStack.length >= 1) {
      _NavigateToTab(
        _m_tabStack[_m_tabStack.length - 1].id,
        undefined,
        true,
        false
      )
  ...
  function _InitResourceManagement(elTab) {
    ...
    $.RegisterEventHandler(
      ...
      elTab,
      elTab.OnPropertyTransitionEndEvent
    )
  ...
  function _InitMainWatchPanel() {
    ...
    $.RegisterForUnhandledEvent(
      ...
      _UpdateMatchListFromMatchListChangeEvent
    )
    ...
    if (_m_bPerfectWorld) {
      ...
      if (elWatchNavBarButtonStreams)
        elWatchNavBarButtonStreams.DeleteAsync(0.0)
  ...
  var _ShowActiveTournamentPage = function (idOfTab = "") {
    while (_CloseSubMenuContent()) continue
    ...
    _NavigateToTab(
      "JsMainMenuSubContent_Tournament" + i,
      "mainmenu_watch_tournament",
      "tournament:" + i,
      true,
      true
    )
    ...
    if (idOfTab && elTournamentActive) {
      var elTabToActivate = elTournamentActive.FindChildTraverse(
...
;(function () {
  ...
  $.RegisterEventHandler(
    "Cancelled",
    $("#JsWatch"),
    mainmenu_watch.CloseSubMenuContent
  )
  $.RegisterEventHandler(
    "ReadyForDisplay",
    $("#JsWatch"),
    mainmenu_watch.OnReadyForDisplay
  )
  $.RegisterForUnhandledEvent(
    "ShowActiveTournamentPage",
    mainmenu_watch.ShowActiveTournamentPage
  )
})()
