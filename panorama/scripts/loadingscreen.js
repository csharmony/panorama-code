...
var LoadingScreen = (function () {
  ...
  var _UpdateLoadingScreenInfo = function (
    mapName,
    prettyMapName,
    prettyGameModeName,
    gameType,
    ...
    if (mapName) {
      ...
      var mapIconFailedToLoad = function () {
        $("#LoadingScreenMapName").RemoveClass(
          "loading-screen-content__info__text-title-short"
        )
        $("#LoadingScreenMapName").AddClass(
      ...
      $.RegisterEventHandler(
        "ImageFailedLoad",
        $("#LoadingScreenIcon"),
        mapIconFailedToLoad.bind(undefined)
      )
      $("#LoadingScreenMapName").RemoveClass(
        "loading-screen-content__info__text-title-long"
      )
      ...
      $.RegisterEventHandler(
        ...
        mapOverviewLoaded.bind(undefined)
      )
      ...
      $.RegisterEventHandler(
        "ImageFailedLoad",
        $("#LoadingScreenOverview"),
        mapOverviewFailed.bind(undefined)
      )
      ...
      if (mapName === "lobby_mapveto") {
        ...
        elOverview.SetImage(
          "file://{images_overviews}/" + mapName + "_radar.dds"
        )
      }
      ...
      if (prettyMapName != "")
        ...
        $("#LoadingScreenMapName").SetLocalizationString(
          GameStateAPI.GetMapDisplayNameToken(mapName)
        )
    }
    ...
    if (gameMode) {
      ...
      if (prettyGameModeName != "")
        ...
        $("#LoadingScreenGameMode").SetLocalizationString(
          "#sfui_gamemode_" + gameMode
        )

      if (
        GameStateAPI.IsQueuedMatchmakingMode_Team() ||
        mapName === "lobby_mapveto"
      )
        $("#LoadingScreenGameModeIcon").SetImage(
        ...
        $("#LoadingScreenGameModeIcon").SetImage(
          "file://{images}/icons/ui/" + gameMode + ".svg"
        )

      if (descriptionText != "")
        $(
          "#LoadingScreenModeDesc"
        ).SetProceduralTextThatIPromiseIsLocalizedAndEscaped(
          descriptionText,
          false
        )
      else $("#LoadingScreenModeDesc").SetLocalizationString("")
    } else elInfoBlock.AddClass("hidden")
  }
  ...
  var _OnMapConfigLoaded = function (overviewKV) {
    ...
    if (elMapOverview) {
      ...
      for (var i = 1; i <= 6; i++) {
        if (
          (elImage = CreateMapIcon(overviewKV, elMapOverview, "Hostage" + i))
        ) {
...
;(function () {
  $.RegisterForUnhandledEvent(
    ...
    LoadingScreen.UpdateLoadingScreenInfo
  )
  $.RegisterForUnhandledEvent("QueueConnectToServer", LoadingScreen.Init)
  $.RegisterForUnhandledEvent(
    "OnMapConfigLoaded",
    LoadingScreen.OnMapConfigLoaded
  )
  $.RegisterForUnhandledEvent(
