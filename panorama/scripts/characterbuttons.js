...
var CharacterButtons = (function () {
  function _PopulateWeaponDropdownForCharacter(elDropdown, modelPanelSettings) {
    ...
    list.forEach(function (entry) {
      ...
      elRarity.style.backgroundColor =
        "gradient( linear, 0% 0%, 100% 0%, from(" +
        rarityColor +
        " ),  color-stop( 0.0125, #00000000 ), to( #00000000 ) );"
  ...
  var _OnUpdateWeaponSelection = function (
    elDropdown,
    modelPanelSettings,
    bPlaySound
  ) {
    modelPanelSettings.weaponItemId = elDropdown.GetSelected()
  ...
  function _ZoomCamera() {
    ...
    var elZoomButton = $.GetContextPanel().FindChildInLayoutFile(
      "LoadoutSingleItemModelZoom"
    )
    data.m_modelPanelSettings.cameraPreset = elZoomButton.checked
      ? data.m_characterToolbarButtonSettings.cameraPresetZoomed
      : data.m_characterToolbarButtonSettings.cameraPresetUnzoomed

    data.m_modelPanelSettings.panel.SetCameraPreset(
  ...
  function _PlayCheer() {
    ...
    if (elZoomButton.checked) elZoomButton.checked = false
    ...
    var modelRenderSettingsOneOffTempCopy =
      ItemInfo.DeepCopyVanityCharacterSettings(data.m_modelPanelSettings)
    modelRenderSettingsOneOffTempCopy.arrModifiers = [
      InventoryAPI.GetCharacterDefaultCheerByItemId(
        modelRenderSettingsOneOffTempCopy.charItemId
      )
    ]
  ...
  function _InitCharacterButtons(
    elButtons,
    elPreviewpanel,
    ...
    var elZoomButton = elButtons.FindChildInLayoutFile(
      "LoadoutSingleItemModelZoom"
    )

    var modelPanelSettings = ItemInfo.GetOrUpdateVanityCharacterSettings(
      characterToolbarButtonSettings.charItemId
    )
    modelPanelSettings.panel = elPreviewpanel
    modelPanelSettings.cameraPreset = elZoomButton.checked
      ? characterToolbarButtonSettings.cameraPresetZoomed
      : characterToolbarButtonSettings.cameraPresetUnzoomed

    var elDropdown = elButtons.FindChildInLayoutFile(
      "LoadoutSingleItemModelWeaponChoice"
    )
    ...
    elButtons.Data().m_characterToolbarButtonSettings =
      characterToolbarButtonSettings
