...
function OnPopupCustomLayoutOperationHub(startPage) {
  ...
  if (startPage) elPanel.SetAttributeInt("start_page", startPage)
}
...
function OnPopupCustomLayoutLoadingScreen() {
  ...
  UiToolkitAPI.ShowCustomLayoutPopup(
    "teams",
    "file://{resources}/layout/teamselectmenu.xml"
  )
}
...
function VideoPlayNextTrailer() {
  ...
  videoPlayer.SetMovie(
    "file://{resources}/videos/trailer_" + g_VideoCurrentTrailer + ".webm"
  )
...
function InitDialogVariables() {
  ...
  $("#DiagVarLabel").text = $.Localize(
    "\tDynamic Label Count: {d:r:count}",
    $("#ControlsLibDiagVars")
  )
  ...
  $.RegisterEventHandler(
    "TextEntryChanged",
    $("#ChildDialogVarTextEntry"),
    UpdateChildDialogVariablesFromTextEntry
  )
}
...
function OnImageFailLoad(panelName, image) {
  ...
  $("#ControlsLibPanelImageFallback").SetImage(
    "file://{images}/icons/knife.vtf"
  )
}
...
function InitPanels() {
  ...
  $.CreatePanel("Label", parent, "", {
    text: "Label, with text property, created dynamically from js."
  })
  $.CreatePanel("Label", parent, "", {
  ...
  $.CreatePanel(
    "ControlLibTestPanel",
    $.FindChildInContext("#ControlsLibPanelsJS"),
    "",
    { MyCustomProp: "Created dynamically from javascript", CreatedFromJS: 1 }
  )

  $.RegisterEventHandler(
    "ImageFailedLoad",
    $("#ControlsLibPanelImageFallback"),
    OnImageFailLoad
  )
...
function OnRssFeedReceived(feed) {
  ...
  feed["items"].forEach(function (item) {
    var itemPanel = $.CreatePanel("Panel", RSSFeedPanel, "", {
      acceptsinput: true
    })
    itemPanel.AddClass("RSSFeed__Item")

    $.CreatePanel("Label", itemPanel, "", {
      text: item.title,
      html: true,
      class: "RSSFeed__ItemTitle"
    })
    ...
    $.CreatePanel("Label", itemPanel, "", {
      ...
      html: true,
      class: "RSSFeed__ItemDesc"
    })
    $.CreatePanel("Label", itemPanel, "", {
      text: item.date,
      html: true,
      class: "RSSFeed__ItemDate"
    })

    itemPanel.SetPanelEvent(
      "onactivate",
      SteamOverlayAPI.OpenURL.bind(SteamOverlayAPI, item.link)
    )
  })
}
...
function JSReadyAddChild() {
  ...
  $.CreatePanel("Panel", elParent, "ControlsLibBugsReadyChild", {
    class: "ControlLibBugs__ReadyChild"
  })
...
;(function () {
  ...
  jsPopupCallbackHandle = UiToolkitAPI.RegisterJSCallback(
    OnControlsLibPopupEvent
  )
  jsContextMenuCallbackHandle = UiToolkitAPI.RegisterJSCallback(
    OnControlsLibContextMenuEvent
  )
  jsPopupLoadingBarCallbackHandle = UiToolkitAPI.RegisterJSCallback(
    OnControlsLibPopupLoadingBarEvent
  )
