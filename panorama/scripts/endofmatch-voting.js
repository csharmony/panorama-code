...
var EOM_Voting = (function () {
  ...
  function _DisplayMe() {
    ...
    var elMapSelectionList = _m_cP.FindChildInLayoutFile(
      "id-map-selection-list"
    )

    Object.keys(oMatchEndVoteData["voting_options"]).forEach(
      function (key, index) {
        ...
        if (type == "separator") {
          ...
          if (type == "skirmish") {
            ...
            if (cfg) {
              var mg =
                cfg.mapgroups[
                  "mg_skirmish_" +
                    GameTypesAPI.GetSkirmishInternalName(skirmishId)
                ]
              if (mg) {
                Object.keys(mg.maps).forEach(function (map, i) {
                  var elMapImage = $.CreatePanel(
                    "Panel",
                    elVoteItem.FindChildInLayoutFile("MapGroupImagesCarousel"),
            ...
            var elMapIcon = elVoteItem.FindChildInLayoutFile(
              "id-map-selection-btn__modeicon"
            )

            var modeIcon =
              "file://{images}/icons/ui/" +
              GameTypesAPI.GetSkrimishIcon(skirmishId) +
              ".svg"
          ...
          var onActivate = function (element) {
            GameInterfaceAPI.ConsoleCommand(
              "endmatch_votenextmap" + " " + element.m_key
            )

            elMapSelectionList
              .FindChildrenWithClassTraverse("map-selection-btn")
              .forEach((btn) => (btn.enabled = false))
            $.DispatchEvent(
              "PlaySoundEffect",
          ...
          elVoteItem.SetPanelEvent(
            "onactivate",
            onActivate.bind(undefined, elVoteItem)
          )
  ...
  var _UpdateVotes = function () {
    ...
    var _GetWinningMaps = function () {
      ...
      Object.keys(oMatchEndVoteData["voting_options"]).forEach(function (key) {
        ...
        if (nVotes > highestVote) highestVote = nVotes
      })
      ...
      if (oMatchEndVoteData["voting_done"] == "1") {
        ...
        if (winner !== -1) {
          ...
          Object.keys(_m_elVoteItemPanels).forEach(function (key) {
            if (_m_elVoteItemPanels[key].m_key == winner) winningIndex = key
          })

          if (_m_elVoteItemPanels[winningIndex]) {
            var elCheckmark = _m_elVoteItemPanels[
              winningIndex
            ].FindChildTraverse("id-map-selection-btn__winner")
          ...
          if (!elVoteItem || !elVoteItem.IsValid()) return

          var panelToHilite = elVoteItem.FindChildTraverse(
            "id-map-selection-btn__gradient"
          )

          if (!panelToHilite || !panelToHilite.IsValid()) return
        ...
        Object.keys(_m_elVoteItemPanels).forEach(function (key) {
          ...
          var oVoteOptions =
            oMatchEndVoteData["voting_options"][_m_elVoteItemPanels[key].m_key]

          var elVoteCountLabel = elVoteItem.FindChildTraverse(
            "id-map-selection-btn__count"
          )
          ...
          elVoteCountLabel.text =
            "<font color='#ffc130'>" + votes + "</font>/" + votesNeeded
        })
      }
  ...
  function _Start() {
    if (
      ...
      !MockAdapter.GetMockData().includes("VOTING")
    ) {
      _End()
      return
    }
