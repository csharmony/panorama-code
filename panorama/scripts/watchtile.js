...
var watchTile = (function () {
  ...
  function _Init(elTile) {
    ...
    elTile.BLoadLayout(
      "file://{resources}/layout/matchtiles/" + matchTileDescriptor + ".xml",
      false,
      false
    )
    ...
    if (elTileMenu) {
      if (elTile.matchListDescriptor === "live") {
        elWatchButton = _AddButton(
          elTileMenu,
          elTile.matchListDescriptor + "_" + elTile.matchId,
          "watch",
          $.Localize("#CSGO_Watch_Info_live"),
          _Watch.bind("", elTile),
          false
        )
      } else {
        elShareButton = _AddButton(
          elTileMenu,
          elTile.matchListDescriptor + "_" + elTile.matchId,
          "link",
          $.Localize("#CSGO_Watch_Copy_Url"),
          _ShareMatch.bind("", elTile),
          false
        )
        elDownloadButton = _AddButton(
          elTileMenu,
          elTile.matchListDescriptor + "_" + elTile.matchId,
          "downloaded",
          $.Localize("#CSGO_Watch_Download"),
          _DownloadMatch.bind("", elTile),
          false
        )
        elDownloadingButton = _AddButton(
          elTileMenu,
          elTile.matchListDescriptor + "_" + elTile.matchId,
          "downloading",
          $.Localize("#SFUI_GameUI_MatchDlDownloading"),
          undefined,
          true
        )
        elDownloadFailedButton = _AddButton(
          elTileMenu,
          elTile.matchListDescriptor + "_" + elTile.matchId,
          "warning",
          $.Localize("#WatchMenu_Info_Download_Failed"),
          _UpdateFailedNotify.bind("", elTile),
          false
        )
        elMoreButton = _AddButton(
          elTileMenu,
          elTile.matchListDescriptor + "_" + elTile.matchId,
          "expand",
          $.Localize("#WatchMenu_Expand_Match_Menu"),
          _OpenContextMenu.bind("", elTile),
          false
        )
      }
    }
    ...
    if (matchTileDescriptor === "tournament") {
      ...
      elTournamentNameLabel.text = $.Localize(
        MatchInfoAPI.GetMatchTournamentStageName(elTile.matchId)
      )
      ...
      $.RegisterEventHandler(
        "ImageFailedLoad",
        elTournamentTeam0Icon,
        setDefaultTeamImage.bind(undefined, elTournamentTeam0Icon)
      )
      $.RegisterEventHandler(
        "ImageFailedLoad",
        elTournamentTeam1Icon,
        setDefaultTeamImage.bind(undefined, elTournamentTeam1Icon)
      )

      var icon0Filename =
        "file://{images}/tournaments/teams/" +
        MatchInfoAPI.GetMatchTournamentTeamTag(
          elTile.matchId,
          team0
        ).toLowerCase() +
        ".svg"
    ...
    if (elScore0Label) {
      elScore0Label.text = MatchInfoAPI.GetMatchRoundScoreForTeam(
        elTile.matchId,
        team0
      )
      if (matchTileDescriptor != "tournament") {
    ...
    if (elScore1Label) {
      elScore1Label.text = MatchInfoAPI.GetMatchRoundScoreForTeam(
        elTile.matchId,
        team1
      )
      if (matchTileDescriptor != "tournament") {
    ...
    if (elViewersLabel) {
      var spectatorCount = parseInt(
        MatchInfoAPI.GetMatchSpectators(elTile.matchId)
      )
      if (!spectatorCount) {
        spectatorCount = 0
      }
      ...
      if (elOutcomeLabel) {
        ...
        if (outcomeCode == undefined) {
          ...
          if (myTeam != 0) {
            ...
            else if (outcomeCode == 2) outcomeCode = 1
          }
          switch (outcomeCode) {
            case 0:
              elTile.AddClass("MatchTied")
              elOutcomeLabel.text = $.Localize(
                "#WatchMenu_Outcome_Tied",
                elOutcomeLabel
              )
              break
            case 1:
    ...
    if (elMatchMapIcon) {
      ...
      elMatchMapIcon.SetImage(
        "file://{images}/map_icons/map_icon_" + mapName + ".svg"
      )
    }
    ...
    if (elSkillGroupImg) {
      ...
      if (skillgroup)
        elSkillGroupImg.SetImage(
          "file://{images}/icons/skillgroups/skillgroup" + skillgroup + ".svg"
        )
    }
  }
