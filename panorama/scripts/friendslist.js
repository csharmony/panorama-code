...
var friendsList = (function () {
  ...
  var _Init = function () {
    ...
    var btnLobbiesTabListFilters = $(
      "#JsFriendsList-lobbies-toolbar-button-" + _m_sLobbiesTabListFiltersString
    )
    if (btnLobbiesTabListFilters) {
      ...
      elParent.Children().forEach((child) => {
        if (!child.BHasClass("friendslist-navbar-lobby-button")) return

        var strPrefix = "JsFriendsList-lobbies-toolbar-button-"
        if (!child.id.startsWith(strPrefix)) return

        var gamemodename = child.id.slice(strPrefix.length)
        if (
          !PartyListAPI.IsPlayerForHireAdvertisingEnabledForGameMode(
            gamemodename
          )
        )
          child.visible = false
  ...
  var _SetLocalPlayerAvatar = function () {
    ...
    var newPanel =
      $.GetContextPanel().FindChildInLayoutFile("JsLocalPlayercard")
    ...
    newPanel.BLoadLayout(
      "file://{resources}/layout/playercard.xml",
      false,
      false
    )

    $.GetContextPanel().MoveChildBefore(
      newPanel,
      $.GetContextPanel().FindChild("AntiAddiction")
    )

    newPanel.FindChildInLayoutFile("JsPlayerCardAvatar")
    _AddOpenPlayerCardAction(
      newPanel.FindChildInLayoutFile("JsPlayerCardAvatar"),
      xuid
    )
  }
  ...
  var _UpdateAntiAddiction = function () {
    ...
    var numSec = _m_isPerfectWorld
      ? MyPersonaAPI.GetAntiAddictionTimeRemaining()
      : -1
    if (numSec < 0) {
    ...
    if (numSec <= 300) szSeverity = "Red"
    else if (numSec <= 1800) szSeverity = "Yellow"
    ...
    var szLocalizedTooltip = $.Localize(
      numSec >= 60
        ? "#UI_AntiAddiction_Tooltip_GameTime"
        : "#UI_AntiAddiction_Tooltip_GameTimeNone",
      elAAGroup
    )
  ...
  var _UpdateFriendsTabList = function () {
    if (_m_activeTabIndex === 1) _UpdateRequestsTabList()
  ...
  var _m_sLobbiesTabListFiltersString = GameInterfaceAPI.GetSettingString(
    "ui_nearbylobbies_filter"
  )
  var _SetLobbiesTabListFilters = function (sFilterString) {
    ...
    GameInterfaceAPI.SetSettingString(
      "ui_nearbylobbies_filter",
      _m_sLobbiesTabListFiltersString
    )
    _RefreshLobbyListings()
  }
  ...
  var _UpdateTabList = function (tabIndex) {
    ...
    if (tabIndex === 2) {
      if (_ShowRecentsLoadingBar()) return
    }

    var elNodata = tabData.elList.FindChild("JsFriendListNoData")
    if (elNodata) elNodata.AddClass("hidden")
  ...
  var _ShowRecentsLoadingBar = function () {
    ...
    if (TeammatesAPI.GetSecondsAgoFinished() < 0) {
      if (elBarOuter.BHasClass("hidden")) elBarOuter.RemoveClass("hidden")
  ...
  var _ShowEmptyListerMessage = function (tabData) {
    ...
    if (!elTile) {
      ...
      elTile.FindChildTraverse("JsFriendsNoDataTitle").text = $.Localize(
        descString + "_title"
      )
      elTile.FindChildTraverse("JsFriendsNoDataDesc").text =
        $.Localize(descString)
  ...
  var _AddTile = function (elList, children, xuid, index, tileXmlToUse, type) {
    ...
    elTile.BLoadLayout(
      "file://{resources}/layout/" + tileXmlToUse + ".xml",
      false,
      false
    )
  ...
  var _UpdateIncomingInvitesContainer = function () {
    ...
    var elInviteContainer = elInviteRoot.FindChildInLayoutFile(
      "JsIncomingInviteContainer"
    )
  ...
  var _UpdateTilePosition = function (
    elList,
    children,
    elTile,
    xuid,
    ...
    if (children[index]) elList.MoveChildBefore(elTile, children[index])
  ...
  var _AddTransitionEndEventHandeler = function (elTile) {
    ...
    $.RegisterEventHandler(
      ...
      elTile,
      elTile.OnPropertyTransitionEndEvent
    )
  }
  ...
  var _SidebarContextMenuActive = function (isSidebarContextMenuActive) {
    if (isSidebarContextMenuActive) _OnSideBarHover(!isSidebarContextMenuActive)
    else _OnSideBarHover(isSidebarContextMenuActive)
  }
  ...
  var _OnSideBarHover = function (isCollapsed) {
    if (!isCollapsed) {
      ...
      for (var i = 0; i < _m_tabs.length; i++) {
        if (i !== _m_activeTabIndex) _m_tabs[i].elTabRadioBtn.AddClass("hidden")
        else _m_tabs[i].elTabRadioBtn.RemoveClass("hidden")
      }
    }
  }
  ...
  var _OnAddFriend = function () {
    UiToolkitAPI.ShowCustomLayoutPopup(
      "",
      "file://{resources}/layout/popups/popup_add_friend.xml"
    )
  }
  ...
  var _SetToTabLobbies = function () {
    _m_tabs[3].elTabRadioBtn.FindChildInLayoutFile("JsFriendsTab").checked =
      true
    _ShowSelectedTab(3)
  }
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    ...
    friendsList.UpdateAllTabsAlertCounts
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_FriendsList_RebuildFriendsList",
    friendsList.UpdateFriendsTabList
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Teammates_Refresh",
    friendsList.UpdateRecentsTabList
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_PartyBrowser_Refresh",
    friendsList.UpdateLobbiesTabList
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_FriendsList_NameChanged",
    friendsList.FriendsListNameChanged
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_MyPersona_GcLogonNotificationReceived",
    friendsList.SetLocalPlayerAvatar
  )
  $.RegisterForUnhandledEvent(
  ...
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_PartyBrowser_InviteConsumed",
    friendsList.UpdateIncomingInvitesContainer
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_PartyBrowser_InviteReceived",
    friendsList.UpdateIncomingInvitesContainer
  )
  $.RegisterForUnhandledEvent("SidebarIsCollapsed", friendsList.OnSideBarHover)
  $.RegisterForUnhandledEvent(
    "SidebarContextMenuActive",
    friendsList.SidebarContextMenuActive
  )
  $.RegisterForUnhandledEvent(
    "FriendInvitedFromContextMenu",
    friendsList.SetInvitedTile
  )
