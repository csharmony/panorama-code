...
var _BIsBatchMode = function () {
  if (
    ...
    m_strShowSelectItemForCapabilityPopupCapability === "batch"
  )
    return true
  else return false
}
...
var SetupPopup = function () {
  ...
  $.GetContextPanel().SetDialogVariable(
    "title",
    $.Localize("#popup_casket_title_" + m_strOperation)
  )

  m_itemidCasket = $.GetContextPanel().GetAttributeString("casket_item_id", "")

  m_strShowSelectItemForCapabilityPopupCapability =
    $.GetContextPanel().GetAttributeString("nextcapability", "")
    ...
    case "casket_added":
      if (m_strShowSelectItemForCapabilityPopupCapability) {
        ...
        $.DispatchEvent(
          ...
          "#popup_casket_message_prompt_bulkstore",
          "casketstore",
          itemid,
          ""
        )
...
function LaunchOperation() {
  ...
  InventoryAPI.PerformItemCasketTransaction(
    nOpRequestNumber,
    m_itemidCasket,
    m_itemidSubject
  )
