...
var PopupPermissionsSettings = (function () {
  ...
  var _SyncDialogsFromSessionSettings = function (settings) {
    ...
    $("#JsPopUpPermissionsPrivate").checked =
      settings.system.access === "private"
    $("#JsPopUpPermissionsPublic").checked = settings.system.access === "public"

    $("#JsPopUpPermissionsPrivate").SetPanelEvent("onactivate", function () {
      _ShowPublicExtraSettings()
    })
    $("#JsPopUpPermissionsPublic").SetPanelEvent("onactivate", function () {
      _ShowPublicExtraSettings()
    })
    ...
    var bClanDropDownSupport =
      clanCount > 0 ? true : settings.game.clanid ? true : false
    ...
    if (bClanDropDownSupport) {
      ...
      if (settings.game.clanid) {
        ...
        if (
          strUsersClan !== "103582791429521408" &&
          strUsersClan !== "175640385467449344"
        ) {
      ...
      elDropdown.SetPanelEvent("oninputsubmit", function () {
        elDropdownCheckbox.checked = elDropdown.GetSelected().id !== "NoGroup"
      })
    }
    ...
    m_nWasAdvertising =
      (settings.system.access === "public" && settings.game.clanid ? 1 : 0) +
      (settings.system.access === "public" && settings.game.nby === 1 ? 2 : 0)
  }

  var _ShowPublicExtraSettings = function () {
    $("#JsPopUpPermissionsGroupsDropdownEnabled").enabled = $(
      "#JsPopUpPermissionsPublic"
    ).checked
    $("#JsPopUpPermissionsGroupsDropdown").enabled = $(
      "#JsPopUpPermissionsPublic"
    ).checked
    $("#JsPopUpPermissionsNearby").enabled = $(
      "#JsPopUpPermissionsPublic"
    ).checked
  }

  var _ApplySessionSettings = function () {
    var selectedGroupEntryId = $(
      "#JsPopUpPermissionsGroupsDropdown"
    ).GetSelected().id
    var bGroupEntryEnabled =
      $("#JsPopUpPermissionsGroupsDropdownEnabled").checked &&
      selectedGroupEntryId !== "NoGroup"
    var nearbySetting = $("#JsPopUpPermissionsNearby").checked ? 1 : 0
    var accessSetting = $("#JsPopUpPermissionsPublic").checked
      ? "public"
      : "private"
    ...
    GameInterfaceAPI.SetSettingString(
      "lobby_default_privacy_clan_enabled",
      bGroupEntryEnabled ? "1" : "0"
    )
    GameInterfaceAPI.SetSettingString(
      "lobby_default_privacy_nearby_enabled",
      "" + nearbySetting
    )
    if (bGroupEntryEnabled) {
    ...
    if (
      $("#JsPopUpPermissionsPublic").checked &&
      GameInterfaceAPI.GetSettingString("lobby_default_privacy_bits") === "0"
    ) {
      GameInterfaceAPI.SetSettingString(
        "lobby_default_privacy_bits",
        $("#JsPopUpPermissionsPublic").checked ? "1" : "0"
      )
    }
    ...
    var nNowAdvertising =
      (accessSetting === "public" && bGroupEntryEnabled ? 1 : 0) +
    ...
    if (nNowAdvertising > 0) {
      ...
      if (
        settings &&
        settings.system &&
        settings.system.network === "offline"
      ) {
