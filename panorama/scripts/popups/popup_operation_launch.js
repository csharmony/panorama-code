...
var PopupOperationLaunchMovie = (function () {
  ...
  var _Init = function () {
    m_previousVersion = GameInterfaceAPI.GetSettingString(
      "ui_popup_weaponupdate_version"
    )

    var strGoalVersion = $.GetContextPanel().GetAttributeString(
      "uisettingversion",
      m_previousVersion
    )
    ...
    var btns = $.GetContextPanel().FindChildInLayoutFile(
      "id-operation-launch-buttons-row-buy-Btns"
    )
    ...
    var elDoNotShow = $.GetContextPanel().FindChildInLayoutFile(
      "id-operation-launch-btn-do-not-show"
    )
    elDoNotShow.SetPanelEvent("onactivate", _DoNotShowAgain)

    var elRepeat = $.GetContextPanel().FindChildInLayoutFile(
      "id-operation-launch-btn-repeat"
    )
    elRepeat.SetPanelEvent("onactivate", _PlayAgain)

    var strMovieFilePath =
      "file://{resources}/videos/op_riptide_launch_" + strMovieSuffix + ".webm"
  ...
  var _DoNotShowAgain = function () {
    if (
      ...
      m_version = $.GetContextPanel().GetAttributeString(
        "uisettingversion",
        m_previousVersion
      )
    }
