...
var CapabilityCanSticker = (function () {
  ...
  var _Init = function () {
    ...
    var strMsg = m_cP.GetAttributeString(
      "sticker-and-itemtosticker",
      "(not found)"
    )
  ...
  var _SetItemModel = function (toolId, itemId) {
    if (!InventoryAPI.IsItemInfoValid(itemId)) return
  ...
  var _SetTitle = function () {
    ...
    if (m_isRemoveStickers) {
      title = $.Localize(
        m_isPatch
          ? "#SFUI_InvContextMenu_can_stick_Wear_full" + m_toolToken
          : "#SFUI_InvContextMenu_can_stick_Wear" + m_toolToken,
        m_cP
      )
    } else {
  ...
  var _SetDescText = function (itemId) {
    ...
    var desc = m_isRemoveStickers
      ? m_isPatch
        ? "#popup_can_stick_scrape_full" + m_toolToken
        : "#popup_can_stick_scrape" + m_toolToken
  ...
  var _SetWarningText = function (toolId, itemId) {
    ...
    if (elWarningLabel) elWarningLabel.text = warningText
  }
    ...
    if (!bIsPerfectWorld) {
      if (InventoryAPI.IsMarketable(itemId)) {
        if (!InventoryAPI.IsPotentiallyMarketable(toolId)) {
          strSpecialParam = InventoryAPI.GetItemAttributeValue(
            toolId,
            "tradable after date"
          )
          if (strSpecialParam !== undefined && strSpecialParam !== null) {
            strSpecialWarning = _GetSpecialWarningString(
              strSpecialParam,
              "#popup_can_stick_warning_marketrestricted" + m_toolToken
            )
          }
        } else {
  ...
  var _GetStickerMarketDateGreater = function (toolId, itemId) {
    var rtTradableAfterSticker = InventoryAPI.GetItemAttributeValue(
      toolId,
      "{uint32}tradable after date"
    )
    var rtTradableAfterWeapon = InventoryAPI.GetItemAttributeValue(
      itemId,
      "{uint32}tradable after date"
    )
    if (
      rtTradableAfterSticker != undefined &&
      rtTradableAfterSticker != null &&
      (rtTradableAfterWeapon == undefined ||
        rtTradableAfterWeapon == null ||
        rtTradableAfterSticker > rtTradableAfterWeapon)
    ) {
      var strSpecialParam = null
      strSpecialParam = InventoryAPI.GetItemAttributeValue(
        toolId,
      ...
      if (strSpecialParam != undefined && strSpecialParam != null) {
        return _GetSpecialWarningString(
          strSpecialParam,
          "#popup_can_stick_warning_traderestricted" + m_toolToken
        )
      }
    }
  ...
  var _ShowStickerIconsToApplyOrRemove = function (toolId, itemId) {
    ...
    if (m_isRemoveStickers) {
      ...
      for (var i = 0; i < slotCount; i++) {
        ...
        if (imagePath) {
          if (!m_SlotSelectedForScratch) m_SlotSelectedForScratch = i

          var elSticker = $.CreatePanel("Button", elStickersToRemove, imagePath)
          elSticker.BLoadLayoutSnippet("ScrapeStickerBtn")
          elSticker
            .FindChildInLayoutFile("ScrapeStickerImage")
            .SetImage("file://{images_econ}" + imagePath + "_large.png")
          elSticker.SetPanelEvent(
            "onactivate",
            _OnScratchSticker.bind(undefined, elSticker, itemId, i)
          )
          elSticker.SetPanelEvent(
            "onmouseover",
            _HighlightStickerBySlot.bind(undefined, i)
          )
          elSticker.SetPanelEvent(
            "onmouseout",
            _HighlightStickerBySlot.bind(undefined, -1)
          )
        }
      }
    } else {
  ...
  var _SetUpAsyncActionBar = function (itemId, toolId) {
    ...
    var elAsyncActionBarPanel = m_cP.FindChildInLayoutFile(
  ...
  var _StickerBtnActions = function (toolId, itemId) {
    var slotsCount = m_isRemoveStickers
      ? InventoryAPI.GetItemStickerSlotCount(itemId)
      : m_emptySlotList.length
    ...
    if (!m_isRemoveStickers) {
      if (slotsCount > 1) {
        if (elContinueBtn)
          elContinueBtn.SetPanelEvent(
            "onactivate",
            _OnContinue.bind(undefined, elContinueBtn)
          )

        if (elNextSlotBtn)
          elNextSlotBtn.SetPanelEvent(
            "onactivate",
            _NextSlot.bind(undefined, elContinueBtn, toolId)
          )
  ...
  var _OnContinue = function (elContinueBtn) {
    ...
    m_cP
      .FindChildInLayoutFile("StickerToAppy")
      .RemoveClass("popup-cansticker-pickedslot--anim")
    m_cP
      .FindChildInLayoutFile("StickerToAppy")
      .AddClass("popup-cansticker-pickedslot--anim")
  }

  var _NextSlot = function (elContinueBtn, toolId, bDontIncrement = false) {
    if (!bDontIncrement) _ActiveSlotIndex.IncrementIndex()

    var activeIndex = _GetSelectedStickerSlot()
    _PreviewStickerInSlot(toolId, activeIndex)

    if (elContinueBtn) elContinueBtn.enabled = true
  ...
  var m_cameraRules = [
    {
      weapontype: "weapon_elite",
      slotsForSecondCamera: [2, 3],
      cameraPreset: 1
    },
    {
      weapontype: "weapon_revolver",
      slotsForSecondCamera: [4],
      cameraPreset: 1
    },
    {
      weapontype: "weapon_nova",
      slotsForSecondCamera: [1, 2, 3],
      cameraPreset: 1
    },
  ...
  var _CameraAnim = function (activeIndex, bTransition) {
    if (m_prevCameraSlot == activeIndex || activeIndex == -1) return

    var elPanel = m_cP.FindChildInLayoutFile("CanStickerItemModel")
    if (!InventoryAPI.IsItemInfoValid(elPanel.Data().id)) return

    var defName = ItemInfo.GetItemDefinitionName(elPanel.Data().id)

    if (m_isSticker) {
      var weaponProp = m_cameraRules.find(
        (weapon) => weapon.weapontype === defName
      )
      ...
      _UpdatePreviewPanelSettingsForPatchPosition(
        elPanel.Data().id,
        settings,
        activeIndex
      )
      CharacterAnims.PlayAnimsOnPanel(settings, !m_isRemoveStickers)
      _UpdatePreviewPanelCameraAndLightingForPatch(
        elPanel,
        elPanel.Data().id,
        activeIndex
      )
  ...
  function _UpdatePreviewPanelSettingsForPatchPosition(
    charItemId,
    oSettings,
    activeIndex = 0
  ) {
    var patchPosition = InventoryAPI.GetCharacterPatchPosition(
      charItemId,
      activeIndex
    )
    ...
    oSettings.weaponItemId = LoadoutAPI.GetItemID(
      oSettings.team,
      oSettings.loadoutSlot
    )
  }

  function _UpdatePreviewPanelCameraAndLightingForPatch(
    elPanel,
    charItemId,
    activeIndex = 0
  ) {
    ...
    var lightpos_torso = [51.1, -9.16, 72.78]
    var lightang_torso = [23.98, 166.5, 0.0]
    var campos_torso = [189.9, -28.08, 46.37]
    var camang_torso = [-2.06, 171.74, 0.0]

    var lightpos_mid = [50.15, -10.03, 70.19]
    var lightang_mid = [23.98, 166.5, 0.0]
    var campos_mid = [188.43, -25.44, 38.53]
    var camang_mid = [-2.06, 171.74, 0.0]

    var lightpos_legs = [50.15, -10.03, 50.19]
    var lightang_legs = [23.98, 166.5, 0.0]
    var campos_legs = [188.43, -25.44, 18.53]
    var camang_legs = [-2.06, 171.74, 0.0]
    ...
    if (lightbrt) elPanel.SetFlashlightAmount(lightbrt)
  ...
  var _GetSlotInfo = function (itemId) {
    ...
    for (var i = 0; i < slotsCount; i++) {
      ...
      slotInfoList.push({
        index: i,
        path: !StickerPath ? "empty" : StickerPath
      })
    }
  ...
  var _GetEmptyStickerSlots = function (slotInfoList) {
    return slotInfoList.filter(function (slot) {
      if (slot.path === "empty") return true
    })
  }
  ...
  return {
    ...
    UpdatePreviewPanelCameraAndLightingForPatch:
      _UpdatePreviewPanelCameraAndLightingForPatch,
    UpdatePreviewPanelSettingsForPatchPosition:
      _UpdatePreviewPanelSettingsForPatchPosition
  }
})()
...
;(function () {
  $.RegisterForUnhandledEvent(
    ...
    CapabilityCanSticker.OnFinishedScratch
  )
})()
