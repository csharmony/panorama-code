...
var CapabiityHeader = (function () {
  ...
  var _Init = function (elPanel, itemId, funcGetSettingCallback) {
    ...
    if (!m_worktype && !m_storeItemid) return
    ...
    m_bShowWarning =
      funcGetSettingCallback("asyncworkitemwarning", "yes") === "no"
        ? false
        : true
  ...
  var _SetUpTitle = function (elPanel) {
    ...
    if (m_isXrayMode) {
      ...
      else elTitle.text = "#popup_" + m_worktype + "_title"
    }
  }
  ...
  var _SetUpWarning = function (elPanel) {
    ...
    if (m_worktype === "decodeable") {
      var sRestriction = m_storeItemid
        ? ""
        : InventoryAPI.GetDecodeableRestriction(m_itemid)
