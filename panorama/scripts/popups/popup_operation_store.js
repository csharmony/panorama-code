...
var CapabilityOperationStore = (function () {
  ...
  var _Init = function () {
    ...
    if (
      !OperationUtil.ValidateOperationInfo(nActiveSeason) ||
      !_CanUserDoShopping()
    ) {
    ...
    var oldStarItemIdsToActivate = $.GetContextPanel().GetAttributeString(
      "oldstarstoactivate",
      ""
    )
    if (oldStarItemIdsToActivate) {
      var oldStarsValue = $.GetContextPanel().GetAttributeInt(
    ...
    $.GetContextPanel().SetDialogVariableInt(
      "after_purchase_stars",
      OperationUtil.GetOperationInfo().nCoinRank
    )
    SetCurrrentStars()
  ...
  var _SetStarsLevelsAfterPurchase = function (nStarsInCart) {
    $.GetContextPanel().SetHasClass(
      "popup-operation-store-cart-empty",
      nStarsInCart <= 0
    )
    ...
    var animName =
      $.GetContextPanel().Data().oldStarsTotal &&
    ...
    $.GetContextPanel()
      .FindChildInLayoutFile("id-operation-store-progress-after")
      .AddClass(animName)
  ...
  var _ClosePopup = function () {
    ...
    $.DispatchEvent(
      "PlaySoundEffect",
      "UIPanorama.mainmenu_press_home",
      "MOUSE"
    )
...
var StarsShoppingCart = (function () {
  ...
  var elParent = $.GetContextPanel().FindChildInLayoutFile(
    "popup-operation-store-rows"
  )

  var _Init = function () {
    var elOK = $.GetContextPanel().FindChildInLayoutFile(
      "AsyncItemWorkAcceptConfirm"
    )
    elOK.SetPanelEvent(
      "onactivate",
      _OnActivate.bind(undefined, elOK, "purchase")
    )

    var elApply = $.GetContextPanel().FindChildInLayoutFile(
      "AsyncItemWorkUseItem"
    )
    elApply.SetPanelEvent(
      "onactivate",
      _OnActivate.bind(undefined, elApply, "useitem")
    )
  }
  ...
  var _UpdateStoreBasedOnCoinRank = function (nRank, bIsPreview = false) {
    ...
    oStoreData.storeids.forEach((element) => {
      ...
      var fauxItemId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
        element.toString(),
        0
      )
      var starsCount = InventoryAPI.GetItemAttributeValue(
        fauxItemId,
        "upgrade level"
      )
      ...
      if (!discount) {
        ...
        elRow.SetDialogVariable(
          "store-item-original-price",
          ItemInfo.GetStoreOriginalPrice(fauxItemId, 1)
        )
      }
      ...
      elRow.SetDialogVariable(
        "store-item-sale-price",
        ItemInfo.GetStoreSalePrice(fauxItemId, 1)
      )
      elRow.SetDialogVariableInt("store-item-quantity", elRow.Data().quantity)

      elRow.FindChildInLayoutFile(
        "popup-operation-store-count-increment"
      ).enabled = !bIsPreview
      elRow
        .FindChildInLayoutFile("popup-operation-store-count-increment")
        .SetPanelEvent("onactivate", _AddItem.bind(undefined, elRow))

      elRow.FindChildInLayoutFile(
        "popup-operation-store-count-decrement"
      ).enabled = !bIsPreview
      elRow
        .FindChildInLayoutFile("popup-operation-store-count-decrement")
        .SetPanelEvent("onactivate", _RemoveItem.bind(undefined, elRow))
    ...
    elParent
      .Children()
      [
        elParent.Children().length - 1
      ].AddClass("popup-operation-store-row-bottom-border")
  }

  var _AddItem = function (elRow) {
    if (elRow.Data().quantity + 1 > MAX_QUANTITY / elRow.Data().starsCount)
      return
    if (elRow.Data().quantity + 1 > MAX_QUANTITY_EACH) return
      ...
      elRow
        ...
        .forEach((elOtherRow) => {
          if (elOtherRow.Data() && elOtherRow.Data().starsCount) {
            while (elOtherRow.Data().quantity > 0) _RemoveItem(elOtherRow)
          }
        })
    }
  ...
  var _UpdateRowPurchasePriceQuantity = function (elRow) {
    ...
    var price = !quantity
      ? "-"
      : ItemInfo.GetStoreSalePrice(elRow.Data().itemid, elRow.Data().quantity)
  ...
  var _UpdateTotals = function () {
    ...
    var dispPrice = !totalStarCount
      ? "-"
      : _GetTotalPriceOfItems(m_aCartItemIds)
  ...
  var _GetTotalStarCount = function (aCartItems) {
    ...
    aCartItems.forEach(function (item) {
      total +=
        item.quantity *
        InventoryAPI.GetItemAttributeValue(item.id, "upgrade level")
    })
  ...
  var _OnActivate = function (btn, workType) {
    if (workType == "useitem") {
      ...
      $.GetContextPanel()
        .FindChildInLayoutFile("op-Store-spinner")
        .RemoveClass("hidden")
      btn.AddClass("hidden")
    }
  ...
  var _CancelWaitforCallBack = function () {
    ...
    var elSpinner =
      $.GetContextPanel().FindChildInLayoutFile("op-Store-spinner")
  ...
  var _AcknowlegeStars = function (aDefNames) {
    ...
    m_NewItemsList = AcknowledgeItems.GetItemsByType(
      aDefNames,
      bShouldAcknowledge
    )
  ...
  var _SetApplyStarsStyles = function () {
    ...
    $.GetContextPanel()
      ...
      .AddClass("hidden")
    $.GetContextPanel()
      .FindChildInLayoutFile("AsyncItemWorkUseItem")
      .RemoveClass("hidden")
    $.GetContextPanel()
  ...
  var _UpdateApplyDialogLabels = function (bhasUnused, bNewStars) {
    if (bNewStars) {
      $.GetContextPanel().FindChildInLayoutFile(
        "id-operation-store-apply_title"
      ).text = "#CSGO_PickEm_Trophy_Status_Gold"
      $.GetContextPanel().FindChildInLayoutFile(
        "id-operation-store-apply_desc"
      ).text = "#op_store_stars_applied"
    } else {
      $.GetContextPanel().FindChildInLayoutFile(
        "id-operation-store-apply_title"
      ).text = bhasUnused
        ? "#op_store_apply_stars_unused"
        : "#op_store_apply_stars"
      $.GetContextPanel().FindChildInLayoutFile(
        "id-operation-store-apply_desc"
      ).text = bhasUnused
        ? "#op_store_apply_stars_desc_unused"
        : "#op_store_apply_stars_desc"
    }

    $.GetContextPanel().FindChildInLayoutFile(
      "id-operation-store-apply_type_desc"
    ).text = bhasUnused ? "#op_store_new_stars_unused" : "#op_store_new_stars"

    $.GetContextPanel().FindChildInLayoutFile(
      "id-operation-store-apply_warning"
    ).visible = !bNewStars
  }
  ...
  var _PreFillCart = function () {
    ...
    var elRows = $.GetContextPanel()
      .FindChildInLayoutFile("popup-operation-store-rows")
      .Children()
    elRows.sort(function (a, b) {
      return b.Data().starsCount - a.Data().starsCount
    })
  ...
  var _OnInventoryUpdate = function () {
    ...
    OperationUtil.ValidateOperationInfo(
    ...
    if (StarsEarned > prevStars) {
      ...
      $.GetContextPanel()
        .FindChildInLayoutFile("id-operation-store-progress-apply")
        .AddClass("popup-operation-apply-stars")
...
;(function () {
  $.RegisterForUnhandledEvent(
    ...
    StarsShoppingCart.OnInventoryUpdate
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Store_PurchaseCompleted",
    StarsShoppingCart.ItemAcquired
  )
})()
