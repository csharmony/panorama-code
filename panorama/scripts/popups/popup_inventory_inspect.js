...
var InventoryInspect = (function () {
  ...
  var _Init = function () {
    ...
    if (!_m_PanelRegisteredForEvents) {
      _m_PanelRegisteredForEvents = $.RegisterForUnhandledEvent(
        ...
        InventoryInspect.ShowNotification
      )
      $.RegisterForUnhandledEvent(
        "PanoramaComponent_Store_PurchaseCompleted",
        InventoryInspect.ItemAcquired
      )
    }
    ...
    var styleforPopUpInspectFullScreenHostContainer =
      $.GetContextPanel().GetAttributeString("extrapopupfullscreenstyle", null)
    if (styleforPopUpInspectFullScreenHostContainer) {
      ...
      elPopUpInspectFullScreenHostContainer.AddClass(
        styleforPopUpInspectFullScreenHostContainer
      )
    }
  ...
  var _UpdatePanelData = function (itemId) {
    var elItemModelImagePanel = $.GetContextPanel().FindChildInLayoutFile(
      "PopUpInspectModelOrImage"
    )
    InspectModelImage.Init(elItemModelImagePanel, itemId, _GetSettingCallback)

    var elActionBarPanel = $.GetContextPanel().FindChildInLayoutFile(
      "PopUpInspectActionBar"
    )
    ...
    var elCapabilityPanel = $.GetContextPanel().FindChildInLayoutFile(
      "PopUpCapabilityHeader"
    )
    CapabiityHeader.Init(elCapabilityPanel, itemId, _GetSettingCallback)

    var elPurchasePanel = $.GetContextPanel().FindChildInLayoutFile(
  ...
  var _SetDescription = function (id) {
    ...
    var shortString = descText.substring(
      0,
      descText.indexOf("</font></b><br><font color='#9da1a9'>")
    )
    $.GetContextPanel().SetDialogVariable(
      "item_description",
      shortString === "" ? descText : shortString
    )
  }
  ...
  var _SetupLootlistNavPanels = function (itemId) {
    ...
    if (aLootlistIds.length < 1) {
      $.GetContextPanel().FindChildInLayoutFile(
        "id-lootlist-btns-container"
      ).visible = false
      $.GetContextPanel().FindChildInLayoutFile(
        "id-lootlist-title-container"
      ).visible = false
      return
    }
    ...
    btnNext.SetPanelEvent("onactivate", function () {
      m_lootlistItemIndex =
        m_lootlistItemIndex < count - 1
          ? m_lootlistItemIndex + 1
          : m_lootlistItemIndex
      _EnableNextPrevBtns(aLootlistIds)
    ...
    btnPrev.SetPanelEvent("onactivate", function () {
      m_lootlistItemIndex =
        m_lootlistItemIndex > 0 ? m_lootlistItemIndex - 1 : m_lootlistItemIndex
      _EnableNextPrevBtns(aLootlistIds)
      _UpdatePanelData(aLootlistIds[m_lootlistItemIndex])
      _UpdateCharacterModelPanel(aLootlistIds[m_lootlistItemIndex])
  ...
  var _UpdateCharacterModelPanel = function (itemId) {
    ...
    InspectActionBar.OnUpdateCharModel(
      false,
  ...
  var _PrecacheCustomMaterials = function (aLootlistIds, count, bPrev, bNext) {
    if (bNext) {
      if (m_lootlistItemIndex + 1 < count - 1) {
        InventoryAPI.PrecacheCustomMaterials(
          aLootlistIds[m_lootlistItemIndex + 1]
        )
      }
    }
  ...
  var _EnableNextPrevBtns = function (aLootlistIds) {
    ...
    btnNext.enabled =
      m_lootlistItemIndex < aLootlistIds.length - 1 &&
      aLootlistIds[m_lootlistItemIndex + 1] !== "0"
  ...
  var _SetBtnLabel = function (btnNext, btnPrev, aLootlistIds) {
    if (btnNext.enabled) {
      btnNext.FindChildInLayoutFile("id-lootlist-label").text =
        InventoryAPI.GetItemName(aLootlistIds[m_lootlistItemIndex + 1])
      var rarityColor = ItemInfo.GetRarityColor(
        aLootlistIds[m_lootlistItemIndex + 1]
      )

      if (rarityColor) {
        btnNext.FindChildInLayoutFile("id-lootlist-rarity").style.washColor =
          rarityColor
      }
    }

    if (btnPrev.enabled) {
      btnPrev.FindChildInLayoutFile("id-lootlist-label").text =
        InventoryAPI.GetItemName(aLootlistIds[m_lootlistItemIndex - 1])
      var rarityColor = ItemInfo.GetRarityColor(
        aLootlistIds[m_lootlistItemIndex - 1]
      )
      ...
      if (rarityColor) {
        btnPrev.FindChildInLayoutFile("id-lootlist-rarity").style.washColor =
          rarityColor
      }
    }
  }
  ...
  var _ItemAcquired = function (ItemId) {
    ...
    if (storeItemId) {
      var storeItemSeasonAccess = InventoryAPI.GetItemAttributeValue(
        storeItemId,
        "season access"
      )
      var acquiredItemSeasonAccess = InventoryAPI.GetItemAttributeValue(
        ItemId,
        "season access"
      )
      if (storeItemSeasonAccess === acquiredItemSeasonAccess) {
        var nSeasonAccess = GameTypesAPI.GetActiveSeasionIndexValue()
        var nCoinRank = MyPersonaAPI.GetMyMedalRankByType(
          nSeasonAccess + 1 + "Operation$OperationCoin"
        )
      ...
      var defName = ItemInfo.GetItemDefinitionName(
      ...
      if (
        ItemInfo.ItemMatchDefName(storeItemId, defName) &&
  ...
  var _ClosePopup = function () {
    ...
    var elInspectBar = $.GetContextPanel().FindChildInLayoutFile(
      "PopUpInspectActionBar"
    )
