...
var PopupAcceptMatch = (function () {
  ...
  var m_lobbySettings
  var m_elTimer = $.GetContextPanel().FindChildInLayoutFile(
  ...
  var _Init = function () {
    var elPlayerSlots =
      $.GetContextPanel().FindChildInLayoutFile("AcceptMatchSlots")
    elPlayerSlots.RemoveAndDeleteChildren()

    var settings = $.GetContextPanel().GetAttributeString(
      "map_and_isreconnect",
      ""
    )
    ...
    if (!m_isReconnect && m_lobbySettings && m_lobbySettings.game) {
      ...
      var elAgreementComp = $.GetContextPanel().FindChildInLayoutFile(
        "AcceptMatchAgreementCompetitive"
      )
  ...
  function _PopulatePlayerList() {
    ...
    $.GetContextPanel()
      .FindChildInLayoutFile("id-map-draft-phase-teams")
      .RemoveClass("hidden")
    ...
    for (var i = 0; i < numPlayers; ++i) {
      ...
      var teamPanelId =
        iYourXuidTeamIdx === iThisPlayerTeamIdx
          ? "id-map-draft-phase-your-team"
          : "id-map-draft-phase-other-team"
      var elTeammates = $.GetContextPanel()
  ...
  var _UpdateUiState = function () {
    ...
    m_elTimer.GetChild(0).text =
      "0:" + (m_numSecondsRemaining < 10 ? "0" : "") + m_numSecondsRemaining
  ...
  var _ReadyForMatch = function (
    shouldShow,
    playersReadyCount,
    numTotalClientsInReservation
  ) {
    if (!shouldShow) {
      if (m_jsTimerUpdateHandle) {
        $.CancelScheduled(m_jsTimerUpdateHandle)
    ...
    if (
      m_hasPressedAccept &&
      m_numPlayersReady &&
      playersReadyCount > m_numPlayersReady
    ) {
    ...
    if (
      playersReadyCount == 1 &&
      numTotalClientsInReservation == 1 &&
      m_numTotalClientsInReservation > 1
    ) {
  ...
  var _UpdatePlayerSlots = function (elPlayerSlots) {
    for (var i = 0; i < m_numTotalClientsInReservation; i++) {
      var Slot = $.GetContextPanel().FindChildInLayoutFile(
        "AcceptMatchSlot" + i
      )
      ...
      Slot.SetHasClass(
        "accept-match__slots__player--accepted",
        i < m_numPlayersReady
      )
    }

    var labelPlayersAccepted = $.GetContextPanel().FindChildInLayoutFile(
      "AcceptMatchPlayersAccepted"
    )
    labelPlayersAccepted.SetDialogVariableInt("accepted", m_numPlayersReady)
    labelPlayersAccepted.SetDialogVariableInt(
      "slots",
      m_numTotalClientsInReservation
    )
    labelPlayersAccepted.text = $.Localize(
      "#match_ready_players_accepted",
      labelPlayersAccepted
    )
  }
  ...
  var _SetMatchData = function (map) {
    ...
    var labelData =
      $.GetContextPanel().FindChildInLayoutFile("AcceptMatchModeMap")
    var strLocalize = "#match_ready_match_data"

    labelData.SetDialogVariable(
      "mode",
      $.Localize("#SFUI_GameMode_" + m_lobbySettings.game.mode)
    )
    ...
    if (
      ...
      SessionUtil.DoesGameModeHavePrimeQueue(m_lobbySettings.game.mode) &&
      (m_lobbySettings.game.prime !== 1 || !SessionUtil.AreLobbyPlayersPrime())
    ) {
      $.GetContextPanel()
        .FindChildInLayoutFile("AcceptMatchWarning")
        .RemoveClass("hidden")
    }
    ...
    if (
      m_lobbySettings.game.mode === "competitive" &&
      map === "lobby_mapveto"
    ) {
      $("#AcceptMatchModeIcon").SetImage(
      ...
      if (m_lobbySettings.options && m_lobbySettings.options.challengekey) {
        ...
        labelData.SetDialogVariable(
          "map",
          $.Localize("#SFUI_Lobby_LeaderMatchmaking_Type_PremierPrivateQueue")
        )
      }
    }
    ...
    var imgMap = $.GetContextPanel().FindChildInLayoutFile(
      "AcceptMatchMapImage"
    )
    imgMap.style.backgroundImage =
      'url("file://{images}/map_icons/screenshots/360p/' + map + '.png")'
  }
...
;(function () {
  $.RegisterForUnhandledEvent(
    ...
    PopupAcceptMatch.FriendsListNameChanged
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Lobby_ReadyUpForMatch",
    PopupAcceptMatch.ReadyForMatch
  )
  $.RegisterForUnhandledEvent(
    "MatchAssistedAccept",
    PopupAcceptMatch.OnAcceptMatchPressed
  )
})()
