...
var CapabilityCanStatTrackSwap = (function () {
  ...
  var _Init = function () {
    ...
    m_itemids.forEach(function (item, idx) {
      m_statNumbersOriginal[idx] = parseInt(
        InventoryAPI.GetItemAttributeValue(item, "kill eater")
      )
      _SetItemModel(idx)
    })

    var elModel = $.GetContextPanel().FindChildInLayoutFile(
      "StatTrackSwapToolModel"
    )
  ...
  var _SetUpButtonStates = function () {
    $.GetContextPanel()
      .FindChildInLayoutFile("StatTrackSwapAcceptConfirm")
      .SetPanelEvent("onactivate", _OnAccept)

    $.GetContextPanel()
      .FindChildInLayoutFile("StatTrackSwapCancelBtn")
      .SetPanelEvent("onactivate", _ClosePopup)
  }
  ...
  var _LerpTimer = function () {
    ...
    var newDistanceLerped =
      m_flLerpProgress < 1.0
        ? Math.round(m_flLerpProgress * originalLen)
        : originalLen
    if (newDistanceLerped != m_distanceLerped) {
      ...
      $.GetContextPanel().FindChildInLayoutFile("StatTrackSwapNumber0").text = (
        m_statNumbersOriginal[0] + m_distanceLerped
      )
        .toString()
        .padStart(6, "0")
      $.GetContextPanel().FindChildInLayoutFile("StatTrackSwapNumber1").text = (
        m_statNumbersOriginal[1] - m_distanceLerped
      )
        .toString()
        .padStart(6, "0")
    }
  ...
  var _OnAccept = function () {
    ...
    $.GetContextPanel()
      .FindChildInLayoutFile("NameableSpinner")
      .RemoveClass("hidden")
...
;(function () {
  $.RegisterForUnhandledEvent(
    ...
    CapabilityCanStatTrackSwap.OnItemCustomization
  )
})()
