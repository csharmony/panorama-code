...
var PopupCommendPlayer = (function () {
  ...
  var _Update = function () {
    var xuid = $.GetContextPanel().GetAttributeString("xuid", "")
    var bAskedServersForCommendation =
      GameStateAPI.QueryServersForCommendation(xuid)
    ...
    if (bAskedServersForCommendation) {
      ...
      if (numTokens == 0) {
        if (m_elStatus && m_elStatus.IsValid()) {
          m_elStatus.text = $.Localize(
            "#SFUI_PlayerDetails_NoCommendations_Left"
          )
        }
      ...
      if (m_elCommend.visible) {
        ...
        if (oCommends["valid"]) {
          ...
          $.GetContextPanel()
            .FindChildInLayoutFile("id-commend")
            .Children()
            .forEach((el) => {
          ...
          if (bHasPrevCommendations) {
            m_elStatus.text = $.Localize(
              "#SFUI_PlayerDetails_Previously_Submitted"
            )
          }
        }
      }
...
;(function () {
  $.RegisterForUnhandledEvent(
    "GameState_CommendPlayerQueryResponse",
    PopupCommendPlayer.ReceivedCommendationFromServer
  )
})()
