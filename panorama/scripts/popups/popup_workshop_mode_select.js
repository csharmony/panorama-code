...
var PopupWorkshopModeSelect = (function () {
  ...
  var _Init = function () {
    ...
    m_elButtonContainer = m_elPopup.FindChildTraverse(
      "popup-workshop-mode-items"
    )

    m_elPopup.FindChildTraverse("GoButton").SetPanelEvent("onactivate", _Apply)
    m_elPopup
      .FindChildTraverse("CancelButton")
      .SetPanelEvent("onactivate", _Cancel)

    var strModes = m_elPopup.GetAttributeString("workshop-modes", "")
    if (!strModes) strModes = "casual"
  ...
  var _InitModes = function (modes) {
    m_elButtons.forEach(function (elButton) {
      elButton.DeleteAsync(0.0)
    })
    m_elButtons = []
    ...
    for (var i = 0; i < modes.length; ++i) {
      ...
      elButton.SetDialogVariable(
        "workshop-mode-item-name",
        $.Localize("#CSGO_Workshop_Mode_" + strMode)
      )

      if (i === 0) elButton.checked = true
  ...
  var _Apply = function () {
    ...
    var elSelectedButton = m_elButtons.find(function (elButton) {
      return elButton.checked
    })
    if (elSelectedButton)
      strGameMode = elSelectedButton.GetAttributeString(
        "data-mode",
        strGameMode
      )
