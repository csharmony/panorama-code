...
var TournamentStore = (function () {
  ...
  var _Init = function () {
    ...
    $("#TournamentTitle").SetLocalizationString(
      "#CSGO_Tournament_Event_NameShort_" + g_ActiveTournamentInfo.eventid
    )
    $("#TournamentInfo").SetPanelEvent("onactivate", function () {
      SteamOverlayAPI.OpenURL(
        "http://www.counter-strike.net/pickem/" +
    ...
    elOrgPanel
      .FindChildTraverse("CategoryImage")
      .SetImage(
        "file://{images_econ}/econ/stickers/" +
          g_ActiveTournamentInfo.location +
          "/" +
    ...
    elOrgPanel
      .FindChildTraverse("CategoryLabel")
      .SetLocalizationString("#CSGO_" + g_ActiveTournamentInfo.organization)
    ...
    if (sRestriction !== "restricted" && sRestriction !== "xray") {
      ...
      elCapsules.FindChildTraverse("CategoryImage").itemid =
        InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
          g_ActiveTournamentCapsules[0],
          0
        )
      elCapsules
        .FindChildTraverse("CategoryLabel")
        .SetLocalizationString("#CSGO_Store_Legends_Challengers")
      _ShowSaleTag(elCapsules, g_ActiveTournamentCapsules[0])
      SetItemsToOffer.apply(undefined, [
        elCapsules,
        ...g_ActiveTournamentCapsules
      ])
    }

    var passItemId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
    ...
    if (StoreAPI.GetStoreItemSalePrice(passItemId, 1, "")) {
      ...
      elPass
        ...
        .SetLocalizationString(
          "#CSGO_TournamentPass_" + g_ActiveTournamentInfo.location
        )
    ...
    function SetupTeamPurchases(elRootPanel, arrTeamList) {
      arrTeamList.forEach(function (teamInfo) {
        ...
        elTeamPanel
          .FindChildTraverse("CategoryImage")
          .SetImage(
            "file://{images}/tournaments/teams/" + teamInfo.team + ".svg"
          )
        elTeamPanel
          .FindChildTraverse("CategoryLabel")
          .SetLocalizationString("#CSGO_TeamID_" + teamInfo.teamid)
    ...
    SetupTeamPurchases(
      $("#Legends"),
      g_ActiveTournamentTeams.filter((team) => team.team_group === "legends")
    )
    SetupTeamPurchases(
      $("#Challengers"),
      g_ActiveTournamentTeams.filter(
        (team) => team.team_group === "challengers"
      )
    )
    SetupTeamPurchases(
      $("#Minors"),
      g_ActiveTournamentTeams.filter((team) => team.team_group === "contenders")
    )
  ...
  var OnItemPressed = function (ids) {
    var contextMenuPanel = UiToolkitAPI.ShowCustomLayoutContextMenuParameters(
      ...
      "items=" + ids.join(",") + "&item_count_func_name=funcNumItemsInCart"
    )
    contextMenuPanel.AddClass("ContextMenu_NoArrow")

    contextMenuPanel.FindChildTraverse(
      "ItemContainer"
    ).Data.funcNumItemsInCart = function (itemDefIdx) {
  ...
  function _ShowSaleTag(elPanel, storeDefIndx) {
    ...
    elPrecent.SetHasClass(
      "hidden",
      reduction === "" || reduction === undefined ? true : false
    )
  ...
  function RefreshCartCounts(itemDefIndex) {
    ...
    if (bHasItems) {
      m_ItemsToCategories[itemDefIndex].SetDialogVariableInt(
        "num_in_cart",
        itemCount
      )
    }
    m_ItemsToCategories[itemDefIndex].SetHasClass(
      m_StyleForCategoriesInCart,
      bHasItems
    )
    ...
    m_elTournamentCart.SetDialogVariable(
      "price",
      StoreAPI.GetStoreItemsSalePrice(purchaseItems)
    )
  }

  var GetMaxItems = function (itemDefIdx) {
    if (itemDefIdx == g_ActiveTournamentInfo.itemid_pass) return 5

    return 20
  }

  var _AddItemToCart = function (itemDefIndex) {
    if (GetTotalItemsInCart() >= m_MaxCartSize) return
  ...
  var _RemoveItemFromCart = function (itemDefIndex) {
    var prevCount = m_CartItems[itemDefIndex]
    if (
      typeof m_CartItems[itemDefIndex] === "number" &&
      m_CartItems[itemDefIndex] > 1
    )
      m_CartItems[itemDefIndex]--
    else delete m_CartItems[itemDefIndex]
  ...
  var _ShowTournamentStorePassPopup = function () {
    ...
    var elContextPanel = OnItemPressed([
      g_ActiveTournamentInfo.itemid_pass,
    ...
    $.Schedule(0.1, function () {
      elContextPanel.FindChildInLayoutFile("ContextMenuBody").style.transform =
        "translate3d( 0%, 0%, 0px );"
    })
  }
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    "RemoveItemFromCart",
    TournamentStore.RemoveItemFromCart
  )
  $.RegisterForUnhandledEvent(
    "ShowTournamentStorePassPopup",
    TournamentStore.ShowTournamentStorePassPopup
  )
})()
