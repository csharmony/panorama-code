...
var PopupTournamentControlRoom = (function () {
  ...
  var _SetTitle = function (type) {
    var titleOverride = $.GetContextPanel().GetAttributeString(
      "titleoverride",
      ""
    )
    var title = titleOverride !== "" ? titleOverride : "#CSGO_" + type
    $.GetContextPanel().FindChildInLayoutFile(
      "id-popup-tournamentcontrolroom-title"
    ).text = $.Localize(title)
  }
  ...
  var _MakeTabs = function (aTypes) {
    ...
    var elNavBar = $.GetContextPanel().FindChildInLayoutFile(
      "id-popup-tournamentcontrolroom-navbar"
    )
    ...
    for (var i = 0; i < aTypes.length; i++) {
      ...
      elTab.FindChildInLayoutFile("tournamentcontrolroom-tab-label").text =
        $.Localize("#CSGO_" + aTypes[i] + "_tab")
    }
  ...
  var _Updatetournamentcontrolroom = function (type) {
    ...
    var elStatus = m_cp.FindChildInLayoutFile(
      "id-popup-tournamentcontrolroom-loading"
    )
    var elData = m_cp.FindChildInLayoutFile(
      "id-popup-tournamentcontrolroom-nodata"
    )
    var eltournamentcontrolroomList = m_cp.FindChildInLayoutFile(
      "id-popup-tournamentcontrolroom-list"
    )
  ...
  var _FillOutEntries = function (type, count) {
    var elParent = m_cp.FindChildInLayoutFile(
      "id-popup-tournamentcontrolroom-entries"
    )
...

    {
      ...
      var elStageDropdown = el.FindChildInLayoutFile(
        "popup-tournamentcontrolroom-edit-type"
      )
      ...
      var elMapDropdown = el.FindChildInLayoutFile(
        "popup-tournamentcontrolroom-edit-map"
      )
      elMapDropdown.RemoveAllOptions()
      _AddDropdownOption(
        elMapDropdown,
        "PickMap",
        $.Localize("#SFUI_Tournament_Pick_Map_Title"),
        "",
        ""
      )
      ...
      for (var kk = 0; kk < 2; ++kk) {
        var elTeamDropdown = el.FindChildInLayoutFile(
          "popup-tournamentcontrolroom-edit-team" + kk
        )
        ...
        var teamCount =
          CompetitiveMatchAPI.GetTournamentTeamCount(strTournament)
      ...
      el.FindChildInLayoutFile("ButtonAdd").SetPanelEvent(
        "onactivate",
        function () {
          ...
          TournamentsAPI.RequestAddManagementMatch(
            m_eventid,
            "add",
    ...
    for (var i = 0; i < count; i++) {
      ...
      var fnCopyToClipboard = function (pwd) {
        ...
        UiToolkitAPI.ShowTextTooltip(
          "popup-tournamentcontrolroom-entry-pwd-copy",
      ...
      el.FindChildInLayoutFile("popup-tournamentcontrolroom-entry-pwd").text =
        jso.pwd
      el.FindChildInLayoutFile("popup-tournamentcontrolroom-entry-team0").text =
        jso.team0
      el.FindChildInLayoutFile("popup-tournamentcontrolroom-entry-team1").text =
        jso.team1
      el.FindChildInLayoutFile("popup-tournamentcontrolroom-entry-score").text =
        jso.info

      var strStatus = ""
      if (!jso.updates) {
        if (jso.flags & 1) strStatus = "#SFUI_Settings_Enabled"
        else strStatus = "#SFUI_Settings_Disabled"
      ...
      if (strStatus)
        el.FindChildInLayoutFile(
          "popup-tournamentcontrolroom-entry-updates"
        ).text = $.Localize(strStatus)

      var sValue = jso.stage
      sValue = sValue.replace(" | ", "\n")
      sValue = sValue.replace("|", "\n")
      el.FindChildInLayoutFile("popup-tournamentcontrolroom-entry-type").text =
        sValue
      el.FindChildInLayoutFile("popup-tournamentcontrolroom-entry-map").text =
        jso.map

      if (i % 2 === 0) el.AddClass("background")
      ...
      if (jso.updates === "final") {
        ...
        el.FindChildInLayoutFile(
          "popup-tournamentcontrolroom-entry-pwd-copy"
        ).SetPanelEvent("onactivate", fnCopyToClipboard.bind(null, jso.pwd))

        el.FindChildInLayoutFile(
          "popup-tournamentcontrolroom-entry-play"
        ).visible = false
        el.FindChildInLayoutFile(
          "popup-tournamentcontrolroom-entry-pause"
        ).SetPanelEvent("onactivate", fnSetFlags.bind(null, jso.id, "0"))
      } else {
        el.FindChildInLayoutFile(
          "popup-tournamentcontrolroom-entry-pwd-copy"
        ).visible = false
        ...
        el.FindChildInLayoutFile(
          "popup-tournamentcontrolroom-entry-play"
        ).SetPanelEvent("onactivate", fnSetFlags.bind(null, jso.id, "1"))
      }
    }
  }
...
;(function () {
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Tournaments_ManagementMatchListReceived",
    PopupTournamentControlRoom.ManagementMatchListReceived
  )
})()
