...
var TournamentJournal = (function () {
  ...
  var m_isInMatch =
    GameStateAPI.IsDemoOrHltv() || GameStateAPI.IsLocalPlayerPlayingMatch()

  var _Init = function () {
    var journalId = $.GetContextPanel().GetAttributeString("journalid", "")
    var tournamentId = InventoryAPI.GetItemAttributeValue(
      journalId,
      "tournament event id"
    )
    ...
    m_tokenItemDefName =
      "tournament_pass_" + g_ActiveTournamentInfo.location + "_charge"

    var nCampaignID = parseInt(
      InventoryAPI.GetItemAttributeValue(journalId, "campaign id")
    )

    var numTotalChallenges = InventoryAPI.GetCampaignNodeCount(nCampaignID)

    for (var jj = 0; jj < numTotalChallenges; ++jj) {
      var nMissionNodeID = InventoryAPI.GetCampaignNodeIDbyIndex(
        nCampaignID,
        jj
      )

      var strNodeState = InventoryAPI.GetCampaignNodeState(
        nCampaignID,
        nMissionNodeID,
        journalId
      )

      var nQuestID = InventoryAPI.GetCampaignNodeQuestID(
        nCampaignID,
        nMissionNodeID
      )
    ...
    var oWinningTeam = !_IsValidJournalId(journalId)
      ? null
      : EventUtil.GetTournamentWinner(tournamentId, 1)
    m_isTournament_active =
      !oWinningTeam || !oWinningTeam.hasOwnProperty("team_id") ? true : false
    ...
    _UpdateSetChargesBtn(
      m_activeTournament === tournamentId
  ...
  var GetPassToActivate = function () {
    ...
    var id = InventoryAPI.GetActiveTournamentCoinItemId(
      g_ActiveTournamentInfo.eventid * -1
    )
    return !_IsValidJournalId(journalId) && id && id !== "0" ? id : ""
  }

  var _UpdateActivateBtn = function () {
    var activateBtn = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-active"
    )

    var id = GetPassToActivate()
    activateBtn.SetPanelEvent("onactivate", _ActivatePass.bind(undefined, id))
  ...
  var _SetBackgroundMovie = function (id) {
    if (id > 15) {
      $.GetContextPanel().FindChildInLayoutFile(
        "id-tournament-journal-container"
      ).style.backgroundImage =
        "url( 'file://{images}/tournaments/backgrounds/background_" +
        id +
        ".png' );"
    }
  }
  ...
  var _SetTitle = function (journalId) {
    $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-title"
    ).text = !_IsValidJournalId(journalId)
      ? $.Localize(
          "CSGO_TournamentPass_" +
            g_ActiveTournamentInfo.location +
            "_store_title"
        )
  ...
  var _SetSubtitle = function (journalId) {
    var srtText = !_IsValidJournalId(journalId)
      ? "#CSGO_TournamentPass_" +
        g_ActiveTournamentInfo.location +
        "_store_desc"
      : "#tournament_coin_desc_token"
    $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-subtitle"
    ).text = $.Localize(srtText)
  }

  var _SetModel = function (id) {
    var fakeId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
      g_ActiveTournamentInfo.itemid_coins[
        g_ActiveTournamentInfo.itemid_coins.length - 1
      ],
      0
    )
    id = !_IsValidJournalId(id) ? fakeId : id

    var elModel = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-model"
    )
  ...
  var _SetBannerColor = function (journalId) {
    var coinLevel = InventoryAPI.GetItemAttributeValue(
      journalId,
      "upgrade level"
    )
      ...
      coinLevel < 1
        ...
        : coinLevel === 1
          ...
          : coinLevel > 1
            ...
            : "bronze"
    $.GetContextPanel()
      .FindChildInLayoutFile("id-tournament-journal-container")
      .AddClass(style)
  }

  var _SetThesholdText = function (journalId, tournamentId) {
    var threshold = InventoryAPI.GetItemAttributeValue(
      journalId,
      "upgrade threshold"
    )
    var completedChallenges = m_test_challenges.filter(function (entry) {
    ...
    if (
      !threshold ||
      completedChallenges.length === m_test_challenges.length ||
      !m_isTournament_active
    ) {
      $.GetContextPanel().FindChildInLayoutFile(
        "id-tournament-journal-remaining"
      ).visible = false
      return
    }
    ...
    $.GetContextPanel().FindChildInLayoutFile(
      ...
      tournamentId < 16
        ? $.Localize(
            "#tournament_coin_remaining_challenges",
            $.GetContextPanel()
          )
        : $.Localize(
  ...
  var _SouvenirsEarned = function (journalId, tournamentId) {
    ...
    $.GetContextPanel().SetDialogVariableInt(
      "redeems_remain",
      redeemsAvailable !== undefined ? redeemsAvailable : 0
    )

    var elLabel = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-redeem-btn-label"
    )
    ...
    elLabel = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-souvenir-earned"
    )
    ...
    elLabel.text =
      redeemsEarned === 1
        ...
        : $.Localize(
            "#tournament_coin_earned_" + locStringModifier + "_multi",
            elLabel
          )

    $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-info-block-subtitle"
    ).text =
      tournamentId < 16
  ...
  var _SouvenirsImage = function (tournamentId) {
    var elImage = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-ticket-icon"
    )
    if (tournamentId < 16) {
      elImage.SetImage(
        "file://{images}/tournaments/souvenir/souvenir_blank_tournament_" +
          tournamentId +
          ".png"
      )
    } else {
      elImage.itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
        g_ActiveTournamentInfo.itemid_charge,
        0
      )
    }
  ...
  var _RedeemBtn = function (redeemsAvailable) {
    var elbtn = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-redeem-btn"
    )
  ...
  var _AnimRedeemValue = function () {
    var elbtn = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-souvenir-earned-value"
    )
  ...
  var _SetEventSticker = function (tournamentId) {
    var elTitleBar = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-title-bar"
    )

    $.CreatePanel("Image", elTitleBar, "", {
      texturewidth: "98",
      textureheight: "-1",
      src:
  ...
  var _UpdateChallengesList = function (journalId, tournamentId) {
    var elList = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal__challenges-list"
    )
    if (!_IsValidJournalId(journalId)) {
      return
    }
    ...
    m_test_challenges.forEach(function (obj, index) {
      var elChallenge = $.GetContextPanel().FindChildInLayoutFile(
        "id-tournament-journal__challenges" + index
      )
    ...
    function _CreateChallenge(objData, elList, index) {
      ...
      $.GetContextPanel()
        .FindChildInLayoutFile("id-tournament-journal-container")
        .AddBlurPanel(elChallenge)

      return elChallenge
    }

    function _UpdateChallenge(objData, elChallenge, index, tournamentId) {
      var elName = elChallenge.FindChildInLayoutFile(
        "id-tournament-journal__challenge__name"
      )
      var elIcon = elChallenge.FindChildInLayoutFile(
        "id-tournament-journal__challenge__icon"
      )
      elName.text = objData.text

      var iconPath =
        objData.value === 1
          ? "file://{images}/icons/ui/check.svg"
    ...
    function _OnActivatePlayChallenge(elPanel, index) {
      ...
      elPanel.SetPanelEvent(
        "onactivate",
        OnActivate.bind(undefined, "mg_" + pickedMap)
      )
      ...
      function OnActivate(mapGroup) {
        ...
        if (!LobbyAPI.IsSessionActive()) return
  ...
  var _OnWatchStream = function () {
    var elbtn = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-watch-stream"
    )
  ...
  var _SetUpSpray = function (journalId) {
    ...
    var journalId = $.GetContextPanel().GetAttributeString("journalid", "")
    var elImage = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-spray"
    )
    elImage.itemid = ItemInfo.GetFauxReplacementItemID(journalId, "graffiti")

    var elIBtn = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-selectspray-btn"
    )
  ...
  var _SetWinners = function (ObjWinner, tournamentId, journalId) {
    ...
    elPanel.visible = true
    $.GetContextPanel()
      .FindChildInLayoutFile("tournament-journal-winners-img")
      .SetImage(
        "file://{images}/tournaments/teams/" + ObjWinner["tag"] + ".svg"
      )
    $.GetContextPanel().FindChildInLayoutFile(
      "tournament-journal-winners-desc"
    ).text = $.Localize("Place_Name_1st")
    $.GetContextPanel().FindChildInLayoutFile(
      "tournament-journal-winners-title"
    ).text = $.Localize("CSGO_TeamID_" + ObjWinner["team_id"])

    var elModel = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-trophy"
    )
    elModel.SetCameraPreset(1, false)
  ...
  var _SetPlayerPhotos = function (ObjWinner, tournamentId) {
    ...
    var elPlayerContainer = $.GetContextPanel().FindChildInLayoutFile(
      "tournament-journal-winners-players"
    )
    ...
    arrPlayers.forEach(function (player) {
      ...
      let elPlayerImage = elPlayer.FindChildTraverse(
        "id-tournament-journal-player-photo"
      )
      if (elPlayerImage) {
        var tournament_event = "tournament:" + tournamentId.toString()
        var StickerID = PredictionsAPI.GetFakeItemIDToRepresentPlayerID(
          tournament_event,
          player[1]["accountid64"]
        )

        let photo_url =
          "file://{images}/tournaments/avatars/" +
          tournamentId +
          "/" +
          player[1]["accountid64"] +
          ".png"
        ...
        elPlayer.FindChildInLayoutFile(
          "id-tournament-journal-player-sticker"
        ).itemid = StickerID
      }
    })
  }
  ...
  var _UpdateSetChargesBtn = function (id) {
    ...
    if (
      ...
      id !== "0" &&
      id !== "-1" &&
      ItemInfo.GetStoreSalePrice(id, 1, "")
    ) {
      btn.visible = true
  ...
  var _UpdateApplyCharges = function () {
    var btn = $.GetContextPanel().FindChildInLayoutFile(
      "id-activate-charges-btn"
    )
    $.GetContextPanel().FindChildInLayoutFile("id-activate-spinner").visible =
      false
    btn.visible = false

    if (!m_tokenItemDefName) return

    InventoryAPI.SetInventorySortAndFilters(
      "inv_sort_age",
      false,
      "item_definition:" + m_tokenItemDefName,
      "",
      ""
    )
  ...
  var _OnItemCustomization = function (numericType, type, itemid) {
    if (type === "ticket_activated") {
      ...
      $.Schedule(
        0.25,
        _SouvenirsEarned.bind(undefined, journalId, tournamentId)
      )
    }
  }
  ...
  var _SetFaqBtn = function (tournamentId, bNoItemsOnSale) {
    ...
    btn.SetPanelEvent("onactivate", function () {
      SteamOverlayAPI.OpenURL(
        "https://store.steampowered.com/sale/csgo" +
          g_ActiveTournamentInfo.location
      )
    })
  ...
  var _UpdateStoreItems = function (bNoItemsOnSale) {
    ...
    var elItemsPanel = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-items"
    )
    var elBlurPanel = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-journal-container"
    )
    ...
    var fnCreateOffering = function (ids, i) {
      if (
        !bCanSellCapsules &&
        i >= g_ActiveTournamentInfo.num_global_offerings
      ) {
        return
      }
      ...
      if (!elItemsPanel.FindChildInLayoutFile("tournament-items" + i)) {
        var itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
          g_ActiveTournamentStoreLayout[i][0],
          0
        )
        var bContainsJustChampions =
          typeof g_ActiveTournamentStoreLayout[i][1] === "string"
        var linkedid = bContainsJustChampions
          ? itemid
          : InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
              g_ActiveTournamentStoreLayout[i][1],
              0
            )
        ...
        elItem.Data().oData = {
          ...
          warningtext: !bIsPurchaseable
            ? "#tournament_items_not_released" +
              (bContainsJustChampions ? "_1" : "")
            : InventoryAPI.GetItemTypeFromEnum(itemid) === "type_tool"
        ...
        var defidxChampionHighlight = [
          g_ActiveTournamentStoreLayout[
            g_ActiveTournamentStoreLayout.length - 1
          ][0]
        ]
  ...
  var _ActivatePass = function (id) {
    UiToolkitAPI.ShowCustomLayoutPopupParameters(
      ...
      "key-and-case=," + id + "&" + "asyncworktype=decodeable"
    )
...
;(function () {
  $.RegisterForUnhandledEvent(
    ...
    TournamentJournal.OnInventoryUpdated
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Inventory_ItemCustomizationNotification",
    TournamentJournal.OnItemCustomization
  )
})()
