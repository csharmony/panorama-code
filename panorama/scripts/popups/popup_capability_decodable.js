...
var CapabilityDecodable = (function () {
  ...
  var _Init = function () {
    ...
    m_isXrayMode =
      $.GetContextPanel().GetAttributeString("isxraymode", "no") === "yes"
        ? true
        : false
    m_isAllowedToInteractWithLootlistItems =
      $.GetContextPanel().GetAttributeString(
        "allowtointeractwithlootlistitems",
        "true"
      ) === "true"
        ? true
        : false
    m_blurOperationPanel =
      $.GetContextPanel().GetAttributeString("bluroperationpanel", "false") ===
      "true"
        ? true
        : false
      ...
      if (m_existingRewardFromXrayId) {
        if (m_existingRewardFromXrayId) {
          if (InventoryAPI.IsFauxItemID(m_existingRewardFromXrayId)) {
            var elPopup = UiToolkitAPI.ShowGenericPopupOk(
              "#popup_xray_first_use_title",
              "#popup_xray_first_use_desc",
              "",
              function () {}
            )
            elPopup.FindChildInLayoutFile("MessageLabel").html = true
            var id = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
              m_existingRewardFromXrayId,
              0
            )
            elPopup.SetDialogVariable(
              "itemname",
              ItemInfo.GetName(m_existingRewardFromXrayId)
            )
            elPopup.FindChildInLayoutFile("MessageLabel").text = $.Localize(
              "#popup_xray_first_use_desc",
              elPopup
            )
          } else if (
            $.GetContextPanel().GetAttributeString("showxraypopup", "no") ===
            "yes"
          ) {
            UiToolkitAPI.ShowGenericPopupOk(
              "#popup_xray_in_use_title",
              "#popup_xray_in_use_desc",
              "",
              function () {}
            )
          }
    ...
    m_styleforPopUpInspectFullScreenHostContainer =
      $.GetContextPanel().GetAttributeString("extrapopupfullscreenstyle", "")
    if (m_styleforPopUpInspectFullScreenHostContainer) {
      var elPopUpInspectFullScreenHostContainer =
        $.GetContextPanel().FindChildInLayoutFile(
          "PopUpInspectFullScreenHostContainer"
        )
      elPopUpInspectFullScreenHostContainer.AddClass(
        m_styleforPopUpInspectFullScreenHostContainer
      )
    }
    ...
    if (!m_keyId) {
      ...
      if (
        (associatedItemCount === 0 || !associatedItemCount) &&
        !m_storeItemId
      ) {
        m_isKeyless = true
  ...
  var _SetUpPanelElements = function () {
    ...
    var sRestriction = m_storeItemId
      ? ""
      : InventoryAPI.GetDecodeableRestriction(m_caseId)

    if (
      (sRestriction !== "restricted" && sRestriction !== "xray") ||
      (m_isXrayMode && sRestriction === "xray")
    ) {
    ...
    if (m_isXrayMode) {
      ...
      if (
        !ItemInfo.ItemMatchDefName(m_caseId, "spray") &&
        !ItemInfo.ItemDefinitionNameSubstrMatch(m_caseId, "tournament_pass_")
      ) {
  ...
  var _SetupHeader = function (caseId) {
    var elCapabilityHeaderPanel = $.GetContextPanel().FindChildInLayoutFile(
      "PopUpCapabilityHeader"
    )
  ...
  var _SetUpAsyncActionBar = function (itemId) {
    var elAsyncActionBarPanel = $.GetContextPanel().FindChildInLayoutFile(
      "PopUpInspectAsyncBar"
    )
  ...
  var _ShowPurchase = function (m_keytoSellId) {
    var elPurchase = $.GetContextPanel().FindChildInLayoutFile(
      "PopUpInspectPurchaseBar"
    )
  ...
  var _SetLootListItems = function (caseId, keyId) {
    var count = ItemInfo.GetLootListCount(caseId)
    var elLootList =
      $.GetContextPanel().FindChildInLayoutFile("DecodableLootlist")
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (!elItem) {
        ...
        var funcActivation = m_isAllowedToInteractWithLootlistItems
          ? _OnActivateLootlistTile
          : _OnActivateLootlistTileDummy
        elItem.SetPanelEvent(
          "onactivate",
          funcActivation.bind(undefined, itemid, caseId, keyId)
        )
        elItem.SetPanelEvent(
          "oncontextmenu",
          funcActivation.bind(undefined, itemid, caseId, keyId)
        ...
        if (i === 0 && m_isAllowedToInteractWithLootlistItems) {
          $.GetContextPanel()
            .FindChildInLayoutFile("CanDecodableBrowseBtn")
            .SetPanelEvent(
              "onactivate",
              callBackFunc.bind(undefined, itemid, caseId, keyId)
            )
        }
  ...
  var _OnActivateLootlistTile = function (itemid, caseId, keyId) {
    if (!InventoryAPI.IsValidItemID(itemid)) return
    ...
    items.push({
      label: "#UI_Inspect",
      jsCallback: callBackFunc.bind(undefined, itemid, caseId, keyId)
    })
  ...
  var callBackFunc = function (itemid, caseId, keyId) {
    ...
    var bluroperationpanel = m_blurOperationPanel
      ? "bluroperationpanel=true"
      : ""

    var additionalParams =
      _GetSettingCallback("inspectonly", "false") === "true"
        ? "inspectonly=true,"
        : ""
    additionalParams =
      _GetSettingCallback("asyncworkbtnstyle", "positive") === "hidden"
        ? additionalParams + "asyncworkbtnstyle=hidden"
        : ""
    additionalParams = m_blurOperationPanel
      ? additionalParams + "," + "bluroperationpanel=true"
      : ""
  ...
  var _UpdateLootListItemInfo = function (elItem, itemid, caseId) {
    if (itemid == "0") {
      m_unusualItemImagePath =
        InventoryAPI.GetLootListUnusualItemImage(caseId) + ".png"
  ...
  var _ShowHideLootList = function (bshow) {
    var elLootListContainer = $.GetContextPanel().FindChildInLayoutFile(
      "DecodableLootlistContainer"
    )
  ...
  var _SetLootlistHintText = function (caseId, count) {
    ...
    $.GetContextPanel().FindChildInLayoutFile("CanDecodableDesc").visible =
      !bAllItems

    if (count > 1 || bAllItems) {
      $.GetContextPanel()
        .FindChildInLayoutFile("CanDecodableDescMulti")
        .SetDialogVariableInt("num_items", count)
      $.GetContextPanel().FindChildInLayoutFile(
        "CanDecodableDescMulti"
      ).visible = count > 1 && bAllItems
    }
  }

  var _UpdateUnusualItemInfo = function (elItem, caseId, unusualItemImagePath) {
    elItem
      .FindChildInLayoutFile("ItemImage")
      .SetImage("file://{images_econ}/" + unusualItemImagePath)
    elItem
      .FindChildInLayoutFile("JsRarity")
      .AddClass("popup-decodable-wash-color-unusual")
  ...
  var _ShowScroll = function (elCase) {
    var elScroll = $.GetContextPanel().FindChildInLayoutFile(
      "DecodableItemsScroll"
    )
  ...
  var _PlayScrollAnim = function (scrolllists) {
    ...
    var xOffsetSlackPercent =
      Math.floor(Math.random() * (90 - 10 + 1) + 10) / 100

    scrolllists.forEach((element) => {
      var xPos = _GetStopPostion(
        $.GetContextPanel().FindChildInLayoutFile(element),
        targetId,
        xOffsetSlackPercent
      )
    ...
    for (var i = 0; i < _TickSoundIntervals.length; ++i) {
      $.Schedule(
        _TickSoundIntervals[i],
        _ScrollTick.bind(undefined, soundEventName)
      )
    }
  }

  var _TickSoundIntervals = [
    0.0, 0.063, 0.125, 0.188, 0.25, 0.313, 0.375, 0.438, 0.5, 0.563, 0.625,
    0.688, 0.75, 0.813, 0.875, 0.938, 1.0, 1.063, 1.125, 1.188, 1.25, 1.313,
    1.375, 1.483, 1.351, 1.62, 1.701, 1.786, 1.872, 2.003, 2.154, 2.313, 2.466,
    2.615, 2.773, 2.941, 3.104, 3.339, 3.63, 3.953, 4.385, 5.004
  ]
  ...
  var _ShowInspect = function () {
    ...
    if (m_itemFromContainer) {
      InventoryAPI.SetItemSessionPropertyValue(
        m_itemFromContainer,
        "recent",
        "1"
      )
      ...
      if (
        ItemInfo.ItemDefinitionNameSubstrMatch(
          m_itemFromContainer,
          "tournament_journal_"
        )
      ) {
  ...
  var _FillScrollsWithItems = function (lists) {
    ...
    for (var i = 0; i < numTilesInScroll; i++) {
      var itemToAdd = GetItemBasedOnDisplayWeight(
        totalWeight,
        m_aItemsInLootlist
      )
  ...
  var _UpdateScrollTile = function (listId, elTile, itemId) {
    ...
    itemId =
      elTile.id === "ItemFromContainer" && m_itemFromContainer
        ? m_itemFromContainer
        : itemId
    ...
    if (InventoryAPI.IsItemUnusual(itemId) && m_unusualItemImagePath) {
      ...
      elTile.FindChildInLayoutFile("JsRarity").style.washColor =
        ItemInfo.GetRarityColor(itemId)
      elTile.FindChildInLayoutFile("JItemTint").style.washColor =
        ItemInfo.GetRarityColor(itemId)
    }
  }
  ...
  var _SetUpCaseOpeningCountdown = function () {
    _UpdateOpeningCounter.SetIsGraffiti(
      _GetContainerType(m_caseId) === "graffiti"
    )
  ...
  var _UpdateOpeningCounter = (function () {
    var counterVal = 6
    var elCountdown =
      $.GetContextPanel().FindChildInLayoutFile("DecodableCountdown")
    var elCountdownLabel = elCountdown.FindChildInLayoutFile(
      "DecodableCountdownLabel"
    )
    var elCountdownRadial = elCountdown.FindChildInLayoutFile(
      "DecodableCountdownRadial"
    )
  ...
  var _SetUpXrayPanel = function () {
    ...
    var elActionsPanel = $.GetContextPanel().FindChildInLayoutFile(
      "XrayItemsActionPanel"
    )
    ...
    if (!m_existingRewardFromXrayId) {
      ...
      $.GetContextPanel().FindChildInLayoutFile("PopUpXrayStatusLabel").text =
        $.Localize("#popup_xray_ready_for_use")
    } else if (m_existingRewardFromXrayId) {
      var elHeaderPanel =
        $.GetContextPanel().FindChildInLayoutFile("PopUpInspectHeader")
      InspectHeader.Init(
        elHeaderPanel,
        m_existingRewardFromXrayId,
        _GetSettingCallback
      ...
      $.GetContextPanel()
        .FindChildInLayoutFile("XrayItemsActionPanelItemName")
        .RemoveClass("hidden")
      var elImagePanel = $.GetContextPanel().FindChildInLayoutFile(
        "PopUpXrayModelOrImageReveal"
      )
      ...
      $.GetContextPanel().FindChildInLayoutFile("PopUpXrayStatusLabel").text =
        $.Localize("#popup_xray_already_in_use")
      $.GetContextPanel()
        .FindChildInLayoutFile("PopUpXrayStatusDot")
        .AddClass("in-use")
    }

    var elXrayPanel =
      $.GetContextPanel().FindChildInLayoutFile("XrayItemsPanel")
    elXrayPanel.RemoveClass("hidden")

    var aPanels = $.GetContextPanel()
      .FindChildInLayoutFile("PopUpXrayBgSquares")
      .Children()
  ...
  var _XrayReveal = function () {
    ...
    var oData = {
      clipValue: 0,
      lineValue: 100,
      clipPanel: $.GetContextPanel().FindChildInLayoutFile(
        "PopUpXrayModelOrImage"
      ),
      linePanel: $.GetContextPanel().FindChildInLayoutFile(
        "PopUpXrayModelOrImageRevealLine"
      )
    }

    oData.clipPanel.AddClass("popup-xray-inverse-effect")
    $.GetContextPanel()
      .FindChildInLayoutFile("PopUpXrayModelOrImageReveal")
      .AddClass("popup-xray-reverse-effect")
  ...
  var _AnimClip = function (oData) {
    if (oData.clipValue <= 100) {
      oData.clipPanel.style.clip =
        "rect( 0%, 100%, 100%, " + oData.clipValue + "% );"
      oData.clipValue = oData.clipValue + 1

      oData.linePanel.style.transform =
        "translatex( -" + oData.lineValue + "%);"
    ...
    if ($.GetContextPanel().FindChildInLayoutFile("XrayItemsPanel").visible) {
      aPanels.forEach((panel) => {
        panel.style.backgroundColor =
          "rgba(255, 255, 255, 0.0" + Math.ceil(Math.random() * 10) + ");"
      })
  ...
  var _UpdateScrollResultTile = function (numericType, type, itemId) {
    if (
      ...
      if (
        $.GetContextPanel()
          .FindChildInLayoutFile("DecodableItemsScroll")
          .BHasClass("hidden")
      ) {
  ...
  var _PlayContainerSound = function (caseId, soundName) {
    $.DispatchEvent(
      "PlaySoundEffect",
      "container_" + _GetContainerType(caseId) + "_" + soundName,
      "MOUSE"
    )
  }
  ...
  var _GetContainerType = function (caseId) {
    ...
    if (
      itemDefName &&
      (itemDefName.indexOf("spray") != -1 ||
        itemDefName.indexOf("tournament_pass_") != -1)
    ) {
      return "graffiti"
  ...
  var _ClosePopUp = function () {
    ...
    if (m_Inspectpanel.IsValid()) {
      ...
      var elAsyncActionBarPanel = m_Inspectpanel.FindChildInLayoutFile(
        "PopUpInspectAsyncBar"
      )
      var elPurchase = m_Inspectpanel.FindChildInLayoutFile(
        "PopUpInspectPurchaseBar"
      )
...
;(function () {
  ...
  if (!_m_PanelRegisteredForEvents) {
    _m_PanelRegisteredForEvents = $.RegisterForUnhandledEvent(
      ...
      CapabilityDecodable.UpdateScrollResultTile
    )
    $.RegisterForUnhandledEvent(
      "PanoramaComponent_Store_PurchaseCompleted",
      CapabilityDecodable.ItemAcquired
    )
    $.RegisterForUnhandledEvent(
      "StartDecodeableAnim",
      CapabilityDecodable.ShowUnlockAnimation
    )
  }
})()
