...
var InpsectPurchaseBar = (function () {
  ...
  var _Init = function (elPanel, itemId, funcGetSettingCallback) {
    ...
    m_bOverridePurchaseMultiple =
      funcGetSettingCallback("overridepurchasemultiple", "") === "1"
        ? true
        : false
    ...
    var priceOriginal = bFauxItemIdForPurchase
      ? ItemInfo.GetStoreOriginalPrice(m_itemid, 1)
      : ""
    ...
    m_allowXrayPurchase =
      funcGetSettingCallback("allowxraypurchase", "no") === "yes" ? true : false
    m_showToolUpsell =
      funcGetSettingCallback("toolid", "") === "" ? true : false
  ...
  var _SetPurchaseImage = function (elPanel, itemId) {
    ...
    elImage.SetHasClass(
      ...
      m_isXrayMode && !m_allowXrayPurchase
    )
  }
  ...
  var _UpdateDecString = function (elPanel) {
    ...
    if (m_isXrayMode) {
      elPanel.SetDialogVariable(
        "itemprice",
        ItemInfo.GetStoreSalePrice(m_itemid, 1)
      )
  ...
  var _UpdatePurchasePrice = function () {
    ...
    var bCanShowQuantityDropdown =
      !m_isXrayMode && _isAllowedToPurchaseMultiple()
  ...
  var _isAllowedToPurchaseMultiple = function () {
    ...
    if (attValue) return false

    var defName = InventoryAPI.GetItemDefinitionName(m_itemid)
    if (defName === "casket") return false

    return true
  }

  var _SetUpPurchaseBtn = function (elPanel) {
    elPanel.FindChildInLayoutFile("PurchaseBtn").enabled =
      !m_isXrayMode || (m_isXrayMode && m_allowXrayPurchase)
    elPanel
      .FindChildInLayoutFile("PurchaseBtn")
      .SetPanelEvent("onactivate", _OnActivate)
  }
  ...
  var _OnActivate = function () {
    ...
    if (
      itemDefitionNameString &&
      itemDefitionNameString.startsWith("coupon - crate_patch_") &&
