...
var AcknowledgeItems = (function () {
  ...
  var _Init = function () {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      elDelayLoadPanel.SetLoadFunction(
        _MakeItemPanel.bind(null, items[i], i, numItems)
      )
  ...
  var _MakeItemPanel = function (item, index, numItems, elParent) {
    ...
    if (defName === "casket" && item.type === "nametag_add") {
      ...
      var typeWithoutParams =
        idxOfExtraParams > 0
          ? titleSuffex.substring(0, idxOfExtraParams)
          : titleSuffex
  ...
  var _ShowModelOrItem = function (elItemTile, id, type = "") {
    ...
    if (modelPath) {
      ...
      if (type && typeof type === "string" && type.startsWith("patch_apply[")) {
        ...
        CapabilityCanSticker.UpdatePreviewPanelSettingsForPatchPosition(
          id,
          settings,
          slot
        )
        CharacterAnims.PlayAnimsOnPanel(settings)
        CapabilityCanSticker.UpdatePreviewPanelCameraAndLightingForPatch(
          elModel,
          id,
          slot
        )
        ...
        elModel.SetCameraPosition(305.6, -7.43, 85.19)
  ...
  var _ShowQuestPanel = function (elItemTile, id) {
    ...
    elPanel.SetHasClass(
      "hidden",
      "quest_reward" !== ItemInfo.GetItemPickUpMethod(id)
    )
  ...
  var _ShowSetPanel = function (elItemTile, id) {
    ...
    elImage.SetImage(
      "file://{images_econ}/econ/set_icons/" + strSetName + "_small.png"
    )
  ...
  var _GetItems = function () {
    ...
    if (
      getUpdateItem &&
      newItems.filter((item) => item.id === getUpdateItem.id).length < 1
    ) {
      newItems.push(getUpdateItem)
    }

    var rewardItems = newItems.filter(
      (item) => item.pickuptype === "quest_reward"
    )
    var otherItems = newItems.filter(
      (item) => item.pickuptype !== "quest_reward"
    )
  ...
  var _GetItemsByType = function (afilters, bShouldAcknowledgeItems) {
    ...
    alist.forEach(function (oItem) {
      aOnlyIds.push(oItem.id)
    })
    return aOnlyIds
  }

  var _GetUpdatedItem = function () {
    var itemidExplicitAcknowledge = $.GetContextPanel().GetAttributeString(
      "ackitemid",
      ""
    )
    if (itemidExplicitAcknowledge === "") return null
  ...
  var _AcknowledgeAllItems = (function () {
    ...
    var _AcknowledgeItems = function () {
      itemsToSave.forEach(function (item) {
        InventoryAPI.SetItemSessionPropertyValue(
          item.id,
          "item_pickup_method",
          ItemInfo.GetItemPickUpMethod(item.id)
        )
    ...
    var _OnActivate = function () {
      ...
      var callbackResetAcknowlegePopupHandle =
        $.GetContextPanel().GetAttributeInt("callback", -1)
