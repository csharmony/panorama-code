...
function SetupPopup() {
  ...
  $.GetContextPanel().SetDialogVariable(
    "title",
    $.LocalizeSafe(strToken + "_title")
  )
  $.GetContextPanel().SetDialogVariable(
    "desc",
    $.LocalizeSafe(strToken + "_desc")
  )

  var currentValue = $.GetContextPanel().GetAttributeString("currentvalue", "")

  var elOptionsContainer = $.GetContextPanel().FindChildTraverse(
    "RadioOptionsContainer"
  )

  function _CreateOption(value, strToken) {
    var elOption = $.CreatePanel(
      "RadioButton",
      elOptionsContainer,
      "id-" + value
    )
    ...
    elOption.SetDialogVariable(
      "optiontextdesc",
      $.Localize(strToken + "_" + value + "_desc")
    )
    elOption.SetPanelEvent(
      "onactivate",
      _OnRadioButtonPressed.bind(undefined, value)
    )
...
function _Return(bProceed) {
  ...
  var cancelCallbackHandle = $.GetContextPanel().GetAttributeInt(
    "cancelcallback",
    -1
  )
  ...
  if (callbackHandle != -1) {
    var resumeMatchmakingHandle = $.GetContextPanel().GetAttributeString(
      "searchfn",
      ""
    )
  ...
  if (callbackHandle != -1) UiToolkitAPI.UnregisterJSCallback(callbackHandle)
