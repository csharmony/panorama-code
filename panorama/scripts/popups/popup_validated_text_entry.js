...
var ValidatedTextEntry = (function () {
  ...
  function _Init() {
    ...
    m_validatorFn = parseInt(
      $.GetContextPanel().GetAttributeInt("validateCallback", -1)
    )
    m_submitFn = parseInt(
      $.GetContextPanel().GetAttributeInt("submitCallback", -1)
    )
    m_cancelFn = parseInt(
      $.GetContextPanel().GetAttributeInt("cancelCallback", -1)
    )
  ...
  function _UnregisterAllCallbacks() {
    if (m_validatorFn != -1) UiToolkitAPI.UnregisterJSCallback(m_validatorFn)
