...
function _InterruptToGetGameModeFlags(mode) {
  function _Callback(value, resumeMatchmakingHandle = "") {
    GameInterfaceAPI.SetSettingString(
      "ui_playsettings_flags_official" + "_" + mode,
      value
    )
...
function LaunchMission() {
  var nRequestedMissonCardId = $.GetContextPanel().GetAttributeInt(
  ...
  var iActiveMissionCard =
    MissionsAPI.GetSeasonalOperationMissionCardActiveIdx(nSeasonAccess)

  if (
    iActiveMissionCard < 0 ||
    MissionsAPI.GetSeasonalOperationMissionCardDetails(
      nSeasonAccess,
      iActiveMissionCard
    ).id !== nRequestedMissonCardId
  ) {
    MissionsAPI.ActionRequestSeasonalOperationMissionCardID(
      nSeasonAccess,
      nRequestedMissonCardId
    )
  } else {
    _CancelMissionActivateTimer()

    var bOnlyActivateMission =
      $.GetContextPanel().GetAttributeString("activateonly", "false") === "true"
        ? true
        : false
    ...
    if (!EnsureHostAndMatchmakingStoppedIfNeeded()) return

    var gameMode = InventoryAPI.GetQuestGameMode(QuestItemID)
    var gameType = ""
    var mapGroup = InventoryAPI.GetQuestMapGroup(QuestItemID)
    if (gameMode === "survival") {
      mapGroup = GameInterfaceAPI.GetSettingString(
        "ui_playsettings_maps_official_" + gameMode
      )
    }
    if (!mapGroup) {
    ...
    var gameModeFlags = GameInterfaceAPI.GetSettingString(
      "ui_playsettings_flags_official_" + gameMode
    )
    ...
    if (LobbyAPI.IsSessionActive()) {
      ...
      if (gameType === "cooperative") {
        settings.update.Game.questid = InventoryAPI.GetItemAttributeValue(
          QuestItemID,
          "quest id"
        )
      }
      ...
      if (gameType === "cooperative" && PartyListAPI.GetCount() <= 1) {
        ...
        PartyListAPI.SessionCommand(
          "Game::ChatReportGreen",
          "run local green #SFUI_QMM_ERROR_PartyRequired2"
        )
