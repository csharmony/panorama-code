...
var PopupTournamentPickBan = (function () {
  ...
  var _DraftUpdate = function () {
    ...
    if (numStartTeamID == undefined) return
  ...
  var _SetUpInitalVotePanel = function (numStartTeamID) {
    ...
    if (isPicker) {
      ...
      var strLoc =
        numMapsCount == 1
          ? "#SFUI_InitalPick_Maps_Picks_One"
          : "#SFUI_InitalPick_Maps_Picks_Three"
  ...
  var _ChooseTypeOfLayout = function () {
    ...
    var allCompetitiveMaps = GameTypesAPI.GetMapGroupAttributeSubKeys(
      "mg_active",
      "maps"
    )
    ...
    function SetTeamLogo(elOption, numTeamID) {
      if (numTeamID != undefined && numTeamID != 0) {
        ...
        var strTeamTag = CompetitiveMatchAPI.GetTournamentTeamTagByID(
          m_strTournament,
          numTeamID
        )
        if (!strTeamTag) {
        ...
        elTeamLogo.SetImage(
          "file://{images}/tournaments/teams/" + strTeamTag + ".svg"
        )
      }
    }
    ...
    var _GetTile = function (
      ...
      numTeamID,
      numCtTeamID,
      numTeamPickingSideID
    ) {
      var strTileName = "Tile" + i
      if (strTileName in m_pickbanTiles) return m_pickbanTiles[strTileName]

      var elDraftContainer =
        $.GetContextPanel().FindChildInLayoutFile("DraftContainer")
    ...
    function GetSelectedDataFromDropdown(elDropdown) {
      if (!elDropdown.GetSelected()) return ""
    ...
    function SetupMapSelection(
      ...
      AddDropdownOption(
        elPickDropdown,
        "default",
        $.Localize(strDefaultDropdownLoc),
        "",
        ""
      )
      for (var i = 0; i < m_availableMaps.length; i++) {
        var strMapName = m_availableMaps[i]
        var NiceMapName = $.Localize(
          GameTypesAPI.GetMapGroupAttribute("mg_" + strMapName, "nameID")
        )
        AddDropdownOption(
          elPickDropdown,
          "map" + i,
          $.Localize(NiceMapName),
          strMapName,
          ""
        )
      }
      ...
      var _OnMapDropdownSelected = function (
        elOption,
        elDropdown,
        elConfirmBtn
      ) {
      ...
      elPickDropdown.SetPanelEvent(
        "oninputsubmit",
        _OnMapDropdownSelected.bind(
          undefined,
          elOption,
          elPickDropdown,
          elConfirmBtn
      ...
      elConfirmBtn.SetPanelEvent(
        "onactivate",
        _OnMapConfirmed.bind(undefined, numIndex, elPickDropdown)
      )
    ...
    function SetMapImage(elOption, strMapName) {
      ...
      elMapImage.SetImage(
        "file://{images}/map_icons/screenshots/360p/" + strMapName + ".png"
      )
    }
    ...
    function SetupSelectedMap(
      ...
      var strTeamName = CompetitiveMatchAPI.GetTournamentTeamNameByID(
        m_strTournament,
        numTeamID
      )
    ...
    function SetupTeamSelection(elOption, numIndex) {
      ...
      AddDropdownOption(
        elPickDropdown,
        "default",
        $.Localize("#SFUI_Tournament_Starting_Side"),
        "",
        ""
      )
      AddDropdownOption(
      ...
      var _OnTeamDropdownSelected = function (
        elOption,
        elDropdown,
        elConfirmBtn
      ) {
      ...
      elConfirmBtn.SetPanelEvent(
        "onactivate",
        _OnTeamConfirmed.bind(undefined, numIndex, elPickDropdown)
      )
    ...
    function SetupSelectedTeam(elOption, numTeamID, numCtTeamID) {
      ...
      var strSideLoc =
        numCtTeamID == numTeamID ? "#counter-terrorists" : "#terrorists"
    ...
    function SetUpVetoTile(
      ...
      if (isActive && strMapName == "") {
        ...
        SetupSelectedMap(
          ...
          strMapName,
          numTeamID,
          "#SFUI_Tournament_Vetoed_Title",
          "pickban__draft_vetoed"
        )
      } else {
    ...
    function SetUpPickTile(
      ...
      elMapOption.SetHasClass(
        "pickban__picking",
        isActive && isChoosingMap && isPickingMap
      )
      elMapOption.SetHasClass(
        "pickban__waiting",
        isActive && isChoosingMap && !isPickingMap
      )
      elTeamOption.SetHasClass(
        "pickban__picking",
        isActive && !isChoosingMap && isChoosingTeam && isPickingSide
      )
      elTeamOption.SetHasClass(
        "pickban__waiting",
        isActive && !isChoosingMap && isChoosingTeam && !isPickingSide
      ...
      if (numActiveIndex == numIndex) {
        if (isChoosingMap) {
          ...
          SetupSelectedMap(
            ...
            strMapName,
            numTeamID,
            "#SFUI_Tournament_Picked_Title",
            "pickban__draft_picked"
          )
...
;(function () {
  $.RegisterForUnhandledEvent(
    ...
    PopupTournamentPickBan.DraftUpdate
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_Lobby_MatchmakingSessionUpdate",
    PopupTournamentPickBan.SessionSettingsUpdate
  )
})()
