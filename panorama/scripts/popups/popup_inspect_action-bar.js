...
var InspectActionBar = (function () {
  ...
  var _Init = function (
    ...
    funcGetSettingCallbackInt,
    elItemModelImagePanel
  ) {
    if (funcGetSettingCallback("inspectonly", "false") === "false") return
    ...
    m_showMaketLink =
      funcGetSettingCallback("showmarketlink", "false") === "true"
    m_showCharSelect =
      funcGetSettingCallback("showcharselect", "true") === "true"
  ...
  var _SetupEquipItemBtns = function (elPanel, id) {
    ...
    if (m_insideCasketID) {
      ...
      elSingleActionBtn.SetPanelEvent(
        "onactivate",
        _OnActivateUpdateSelectionForMultiSelect.bind(this, id)
      )
      return
    }
    ...
    var isFanToken = ItemInfo.ItemDefinitionNameSubstrMatch(
      id,
      "tournament_pass_"
    )
    ...
    if (
      ...
      if (!isEquipped) {
        elSingleActionBtn.RemoveClass("hidden")
        _SetUpSingleActionBtn(
          elPanel,
          id,
          isSticker || isSpraySealed || isFanToken || isPatch
        )
      }
  ...
  var _SetUpSingleActionBtn = function (elPanel, id, isSticker) {
    ...
    for (var i = 0; i < validEntries.length; i++) {
      ...
      if (entry.AvailableForItem(id)) {
        ...
        elSingleActionBtn.SetPanelEvent(
          "onactivate",
          _OnSingleAction.bind(this, entry, id, closeInspect)
        )
      }
    }
  }
  ...
  var _ShowButtonsForWeaponInspect = function (elPanel, id) {
    ...
    if (
      ...
      elPanel
        .FindChildInLayoutFile("InspectCharBtn")
        .SetHasClass("hidden", !hasAnims)
      elPanel
        .FindChildInLayoutFile("InspectWeaponBtn")
        .SetHasClass("hidden", !hasAnims)

      var list = CharacterAnims.GetValidCharacterModels(true).filter(
        function (entry) {
          return (
            (ItemInfo.IsItemCt(id) &&
              (entry.team === "ct" || entry.team === "any")) ||
            (ItemInfo.IsItemT(id) &&
              (entry.team === "t" || entry.team === "any")) ||
      ...
      if (
        list &&
        list.length > 0 &&
        !elPanel.FindChildInLayoutFile("InspectDropdownCharModels").Data()
          .selectedId
      )
  ...
  function _ShowButtonsForCharacterInspect(elPanel, id) {
    ...
    if (!ItemInfo.IsCharacter(id)) return

    elPanel
      .FindChildInLayoutFile("id-character-button-container")
    ...
    if (
      $.GetContextPanel().BAscendantHasClass("AspectRatio16x9") ||
    ...
    var elCharacterButtons = elPanel.FindChildInLayoutFile(
  ...
  var _SetDropdown = function (elPanel, vaildEntiresList, id) {
    var currentMainMenuVanitySettings =
      ItemInfo.GetOrUpdateVanityCharacterSettings(
        ItemInfo.IsItemAnyTeam(id)
          ? null
          : LoadoutAPI.GetItemID(
              ItemInfo.IsItemCt(id) ? "ct" : "t",
              "customplayer"
            )
      )
    ...
    elDropdown.SetPanelEvent(
      "oninputsubmit",
      InspectActionBar.OnUpdateCharModel.bind(undefined, false, elDropdown, id)
    )
    elDropdown.SetSelected(currentMainMenuVanitySettings.charItemId)
    elDropdown.SetPanelEvent(
      "oninputsubmit",
      InspectActionBar.OnUpdateCharModel.bind(undefined, true, elDropdown, id)
    )
  }
  ...
  var _OnUpdateCharModel = function (bPlaySound, elDropdown, weaponItemId) {
    ...
    InspectModelImage.SetCharScene(
      m_modelImagePanel,
      characterItemId,
      weaponItemId
    )
  ...
  var _NavigateModelPanel = function (type) {
    InspectModelImage.ShowHideItemPanel(
      m_modelImagePanel,
      type !== "InspectModelChar"
    )
    InspectModelImage.ShowHideCharPanel(
      m_modelImagePanel,
      type === "InspectModelChar"
    )

    $.GetContextPanel()
      .FindChildTraverse("InspectCharModelsControls")
      .SetHasClass("hidden", type !== "InspectModelChar")
  }

  var _InspectPlayMusic = function (type) {
    if (!m_previewingMusic) return

    if (type === "mvp") {
      if (m_schfnMusicMvpPreviewEnd) return

      InventoryAPI.StopItemPreviewMusic()
      InventoryAPI.PlayItemPreviewMusic(m_itemId, "roundmvpanthem_01.mp3")

      m_schfnMusicMvpPreviewEnd = $.Schedule(
        6.8,
        InspectActionBar.InspectPlayMusic.bind(null, "schfn")
      )
    } else if (type === "schfn") {
