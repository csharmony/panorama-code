...
var StreamPanel = (function () {
  ...
  var _UpdateEmbeddedStream = function () {
    ...
    var elStreamPanelFeed =
      m_elSteamPanel.FindChildInLayoutFile("StreamPanelFeed")
    ...
    if (urlStreamFeed) {
      if (!elStreamPanelFeed) {
        ...
        if (elSlider) {
          ...
          elSlider.SetPanelEvent(
            "onvaluechanged",
            StreamPanel.OnVolumeSliderValueChanged
          )
        }

        _UpdateVolumeImageFromSlider()
        var elVolumeImage =
          elStreamPanelFeed.FindChildInLayoutFile("VolumeImage")
        if (elVolumeImage) {
          elVolumeImage.SetPanelEvent(
            "onactivate",
            StreamPanel.ToggleVolumeMute
          )
        }

        elStreamPanelFeed
          .FindChildInLayoutFile("id-close-btn")
          .SetPanelEvent("onactivate", _CloseStream)
        elStreamPanelFeed
          .FindChildInLayoutFile("id-minimize-btn")
          .SetPanelEvent("onactivate", _MinimizeStream)
        elStreamPanelFeed
          .FindChildInLayoutFile("id-full-size-btn")
          .SetPanelEvent("onactivate", _FullSizeStream)
      }
    }
  ...
  var _UpdateVolumeImageFromSlider = function () {
    ...
    if (elSlider && elVolumeImage) {
      elVolumeImage.SetImage(
        elSlider.value > 0
          ? "file://{images}/icons/ui/unmuted.svg"
          : "file://{images}/icons/ui/sound_off.svg"
      )
    }
  }
  ...
  var _SetClassesForVideoPlaying = function (bIsVideoPlaying) {
    if (m_elSteamPanel) {
      if (bIsVideoPlaying) {
        ...
        if (elTitle) {
          elTitle.text = $.Localize(
            "#SFUI_MajorEventVenue_StreamTitle_" +
              NewsAPI.GetActiveTournamentEventID() +
              "_" +
              EmbeddedStreamAPI.GetStreamEventVenueID()
          )
        }

        var elNavBarWatchExternalExtraButtons =
          m_elSteamPanel.FindChildInLayoutFile(
            "NavBarWatchExternalExtraButtons"
          )
        var sSupportedStreamTypes =
          EmbeddedStreamAPI.GetStreamExternalLinkTypes()
        var sChildrenWithTypeName = "NavBarWatchExternal"
        elNavBarWatchExternalExtraButtons
          .Children()
          .forEach(function (elchild) {
            if (elchild.id.startsWith(sChildrenWithTypeName)) {
              var chrLookupTypeCharacter = elchild.id.substring(
                sChildrenWithTypeName.length,
                sChildrenWithTypeName.length + 1
              )
              elchild.SetHasClass(
                "hidden",
                sSupportedStreamTypes.indexOf(chrLookupTypeCharacter) < 0
              )
            }
          })
      } else {
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_EmbeddedStream_VideoReload",
    StreamPanel.UpdateEmbeddedStream
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_EmbeddedStream_VideoPlaying",
    StreamPanel.UpdateEmbeddedStreamVisibility
  )
  $.RegisterForUnhandledEvent(
    "PanoramaComponent_EmbeddedStream_VolumeChanged",
    StreamPanel.OnVolumeCodeValueChanged
  )
  ...
  $.RegisterEventHandler(
    "HTMLJSAlert",
    $.GetContextPanel(),
    StreamPanel.HTMLJSAlert
  )
  $.RegisterEventHandler(
    "HTMLFinishRequest",
    $.GetContextPanel(),
    StreamPanel.HTMLFinishRequest
  )
})()
