...
var ItemTile = (function () {
  var _OnTileUpdated = function () {
    ...
    var idForDisplay = id
    if (
      $.GetContextPanel().GetAttributeString("filter_category", "") ===
      "inv_graphic_art"
    ) {
  ...
  var _SetMultiSelect = function (id) {
    var bSelectedInMultiSelect =
      $.GetContextPanel().GetParent() &&
      $.GetContextPanel()
        .GetParent()
        .GetAttributeInt("capability_multistatus_selected", 0) &&
  ...
  var _SetEquippedState = function (id) {
    ...
    for (var team of ["t", "ct", "noteam"]) {
      if (
        ...
        ItemInfo.IsShuffleEnabled(id, team) &&
        ItemInfo.IsItemInShuffleForTeam(id, team)
      ) {
  ...
  var _SetEquipIcon = function (isShuffle, team) {
    ...
    var elCtDot = $.GetContextPanel().FindChildInLayoutFile(
      "ItemEquipped-" + team
    )
  ...
  var _SetRecentLabel = function (id) {
    ...
    if (isUpdatedValue === "1" || isRecentValue === "1") {
      ...
      if (isRecentValue === "1") {
        var strItemPickupMethod = InventoryAPI.GetItemSessionPropertyValue(
          id,
          "item_pickup_method"
        )
  ...
  var _OnActivate = function () {
    ...
    if (capabilityInfo) {
      ...
      if (capabilityInfo.capability === "nameable") {
        _CapabilityNameableAction(
          SortIdsIntoToolAndItemID(id, capabilityInfo.initialItemId)
        )
      } else if (capabilityInfo.capability === "can_sticker") {
        _CapabilityCanStickerAction(
          SortIdsIntoToolAndItemID(id, capabilityInfo.initialItemId)
        )
      } else if (capabilityInfo.capability === "can_patch") {
        _CapabilityCanPatchAction(
          SortIdsIntoToolAndItemID(id, capabilityInfo.initialItemId)
        )
      } else if (capabilityInfo.capability === "decodable") {
        _CapabilityDecodableAction(
          SortIdsIntoToolAndItemID(id, capabilityInfo.initialItemId)
        )
    ...
    var filterValue = $.GetContextPanel().GetAttributeString(
      "context_menu_filter",
      null
    )
    var filterForContextMenuEntries = filterValue
  ...
  var jsUpdateItemListCallback = UiToolkitAPI.RegisterJSCallback(
    _UpdatePopUpCapabilityList
  )
...
;(function () {
  $.RegisterEventHandler(
    "CSGOInventoryItemLoaded",
    $.GetContextPanel(),
    ItemTile.OnTileUpdated
  )
  $.RegisterEventHandler(
    "UpdateItemTile",
    $.GetContextPanel(),
