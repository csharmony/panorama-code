...
var MainMenuTournamentPassStatus = (function () {
  ...
  var _RegisterForInventoryUpdate = function () {
    if (_m_bRegisteredForAllEvents) return
  ...
  var _UpdatePanel = function () {
    var elCountdownLabel = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-pass-status-game-countdown"
    )
    ...
    $.GetContextPanel()
      .FindChildInLayoutFile("id-tournament-pass-status")
      .SetPanelEvent(
        "onactivate",
        _ShowTournamentJournalPanel.bind(undefined, id)
      )

    _AddItemImages()
    _ItemsOnSale()

    $.GetContextPanel().style.backgroundImage =
      'url("file://{images}/store/tournament_items_' +
      g_ActiveTournamentInfo.eventid +
      '.png")'
  ...
  var _AddItemImages = function () {
    var aImages = $.GetContextPanel()
      .FindChildInLayoutFile("id-tournament-pass-status-images-row")
      .Children()
    aImages[0].itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
      g_ActiveTournamentStoreLayout[0][1],
      0
    )
    aImages[1].itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
      g_ActiveTournamentStoreLayout[1][0],
      0
    )
    aImages[2].itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
      g_ActiveTournamentStoreLayout[2][0],
      0
    )
  }
  ...
  var _ItemsOnSale = function () {
    var elPrecent = $.GetContextPanel().FindChildInLayoutFile(
      "id-tournament-pass-saletag"
    )
    elPrecent.SetHasClass("hidden", true)

    var reduction = ItemInfo.GetStoreSalePercentReduction(
      g_ActiveTournamentStoreLayout[1][0],
      1
    )
  ...
  var _Timer = function (elPanelParam) {
    ...
    if (!elPanelParam || !elPanelParam.IsValid()) return

    var elCountdown = elPanelParam.FindChildInLayoutFile(
      "id-tournament-pass-status-game-countdown"
    )
    var secRemaining =
      PredictionsAPI.GetGroupRemainingPredictionSeconds("global")
    if (!secRemaining) {
      _ShowStatus(elPanelParam)
      return
    }

    elCountdown.SetDialogVariable(
      "time",
  ...
  var _ShowStatus = function (elPanel) {
    ...
    var nCampaignID = parseInt(
      InventoryAPI.GetItemAttributeValue(id, "campaign id")
    )
    ...
    var elLabel = elPanel.FindChildInLayoutFile(
      "id-tournament-pass-status-game-countdown"
    )
...
;(function () {
  ...
  $.RegisterForUnhandledEvent(
    ...
    MainMenuTournamentPassStatus.PurchaseComplete
  )
})()
