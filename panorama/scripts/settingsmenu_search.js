...
var SettingsMenuSearch = (function () {
  ...
  function _Init() {
    ...
    m_SettingsSearchTextEntry.SetPanelEvent(
      "ontextentrychange",
      OnTextEntryChanged
    )
    OnTextEntryChanged()
  ...
  function OnTextEntryChanged() {
    ...
    var arrStrings = m_SettingsSearchTextEntry.text
      .split(/\s/)
      .filter((s) => /^\w+$/.test(s))
    ...
    searchableMenus.forEach((id) => {
      ...
      function TraverseChildren(elRoot, fnSearch) {
        ...
        elRoot.Children().forEach((c) => {
          TraverseChildren(c, fnSearch)
          fnSearch(c)
        })
      }
      ...
      function SearchSettingText(setting) {
        ...
        function ShouldSearchPanelText(setting) {
          if (!setting.hasOwnProperty("text")) return false

          if (setting.paneltype === "TextEntry") return false

          if (setting.BHasClass("DropDownChild")) return false

          if (setting.BHasClass("BindingRowButton")) return false

          if (setting.GetParent().paneltype === "RadioButton") return false
  ...
  function CreateSearchResultPanel(text, menuid, panel) {
    var elSearchResult = $.CreatePanel(
      "Panel",
      m_ResultsContainer,
      "setting_result_link"
    )
    if (elSearchResult.BLoadLayoutSnippet("SearchResult")) {
      elSearchResult
        .FindChild("ResultString")
        .SetProceduralTextThatIPromiseIsLocalizedAndEscaped(text, true)
  ...
  function PopulateWithPromotedSettings() {
    var elTitle = $.CreatePanel(
      "Label",
      m_ResultsContainer,
      "promoted_settings_title"
    )
