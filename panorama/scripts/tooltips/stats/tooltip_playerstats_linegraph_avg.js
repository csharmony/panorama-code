...
var PlayerStatsLineGraphAvgTooltip = (function () {
  ...
  function _GetUnitSuffix(index) {
    if (_IsPerRound(index)) return $.Localize("playerstats_suffix_per_round")
    else if (_isPerDeath(index)) return ""
  ...
  function _Init() {
    var unixtime =
      Number($.GetContextPanel().GetAttributeString("timestamp", "")) * 1000
    ...
    $.GetContextPanel().SetDialogVariable(
      "statvalue",
      statvalue + _GetUnitSuffix(stat_index)
    )
  }
