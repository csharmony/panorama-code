...
var PlayerStatsCalendarDayTooltip = (function () {
  function _Init() {
    var arrMatches = JSON.parse(
      $.GetContextPanel().GetAttributeString("data", "[]")
    )
    ...
    if (elBg) elBg.SetImage("file://{images}/icons/ui/competitive.svg")

    $.GetContextPanel().SetDialogVariable("date", DateUtil.UUU_MMM_dd(date))

    var elMatchList = $.GetContextPanel().FindChildTraverse(
      "TTCalendarDayMatchList"
    )
    ...
    arrMatches.forEach(function (oMatch, index) {
      ...
      elMatch.SetHasClass(
        "playerstats-tt__match--win",
        (oMatch.match_outcome & 0x3) == 1
      )
      elMatch.SetHasClass(
        "playerstats-tt__match--loss",
        (oMatch.match_outcome & 0x3) == 2
      )
      elMatch.SetHasClass(
        "playerstats-tt__match--tie",
        (oMatch.match_outcome & 0x3) == 0
      )

      elMatch.SetHasClass(
        "playerstats-tt__match--dnf",
