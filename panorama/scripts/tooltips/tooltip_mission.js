...
var TooltipMission = (function () {
  function _Init() {
    var strMissionIds = $.GetContextPanel().GetAttributeString(
      "sub-mission-ids",
      "not-found"
    )
    ...
    aCheckboxes[aCheckboxes.length - 1].SetHasClass(
      "hide-connector",
      strType === "sequential"
    )

    var gamemode = $.GetContextPanel().GetAttributeString(
      "gamemode",
      "not-found"
    )
    var elIcon = $.GetContextPanel().FindChildInLayoutFile(
      "id-tooltip-mission-icon"
    )
    ...
    var missionId = $.GetContextPanel().GetAttributeString(
      "mission-id",
      "not-found"
    )
    ...
    $.GetContextPanel().FindChildInLayoutFile("id-tooltip-mission-title").text =
      InventoryAPI.GetItemName(MissionItemID)
    $.GetContextPanel()
      .FindChildInLayoutFile("id-tooltip-mission-desc")
    ...
    $.GetContextPanel().FindChildInLayoutFile(
      "id-tooltip-mission-type_desc"
    ).text = $.Localize("op_misson_" + strType + "_tooltip")
  }
