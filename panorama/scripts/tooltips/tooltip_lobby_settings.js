...
var TooltipLobby = (function () {
  ...
  var _GetLobbyStatistics = function () {
    ...
    var elMatchStats =
      $.GetContextPanel().FindChildInLayoutFile("LobbyTooltipStats")
    var isSearching =
      searchingStatus !== "" && searchingStatus !== undefined ? true : false

    elMatchStats.SetHasClass("hidden", !isSearching)

    if (!isSearching) return

    var elMatchStatsLabel = elMatchStats.FindChildInLayoutFile(
      "LobbyTooltipStatsTitle"
    )
    ...
    var MakeStatsRow = function (statType, iconName) {
      ...
      if (statType === "avgSearchTimeSeconds") {
        var time = FormatText.SecondsToDDHHMMSSWithSymbolSeperator(
          matchmakeingStats[statType]
        )
        p.SetDialogVariable("stat", time)
      } else p.SetDialogVariableInt("stat", matchmakeingStats[statType])

      p.FindChildInLayoutFile("SettingText").text = $.Localize(
        "matchmaking_stat_" + statType,
        p
      )
      p.FindChildInLayoutFile("SettingImage").SetImage(
        "file://{images}/icons/ui/" + iconName + ".svg"
      )
  ...
  var _SetPrimeStatus = function () {
    ...
    var displayText = !isLocalPlayerPrime
    ...
    var elPrimeText =
      $.GetContextPanel().FindChildInLayoutFile("LobbyTooltipPrime")
  ...
  function _SetDirectChallengeSettings() {
    var elDirectChallengeText = $.GetContextPanel().FindChildInLayoutFile(
      "LobbyDirectChallenge"
    )

    var gss = LobbyAPI.GetSessionSettings()
    var bPrivate =
      gss.options.hasOwnProperty("challengekey") &&
      gss.options.challengekey != ""

    elDirectChallengeText.text = bPrivate
      ? $.Localize("#DirectChallenge_lobbysettings_on")
      : $.Localize("#DirectChallenge_lobbysettings_off")
    var elContainer = $.GetContextPanel().FindChildInLayoutFile(
      "LobbyTooltipDirectChallengeContainer"
    )
  ...
  function _SetGameModeFlags() {
    var elContainer = $.GetContextPanel().FindChildInLayoutFile(
      "LobbyTooltipGameModeFlagsContainer"
    )
    ...
    var displayTextToken =
      "play_setting_gamemodeflags_" +
      m_GameSettings.mode +
      "_" +
      m_GameSettings.gamemodeflags
    elContainer.SetDialogVariable("gamemodeflags", $.Localize(displayTextToken))

    var elIcon = $.GetContextPanel().FindChildTraverse(
      "LobbyTooltipGamdeModeFlagsImage"
    )
  ...
  var _SetRankedStatus = function (isLocalPlayerPrime) {
    var elRankedText =
      $.GetContextPanel().FindChildInLayoutFile("LobbyTooltipRanked")

    if (
      !isLocalPlayerPrime ||
      !SessionUtil.DoesGameModeHavePrimeQueue(m_GameSettings.mode)
    ) {
    ...
    var isRanked =
      m_GameSettings.prime === 1 && SessionUtil.AreLobbyPlayersPrime()
    elRankedText.text = isRanked
      ? $.Localize("#prime_ranked")
      : $.Localize("#prime_unranked")
  }
  ...
  var _Permissions = function () {
    ...
    $.GetContextPanel().FindChildInLayoutFile("LobbyTooltipPermissions").text =
      $.Localize(displayText)
  }

  var _SetMode = function () {
    var elGameModeTitle = $.GetContextPanel().FindChildInLayoutFile(
      "LobbyTooltipGameMode"
    )
    elGameModeTitle.FindChild("SettingText").text = $.Localize(
      "SFUI_GameMode" + m_GameSettings.mode
    )
    elGameModeTitle
      .FindChild("SettingImage")
      .SetImage("file://{images}/icons/ui/" + m_GameSettings.mode + ".svg")

    elGameModeTitle
      .FindChild("SettingImage")
      .SetHasClass("tint", m_GameSettings.mode !== "competitive")
  }

  var _SetMaps = function () {
    if (!m_GameSettings.mapgroupname) return

    var mapsList = m_GameSettings.mapgroupname.split(",")

    var elMapsSection = $.GetContextPanel().FindChildInLayoutFile(
      "LobbyTooltipMapsList"
    )
    ...
    mapsList.forEach(function (element) {
      ...
      var strMapText = $.Localize(
        GameTypesAPI.GetMapGroupAttribute(element, "nameID")
      )
      if (
        element === "mg_lobby_mapveto" &&
        m_GameOptions &&
        m_GameOptions.challengekey
      ) {
        strMapText = $.Localize(
          "#SFUI_Lobby_LeaderMatchmaking_Type_PremierPrivateQueue"
        )
      }

      p.FindChildInLayoutFile("SettingText").text = strMapText

      var maps = GameTypesAPI.GetMapGroupAttributeSubKeys(
        element,
        "maps"
      ).split(",")
      p.FindChildInLayoutFile("SettingImage").SetImage(
        "file://{images}/map_icons/map_icon_" + maps[0] + ".svg"
      )
    })
  }
