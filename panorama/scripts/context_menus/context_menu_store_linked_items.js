...
var StoreLinkedItems = (function () {
  function _Init() {
    ...
    var extrapopupfullscreenstyle = $.GetContextPanel().GetAttributeString(
      "extrapopupfullscreenstyle",
      ""
    )
    var isDisabled =
      $.GetContextPanel().GetAttributeString("disablepurchase", "") !== "true"
        ? false
        : true
    var aItemIds = itemId.split(",")

    var usetinynames = $.GetContextPanel().GetAttributeString(
      "usetinynames",
      ""
    )
  ...
  function _ShowWarningText() {
    ...
    if (warningText) {
      ...
      $.GetContextPanel().FindChildInLayoutFile(
        "id-store-linked-items-warning"
      ).text = $.Localize(warningText)
    } else {
