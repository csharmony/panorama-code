...
var SelectMissionCardContextMenu = (function () {
  var _SetupContextMenu = function () {
    ...
    var nMissionCards =
      MissionsAPI.GetSeasonalOperationMissionCardsCount(nSeasonIndex)
    var elParent = $.GetContextPanel().FindChildInLayoutFile(
      "ContextMenuMissionsBody"
    )
    ...
    for (var i = 0; i < nMissionCards; i++) {
      ...
      newEntry.SetHasClass(
        "show-check",
        oPoints.totalCardPoints === oPoints.totalPossilbePoints && isUnlocked
      )
      newEntry.SetPanelEvent("onactivate", _OnActivate.bind(undefined, i))

      var elTimer = newEntry.FindChildInLayoutFile(
        "id-mission-context-entry-timer"
      )
      var seconds = InventoryAPI.GetSecondsUntilNextMission()

      if (seconds && seconds > 0 && i === nBacklog) {
        newEntry.SetDialogVariable(
          "time",
