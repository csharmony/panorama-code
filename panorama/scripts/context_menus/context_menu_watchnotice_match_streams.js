...
var ContextMenuWatchNoticeMatchStream = (function () {
  ...
  function _Init() {
    $.RegisterForUnhandledEvent(
      ...
      _RequestMatchString_Received
    )

    var matchId = _m_cP.GetAttributeString("match_id", "")
    _m_isOfficial =
      _m_cP.GetAttributeString("is_official", "") === "true" ? true : false
  ...
  function _RequestMatchString_Received(matchString) {
    ...
    var elStreamContainer = $.GetContextPanel().FindChildTraverse(
      "id-watchnotice__event__match__stream-container"
    )
    ...
    if (_m_arrStreams.length == 0) {
      var elNoStreamLabel = $.GetContextPanel().FindChildTraverse(
        "id-watchnotice__event__match__no-stream"
      )
    ...
    for (var jdx in _m_arrStreams) {
      ...
      var languageCode = oStream.hasOwnProperty("language")
        ? oStream.language
        : ""

      var bIsGotv = oStream["site"].toLowerCase() === "gotv"
      var elGotvBtn = $.GetContextPanel().FindChildTraverse(
        "id-watchnotice__event__match_gotv"
      )
      ...
      if (bIsGotv) {
        ...
        elGotvBtn.SetPanelEvent(
          "onactivate",
          onActivate.bind(undefined, oStream["resolved_embed"])
        )
        ...
        if (languageCode) CommonUtil.SetLanguageOnLabel(languageCode, elStream)
        else CommonUtil.SetRegionOnLabel(countryCode, elStream)

        var elStreamName = elStream.FindChildTraverse(
          "id-eventsched-match__stream__name"
        )
        if (elStreamName) {
          var streamName = ""
          ...
          if (oStream["resolved_embed"].search("channel=") != -1) {
            streamName =
              "Twitch: " +
              oStream["resolved_embed"].match("channel=(.*?(?=&))")[1]
    ...
    if ("match_page_url" in oMatch) {
      ...
      var elLinkBtn = $.GetContextPanel().FindChildTraverse(
        "id-watchnotice__event__match"
      )
