...
var contextmenuPlayerCard = (function () {
  ...
  var _LoadPlayerCard = function (xuid) {
    ...
    var newPanel = $.GetContextPanel().FindChildInLayoutFile(
      "JsContextMenuPlayercard"
    )

    if (newPanel) newPanel.DeleteAsync(0.0)

    var newPanel = $.CreatePanel(
      "Panel",
      $.GetContextPanel().FindChildInLayoutFile("JsContextMenuSections"),
      "JsContextMenuPlayercard"
    )
  ...
  var _ContextMenus = [
    ...
    },
    {
      ...
      AvailableForItem: function (id) {
        if (FriendsListAPI.IsFriendJoinable(id)) {
          if (GameStateAPI.IsPlayerConnected(id)) return false

          if (LobbyAPI.IsSessionActive()) {
            var party = LobbyAPI.GetSessionSettings().members

            for (var i = 0; i < party.numPlayers; i++) {
              if (id === party["machine" + i].player0.xuid) return false
            }
          }
    ...
    },
    {
      ...
      icon: "friendignore",
      AvailableForItem: function (id) {
        if (GameStateAPI.IsLocalPlayerPlayingMatch()) return false
    ...
    },
    {
      ...
      AvailableForItem: function (id) {
        return (
          FriendsListAPI.GetFriendStatusBucket(id) === "AwaitingLocalAccept"
        )
      },
      OnSelected: function (id) {
    ...
    },
    {
      ...
      AvailableForItem: function (id) {
        return (
          FriendsListAPI.GetFriendStatusBucket(id) === "AwaitingRemoteAccept"
        )
      },
      OnSelected: function (id) {
    ...
    },
    {
      ...
      AvailableForItem: function (id) {
        if (MyPersonaAPI.GetLauncherType() === "perfectworld") {
          ...
          return (
            status !== "AwaitingRemoteAccept" &&
            status !== "AwaitingLocalAccept"
          )
        }
    ...
    },
    {
      ...
      AvailableForItem: function (id) {
        var status = FriendsListAPI.GetFriendStatusBucket(id)
        var isRequest =
          status === "AwaitingRemoteAccept" || status === "AwaitingLocalAccept"
    ...
    },
    {
      ...
      OnSelected: function (id) {
        ...
        SteamOverlayAPI.OpenURL(
          communityUrl + "/profiles/" + id + "/minimaledit"
        )
        ...
        return (
          (GameStateAPI.IsLocalPlayerPlayingMatch() ||
            (GameStateAPI.IsLocalPlayerWatchingOwnDemo() &&
              MatchInfoAPI.CanReportFromCurrentlyPlayingDemo()) ||
    ...
    },
    {
      ...
      AvailableForItem: function (id) {
        var borrowedPlayerIndex = parseInt(
          GameInterfaceAPI.GetSettingString("cl_borrow_music_from_player_index")
        )
        return (
          GameStateAPI.IsLocalPlayerPlayingMatch() &&
          !_IsSelf(id) &&
      ...
      OnSelected: function (id) {
        GameInterfaceAPI.SetSettingString(
          ...
          "" + GameStateAPI.GetPlayerIndex(id)
        )
        $.DispatchEvent("ContextMenuEvent", "")
      }
    },
    {
      ...
      AvailableForItem: function (id) {
        ...
        if (borrowedPlayerIndex === 0) return false
  ...
  var _GetContextMenuEnteries = function () {
    $.CreatePanel("Panel", $.GetContextPanel(), "", {
      class: "context-menu-playercard-seperator"
    })
    var elContextMenuBtns = $.CreatePanel(
      "Panel",
      $.GetContextPanel(),
      "JsContextMenuPlayercardBtns",
      { class: "context-menu-playercard-btns" }
    )
    ...
    _ContextMenus.forEach(function (entry) {
      if (entry.AvailableForItem(xuid)) {
        ...
        if ("xml" in entry) {
          ...
          $.CreatePanel("Image", elEntryBtn, entry.name, {
            src: "file://{images}/icons/ui/" + entry.icon + ".svg"
          })

          elEntryBtn.SetPanelEvent(
            "onactivate",
            entry.OnSelected.bind(this, xuid, type)
          )
