...
var ItemContextMenu = (function () {
  ...
  function GetMapTargets() {
    ...
    if (strMaps === null || strMaps === "") return []

    var mapsInMapGroup = strMaps.split(",")

    return mapsInMapGroup.map(function (curMap) {
      return {
        displayName: $.Localize(GetMapDisplayName(curMap)).toUpperCase(),
        voteParam: curMap
      }
    })
  }
  ...
  function GetBackupFilenames(elPopup) {
    ...
    m_hBackupFileNamesEventReg = $.RegisterForUnhandledEvent(
      "BackupFileNamesReceived",
  ...
  function _Event_BackupFileNamesReceived(elPopup, objFileNames) {
    ...
    arrDisplayNames.forEach(function (item, idx) {
      ...
      elButton
        .FindChild("Icon")
        .SetImage("file://{images}/icons/ui/competitive.svg")
      $.UnregisterForUnhandledEvent(
        "BackupFileNamesReceived",
        m_hBackupFileNamesEventReg
      )
    })
  }
  ...
  function CreateVotePopupChoices(locToken) {
    var elPopup = UiToolkitAPI.ShowCustomLayoutPopup(
      "voteissuechoices",
      "file://{resources}/layout/popups/popup_choices.xml"
    )
  ...
  function ConstructPopupMapChoicesVoteIssue(
    voteCommand,
    locToken,
    choiceListFunc
  ) {
    ...
    this.cmd = function () {
      ...
      choiceListFunc().forEach(function (item) {
        ...
        IconUtil.SetupFallbackMapIcon(
          elIcon,
          "file://{images}/map_icons/map_icon_" + item.voteParam
        )
        elIcon.SetImage(
          "file://{images}/map_icons/map_icon_" + item.voteParam + ".svg"
        )
      })
    }
  ...
  function ConstructPopupAvatarChoicesVoteIssue(
    voteCommand,
    locToken,
    choiceListFunc
  ) {
    ...
    this.cmd = function () {
      ...
      choiceListFunc().forEach(function (item) {
        ...
        elButton.FindChild("AvatarImage").steamid =
          GameStateAPI.GetPlayerXuidFromUserID(item.voteParam)
      })
    }
    this.enabled = choiceListFunc().length > 0
  }

  function ConstructRoundBackupVoteIssue(
    voteCommand,
    locToken,
    bEnabled,
    pollForBackups
  ...
  function GetCurrentVoteIssues() {
    ...
    var nLocalPlayerTeamNum = GameStateAPI.GetAssociatedTeamNumber(
      GameStateAPI.GetLocalPlayerXuid()
    )
    var bLocalPlayerActiveTeam =
      nLocalPlayerTeamNum === 2 || nLocalPlayerTeamNum === 3

    if (!bIsQueuedMatchmaking) {
      VoteIssues.push(
        new ConstructPopupMapChoicesVoteIssue(
          "ChangeLevel",
          "#SFUI_Vote_ChangeMap",
          GetMapTargets
        )
    ...
    if (bIsQueuedMatchmaking && !bIsTournamentMatch) {
      VoteIssues.push(
        new ConstructBaseVoteIssue("Surrender", "#SFUI_vote_surrender")
      )
    }
    ...
    if (bIsQueuedMatchmaking && bIsTournamentMatch) {
      if (bLocalPlayerActiveTeam) {
        VoteIssues.push(
          new ConstructBaseVoteIssueEnabled(
            "starttimeout",
            "#SFUI_Vote_StartTimeout",
            !bIsWarmup && !bIsPaused
          )
        )
        VoteIssues.push(
        ...
        if (bIsWarmup) {
          if (!bIsPaused)
            VoteIssues.push(
              new ConstructBaseVoteIssue(
                "NotReadyForMatch",
                "#SFUI_Vote_not_ready_for_match"
              )
            )
          else
            VoteIssues.push(
          ...
          if (bIsPaused)
            ...
            VoteIssues.push(
              new ConstructBaseVoteIssue("PauseMatch", "#SFUI_Vote_pause_match")
            )
        }

        VoteIssues.push(
          new ConstructRoundBackupVoteIssue(
            "LoadBackup",
            "#SFUI_Vote_loadbackup1",
            !bIsWarmup,
            GetBackupFilenames
          )
        )
    ...
    if (!bIsTournamentMatch) {
      if (
        ...
        curGameMode === "scrimcomp5v5" ||
        curGameMode === "scrimcomp2v2"
      ) {
        VoteIssues.push(
          new ConstructBaseVoteIssue("starttimeout", "#SFUI_Vote_StartTimeout")
        )
      }

      VoteIssues.push(
        new ConstructPopupAvatarChoicesVoteIssue(
          "Kick",
          "#SFUI_Vote_KickPlayer",
          GetKickTargets
        )
      )
    }
