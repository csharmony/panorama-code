  ...
  var _Init = function () {
    var _OnActivateStoreItem = function (elItem, id, type) {
      if (type === "megabundle") {
        elItem.SetPanelEvent(
          "onactivate",
          _ShowInspectPopup.bind(undefined, id)
        )
    ...
    function AddItemEntry(elParent, defIdx) {
      ...
      if (count == 0) {
        ...
        elItemEntry.SetDialogVariable(
          "name",
          $.Localize(InventoryAPI.GetItemBaseName(containedItem))
        )
        _OnActivateStoreItem(
          elItemImage,
      ...
      var itemCountFuncName = $.GetContextPanel().GetAttributeString(
        "item_count_func_name",
        null
      )
      ...
      function UpdateCount() {
        ...
        var reduction = ItemInfo.GetStoreSalePercentReduction(
          itemId,
          nCountForPriceDisplay
        )
        var elOriginalPrice =
          elItemEntry.FindChildInLayoutFile("ItemPriceOriginal")

        if (reduction) {
          elOriginalPrice.visible = true
          var originalPrice = ItemInfo.GetStoreOriginalPrice(
            itemId,
            nCountForPriceDisplay
          )
