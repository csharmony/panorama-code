...
var watchMatchTile = (function () {
  ...
  function _Init(elTile, oMatch, oTeam1, oTeam2) {
    elTile.SetDialogVariable(
      "match_versus_tooltip",
      $.Localize("#eventsched_match_tooltip", elTile)
    )

    function _CreateTeamPanelsInContainerPanel(
      elTeamContainer,
      teamNum,
      oTeam
    ) {
      ...
      var elTeamLogo = $.CreatePanel(
        "Image",
        elTeamContainer,
        "id-wmt-logo-" + teamName,
        {
      ...
      var elLabel = $.CreatePanel(
        "Label",
        elTeamContainer,
        "id-wmt-label-" + oMatch["team" + teamNum + "_name"],
        {
    ...
    if (elWatchButton) {
      ...
      }
      {
        ...
        function OnSimplePerfectWorldContextMenu() {
          ...
          if (
            EmbeddedStreamAPI.GetStreamExternalLinkTypes().indexOf("B") >= 0
          ) {
            items.push({
              label: $.Localize("#CSGO_Watch_Info_live"),
              jsCallback: function () {
          ...
          if (
            EmbeddedStreamAPI.GetStreamExternalLinkTypes().indexOf("G") >= 0
          ) {
            items.push({
              label: $.Localize("#CSGO_Watch_Watch_GOTV"),
              jsCallback: function () {
                StoreAPI.RecordUIEvent("WatchNoticeSchedEventLink")
        ...
        if (_m_isPerfectWorld) {
          elWatchButton.SetPanelEvent(
            "onactivate",
            OnSimplePerfectWorldContextMenu
          )
        } else {
          elWatchButton.SetPanelEvent(
            "onactivate",
            OnActivate.bind(undefined, oMatch["match_id"])
          )
        }
      }
    }
  ...
  function _CreateTeamTooltip(panel, oTeam) {
    ...
    if ("lineup" in oTeam) {
      ...
      if (bHaveAnyPhoto) {
        ...
        panel.SetPanelEvent(
          "onactivate",
          _OnTeamContextMenu.bind(undefined, xmlsrc, parms)
        )
