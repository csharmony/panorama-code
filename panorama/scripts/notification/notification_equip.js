...
var EquipNotification = (function () {
  var _ShowNotification = function (elPanel, slot, itemId) {
    if (!elPanel || !InventoryAPI.IsItemInfoValid(itemId)) return
    ...
    for (var i = 0; i < teamsList.length; i++) {
      itemId =
        itemId === "0" ? LoadoutAPI.GetDefaultItem(teamsList[i], slot) : itemId
      var teamsEquippedList = _WhatTeamIsDefaultItemEquippedFor(
        itemId,
        teamsList
      )
