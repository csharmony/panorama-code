...
var SettingsMenuShared = (function () {
  var _ResetControlsRecursive = function (panel) {
    ...
    if (
      panel.paneltype == "CSGOSettingsSlider" ||
      panel.paneltype == "CSGOSettingsEnumDropDown"
    ) {
  ...
  var _ShowConfirmDiscard = function (discardCall) {
    UiToolkitAPI.ShowGenericPopupTwoOptionsBgStyle(
      "#settings_discard_confirm_title",
  ...
  var _NewTabOpened = function (newTab) {
    ...
    if (newTab == videoSettingsStr) {
      var videoSettingsPanel =
        $.GetContextPanel().FindChildInLayoutFile(videoSettingsStr)

      gBtnApplyVideoSettingsButton = videoSettingsPanel.FindChildInLayoutFile(
        "BtnApplyVideoSettings"
      )
      gBtnDiscardVideoSettingChanges = videoSettingsPanel.FindChildInLayoutFile(
        "BtnDiscardVideoSettingChanges"
      )
  ...
  var _ShowHudEdgePositions = function () {
    UiToolkitAPI.ShowCustomLayoutPopup(
      "",
      "file://{resources}/layout/popups/popup_hud_edge_positions.xml"
    )
  }
